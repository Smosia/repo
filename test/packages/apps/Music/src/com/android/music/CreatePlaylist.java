...
public class CreatePlaylist extends Activity
{
    ...
    public void onCreate(Bundle icicle) {
        ...
        String defaultname = icicle != null ? icicle.getString("defaultname")
                : MusicUtils.makePlaylistName(getApplicationContext(),
        ...
        public void onClick(DialogInterface dialogInterface, int button) {
            if (button == DialogInterface.BUTTON_POSITIVE) {
                ...
                if (name != null && name.length() > 0) {
                    ...
                    if (id >= 0 && (mPlaylistFlag.equals(MusicUtils.NEW_PLAYLIST))) {
                        ...
                        if (!name.equals(mPlaylistName) && name.equalsIgnoreCase(mPlaylistName)) {
                         ...
                         !name.equalsIgnoreCase(mPlaylistName)) {
                            ...
                            try {
                                int newPid =
                                MusicUtils.idForplaylist(getApplicationContext(), name);
                                if (newPid >= 0) {
                                    uri = ContentUris
                                            .withAppendedId(
                                                    MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI,
                                                    newPid);
                                    if (MusicUtils.clearPlaylist(CreatePlaylist.this,
                                    newPid) == -1) {
                                        finish();
                                        return;
                                    }
                    ...
                    /// M: Return the start activity tab to MusicBrowser
                    /// to return result to right activity.
