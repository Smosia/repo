...
public class WebAccessTools {
	...
	public String getWebContent(String url) {
		if (!Utils.isNetworkAvailable(context)) {
			Log.i("guocl", "getWebContent, Network NOT Available");
			return null;
		}
		// 创建一个http请求对象
		HttpGet request = new HttpGet(url);
		android.util.Log.d("guocl", "getWebContent, request:" + request);
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				//android.util.Log
				//		.d("guocl", "getWebContent, content:" + content);
				return content;
			} else {
				Log.d("guocl", "联网失败");
			}
		...
		android.util.Log.w("guocl", "getWebContent, null");
		return null;
	}
}