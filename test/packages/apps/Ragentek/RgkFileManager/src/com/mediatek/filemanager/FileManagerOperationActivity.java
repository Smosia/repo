...
public class FileManagerOperationActivity extends AbsBaseActivity implements
    ...
    // add by chenweihua for SQM's auto recording (start)
    private static final String OPEN_SPECIAL_PATH = "com.android.ragentek.OPEN_SPECIAL_PATH";
    private boolean isForAutoRecording = false;
    // add by chenweihua for SQM's auto recording (end)
    ...
    private BroadcastReceiver mLocaleChangedReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_LOCALE_CHANGED.equals(intent.getAction())) {
            	//M BUG_ID:JWBLB-635 shasha.fang 20150603(start)
                mFileInfoManager.clearShowFilesInfoList(mAdapter);
                //M BUG_ID:JWBLB-635 shasha.fang 20150603(end)
                finish();
    ...
    protected void onResume() {
        ...
        // add by chenweihua for SQM's auto recording (start)
        if (action != null && action.equalsIgnoreCase(OPEN_SPECIAL_PATH)) {
        	isForAutoRecording = true;            
        }
        // add by chenweihua for SQM's auto recording (end)
...
//        mNavigationView.setVisibility(View.VISIBLE);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mCurrentPath != null && mMountPointManager.isRootPath(mCurrentPath)) {
          ...
          //A BUG_ID:JWBLB-900 shasha.fang 20150612(start)
            menu.findItem(R.id.hide).setVisible(false);
            menu.findItem(R.id.hide).setEnabled(false);
            //A BUG_ID:JWBLB-900 shasha.fang 20150612(end)
            // more items
    ...
    protected void onNewIntent(Intent intent) {
        ...
        //add JWBLB-2005 songqingming 20150901 (start)
        if (action != null && action.equalsIgnoreCase(OPEN_SPECIAL_PATH)) {
            isForAutoRecording = true;
            ...
            if (path != null && mService != null && !mService.isBusy(this.getClass().getName())) {
                ...
                addToNavigationList(path, null, -1);
                showDirectoryContent(path);
            }
        }
        //add JWBLB-2005 songqingming 20150901 (end)
    }
    ...
    protected class DetailInfoListener implements FileManagerService.OperationEventListener,
        ...
        public DetailInfoListener(FileInfo fileInfo) {
            ...
            mSize = mStringBuilder.append(getString(R.string.size)).append(": ").append(
                    ...
                    FileUtils.sizeToString(0,getApplicationContext())).append(" \n").toString();
        ...
        public String getDetailInfo(long size) {
            ...
            mSize = getString(R.string.size) + ": "
                    + FileUtils.sizeToString(size,getApplicationContext()) + " \n";
            // M:BUG_ID:JWBLB-968 shasha.fang 20150616(end) 
        ...
        public void onTaskProgress(ProgressInfo progressInfo) {
            ...
            // M:BUG_ID:JWBLB-968 shasha.fang 20150616(start) 
            mSize = getString(R.string.size) + ": "
                    + FileUtils.sizeToString(progressInfo.getTotal(),getApplicationContext()) + " \n";
            // M:BUG_ID:JWBLB-968 shasha.fang 20150616(end) 
            if (mDetailsText != null) {
                mStringBuilder.setLength(0);
...
//add  BUG_ID:JWYYL-12 zhaolei.ding 20141216 (end)
    ...
    private String getTotalSize(ArrayList<BrowseInfo> browseInfoList) {
        ...
        // M:BUG_ID:JWBLB-968 shasha.fang 20150616(start) 
        return FileUtils.sizeToString(size,getApplicationContext());
        // M:BUG_ID:JWBLB-968 shasha.fang 20150616(end) 
    }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            //M:for NullPointerException guoshuai 20160119(atart)
            try{
            ...
            //M:for NullPointerException guoshuai 20160119(end)
        }
    };
