...
/****************************************
History:
1. zhaolei.ding@ragentek.com 20130711 JBLWKK-140
Description:apk does not display the corresponding icon
****************************************/
...
import com.mediatek.filemanager.FileInfoAdapter.FileViewHolder;
...
//add BUG_ID:JBLWKK-140 zhaolei.ding 20140609(start) 
...
//add BUG_ID:JBLWKK-140 zhaolei.ding 20140609 (end)
...
//add BUG_ID:JBLWKK-140 zhaolei.ding 20140609 (start)
            if (mimeType.startsWith("application/vnd.android.package-archive")) {
            	return	getApkIcon(fileInfo.getFileAbsolutePath());
            }
//add BUG_ID:JBLWKK-140 zhaolei.ding 20140609 (end)
...
//add BUG_ID:JBLWKK-140 zhaolei.ding 20140609 (start)
    private Bitmap getApkIcon(String apkPath) {
    	...
    	if (info != null) {
    		...
    		try {
    			return drawableToBitmap(appInfo.loadIcon(pm));
    		} catch (OutOfMemoryError e) {
    			LogUtils.e("IconManager", e.toString());
    		}
    	}
    	return null;
    }
...
//add BUG_ID:JBLWKK-140 zhaolei.ding 20140609 (end)
    ...
    public void displayThumbnail(FileInfo fileInfo, FileManagerService service,
            int viewDirection,ImageView image) {
        if (mCurrentDirection != viewDirection) {
            mDirectionChanged = true;
            mCurrentDirection = viewDirection;
        }
        ...
        Bitmap icon = null;
        if (isExternal) {
            icon =  getExternalIcon(iconId);
        } else {
            icon = getDefaultIcon(iconId);
        }
        String path = fileInfo.getFileAbsolutePath();
        ...
        if (bitmap != null) {
            ...
            queueThumbnail(image,mimeType,path,icon,isExternal);
        }
    }
    
    private void queueThumbnail(ImageView image,String mimeType,String path,Bitmap sourceBitmap,boolean isExternal) {
        executorService.submit(new ThumbnailLoad(image,mimeType,path,sourceBitmap,isExternal));
    }
    
    private class ThumbnailLoad implements Runnable {
        private ImageView imageView;
        private String mimeType;
        private String path;
        private Bitmap sourceBitmap;
        private boolean isExternal;
        public ThumbnailLoad(ImageView imageView,String mimeType,String path,Bitmap sourceBitmap,boolean isExternal) {
        ...
        public void run() {
            ...
            if (mimeType.startsWith("image/")) {
                bitmap = getPicThumbnail(path,width,height);
            } else if (mimeType.startsWith("video/")) {
            ...
            ThumbnailDisplay display = new ThumbnailDisplay(bitmap,imageView);
            Activity activity = (Activity) imageView.getContext();
            activity.runOnUiThread(display);
        }
        
    }
    
    private class ThumbnailDisplay implements Runnable {
        private ImageView imageView;
        private Bitmap bitmap;
        public ThumbnailDisplay(Bitmap bitmap,ImageView imageView) {
            this.imageView = imageView;
            this.bitmap = bitmap;
        }
        @Override
    ...
    private Bitmap getPicThumbnail(final String path,final int width,final int height) {
        Bitmap bm = null;
