...
public abstract class AbsBaseActivity extends Activity implements OnItemClickListener,
    ...
    protected View mBrowseView;
    protected View mFileView;
    protected ViewPager mViewPager;
    protected List<View> mViews;
    protected ProgressBarAdapter mProgressBarAdapter;
    protected TextView mTabBrowse;
    protected TextView mTabFile;
    protected CustomUnderLine mCustomUnderLine;
    protected String mCurPathFromBrowseInRename;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mNavigationBar = (SlowHorizontalScrollView) mFileView.findViewById(R.id.navigation_bar);
        ...
        // set up a list view
        mListView = (ListView) mFileView.findViewById(R.id.list_view);
        if (mListView != null) {
            mListView.setEmptyView(mFileView.findViewById(R.id.empty_view));
    ...
    protected void onDestroy() {
        ...
        IconManager.getInstance().recycle();
    ...
    protected class TabManager {
        ...
        private final TextView mBlankTab;
        private LinearLayout.LayoutParams mBlanckBtnParam = null;

        public TabManager() {
            mTabsHolder = (LinearLayout) mFileView.findViewById(R.id.tabs_holder);
            mBlankTab = new TextView(AbsBaseActivity.this);
//            mBlankTab.setBackgroundColor(R.color.custom_actionbar_main);
//            mBlankTab.setBackgroundDrawable(getResources().getDrawable(R.drawable.fm_blank_tab));
        ...
        public void refreshBorowseTab(String text) {
...
//            ImageButton homeBtn = (ImageButton) mTabsHolder.getChildAt(0);
            TextView textView = (TextView) mTabsHolder.getChildAt(0);
            if (textView == null) {
            ...
            textView.setText("/"+MountPointManager.HOME);
            textView.setTextSize(16);
            textView.setPadding((int) resources.getDimension(R.dimen.home_btn_padding), 0,0, 0);
            textView.setTextColor(R.color.custom_tab_txt_style);
//            Resources resources = getResources();
...
//            }
        ...
        protected void addTab(String text) {
            ...
            View viewLikeTxt = null;
            int count = mTabsHolder.getChildCount();
            TextView lastTextView = null;
            if (mTabNameList.isEmpty()) {
                viewLikeTxt = new TextView(AbsBaseActivity.this);
                mlp = new LinearLayout.LayoutParams(new ViewGroup.MarginLayoutParams(
                        LinearLayout.LayoutParams.WRAP_CONTENT,
                        LinearLayout.LayoutParams.MATCH_PARENT));
                mlp.setMargins(0, 0, 0, 0);
                viewLikeTxt.setLayoutParams(mlp);
            } else {
                lastTextView = (TextView) mTabsHolder.getChildAt(count - 1);
                TextView textView = new TextView(AbsBaseActivity.this);
                textView.setClickable(true);
                textView.setTextSize(16);
                textView.setGravity(Gravity.CENTER_VERTICAL);
                textView.setEllipsize(TextUtils.TruncateAt.END);
                textView.setTextAppearance(getApplicationContext(), R.style.TabBarTxtStyle);
//                textView.setTextColor(Color.BLACK);
                //button.setBackgroundDrawable(getResources().getDrawdable(R.drawable.custom_tab));
                textView.setTextColor(R.color.custom_tab_txt_style);
                textView.setMaxWidth(TAB_TET_MAX_LENGTH);
                LongStringUtils.fadeOutLongString(textView);
                ...
                if (getViewDirection() == ViewGroup.LAYOUT_DIRECTION_LTR) {
                    mlp.setMargins(lastTextView.getWidth(), 0, 0, 0);
                } else if (getViewDirection() == ViewGroup.LAYOUT_DIRECTION_RTL) {
                    mlp.setMargins(0, 0, lastTextView.getWidth(), 0);
                }
                textView.setLayoutParams(mlp);
                viewLikeTxt = textView;
            }
            viewLikeTxt.setOnClickListener(AbsBaseActivity.this);
            viewLikeTxt.setId(mTabNameList.size());
            mTabsHolder.addView(viewLikeTxt);
            mTabNameList.add(text);
            mTabsHolder.addView(mBlankTab);
            viewLikeTxt.setOutlineProvider(null);
        }
        ...
        protected void updateNavigationBar(int id) {
            ...
            if (FileUtils.mCurTabIndex == FileUtils.TAB_ON_BROWSE_ITEM && id == 0) {
                showBrowseTab();
                return;
            }
    ...
    public void onBackPressed() {
        ...
        if (mViewPager != null && mViewPager.getCurrentItem() 
                == FileUtils.TAB_ON_BROWSE) {
            super.onBackPressed();
            return;
        }
    ...
    protected class LightOperationListener implements FileManagerService.OperationEventListener {
        ...
        public void onTaskResult(int errorType) {
            ...
            case ERROR_CODE_USER_CANCEL:
                ...
                if (FileUtils.mCurTabIndex == FileUtils.TAB_ON_BROWSE_ITEM) {
                    fileInfo = mFileInfoManager.updateOneFileInfoList(mCurPathFromBrowseInRename, mSortType);
                } else {
                    fileInfo = mFileInfoManager.updateOneFileInfoList(mCurrentPath, mSortType);
                }
                mAdapter.notifyDataSetChanged();
                if (fileInfo != null) {
    ...
    protected class ProgressBarAdapter extends BaseAdapter {
        @Override
        public int getCount() {
            int count = MountPointManager.getInstance().getMountCount();
            return count;
        }

        @Override
        public Object getItem(int arg0) {
            return MountPointManager.getInstance().getMountPointFileInfo().get(arg0);
        }
        ...
        public View getView(int position, View view, ViewGroup arg2) {
            ProgressBarViewHolder viewHolder = null;
            if (view == null) {
                viewHolder = new ProgressBarViewHolder();
                view = LayoutInflater.from(getApplicationContext()).inflate(R.layout.browse_progressbar_item, null);
                viewHolder.pathDescription = (TextView) view.findViewById(R.id.storage_all_space);
                viewHolder.progressBar = (ProgressBar) view.findViewById(R.id.progressbar);
                view.setTag(viewHolder);
            } else {
                viewHolder = (ProgressBarViewHolder) view.getTag();
            }
            List<FileInfo> fileInfos = MountPointManager.getInstance().getMountPointFileInfo();
            Collections.sort(fileInfos, FileInfoComparator.getInstance(mSortType));
         ...
         // M:BUG_ID:JWBLB-968 shasha.fang 20150616(start) 
            String freeSpaceString = FileUtils.sizeToString(freeSpace,getApplicationContext());
         // M:BUG_ID:JWBLB-968 shasha.fang 20150616(end)  
            long totalSpace = MountPointManager.getInstance().getMountPointTotalSpace(fileInfo.getFileAbsolutePath());
         // M:BUG_ID:JWBLB-968 shasha.fang 20150616(start)  
            String totalSpaces = FileUtils.sizeToString(totalSpace,getApplicationContext());
         // M:BUG_ID:JWBLB-968 shasha.fang 20150616(end)  
            long usedSpace = totalSpace - freeSpace;
            sb.append(fileInfo.getShowPath() + ": ");
            sb.append(totalSpaces + "  ");
            Resources rs = getApplicationContext().getResources();
            sb.append(rs.getString(R.string.free_space) + ": ");
            sb.append(freeSpaceString);
            viewHolder.pathDescription.setText(sb.toString());
            int progress = (int) ((usedSpace / (totalSpace * 1.0f)) * MAX_PROGRESS);
