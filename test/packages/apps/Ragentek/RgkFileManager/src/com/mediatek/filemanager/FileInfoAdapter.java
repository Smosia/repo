...
public class FileInfoAdapter extends BaseAdapter {
        ...
        if (fileInfo.isDirectory()) {
            if (MountPointManager.getInstance().isMountPoint(fileInfo.getFileAbsolutePath())) {
             ...
             // M:BUG_ID:JWBLB-968 shasha.fang 20150616(start) 
                String freeSpaceString = FileUtils.sizeToString(MountPointManager.getInstance().getMountPointFreeSpace(fileInfo.getFileAbsolutePath()),mContext);
                String totalSpaces = FileUtils.sizeToString(MountPointManager.getInstance().getMountPointTotalSpace(fileInfo.getFileAbsolutePath()),mContext);
         ...
         // M:BUG_ID:JWBLB-968 shasha.fang 20150616(start) 
            sb.append(fileInfo.getFileSizeStr(mContext));
         // M:BUG_ID:JWBLB-968 shasha.fang 20150616(end) 
            textView.setText(sb.toString());
            textView.setVisibility(View.VISIBLE);
        }
	...
	//D BUG_ID DWYLL-609 shasha.fang 20150521(start)
       // String mimeType = fileInfo.getFileMimeType(mService);
        //if (mimeType != null && (mimeType.startsWith("image/") || mimeType.startsWith("video/"))) {
           // IconManager.getInstance().displayThumbnail(fileInfo, mService, viewDirection, viewHolder.mIcon);
       // } else {
       ...
       // }
	//D BUG_ID DWYLL-609 shasha.fang 20150521(end)
