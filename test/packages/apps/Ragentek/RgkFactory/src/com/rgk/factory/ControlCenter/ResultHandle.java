...
public class ResultHandle {
	...
	public static int readTestResult() {
		...
		if(Util.subCamera) {
			getTextFromSystem(SubCamera.TAG);
		}
		if(Util.flash) {
			getTextFromSystem(FlashlightManager.TAG);
		}
		if(Util.proactiveFlash) {
			getTextFromSystem(ProactiveFlashlightManager.TAG);
		}		
		if(Util.earphone) {
			getTextFromSystem(EarPhone.TAG);
		}
		if(Util.fm) {
			getTextFromSystem(FM.TAG);
		}
		if(Util.battery) {
		...
		if(Util.buttonLight) {
			getTextFromSystem(ButtonLightActivity.TAG);
		}
		if(Util.hall) {
		...
		if(Util.temperature) {
			getTextFromSystem(Temperature.TAG);
		}
