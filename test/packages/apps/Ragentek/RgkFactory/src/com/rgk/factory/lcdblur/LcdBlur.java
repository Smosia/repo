...
public class LcdBlur extends Activity implements OnClickListener{
	...
	requestWindowFeature(Window.FEATURE_NO_TITLE);
        ...
        //color=context.getResources(R.color.flicker);
	...
	class Panel extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			for (; x < widthPix; x += rectangle_x, i++) {
				for (; y < hightPix+rectangle_y; y += rectangle_y, j++) {
					if((i%2 != 0 && j%2 == 0) || (i%2 == 0 && j%2 != 0)) {
						mPaint.setColor(getResources().getColor(R.color.blur_color_one));
					} else {
						mPaint.setColor(getResources().getColor(R.color.blur_color_two));
					}
