...
public class EarPhone extends Activity implements View.OnClickListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mPass = (Button) findViewById(R.id.earphone_pass);
		mPass.setOnClickListener(this);
		mPass.setEnabled(false);
		((Button) findViewById(R.id.earphone_fail)).setOnClickListener(this);
		mRecord.setOnClickListener(this);
		//add Bug_id:DWYQLSSB-105 taoqi 20160527 (start)
        Settings.Secure.putInt(this.getContentResolver(),"com.rgk.factory.headphonemic.stopFM", 1);
		//add Bug_id:DWYQLSSB-105 taoqi 20160527 (end)
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(Intent.ACTION_HEADSET_PLUG)) {
				...
				mRecord.setText(status ? R.string.start_record
						: R.string.insert_earphone);
				if (status) {
	...
	private void HandlerRecord() {
		...
		mRecord.setText(Text.equals(getResources().getString(
				R.string.start_record)) ? R.string.stop_record
	...
	private Runnable update = new Runnable() {
		...
		public void run() {
			if (!playfinish) {
				if (mMediaPlayer != null)
					mRecordTime.setText(makeTimeString(mMediaPlayer
							.getCurrentPosition()));
	...
	public void onDestroy() {
		//add Bug_id:DWYQLSSB-105 taoqi 20160527 (start)
	    Settings.Secure.putInt(this.getContentResolver(),"com.rgk.factory.headphonemic.stopFM", 0);
		//add Bug_id:DWYQLSSB-105 taoqi 20160527 (end)
		if (mMediaPlayer != null)
			mMediaPlayer.release();
		if (mMediaRecorder != null)
