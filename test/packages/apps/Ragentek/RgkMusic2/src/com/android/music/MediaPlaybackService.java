...
public class MediaPlaybackService extends Service {
    ...
    public static final int MEDIA_ERROR_MUSICFX_DIED = -22; //0x800000FF;
	 ...
	 //add BUG_ID:JSLEL-239 gaoxueyan 20140917 start
    public static final String PAUSE_PLAY_ACTION = "com.android.music.musicservicecommand.pauseplay";
    public static final String CLOSE_ACTION = "com.android.music.musicservicecommand.close";
    //add BUG_ID:JSLEL-239 gaoxueyan 20140917 end
    // add BUG_ID:JEYS-570 chenchunyong 20140307 (start)
    //m DWYBL-933 yejianming 20150429
    private boolean mIsNeedNotifyWidget = false;
    private static final int WIDGET_REFRESH = 15;
	// add BUG_ID:JEYS-570 chenchunyong 20140307 (end)
            ...
            switch (msg.what) {
                ...
                case TRACK_WENT_TO_NEXT:
                    ...
                    if (mPlayPos >= 0 && mPlayListLen > mPlayPos) {
                      ...
                      //add BUG_ID:JSLEL-239 gaoxueyan 20140917 start
                        updateNotification(MediaPlaybackService.this, null, true);
                        //add BUG_ID:JSLEL-239 gaoxueyan 20140917 end
    ...
    private final BroadcastReceiver mSkinChangedReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (isPlaying()) {
            	//add BUG_ID:JSLEL-239 gaoxueyan 20140917 start
                updateNotification(MediaPlaybackService.this, mAlbumArt, true);
            //add BUG_ID:JSLEL-239 gaoxueyan 20140917 end
            }
        }
    };
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && !isEventFromMonkey()) {
            ...
            if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
                ...
                //}
                /// @}
                    //add PAUSE_PLAY_ACTION for JSLEL-239 gaoxueyan 20140613   
            } else if (CMDTOGGLEPAUSE.equals(cmd) || TOGGLEPAUSE_ACTION.equals(action)||PAUSE_PLAY_ACTION.equals(action)) {
                if (isPlaying()) {
                    pause();
                    mPausedByTransientLossOfFocus = false;
                } else {
            ...
            /// @}
                ...
                int[] appWidgetIds = intent.getIntArrayExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS);
                mAppWidgetProvider.performUpdate(MediaPlaybackService.this, appWidgetIds);
            } //add JSLEL_239 gaoxueyan  20140613 (start)
            else if (CLOSE_ACTION.equals(action)){
                pause();
                mPausedByTransientLossOfFocus = false;
                stopForeground(true);
            }
            //add JSLEL_239 gaoxueyan 20140613 (end)
        ...
        synchronized (this) {
           final long short_songe_length = 10000;
            ...
            if (mPlayer.isInitialized() && mIsPlayerReady) {
             ...
             // add BUG_ID:JSLEL- 239 gaoxueyan start 
                updateNotification(this, null, true);
				//add BUG_ID:JSLEL- 239 gaoxueyan end 
                if (!mIsSupposedToBePlaying) {
     ...
     */
    //add JSLEL-239 gaoxueyan  20140613 (start)
    private void updateNotification(Context context, Bitmap bitmap, boolean playing) {
        //add JSLEL-239 gaoxueyan 20140613 (end)
        ...
        //modify JSLEL-239 gaoxueyan20140613 (start)
        //intent = new Intent(PAUSE_ACTION);
        intent = new Intent(PAUSE_PLAY_ACTION);
        //modify JSLEL-239 gaoxueyan 20140613 (end)
        intent.setClass(context, MediaPlaybackService.class);
        pIntent = PendingIntent.getService(context, 0, intent, 0);
      ...
      ////add  JSLEL-239 gaoxueyan20140613 (end)    
        ...
        status.contentIntent = PendingIntent.getActivity(context, 0, intent, 0);
		 status.tickerText = getTrackName(); // silongjun import for smart cover
    ...
    private void gotoIdleState() {
      ...
      //  } else {
        	 //add  JSLEL-239 gaoxueyan20140613 (start) 
            //stopForeground(true);
            updateNotification(this, bm, false);
            //add  JSLEL-239 gaoxueyan 20140613 (end)
      //  }
        /// @}
    ...
    private class AlbumArtWorker extends AsyncTask<Long, Void, Bitmap> {
        ...
        protected void onPostExecute(Bitmap bm) {
            ...
            if (mIsSupposedToBePlaying) {
                ...
                ed.putLong(MediaMetadataRetriever.METADATA_KEY_DURATION, duration());
                //modify bug_id:DWYLL-461 gaoxueyan 20150508 start
                if(getResources().getBoolean(R.bool.isD5116_infinix))
                {  if (bm != null) {
                   ed.putBitmap(MetadataEditor.BITMAP_KEY_ARTWORK, bm);
               }
                }
              //modify bug_id:DWYLL-461 gaoxueyan 20150508 end
                ed.apply();
                /// 2. update notification album art
              //modify BUG-ID:JSLEL-239 gaoxueyan start
                updateNotification(MediaPlaybackService.this, bm, true);
           	//modify BUG-ID:JSLEL-239 gaoxueyan end
            }
        }
    }
    ...
    // add BUG_ID:JEYS-570 chenchunyong 20140307 (start)
    public void notifyFmMusicWidget(){	
	     int progress = 0;
	     long pos = position();
	   
        long duration = duration();
        Log.i("gxy", ">>notifyFmMusicWidget");
        if(duration!=0){
            if(pos>=duration()){
                pos=duration();
             }
            progress = (int) (1000 * pos / duration);
        }   
	     Intent intent = new Intent("com.mediatek.FMRadio.widget.music.refresh");
	     intent.putExtra("bitmap", getAlbumId());
	     intent.putExtra("progress", progress);
	     intent.putExtra("artist", getArtistName());
	     intent.putExtra("track", getTrackName());
	     intent.putExtra("playing", isPlaying());
	     intent.putExtra("totaltime", MusicUtils.makeTimeString(this, duration()/ 1000));
	     intent.putExtra("currenttime", MusicUtils.makeTimeString(this, pos/ 1000));
	     //if(mIsNeedNotifyWidget){
	    	 sendBroadcast(intent);
	    // }
	     mMediaplayerHandler.removeMessages(WIDGET_REFRESH);
	     if(isPlaying()){
	    	 mMediaplayerHandler.sendEmptyMessageDelayed(WIDGET_REFRESH, 1000);
	    	 Log.i("gxy", "notifyFmMusicWidget:progress="+progress);
	     }
    }
	// add BUG_ID:JEYS-570 chenchunyong 20140307 (end)
}
