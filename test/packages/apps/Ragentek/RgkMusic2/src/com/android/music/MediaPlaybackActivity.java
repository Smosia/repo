...
//    private ImageView mNextButton;
    ...
    private Drawable drawableAlbum = null;
    ...
    /**@}**/
    //A: JWBLB-1132 liuhao 20150722{
    private static final String NON_TOUCH_GESTURE_MUSIC_ENABLE = "non_touch_gesture_music_enable";
    ...
    private SensorEventListener mSensorEventListener = new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
			boolean nonTouchGestureEnable = Settings.Secure.getInt(
					getContentResolver(), 
					NON_TOUCH_GESTURE_MUSIC_ENABLE, 
					0) != 0;
			if(!nonTouchGestureEnable) return;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        switch (requestCode) {
            case NEW_PLAYLIST:
                ...
                break;
            //add DWYSLM-227 guowen 20160407 start
            case REQUEST_SIM_SELECT:
                if (resultCode == RESULT_OK) {
                    mSimId = intent.getLongExtra(EXTRA_SLOTID, -1);
                    if(mService != null) {
                        try {
                            MusicUtils.setRingtone(this, mService.getAudioId(), mSimId);
                        } catch (RemoteException ex) {
                            MusicLogUtils.e(TAG, "Can not get the audioId!");
                        }
                    } else {
                        mHandler.sendEmptyMessageDelayed(SET_RINGTONE, 500);
                    }
                }
                break;
            //add DWYSLM-227 guowen 20160407 end
...
*/
    ...
    private void toggleShuffle() {
        ...
        try {
            ...
            if (shuffle == MediaPlaybackService.SHUFFLE_NONE) {
                ...
                //modify bug_id:JWYYY-435 gaoxueyan 20141218 start
               invalidateOptionsMenu();
               // showToast(R.string.shuffle_off_notif);
             //modify bug_id:JWYYY-435 gaoxueyan 20141218 end
              //modify BUG_ID:JWYYY-130 gaoxueyan 20141217 start
				if(shuffle == MediaPlaybackService.SHUFFLE_AUTO)
				{
				showToast(R.string.party_shuffle_off_notif);
				}
				else
				{
                showToast(R.string.shuffle_off_notif);
				}
				//modify BUG_ID:JWYYY-130 gaoxueyan 20141217 end
            } else {
    ...
    private ServiceConnection osc = new ServiceConnection() {
			...
			//try{
				...
				getActionBar().setSubtitle(mService.getArtistName());
        		//	}catch(Exception e){
    ...
    private void setPauseButtonImage() {
		...
		mPauseButton.setImageResource(R.drawable.rgk_playback_pause_stat);
		...
		mPauseButton.setImageResource(R.drawable.rgk_playback_play_stat);
            }
        } catch (RemoteException ex) {
        }
    }
    ...
    //add DWYSLM-227 guowen 20160407 start
    private static final int SET_RINGTONE = 8;
    //add DWYSLM-227 guowen 20160407 end
    ...
    private final Handler mHandler = new Handler() {
		...
		mAlbum.setBackground((Drawable) msg.obj);
		//mAlbum.setBackground(msg.obj);
		//mAlbum.setBackgroundResource((Integer) msg.obj);
                    break;
                ...
                case SET_RINGTONE:
                    if(mService != null) {
                        try {
                            MusicUtils.setRingtone(MediaPlaybackActivity.this, mService.getAudioId(), mSimId);
                        } catch (RemoteException ex) {
                            MusicLogUtils.e(TAG, "Can not get the audioId!");
                        }
                    }else {
                        MusicLogUtils.e(TAG, "Can not setRingtone: mService==null");
                    }
                    break;
                //add DWYSLM-227 guowen 20160407 end
    ...
    private void updateTrackInfo() {
		...
		((View) mArtistName.getParent()).setVisibility(View.GONE);
		((View) mAlbumName.getParent()).setVisibility(View.GONE);
    ...
    public class AlbumArtHandler extends Handler {
		...
		if (drawableAlbum == null || mArtSongId != songid) {
		    ...
		    drawableAlbum = new BitmapDrawable(mArtBitmap);
		    ...
		    numsg = mHandler.obtainMessage(ALBUM_ART_DECODED, drawableAlbum);
                    mHandler.removeMessages(ALBUM_ART_DECODED);
    ...
    private void updateUI() {
        ...
        v.setOnTouchListener(this);
        v.setOnLongClickListener(this);
//modify  bug_id:DWYBL-1191 gaoxueyan 201560421 start
	...
	//mNextButton.setOnClickListener(mNextListener);
        seekmethod = 1;
      //modify  bug_id:DWYBL-1191 gaoxueyan 201560421 end
    ...
    private void setRingtone() {
        ...
        if(SystemProperties.get("ro.mtk_multisim_ringtone").equals("1") && simInfos > 1) {
            Intent intent = new Intent();
            intent.setAction(INTENT_CARD_SELECT);
            startActivityForResult(intent, REQUEST_SIM_SELECT);
        }else if (simInfos == 1) {
            mSimId = SubscriptionManager.from(this).getActiveSubscriptionIdList()[0];
            try {
                MusicUtils.setRingtone(this, mService.getAudioId(), mSimId);
            } catch (RemoteException ex) {
                MusicLogUtils.e(TAG, "Can not get the audioId!");
