    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_UPDATE_WIDGET.equals(action)) {
            ...
            if (widgetsNotConfig.size() > 0) {
                for (Integer notConfigWidgetId : widgetsNotConfig) {
                    updateNotConfigWidget(context, notConfigWidgetId);
                }
            }
...
*/
            ...
            updateNotConfigWidget(context, appWidgetId);
            return;
        } else {
    ...
    private void updateNotConfigWidget(Context context, int appWidgetId) {
        final RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.widget);
        // Widget has not been configured yet
        remoteViews.setViewVisibility(R.id.widget_folder, View.GONE);
        remoteViews.setViewVisibility(R.id.widget_compose, View.GONE);
        ...
        PendingIntent clickIntent = PendingIntent.getActivity(context, 0, configureIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        remoteViews.setOnClickPendingIntent(R.id.widget_configuration, clickIntent);
        LogUtils.d(LOG_TAG, "BaseWidgetProvider.updateNotConfigWidget invalid handling, widgetId %d"
                , appWidgetId);
