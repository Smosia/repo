...
//A:A:A:ADD BUG ID :DWYSLM-131 liupeng 20160315 {
...
public class GeneralPrefsFragment extends MailPreferenceFragment
    ...
    //A:A:ADD BUG ID :DWYSLM-131 liupeng 20160315 {
 	private static final String PREFERENCE_KEY_CONFIRM_DELETE_SIGNATURE = "confirm_delete_signature";
	private boolean mSignatureEnable = false;
    private Preferences mPreferences;
	private CheckBoxPreference mConfirmDeleteSignature;
	//A}
    @Override
    ...
    public void onResume() {
		...
		mSignatureEnable = context.getResources().getBoolean(R.bool.signature_enable);
		//A}
        loadMtkSettings();
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (PERFERENCE_KEY_VIP_MEMBERS.equals(key)) {
            ...
            return true;
        }//A:ADD BUG ID :DWYSLM-131 liupeng 20160315 (start) {
		else if (mSignatureEnable && PREFERENCE_KEY_CONFIRM_DELETE_SIGNATURE.equals(key)) {
			Log.d("qinzhaojin"," mConfirmDeleteSignature.isChecked() === " + mConfirmDeleteSignature.isChecked());
			mPreferences.setConfirmDeleteSignature(mConfirmDeleteSignature.isChecked());
            return true;
        } 
		//A:ADD BUG ID :DWYSLM-131 liupeng 20160315 (end)}
        /** @} */
        return false;
    }
    ...
    private void loadMtkSettings() {
        ...
        mPreferences = Preferences.getPreferences(getActivity());
     	mConfirmDeleteSignature = (CheckBoxPreference) findPreference(PREFERENCE_KEY_CONFIRM_DELETE_SIGNATURE);
     	if(!mSignatureEnable && mConfirmDeleteSignature != null){
     		getPreferenceScreen().removePreference(findPreference(PREFERENCE_KEY_CONFIRM_DELETE_SIGNATURE));
     	}
     	//A:ADD BUG ID :DWYSLM-131 liupeng 20160315 (end)}
        loadVipData();
    }
    ...
    private void setRingtoneSummary(Context context, String ringtoneUri) {
        ...
        if (!TextUtils.isEmpty(ringtoneUri)) {
            //modify DWYSEM-180 guowen 20160413 start
            if(-1 == RingtoneManager.validRingtoneUri(context,Uri.parse(ringtoneUri),RingtoneManager.TYPE_NOTIFICATION)){
