...
public class AccountSettingsFragment extends MailAccountPrefsFragment
	...
	//A:DWLEBM-327 LIUPENG 20160408 {
    private static final String PREFERENCE_DELETE = "delete_account";
    private static final String PREFERENCE_REMOVE = "remove_account";
    //A}
    ...
    private void setRingtoneSummary() {
        //add DWYSEM-180 guowen 201604013 start
        if (mRingtone != null) {
            if(-1 == RingtoneManager.validRingtoneUri(mContext,mRingtone.getUri(),RingtoneManager.TYPE_NOTIFICATION)){
                mRingtone = RingtoneManager.getRingtone(getActivity(), Settings.System.DEFAULT_NOTIFICATION_URI);
            }
        }
        //add DWYSEM-180 guowen 201604013 end
    ...
    private static class AccountLoader extends MailAsyncTaskLoader<Map<String, Object>> {
        ...
        protected void onDiscardResult(Map<String, Object> result) {
            ...
            if (account != null) {
				...
				if (isDefSignature(getContext().getResources().getStringArray(R.array.all_default_signature),accountSignature)
						&& !accountSignature.equals(getContext().getString(R.string.default_signature))) {
					account.setSignature(getContext().getString(R.string.default_signature));
					ContentValues cv = AccountSettingsUtils.getAccountContentValues(account);
					account.update(getContext(), cv);
				}
	...
	//add bug DWLEBM-4 20160309 liupeng (start)
	private static boolean isDefSignature(String[] signature, String temp) {
    ...
    */
        ...
        if (prefOutgoing != null) {
            if (mServiceInfo.usesSmtp && mAccount.mHostAuthSend != null) {
			  ...
			  //Add bug DWLEBM-327 LIUPENG 20160408(start) {
        findPreference(PREFERENCE_REMOVE).setOnPreferenceClickListener(
        		new Preference.OnPreferenceClickListener() {
			
			@Override
			public boolean onPreferenceClick(Preference arg0) {
				// TODO Auto-generated method stub
				//Toast.makeText(getActivity(), "Account name :" + mAccount.getDisplayName(), Toast.LENGTH_LONG).show();
				
				List<android.accounts.Account> amAccounts = AccountReconciler.getAllAmAccounts(mContext);
				for (final android.accounts.Account acct : amAccounts) {
					
					if (mAccount.getEmailAddress().equalsIgnoreCase(acct.name)) {
						...
						.setPositiveButton(R.string.delete_account, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dia, int which) {
								// TODO Auto-generated method stub
								AccountManager.get(getActivity()).removeAccount(acct, new AccountManagerCallback<Boolean>() {

									@Override
									public void run(AccountManagerFuture<Boolean> future) {
										// TODO Auto-generated method stub
										if (!AccountSettingsFragment.this.isResumed()) {
											return;
										}
				                        ...
				                        if (failed && getActivity() != null &&
				                        	...
				                        	Toast.makeText(getActivity(), getString(R.string.remove_account_failed) + "  " + mAccount.getDisplayName(), Toast.LENGTH_LONG).show();
				                        } else {
				                        	Toast.makeText(getActivity(), getString(R.string.remove_account_successed) + "  "+ mAccount.getDisplayName(), Toast.LENGTH_LONG).show();
				                        }
									}
								}, null);
							}
        ...
        //Add bug DWLEBM-327 LIUPENG 20160408(end)}
        }
