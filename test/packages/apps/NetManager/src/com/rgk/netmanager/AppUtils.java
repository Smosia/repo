...
public class AppUtils {
	private static ArrayList<AppDetail> apps = null;
	...
	public static ArrayList<AppDetail> getAllAppsStatsForCurrentMonth(
			Context ctx) throws NetRemoteException {
		ArrayList<AppDetail> appDetails = getApps(ctx);
		...
		for (AppDetail app : appDetails) {
			...
			app.timeType = AppDetail.MONTH;
		}
		Collections.sort(appDetails);
		return appDetails;
	}

	public static ArrayList<AppDetail> getAllAppsHasStatsForOneDay(Context ctx,
			long dayStart, long dayEnd) throws NetRemoteException {
		ArrayList<AppDetail> appDetails = getApps(ctx);
		ArrayList<AppDetail> hasStatsApps = new ArrayList<AppDetail>();
		int selected_mobile[] = getUidsWithMobile(ctx);
		int selected_wifi[] = getUidsWithWifi(ctx);
		// long[] time = getTime(DAY_TYPE, date);
		Log.d(NetApplication.TAG, "AppUtil start " + dayStart + ", end "
				+ dayEnd);
		NetAdapter netAdapter = NetAdapter.getInstance(ctx);
		// netAdapter.forceUpdate();
		ArrayList<NetAdapter.AppItem> appItemsWithMobile1 = netAdapter
		...
		for (AppDetail app : appDetails) {
			...
			appDetail.timeType = AppDetail.DAY;
        ...
        public static ArrayList<AppDetail> getMobileAllAppsHasStatsForOneDay(Context ctx,
        ...
        public static ArrayList<AppDetail> getWifiAllAppsHasStatsForOneDay(Context ctx,
			long dayStart, long dayEnd) throws NetRemoteException {
		ArrayList<AppDetail> appDetails = getApps(ctx);
		ArrayList<AppDetail> hasStatsApps = new ArrayList<AppDetail>();
		int selected_mobile[] = getUidsWithMobile(ctx);
	 ...
	 */
	public static ArrayList<AppDetail> getApps(Context ctx) {
		...
		try {
			...
			final HashMap<Integer, AppDetail> map = new HashMap<Integer, AppDetail>();
			String name = null;
			...
			for (AppDetail application : map.values()) {
