...
public class NetAdapter {
	...
	Log.d(NetApplication.TAG, "NetAdapter [getInstance]");
	if (mInstance == null) {
	...
	mContext = context;
        ...
        try {
            ...
            Log.d(NetApplication.TAG, "exception happen: " + e + " , so finish current activity");
        } catch (Exception e) {
            Log.d(NetApplication.TAG, "exception happen: " + e);
        }
    }
...
*/
	...
	Log.d(NetApplication.TAG, "[setFirewallEnabled] enabled "+enabled);
        try {
            mNetworkService.setFirewallEnabled(enabled);
        } catch (RemoteException e) {
            Log.w(NetApplication.TAG, "problem talking with INetworkManagementService: " + e);
	...
	Log.d(NetApplication.TAG, "[setFirewallUidChainRule] uid "+uid+",networkType "+networkType+",allow "+allow);
        try {
	...
	NetworkTemplate networkTemplate = NetworkTemplate.buildTemplateMobileAll(subscriberId);
        ...
        try {
            ...
            Log.w(NetApplication.TAG, "problem taking with INetworkManagementService: " + e);
    ...
    public void advisePersistThreshold(long thresholdBytes) {
        try {
            ...
            //throw new NetRemoteException(e.toString());
            Log.d(NetApplication.TAG, "RemoteException when advisePersistThreshold()");
        }
    }
