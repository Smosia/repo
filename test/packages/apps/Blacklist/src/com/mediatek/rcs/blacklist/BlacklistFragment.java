...
public class BlacklistFragment extends ListFragment
    ...
    //add requestPermission start
	final private int REQUEST_CODE_ASK_PERMISSIONS = 111;
	int mRequestCode, mResultCode;
    //add requestPermission end
    @Override
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        //add requestPermission start
        int hasWriteContactsPermission = getActivity().checkSelfPermission(Manifest.permission.WRITE_CONTACTS);
        ...
        switch (requestCode) {
            case REQUEST_CODE_ASK_PERMISSIONS:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (mAddTask.getStatus() != AsyncTask.Status.RUNNING) {
                        mAddTask.execute(mRequestCode, mResultCode);
                    }
                } else {
                    // Permission Denied
