...
public class VcsController {
            ...
            AsyncTask<ArrayList<String>, Void, Cursor> task = new AsyncTask<ArrayList<String>,
                ...
                protected void onPostExecute(Cursor cursor) {
                    ...
                    if (dialog != null && dialog.isShowing()
                            && getTabPositionForTextDirection(mActionBarAdapter.getCurrentTab()) != TabState.ALL) {
                        dialog.dismiss();
                    }
                    if (dialog != null && dialog.isShowing()) {
        ...
        public void onSpeechDetected() {
            ...
            if (!mVoiceSearchDialogFragment.isShowing() && (mContactsCount > 0)
                ...
                Log.w(TAG, "[onSpeechDetected] not show Dialog,mContactsCount:"
                        + mContactsCount + ",tab:" + getTabPositionForTextDirection(mActionBarAdapter.getCurrentTab())
    ...
    private VoiceSearchDialogFragment.VoiceDialogListener mVoiceDialogListener =
        ...
        public void onRefreshDone() {
            ...
            Log.d(TAG, "[refreshDone][vcs] table: " + getTabPositionForTextDirection(mActionBarAdapter.getCurrentTab()));
        }
    ...
    private boolean checkStartVcsCondition() {
        int tab = mActionBarAdapter == null ? TabState.DEFAULT : getTabPositionForTextDirection(mActionBarAdapter.getCurrentTab());
    ...
    public void dispatchTouchEventVcs(MotionEvent ev) {
        ...
        if (VcsUtils.isVcsFeatureEnable()) {
            ...
            if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_CANCEL) {
                if (getTabPositionForTextDirection(mActionBarAdapter.getCurrentTab()) == TabState.ALL) {
                    mVoiceSearchManager.disableVoice();
                }
    ...
    public void onPageSelectedVcs() {
        if (VcsUtils.isVcsFeatureEnable() && getTabPositionForTextDirection(mActionBarAdapter.getCurrentTab()) == TabState.ALL) {
