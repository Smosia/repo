...
public class MultiSelectContactsListFragment extends DefaultContactBrowseListFragment
    ...
    protected boolean onItemLongClick(int position, long id) {
        ...
        if (uri != null && (position > 0 || !getAdapter().hasProfile())
                && !getAdapter().isSdnNumber(position) && !getAdapter().isReadOnlyNumber(position)) {
    ...
    protected void onItemClick(int position, long id) {
        ...
        if (getAdapter().isDisplayingCheckBoxes() && !getAdapter().isSdnNumber(position) && !getAdapter().isReadOnlyNumber(position)) {
            final String contactId = uri.getLastPathSegment();
            if (!TextUtils.isEmpty(contactId)) {
                getAdapter().toggleSelectionOfContactId(Long.valueOf(contactId));
            }
    ...
    public void updateCheckBoxState(boolean checked) {
        ...
        /// @}
                if (getAdapter().isUserProfile(position) || getAdapter().isSdnNumber(position) || getAdapter().isReadOnlyNumber(position)) {
                    continue;
                }
