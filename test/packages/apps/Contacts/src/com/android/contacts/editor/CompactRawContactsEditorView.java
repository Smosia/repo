...
public class CompactRawContactsEditorView extends LinearLayout implements View.OnClickListener {
    ...
    private void addAccountInfo(RawContactDeltaList rawContactDeltas, boolean hasNewContact,
        ...
        android.util.Log.i("jingyi", accountInfo.second+ accountInfo.first);
        if (getResources().getBoolean(R.bool.show_account_type_sim1_sim2)) {
            int subId = AccountTypeUtils.getSubIdBySimAccountName(getContext(),
                    accountInfo.first);
            int slot = SubInfoUtils.getSlotIdUsingSubId(subId);
            android.util.Log.i("jingyi", "subId=" + subId + " slot="+slot);
            if (slot == 0) {
            	mAccountTypeView.setText(getContext().getString(R.string.account_type_format, "SIM1"));
            } else if (slot == 1){
            	mAccountTypeView.setText(getContext().getString(R.string.account_type_format, "SIM2"));
            }
        }
