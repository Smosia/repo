...
public class GroupBrowseListAdapter extends BaseAdapter {
    ...
    private void bindHeaderView(GroupListItem entry, GroupListItemViewCache viewCache) {
        ...
        if (mContext.getResources().getBoolean(R.bool.show_account_type_sim1_sim2)) {
	        if (accountType.isIccCardAccount()) {
	    		int subId = AccountTypeUtils.getSubIdBySimAccountName(mContext,
	    				entry.getAccountName());
	    		int slot = SubInfoUtils.getSlotIdUsingSubId(subId);
	            if (slot == 0) {
	            	viewCache.accountType.setText("SIM1");
	            } else {
	            	viewCache.accountType.setText("SIM2");
	            }
	        }
        }
