...
public class QuickContactActivity extends ContactsActivity {
    ...
    private final OnCreateContextMenuListener mEntryContextMenuListener =
        ...
        public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
            ...
            if (Phone.CONTENT_ITEM_TYPE.equals(info.getMimeType()) &&
                ...
                if (!getResources().getBoolean(R.bool.disable_ip_call)) {
    ...
    private final String DB_SMART_CALL = "smart_call";
    private final SensorCallback mSensorCallback = new SensorCallback(){
        ...
        public void onCalling() {
            if (!TextUtils.isEmpty(mPhoneNumber)) {
                Log.d(TAG, "onCalling - mPhoneNumber is " + mPhoneNumber + ".");
                TelephonyManager telephony = (TelephonyManager)getSystemService(Context.TELEPHONY_SERVICE);
                if (telephony != null && telephony.isVoiceCapable() && telephony.isIdle()) {   		
                    final Intent intent = new Intent(Intent.ACTION_CALL_PRIVILEGED, Uri.fromParts("tel", mPhoneNumber, null));
                    intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    startActivity(intent);
                } else {
                    Log.d(TAG, "onCalling - telephony is not idle.");
                }
            } else {
                Log.d(TAG, "onCalling - the phone number is empty.");
            }
		}
        public void onRotate() {}
    };

    private boolean getSmartCallState() {
        return Settings.System.getInt(getContentResolver(), DB_SMART_CALL, -1) == 1;
    }
    //A: kuangyunfeng 20160112(end)
    ...
    private void setHeaderNameText(String value) {
        if (!TextUtils.isEmpty(value)) {
            if (mScroller != null) {
                //add DWYYLM-8 liujingyi 20151125 (start)
                if (isNumber(value)) {
        	        value = numberLeftToRight(value);
                }
               //add DWYYLM-8 liujingyi 20151125 (end)
    ...
    private boolean isNumber(String text) {
    	...
    	for (int i = 0; i < length; i++) {
    		is = text.charAt(i) >= '0' && text.charAt(i) <= '9' || 
    ...
    protected void onResume() {
        ...
        if(mSensorListener != null) {
            if(getSmartCallState()){
                Log.d(TAG, "Set sensor enable for Smart Call.");
                mSensorListener.enable(true);
            }
        }
        //A: kuangyunfeng 20160112(end)
    ...
    protected void onPause() {
        super.onPause();
        if(mSensorListener != null) {
            Log.d(TAG, "Set sensor disable for Smart Call.");
            ...
            if (intent != null) {
                ...
                switch (mimetype) {
                    ...
                    case MIMETYPE_HANGOUTS:
                        ...
                        break;
                    default:
                        if(kind.resourcePackageName.compareTo("com.whatsapp") == 0) {
                            header = textNumberLeftToRight(header);
                        }
    ...
    //add DWYYLM-8 liujingyi 20151125 (start)
    private static String numberLeftToRight(String origin) {
        return TextUtils.isEmpty(origin) ? origin : '\u202D' + origin + '\u202C';// or '\u202A' , '\u202C';
    }
    //add DWYYLM-8 liujingyi 20151125 (end)
    
	private static String textNumberLeftToRight(String text) {
        int start = text.indexOf('+');
        String number = text.substring(start);
		String numberTemp = numberLeftToRight(number);
	    Log.i("jingyi", text.substring(0, start) + " number="+number+" text.substring(0, start)=");
    ...
    private boolean isContactEditable() {
        return mContactData != null && !mContactData.isDirectoryEntry() &&
            ...
            //add preset contacts(start)
            !mContactData.isReadOnlyContact();
            //add preset contacts(end)
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mContactData != null) {
            ...
            /** @} */
            //add black list
            ExtensionManager.getInstance().getQuickContactExtension().onPrepareOptionsMenu(this, menu, mContactData.getNameRawContactId());
            //add black list end
            return true;
        }
        return false;
