...
public class LauncherProvider extends ContentProvider {
    ...
    synchronized public void loadDefaultFavoritesIfNecessary() {
        ...
        if (sp.getBoolean(EMPTY_DATABASE_CREATED, false)) {
            ...
            if (loader == null) {
                //Add BUG_ID:none zhaopenglin 20150610(start)
            	if(getContext().getResources().getBoolean(R.bool.support_more_defaultworkspace))
            	{
            		loader = getDefaultWorkspace();
            	}else
            	//Add BUG_ID:none zhaopenglin 20150610(end)
	...
	//Add BUG_ID:none zhaopenglin 20150610(start)
    private DefaultLayoutParser getDefaultWorkspace() {
    	DefaultLayoutParser myDefaultLayout=null;
		int id = 0;
		...
		if (null != cursor) {
                ...
                if ("country".equals(name)) {
					...
					if (country != null) {
						if (country.equalsIgnoreCase("Nigeria")) {
							id = R.xml.default_workspace_ni;
						} else if (country.equalsIgnoreCase("Saudi Arabia")||country.equalsIgnoreCase("Egypt")
                        ||country.equalsIgnoreCase("Arab emirates")) {
							id = R.xml.default_workspace_ar;
						}else if (country.equalsIgnoreCase("Kenya")) {
							id = R.xml.default_workspace_ke;
						}else if (country.equalsIgnoreCase("Pakistan")) {
							id = R.xml.default_workspace_pkst;
						}
						break;
					}
				}
			...
			cursor.close();
			myDefaultLayout = new DefaultLayoutParser(getContext(), mOpenHelper.mAppWidgetHost,
		                mOpenHelper, getContext().getResources(), id);
		}
		if (id == 0) {
			myDefaultLayout = getDefaultLayoutParser();
		}
		return myDefaultLayout;
	}   
    //Add BUG_ID:none zhaopenglin 20150610(end)
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			//Modify BUG_ID:DWYYL-2142 zhaopenglin 20150909(start)
            if (version < 21) {
            ...
            //Modify BUG_ID:DWYYL-2142 zhaopenglin 20150909(end)
        }
