...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    private static boolean mIconWithFrame = false;//Add BUG_ID:DWYYL-1587 zhaopenglin 20150611
    ...
    private static class PendingInstallShortcutInfo {
        ...
        public String encodeToString() {
            ...
            //Modify BUG_ID:DWYYL-1587 zhaopenglin 20150611(start)
            //Bitmap icon = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_ICON);
            Bitmap icon = null;
            mIconWithFrame = mContext.getResources().getBoolean(R.bool.support_icon_frame);
            if (mIconWithFrame) {
	    			icon = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_ICON);
	    			if (icon == null) {
	    				ShortcutInfo info = getShortcutInfo();
	    				icon = info.getIcon(new IconCache(mContext));
	    			}
	    			BitmapDrawable drawable = new BitmapDrawable(icon);
					icon = Utilities.createIconBitmapWithFrame(drawable, mContext);
	            } else {
	            	icon = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_ICON);
    		}
            //Modify BUG_ID:DWYYL-1587 zhaopenglin 20150611(end)
