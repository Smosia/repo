...
//add headphone bug_id:DSYYWM-48 wanglin 20160303 start
...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    //add headphone bug_id:DSYYWM-48 wanglin 20160303 start
    static final String FM_SEEK_NEXT = "fmradio.seek.next";
    //add headphone end
    private static final String FM_TURN_OFF = "fmradio.turnoff";
    private static final String CMDPAUSE = "pause";
    //add headphone bug_id:DSYYWM-48 wanglin 20160303 start
    static final String CMDPAUSE_PLAY = "pause_play";    
    //add headphone end
    // HandlerThread Keys
    ...
    private class FmServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ((SOUND_POWER_DOWN_MSG.equals(action) && CMDPAUSE.equals(command)) || Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action)) { //A:DSYYLM-9 wanglin add ACTION_AIRPLANE_MODE_CHANGED
                // need remove all messages, make power down will be execute
                mFmServiceHandler.removeCallbacksAndMessages(null);
                //A:DSYYLM-9 wanglin {
                if(Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action)){
					Toast.makeText(FmService.this, R.string.cannot_startup_during_airplane, Toast.LENGTH_SHORT).show();
				}
                //A}
                exitFm();
                stopSelf();
                ...
                if ((0 == mValueHeadSetPlug) && isActivityForeground() ) {
                	//modify BUG_ID DWYQLSSMY-758 wanglin 20160425
    ...
    public void powerUpAsync(float frequency) {
      ...
      //add headphone bug_id:DSYYWM-48 wanglin 20160303 start
        ComponentName rec = new ComponentName(getPackageName(),
        		FmMediabuttionIntentReceviver.class.getName());
    ...
    private void saveRecording(String newName) {
        ...
        if (mFmRecorder != null) {
            if (newName != null) {
                ...
                String path = mFmRecorder.mRecordFile.getPath();
                String Filepath = path;
                if (!StorageManagerEx.getSdSwapState()) {
                	if (path.contains("emulated/0")) {
            			Filepath = path.replace("emulated/0", "phone");
            			}else {
           ...
           Toast.makeText(mContext, mContext.getResources().getString(R.string.toast_savefile_path)+Filepath, Toast.LENGTH_SHORT).show();
                }
    ...
    public void onDestroy() {
        ...
        //A: DWLEBM-244 yejianming 20160406
        mCallback = null;
        exitRenderThread();
        releaseAudioPatch();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (FM_SEEK_PREVIOUS.equals(action)) {
            	...
            	Log.d(TAG, "action="+FM_TURN_OFF);
                powerDownAsync();
                  }else if(CMDPAUSE_PLAY.equals(action)){
                	  Log.d(TAG, "action="+CMDPAUSE_PLAY);
                if(getPowerStatus()==POWER_DOWN)
                {
                    powerUpAsync(FmUtils.computeFrequency(mCurrentStation));
                }
                    ...
                    switch (focusChange) {
                        ...
                        case AudioManager.AUDIOFOCUS_GAIN:
                          ...
                          ComponentName rec1 = new ComponentName(getPackageName(),
                          		FmMediabuttionIntentReceviver.class.getName());        
                          mAudioManager.registerMediaButtonEventReceiver(rec1);
                            //}
                            synchronized (this) {
    ...
    //A: DWLEBM-244 yejianming 20160406 (start)
    IFmRadioServiceCallback  mCallback;
    private NRRadioService mRemoteBinder = new NRRadioService() ;
    public class NRRadioService extends  IFmRadioService.Stub {
        FmService getService() {
            return FmService.this;
        }
        ...
        public boolean isPowerUp() throws RemoteException {
            Log.d(TAG, "FmRadioService.isPowerUp: " + (mPowerStatus == POWER_UP));
            return mPowerStatus == POWER_UP;
        }
        ...
        @Override
        public void tuneDirection(int direction) throws RemoteException {
            mCurrentStation += direction;
            tuneStationAsync(FmUtils.computeFrequency(mCurrentStation));
        }

        @Override
        public void tuneStationAsync(float frequency) throws RemoteException {
            mFmServiceHandler.removeMessages(FmListener.MSGID_TUNE_FINISHED);
            final int bundleSize = 1;
            Bundle bundle = new Bundle(bundleSize);
            bundle.putFloat(FM_FREQUENCY, frequency);
        ...
        @Override
        public void initService(int iCurrentStation) throws RemoteException {
        ...
        @Override
        public void setMuteAsync(boolean mute) throws RemoteException {
            mFmServiceHandler.removeMessages(FmListener.MSGID_SET_MUTE_FINISHED);
            final int bundleSize = 1;
            Bundle bundle = new Bundle(bundleSize);
            bundle.putBoolean(OPTION, mute);
        ...
        @Override
        public void seekStationAsync(float frequency, boolean isUp) throws RemoteException {
            mFmServiceHandler.removeMessages(FmListener.MSGID_SEEK_FINISHED);
            final int bundleSize = 2;
            Bundle bundle = new Bundle(bundleSize);
            bundle.putFloat(FM_FREQUENCY, frequency);
        ...
        @Override
        public boolean isServiceInited() throws RemoteException {
            Log.d(TAG, "FmRadioService.isServiceInit: " + mIsServiceInited);
            return mIsServiceInited;
        }

        @Override
        public boolean isModeNormal() throws RemoteException {
        ...
        @Override
        public void powerUpAsync(float frequency) throws RemoteException {
            //mIsPowerUping = true;
            final int bundleSize = 1;
            mFmServiceHandler.removeMessages(FmListener.MSGID_POWERUP_FINISHED);
            mFmServiceHandler.removeMessages(FmListener.MSGID_POWERDOWN_FINISHED);
        ...
        @Override
        public void powerDownAsync() throws RemoteException {
            mFmServiceHandler.removeMessages(FmListener.MSGID_SCAN_FINISHED);
            mFmServiceHandler.removeMessages(FmListener.MSGID_SEEK_FINISHED);
            mFmServiceHandler.removeMessages(FmListener.MSGID_TUNE_FINISHED);
            mFmServiceHandler.removeMessages(FmListener.MSGID_POWERDOWN_FINISHED);
