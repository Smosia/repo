...
public class FmStation {
     ...
     * @param values The new station's values
     */
    public static void updateFreAndNameToDb(Context context, String stationName, int oldStationFreq,
            int newStationFreq) {
        final int size = 3;
        ContentValues values = new ContentValues(size);
        values.put(Station.STATION_NAME, stationName);
        values.put(Station.FREQUENCY, newStationFreq);
        context.getContentResolver().update(
                Station.CONTENT_URI,
                values,
                Station.FREQUENCY + "=?  " ,
                new String[] {
                        String.valueOf(oldStationFreq)
                });    
    }
