...
//add DWQBLJ-288 songqingming 20150910 (start)
import com.mediatek.contacts.simcontact.SubInfoUtils;
//add DWQBLJ-288 songqingming 20150910 (end)
...
public class ContactsCommonListUtils {
    ...
    public static void addToAccountFilter(Context context, AccountWithDataSet account,
        ...
        if (context.getResources().getBoolean(R.bool.show_account_type_sim1_sim2)) {
            int slot = SubInfoUtils.getSlotIdUsingSubId(subId);
            accountFilters.add(ContactListFilter.createAccountFilter(
                    account.type, account.name, account.dataSet, icon, displayName, slot));
        } else {
    ...
    private static Cursor loadReadOnlySDN(Context context,
            ProfileAndContactsLoader profileAndContactsLoader) {
        Log.d(TAG, "[loadSDN]...");
        Cursor sdnCursor = null;
        if (null != profileAndContactsLoader.getSelection()
                && profileAndContactsLoader.getSelection().indexOf(
                        RawContacts.IS_SDN_CONTACT + " != -2") >= 0) {
            Uri uri = profileAndContactsLoader.getUri();
            String[] projection = profileAndContactsLoader.getProjection();
            String newSelection = profileAndContactsLoader.getSelection().replace(
                    RawContacts.IS_SDN_CONTACT + " != -2", RawContacts.IS_SDN_CONTACT + " = -2");
            String[] selectionArgs = profileAndContactsLoader.getSelectionArgs();
            String sortOrder = profileAndContactsLoader.getSortOrder();
            sdnCursor = context.getContentResolver().query(uri, projection, newSelection,
                    selectionArgs, sortOrder);
    ...
    public static int addCursorAndSetSelection(Context context,
        ...
        //show readonly contact start
        if (context.getResources().getBoolean(R.bool.show_read_only_contact_on_top)) {
        	Cursor readonlyCursor = loadReadOnlySDN(context, profileAndContactsLoader);
	        int readonlyContactCount = 0;
	        if (readonlyCursor != null) {
	            readonlyContactCount = readonlyCursor.getCount();
	            cursors.add(readonlyCursor);
	        }
	        profileAndContactsLoader.setReadOnlyContactCount(readonlyContactCount);
        }
       //show readonly contact end
