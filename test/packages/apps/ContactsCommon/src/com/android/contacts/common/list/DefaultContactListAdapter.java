...
public class DefaultContactListAdapter extends ContactListAdapter {
    ...
    protected void configureSelection(
        ...
        switch (filter.filterType) {
            case ContactListFilter.FILTER_TYPE_ALL_ACCOUNTS: {
                ...
                // filter
                /** M: New Feature SDN. */
                if (SystemProperties.getInt("ro.rgk_disable_sdn_filter", 0) == 1) {
                    selection.append(RawContacts.IS_SDN_CONTACT + " < 2"); 
                } else {
                    selection.append(RawContacts.IS_SDN_CONTACT + " < 1");
                }
                //show readonly contact
                if (getContext().getResources().getBoolean(R.bool.show_read_only_contact_on_top)) {
    ...
    public void updateIndexer(Cursor cursor) {
        ...
        //show readonly contact start
        if (mSDNLoader != null) {
            if (mSDNLoader.getReadOnlyContactCount() > 0) {
                sectionIndexer.setSdnHeader("ECC", mSDNLoader.getReadOnlyContactCount());
            }
        }
        //show readonly contact end
    }
    ...
    private void buildSelectionForFilterAccount(ContactListFilter filter, StringBuilder selection,
        ...
        if (AccountTypeUtils.ACCOUNT_TYPE_LOCAL_PHONE.equals(filter.accountType)) {
            selection.append("EXISTS ("
                            ...
                            + " WHERE ( ");
            if (SystemProperties.getInt("ro.rgk_disable_sdn_filter", 0) == 0) {
                selection.append(RawContacts.IS_SDN_CONTACT + " < 1 AND ");
            }
            //show readonly contact
            if (getContext().getResources().getBoolean(R.bool.show_read_only_contact_on_top)) {
                selection.append(RawContacts.IS_SDN_CONTACT + " != -2 AND ");
            }
	...
	public void changeCursor(int partitionIndex, Cursor cursor) {
		...
		if (mOnChangeCursorListener != null) {
			mOnChangeCursorListener.changed();
		}
	}

	public void setOnChangeCursorListener(OnChangeCursorListener listener) {
		mOnChangeCursorListener = listener;
	}
	...
	private OnPinnedSectionTitleChangeListener mOnPinnedSectionTitleChangeListener;

	@Override
	protected void setPinnedSectionTitle(View pinnedHeaderView, String title) {
		super.setPinnedSectionTitle(pinnedHeaderView, title);
                if (SystemProperties.getInt("ro.rgk_contacts_letter_search", 0) == 1) {
		    ((ContactListPinnedHeaderView)pinnedHeaderView).setSectionHeaderTitle(title, 0xFFFF8F00);
		    if (mOnPinnedSectionTitleChangeListener != null) {
			mOnPinnedSectionTitleChangeListener.onPinnedSectionTitleChanged(title);
		    }
                }
	}

	public void setOnPinnedSectionTitleChangeListener(OnPinnedSectionTitleChangeListener listener) {
		mOnPinnedSectionTitleChangeListener = listener;
	}
	
	public interface OnPinnedSectionTitleChangeListener {
		void onPinnedSectionTitleChanged(String title);
	}
	//add DWYYL-863 songqingming 20150526 (end)
