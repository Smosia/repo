...
public class CallButtonPresenter extends Presenter<CallButtonPresenter.CallButtonUi>
    ...
    public void onStateChange(InCallState oldState, InCallState newState, CallList callList) {
        ...
        //add DWYQLSSMY-70 songqingming 20160324 (start)
        if (mCall != null && mCall.isConferenceCall() 
        	&& !InCallPresenter.getInstance().isRecording()
                && getAutoCallRecordValue() == 1) {
            int subId = SubscriptionManager.getDefaultSubId();
            PhoneAccountHandle accountHandle = mCall.getAccountHandle();
            if (accountHandle != null && TextUtils.isDigitsOnly(accountHandle.getId())) {
                subId = Integer.parseInt(accountHandle.getId());
            }
            int slot = SubscriptionManager.getSlotId(subId);
            String number = mCall.getNumber();
            TelecomAdapter.getInstance().startVoiceRecording(slot, number, true);
        }
        //add DWYQLSSMY-70 songqingming 20160324 (end)
    ...
    //add DWYQLSSMY-70 songqingming (start)
    private int getAutoCallRecordValue() {
        int autoRecordValue = Settings.Global.getInt(getUi().getContext().getContentResolver(),
                    Settings.Global.TELEPHONY_AUTO_CALL_RECORD_CONFIG, 0);
        Log.d(this, ">>>getAutoCallRecordValue(): autoRecordValue="+autoRecordValue);
        return autoRecordValue;
    }
    //add DWYQLSSMY-70 songqingming (end)
    ...
    public void voiceRecordClicked() {
        ...
        /*
        if (android.os.SystemProperties.getInt("ro.rgk_auto_call_record_support", 0) == 1) {
    	    if (mCall != null) {
                int subId = SubscriptionManager.getDefaultSubId();
                PhoneAccountHandle accountHandle = mCall.getAccountHandle();
                if (accountHandle != null && TextUtils.isDigitsOnly(accountHandle.getId())) {
                ...
                int slot = SubscriptionManager.getSlotId(subId);
    	        String number = mCall.getNumber();
    	        TelecomAdapter.getInstance().startVoiceRecording(slot, number, mCall.isConferenceCall());
    	    } else {
                TelecomAdapter.getInstance().startVoiceRecording();
            }
        } else {
