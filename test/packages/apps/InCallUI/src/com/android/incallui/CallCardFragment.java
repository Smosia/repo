...
//add JLYLB-190 songqingming 20151117 (start)
import android.os.Message;
//add JLYLB-190 songqingming 20151117 (end)
...
/**
    ...
    //add JLYLB-190 songqingming 20151117 (start)
    private View mVoiceRecorderContainer;
    private TextView mVoiceRecorderText;
    
    private static final int MSG_VOICE_RECORDER_START = 101;

    private final Handler recordHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_VOICE_RECORDER_START:
                    Log.d(this, "handleMessage, MSG_VOICE_RECORDER_START");
                    mVoiceRecorderText.setText(getRecordTime());
                    recordHandler.sendEmptyMessageDelayed(MSG_VOICE_RECORDER_START, 1000);
                    break;

                default:
                    Log.w(this, "recordHandler: unexpected message: " + msg);
                    break;
            }
        }
    };
    ...
    public String getRecordTime() {
        int mRecordTime = (int)((System.currentTimeMillis() 
                - TelecomAdapter.getInstance().getSampleStart())/1000);
		
        final int oneHour = 3600;
        int hour = mRecordTime / oneHour;
        final int minuteSecond = 60;
        int minute = (mRecordTime / minuteSecond) % minuteSecond;
        int second = mRecordTime % minuteSecond;
    ...
    private void initVoiceRecorderIcon(View view) {
        ...
        //add JLYLB-190 songqingming 20151117 (start)
        mVoiceRecorderContainer = view.findViewById(R.id.voiceRecorderContainer);
        mVoiceRecorderContainer.setVisibility(View.INVISIBLE);
        mVoiceRecorderText = (TextView) view.findViewById(R.id.voiceRecorderText);
        mVoiceRecorderText.setVisibility(View.INVISIBLE);
        //add JLYLB-190 songqingming 20151117 (end)
    }
    ...
    public void updateVoiceRecordIcon(boolean show) {
        ...
        //add JLYLB-190 songqingming 20151117 (start)
        mVoiceRecorderContainer.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
        mVoiceRecorderText.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
        if (show) {
            recordHandler.sendEmptyMessage(MSG_VOICE_RECORDER_START);
        } else {
            recordHandler.removeMessages(MSG_VOICE_RECORDER_START);
        }
        //add JLYLB-190 songqingming 20151117 (end)
    }
    /// @}
