...
public class CallerInfoAsyncQuery {
            ...
            try {
                ...
                if (mCallerInfo == null) {
                    ...
                    if (cw.event == EVENT_EMERGENCY_NUMBER) {
                        ...
                        /// M: add for CDMA 6M @{
                        // when plug-in ready for CMDA 6M, this will be moved to plug-in
                        // fix ALPS02263380
                        ...
                        if (!TextUtils.isEmpty(cw.number)) {
                            ...
                            if (spentTime > 1000) {
                                Log.w(this, "It spent too long to formatNumber: " + spentTime);
                            }
                            /** @} */
                        }
                    }
    ...
    public static CallerInfoAsyncQuery startQuery(int token, Context context, CallerInfo info,
        ...
        //modify DWYSLM-865 songqingming 20160503 (start)
        //if (PhoneNumberUtils.isLocalEmergencyNumber(context, info.phoneNumber)) {
        if (!isCustomEcc(info.phoneNumber) && PhoneNumberUtils.isLocalEmergencyNumber(context, info.phoneNumber)) {
        //modify DWYSLM-865 songqingming 20160503 (end)
    ...
    //add DWYSLM-865 songqingming 20160503 (start)
    private static boolean isCustomEcc(String number) {
        android.util.Log.d("sqm", "[isCustomEcc] incallui ==> number: " + number);
        if (!"1".equals(android.os.SystemProperties.get("ro.qmobile_ecc_config"))) {
            return false;
        }
    	return "15".equals(number)
    		|| "16".equals(number)
    		|| "1122".equals(number)
    		|| "1717".equals(number)
    		|| "130".equals(number)
    ...
    //add DWYSLM-865 songqingming 20160503 (end)
