...
public class DialerPhoneNumberListAdapter extends PhoneNumberListAdapter {
    ...
    private int getNumberType(PhoneAccountHandle accountHandle, CharSequence number,
        ...
        //add JLLBLS-526 songqingming 20151113 (start)
        if (!isCustomEcc(number.toString()) && PhoneNumberUtils.isEmergencyNumber(number.toString())) {
            type = NUMBER_TYPE_EMERGENCY;
        } else 
        //add JLLBLS-526 songqingming 20151113 (end)
    ...
    private boolean isCustomEcc(String number) {
        android.util.Log.d("sqm", "[isCustomEcc] dialer==> number: " + number);
