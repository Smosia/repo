...
//add BUG_ID:DSLLE-767 dushuaihui 20160317(start)
import android.content.pm.PackageManager;
import android.content.pm.PackageInfo;
//add BUG_ID:DSLLE-767 dushuaihui 20160317(end)
...
public class SpecialCharSequenceMgr {
    ...
    //add for tecno aging test
    private static final String TECNO_AGING_TEST = "*#2727#";	
    //add for intex sales test
    private static final String INTEX_SALES_TEST = "*#0748#";
    ...
    public static boolean handleChars(Context context, String input, EditText textField) {
        ...
        if (handleDeviceIdDisplay(context, dialString)
                ...
                || ExtensionManager.getInstance().getDialPadExtension().handleChars(context,
                        dialString)
                /// @}
				|| handleStartAgingTest(context, dialString)
                ) {
            return true;
        }
    ...
    static boolean handleSecretCode(Context context, String input) {	
        //add BUG_ID:DSLLE-767 dushuaihui 20160317(start)
        boolean salesHiddenCodeSupport = context.getResources().getBoolean(R.bool.teletalk_sales_hidden_code_support);
        if(salesHiddenCodeSupport && isApkExist(context, "com.teletalk.salestrackerdata.salestrackerdata")){
            if(input.equals("*#*#2581*#")){
                Intent intent = new Intent();
                intent.setClassName("com.teletalk.salestrackerdata.salestrackerdata",
                            "com.teletalk.salestrackerdata.salestrackerdata.MainActivity");
                try{
				    context.startActivity(intent);
                    return true;
                }catch(Exception e){
                    Log.i(TAG, "com.teletalk.salestrackerdata.salestrackerdata.MainActivity is not available");
                }
            }
        }
        //add BUG_ID:DSLLE-767 dushuaihui 20160317(end)
        ...
        /// @}
		//add BUG_ID:DELSLMY-5 xuemingwei 20160304(start)
	     if(input.equals("*#87#") /*|| input.equals("*#8378464#")*/ || input.equals("*#3232*#")){
	       input = "*#*#7#*#*";
	     }else if(input.equals("*#9646633#") || input.equals("*#*#74655577#*#*") || input.equals("*#435763#") || input.equals("*#9900#")){
	       input = "*#*#3646633#*#*";
		 }else if(input.equals("*#999*#")){
		   input = "*#*#999#*#*";
		 }else if(input.equals("*#888*#")){
		   input = "*#*#888#*#*";
		 }else if(input.equals("*#08#") || input.equals("*#8375#")){
		   input = "*#*#9375#*#*";
		 }else if(input.equals("*#79#")) {
            input = "*#*#9875#*#*";		   
		 }else if(input.equals("*#72537#")) {
            input = "*#*#72537#*#*";
         }else if(input.equals("*#1100#")) {
            input = "*#*#4636#*#*";
         }
		//add BUG_ID:DELSLMY-5 xuemingwei 20160304(end)
        ...
        if (len > 8 && input.startsWith("*#837") && input.endsWith("8464#")) {
            final Intent intent = new Intent(SECRET_CODE_ACTION,
                    Uri.parse("android_secret_code://" + input.substring(2, len - 1)));
            context.sendBroadcast(intent);
            return true;
        }		
		//add BUG_ID:DWYQLSSMY-53 huangerhui 20160321 (start)
        if (context.getResources().getBoolean(R.bool.config_to_control_sar_display)) {
            if (input.equals("*#07#")) {
                String message = context.getResources().getString(R.string.sar_meaasge);
        ...
        //add BUG_ID:DWYQLSSMY-53 huangerhui 20160321 (end)
        //add BUG_ID:DWYQLSSB-37 chenshu 20160531 start
        if (android.os.SystemProperties.getBoolean("persist.sys.intex.sales", false)) {
            if (input.equals(INTEX_SALES_TEST)) {
                Intent intent = new Intent();
                intent.setClassName("com.chenlong.smsservice",
                            "com.chenlong.smsservice.MainActivity");
                try{
				    context.startActivity(intent);
                    return true;
                }catch(Exception e){
        ...
        //add BUG_ID:DWYQLSSB-37 chenshu 20160531 end
        return false;
    }
	
    //add BUG_ID:DSLLE-767 dushuaihui 20160317(start)
    private static boolean isApkExist(Context ctx, String packageName){
        PackageManager pm = ctx.getPackageManager();
        PackageInfo packageInfo = null;
        Log.i(TAG, "isApkExist - packageName = " + packageName);
        try {
            packageInfo = pm.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES);
            Log.i(TAG, "isApkExist - packageInfo = " + packageInfo);
        } catch (PackageManager.NameNotFoundException e) {
            Log.i(TAG, packageName + " is not found.");
            return false;
        }
    ...
    static boolean handlePinEntry(final Context context, final String input) {
	    ...
	    static boolean handleStartAgingTest(Context context, String input) {
        if(context.getResources().getBoolean(R.bool.has_tecno_aging_test) && input.equals(TECNO_AGING_TEST)) {
            Intent intent = new Intent();
            intent.setClassName("com.example.jintest", "com.example.jintest.MainActivity");
            context.startActivity(intent);
            return true;
        }       
        return false;
