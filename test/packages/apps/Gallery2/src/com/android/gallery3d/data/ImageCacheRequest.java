...
abstract class ImageCacheRequest implements Job<Bitmap> {
	...
	protected boolean isScreenShotCover(GalleryApp application, MediaData data,
			Path path) {
		if (data == null) {
		...
		Path mediaSetPath = FancyHelper.getMediaSetPath(data);
		MediaSet mediaSet = application.getDataManager().getMediaSet(
				mediaSetPath);
		if (mediaSet == null) {
		...
		if (!albumName.equals(application.getResources().getString(
				R.string.folder_screenshot))) {
			return false;
		}
		if (mediaSet.getCoverMediaItem() != null
				&& mediaSet.getCoverMediaItem().getPath()
						.equalsIgnoreCase(path.toString())
				&& data.mediaType != MediaType.CONTAINER) {
			Log.d(TAG, "<isScreenShotCover> " + data.filePath
					+ " is ScreenShotCover");
			return true;
		}
		return false;
    ...
    public Bitmap resizeAndCropFancyThumbnail(Bitmap bitmap, MediaItem item,
        ...
        //////////////////add////////////////
        boolean isScreenShotCover = isScreenShotCover(mApplication, data, item.getPath());
