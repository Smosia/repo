...
public class DateTimeSettings extends SettingsPreferenceFragment
    ...
    private static int AUTO_TIME_GPS_INDEX = 1;
    ...
    private void removeTimeGps(){
        CharSequence[] listEntries = mAutoTimePref.getEntries();
        CharSequence[] listEntryValues = mAutoTimePref.getEntryValues();
        CharSequence[] listEntriesNew = new CharSequence[listEntries.length-1];
        CharSequence[] listEntryValuesNew = new CharSequence[listEntries.length-1];

        for(int i=0;i<listEntriesNew.length;i++){
            listEntriesNew[i] = listEntries[i<1?i:i+1];
            listEntryValuesNew[i] = listEntryValues[i<1?i:i+1];
        }
        mAutoTimePref.setEntries(listEntriesNew);
        mAutoTimePref.setEntryValues(listEntryValuesNew);
        AUTO_TIME_GPS_INDEX = 2;
    ...
    private void initUI() {
        ...
        //add DELSLMY-780 yejianming 20160315 (start)
        boolean removeGps = getResources().getBoolean(R.bool.config_remove_time_gps);
        if (removeGps) {
            removeTimeGps();
        }
        //add DELSLMY-780 yejianming 20160315 (end)
        ...
        //M: DELSLMY-780 yejianming 20160315
        boolean autoTimeGpsEnabled = getAutoState(Settings.System.AUTO_TIME_GPS) && !removeGps;
        if (autoTimeEnabled) {
        ...
        mDateFormat = (ListPreference)findPreference(KEY_DATE_FORMAT); //add BUG_ID:DWYYLM-42  liqiang 20151229  
        
        if (isFirstRun) {
            getPreferenceScreen().removePreference(mTime24Pref);
            getPreferenceScreen().removePreference(mDateFormat);//add BUG_ID:DWYYLM-42  liqiang 20151229 
        }
        //add BUG_ID:DWYYLM-42  liqiang 20151229 (start)
        ...
        Log.v("DateTime", "formattedDates"+formattedDates);
    ...
    public void onResume() {
        ...
        updateDateFormatEntries(); //add BUG_ID:DWYYLM-42  liqiang 20151229 
    }
    ...
    public void updateTimeAndDateDisplay(Context context) {
       ...
       // mDatePref.setSummary(DateFormat.getLongDateFormat(context).format(now.getTime()));
        mDatePref.setSummary(DateFormat.getDateFormat(context).format(now.getTime()));
        mDateFormat.setSummary(DateFormat.getDateFormat(context).format(dummyDate));
        //add BUG_ID:DWYYLM-42  liqiang 20151229 (end)
    ...
    static void configureDatePicker(DatePicker datePicker) {
        ...
        t.clear();
	 //modify DELSLMY-715 liujia 20160315 (start)
       // t.set(2037, Calendar.DECEMBER, 31);
        t.set(2036, Calendar.DECEMBER, 31);
	   //modify DELSLMY-715 liujia 20160315 (end)
