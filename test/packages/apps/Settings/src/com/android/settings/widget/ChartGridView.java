...
public class ChartGridView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mid != null) {
            final int saveCount = canvas.save();
            //modify DWYYLM-144 songqingming 20151209 (start)
            //canvas.translate((width - mid.getWidth()) / 2, height + padding);
            int tx = (width - mid.getWidth()) / 2;
            if (tx > start.getWidth()) {
            	canvas.translate(tx, height + padding);
            } else {
            	canvas.translate(start.getWidth()+2, height + padding);
            }
            //modify DWYYLM-144 songqingming 20151209 (end)
            mid.draw(canvas);
            canvas.restoreToCount(saveCount);
        }
