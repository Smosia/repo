...
public class SimSettings extends RestrictedSettingsFragment implements Indexable,
    ...
    private void updateSubscriptions() {
		...
		//modify BUG_ID:DWLEBM-48 dushuaihui 20160314(start)
        boolean modifySimPerference = false;
        if(getActivity() != null){
            modifySimPerference = getActivity().getResources().getBoolean(R.bool.customized_sim_preference_num);
        } else if (mContext != null){
            modifySimPerference = mContext.getResources().getBoolean(R.bool.customized_sim_preference_num);
        }
        for (int i = 0; i < mNumSlots; ++i) {
            if(modifySimPerference){
		...
		//modify BUG_ID:DWLEBM-48 dushuaihui 20160314(end)
            final SubscriptionInfo sir = mSubscriptionManager
                    .getActiveSubscriptionInfoForSimSlotIndex(i);
            SimPreference simPreference = new SimPreference(mContext, sir, i);
            simPreference.setOrder(i-mNumSlots);
    ...
    private void updateCallValues() {
        ...
        if (simPref != null) {
            ...
            /// @}
            //add BUG_ID:DWYQLSSB-146 dushuaihui 20160603(start)
            /// M: [C2K solution 2 enhancement] [C2K solution 1.5] @{
            if (!shouldDisableActivitesCategory(getActivity())) {
                simPref.setEnabled(allPhoneAccounts.size() > 0);
                mSimManagementExt.configSimPreferenceScreen(simPref, KEY_CALLS,
                        allPhoneAccounts.size());
            }
            //add BUG_ID:DWYQLSSB-146 dushuaihui 20160603(end)
            /// @}
        }
    }
    ...
    */
    ...
    private class SimPreference extends RadioPowerPreference{
        ...
        public void update() {
            ...
            //add BUG_ID:DWYQLSSMY-934 dushuaihui 20160511(start)
            boolean removeSummary = res.getBoolean(R.bool.config_remove_summary);
            //add BUG_ID:DWYQLSSMY-934 dushuaihui 20160511(end)
			
            if (mSubInfoRecord != null) {
                if (TextUtils.isEmpty(getPhoneNumber(mSubInfoRecord))) {
                    //modify BUG_ID:DWYQLSSMY-934 dushuaihui 20160511(start)
                    if(removeSummary){
                        setTitle(mSubInfoRecord.getDisplayName());
                    }else{
                        setSummary(mSubInfoRecord.getDisplayName());
                    }
                    //modify BUG_ID:DWYQLSSMY-934 dushuaihui 20160511(end)
                } else {
                    //modify BUG_ID:DWYQLSSMY-934 dushuaihui 20160511(start)
                    if(removeSummary){
                        setTitle(mSubInfoRecord.getDisplayName());
    ...
    private void updateSimPref() {
        if (shouldDisableActivitesCategory(getActivity())) {
            ...
            if (simCallsPref != null) {
                ...
                //add BUG_ID:DWYQLSSB-146 dushuaihui 20160603(start)
                simCallsPref.setEnabled(accoutSum > 0
