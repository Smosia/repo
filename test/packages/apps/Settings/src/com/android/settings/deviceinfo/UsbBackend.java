...
public class UsbBackend {
    ...
    public static final String PROP_DEFAULT_USB = "persist.sys.default.usb";//M by shubin for default usb
    ...
    private void setUsbFunction(int mode) {
        switch (mode) {
            case MODE_DATA_MTP:
                ...
                setDefaultUsb(UsbManager.USB_FUNCTION_MTP);//M by shubin for default usb
                break;
            case MODE_DATA_PTP:
                mUsbManager.setCurrentFunction(UsbManager.USB_FUNCTION_PTP);
                mUsbManager.setUsbDataUnlocked(true);
                setDefaultUsb(UsbManager.USB_FUNCTION_PTP);//M by shubin for default usb
                break;
            case MODE_DATA_MIDI:
                mUsbManager.setCurrentFunction(UsbManager.USB_FUNCTION_MIDI);
                mUsbManager.setUsbDataUnlocked(true);
                setDefaultUsb(UsbManager.USB_FUNCTION_MIDI);//M by shubin for default usb
                break;
            /// M: Add for Built-in CD-ROM and USB Mass Storage @{
            case MODE_DATA_MASS_STORAGE:
                mUsbManager.setCurrentFunction(UsbManager.USB_FUNCTION_MASS_STORAGE);
                mUsbManager.setUsbDataUnlocked(true);
                setDefaultUsb(UsbManager.USB_FUNCTION_MASS_STORAGE);//M by shubin for default usb
                break;
            case MODE_DATA_BICR:
                mUsbManager.setCurrentFunction(UsbManager.USB_FUNCTION_BICR);
                mUsbManager.setUsbDataUnlocked(true);
                setDefaultUsb(UsbManager.USB_FUNCTION_BICR);//M by shubin for default usb
                break;
            /// M: @}
            default:
                mUsbManager.setCurrentFunction(null);
                mUsbManager.setUsbDataUnlocked(false);
                setDefaultUsb("charging");//M by shubin for default usb
                break;
        }
    }
    //A by shubin for default usb {
    private void setDefaultUsb(String function){
    	try {
			SystemProperties.set(PROP_DEFAULT_USB, function);
