...
public class MasterClearConfirm extends InstrumentedFragment {
    ...
    private Button.OnClickListener mFinalClickListener = new Button.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            //modify DWYQLSSB-72 guowen 20160525 start
            IntentFilter filter = new IntentFilter(Intent.ACTION_BATTERY_CHANGED);
            getActivity().registerReceiver(new BroadcastReceiver() {
                @Override
                public void onReceive(Context context, Intent intent) {
                    // TODO Auto-generated method stub
                    int level = Utils.getBatteryLevel(intent);
                    getActivity().unregisterReceiver(this);
                    if(getActivity().getResources().getBoolean(R.bool.show_battery_low) && level < 15){
                        Toast.makeText(getActivity(), R.string.battery_low, Toast.LENGTH_LONG).show();
                    } else {
            ...
            //modify DWYQLSSB-72 guowen 20160525 end
        }
    ...
    private void doMasterClear() {
    	//A: JWBLJ-697 guochenlong  20151027 (START) @{
        if (android.os.SystemProperties.getBoolean("persist.sys.intex.sales", false)) {
            NVUtils.writeResetPhoneNVFlag(NVUtils.BOOT_TYPE_RESET_PHONE_STRING);
        }
    	//A: JWBLJ-697 guochenlong  20151027 (END) }@
