            ...
            public void onClick(DialogInterface dialog, int which) {
                if (which == DialogInterface.BUTTON_POSITIVE) {
                    if (mWifiP2pManager != null) {
						...
						int nameNotBlankLength = mDeviceNameText.getText().toString().trim().length();    //A:DELSLMY-808 guoshuai 20160311
                        if (name != null) {
                            for (int i = 0; i < name.length(); i++) {
                                char cur = name.charAt(i);
                                //M:DELSLMY-808 guoshuai 20160311(start)
                                /*if(!Character.isDigit(cur) && !Character.isLetter(cur)
                                        && cur != '-' && cur != '_' && cur != ' ') {*/
                                if((!Character.isDigit(cur) && !Character.isLetter(cur)
                                        && cur != '-' && cur != '_' && cur != ' ') || nameNotBlankLength == 0) {
                                //M:DELSLMY-808 guoshuai 20160311(end)
