...
//----------------------------For volte ims call only------------------------------------
    ...
    /**
     * This function to get display-name from pau information.
     * @param pauField
     * @return
     */
    public static String getDisplayFromPauField(String pauField) {
        String displayName = "";
        if (!TextUtils.isEmpty(pauField)) {
            displayName = getNameFromPAU(pauField);
            log("getDisplayFromPauField()... displayName: " + displayName);
        }
        return displayName;
    }
