...
public class PhoneRecorder extends Recorder {
    ...
    private static final String AUDIO_OGG = "application/ogg";
    ...
    String mRequestedType = AUDIO_OGG;
    //add DWYQLSSMY-70 songqingming 20160324 (end)
    long mMaxFileSize = -1;
        ...
        if (!RecorderUtils.isExternalStorageMounted(mContext)) {
            ...
            try {
                if (AUDIO_AMR.equals(mRequestedType)) {
                    ...
                    startRecording(MediaRecorder.OutputFormat.THREE_GPP, ".3gpp");
                } else if (AUDIO_OGG.equals(mRequestedType)) {
                    startRecording(MediaRecorder.OutputFormat.OUTPUT_FORMAT_OGG, ".ogg");
                //add DWYQLSSMY-70 songqingming 20160324 (end)
                }else {
    ...
    //add DWYQLSSMY-70 songqingming 20160324 (start)
    public void startRecord(int slot, String number, boolean isConference) {
        log("startRecord, mRequestedType = " + mRequestedType);
        if (sIsRecording) {
            return;
        }
        if (!RecorderUtils.isExternalStorageMounted(mContext)) {
            ...
            try {
                if (AUDIO_AMR.equals(mRequestedType)) {
                    startRecording(MediaRecorder.OutputFormat.RAW_AMR, ".amr", slot, number, isConference);
                } else if (AUDIO_3GPP.equals(mRequestedType)) {
                    startRecording(MediaRecorder.OutputFormat.THREE_GPP, ".3gpp", slot, number, isConference);
                //add DWYQLSSMY-70 songqingming 20160324 (start)
                }else if (AUDIO_OGG.equals(mRequestedType)) {
                    startRecording(MediaRecorder.OutputFormat.OUTPUT_FORMAT_OGG, ".ogg", slot, number, isConference);
                //add DWYQLSSMY-70 songqingming 20160324 (end)
                } else if (AUDIO_ANY.equals(mRequestedType)) {
                    // mRequestedType = AUDIO_3GPP;
                    startRecording(MediaRecorder.OutputFormat.THREE_GPP, ".3gpp", slot, number, isConference);
