...
public class PhoneRecorderServices extends Service {
    ...
    private static final int REQUEST_START_RECORDING_CUSTOM = 10;
    ...
    private final IPhoneRecorder.Stub mBinder = new IPhoneRecorder.Stub() {
         ...
         */
        public void startRecordCustom(int slot, String number, boolean isConference) {
            log("startRecord");
            mRecordHandler.obtainMessage(REQUEST_START_RECORDING_CUSTOM, 
                    new TempObj(slot, number, isConference)).sendToTarget();
        }
        //add DWYQLSSMY-70 songqingming 20160324 (end)
    ...
    class TempObj {
    	...
    	TempObj(int slot, String number, boolean isConference) {
    		this.slot = slot;
    		this.number = number;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case REQUEST_START_RECORDING_CUSTOM:
                    if (null != mPhoneRecorder) {
                        log("[handleMessage]do start recording");
                        TempObj tempObj = (TempObj)msg.obj;
                        mRecordStoragePath = RecorderUtils.getExternalStorageDefaultPath();
                        mPhoneRecorder.startRecord(tempObj.slot, tempObj.number, tempObj.isConference);
                    }
                    break;
                //add DWYQLSSMY-70 songqingming 20160324 (end)
