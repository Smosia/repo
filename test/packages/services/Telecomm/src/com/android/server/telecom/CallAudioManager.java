...
final class CallAudioManager extends CallsManagerListenerBase
    ...
    private static final String MODE_DESCRIPTION_IN_CALL_EXTERNAL = "MODE_IN_CALL_EXTERNAL";
    /* M: CC part end */
    ...
    public void onConnectionLost(Call call) {
        ...
        ///M: ALPS02512168 ignore when audio mode is communication @{
        if (mAudioManager.getMode() == AudioManager.MODE_IN_COMMUNICATION) {
            Log.d(this, "ignore to switch to normal because sip call is foreground call");
            return;
        }
        /// @}
        ...
        ///M: ALPS02469083 @{
        // plug out sim when ecc is ongoing
        // onconnectionlost will be trigger and ecc will redial
        // no any call will co-exist with ECC
        // so do not set this flag when call is ECC
        // ALPS02621164 set this flag as true under ECC
        // if trigger ECC redial, this flag will be reset in onCallStateChanged()
        if (call != null && !mCallsManager.hasVoipCall()) {
            mTRMFlag = true;
        }
        ///@}
        /// @}
    ...
    private void setSystemAudioState(
        ...
        if (mCallAudioState.getRoute() == CallAudioState.ROUTE_BLUETOOTH) {
            ...
            if (oldAudioState.getRoute() != mCallAudioState.getRoute()) {
                Log.i(this, "oldAudioState.getRoute() != mCallAudioState.getRoute()");
            /// @}
                turnOnBluetooth(false);
            }
            turnOnSpeaker(false);
            ...
            */
            /// @}
                ...
                if (foregroundCall.isAlive()) {
                    ...
                    requestAudioFocusAndSetMode(AudioManager.STREAM_VOICE_CALL, mode);
                    ///M: ALPS02282259
                    // move this operation to complete audio mode setting @{
                    /*
    ...
    private CallAudioState getInitialAudioState(Call call) {
        ...
        /// M: change route as speaker if initial route is earpiece @{
        if (call != null && call.isVideoCall() && route == CallAudioState.ROUTE_EARPIECE) {
    ...
    private String modeToString(int mode) {
        switch (mode) {
            ...
            case AudioManager.MODE_IN_CALL_EXTERNAL:
                return MODE_DESCRIPTION_IN_CALL_EXTERNAL;
            /* M: CC part end */
            default:
    ...
    /**
     * Since the getId() of PhoneAccountHandle not stands for
    ...
    private int getPhoneId(PhoneAccountHandle handle) {
        ...
        if (tcm != null && tem != null) {
            PhoneAccount account = tcm.getPhoneAccount(handle);
            if (account != null) {
    ...
    private int audioModeUpdateDualModem(int mode) {
        ...
        if ((isEvdoDualtalk() || isC2kSupported())
                && getForegroundCall() != null && getForegroundCall().isAlive()
                && isInCallMode(oldMode)) {
