...
public class Call implements CreateConnectionResponse {
    ...
    public void updateExtras(Bundle bundle) {
        ...
        if (isIncoming() && TelecomVolteUtils.isPauFieldChanged(bundle, mPauField)) {
            ...
            String displayName = TelecomVolteUtils.getDisplayFromPauField(mPauField);
            if (!TextUtils.isEmpty(displayName)
                    && !Objects.equals(displayName, getCallerDisplayName())) {
                setCallerDisplayName(displayName, getCallerDisplayNamePresentation());
            }
        }
    }
    ...
    private boolean shouldUpdateHoldForViLTE() {
        final List<String> cMccMncMccList =
                Arrays.asList("46000", "46002", "46007", "46008", "46011");
        String simOperator = "";
        PhoneAccountHandle accountHandle = getTargetPhoneAccount();
        if (mContext != null && accountHandle != null) {
            PhoneAccount account = TelecomManager.from(mContext).getPhoneAccount(accountHandle);
