...
public class TelephonyConnectionService extends ConnectionService {
    ...
    public Connection onCreateOutgoingConnection(
        ...
        Log.d(this, "onCreateOutgoingConnection, request: " + request);
        ...
        if (isEmergencyNumber) {
            ...
            /// M: Timing issue, service state may be power off even radio on
            // if (!phone.isRadioOn()) {
            if (state == ServiceState.STATE_POWER_OFF || isAirplaneModeOn) {
            /// @}
        ...
        if (useEmergencyCallHelper) {
            ...
            if (TelephonyConnectionServiceUtil.getInstance().maySwitchPhone()) {
                mEmergencyCallHelper.startExitAirplaneModeSequence(
                        ...
                        new EmergencyCallHelper.Callback() {
                            ...
                            public void onComplete(boolean isRadioReady) {
                                ...
                                if (TelephonyConnectionServiceUtil.getInstance().
                                    ...
                                    Log.d(this, "onCreateOutgoingConnection, 4G data only");
                                    /// M: ECC Retry @{
                                    // Assume only one ECC exists
                                    if (TelephonyConnectionServiceUtil.getInstance()
                                            .isEccRetryOn()) {
    ...
    private void startTurnOnRadio(final TelephonyConnection connection,
            final ConnectionRequest request, String number) {
        /// M: ECC Retry @{
        if (TelephonyConnectionServiceUtil.getInstance().isEccRetryOn()) {
            Log.d(this, "startTurnOnRadio, clear ECC param");
            TelephonyConnectionServiceUtil.getInstance().clearEccRetryParams();
        }
        /// @}
        Phone phone = TelephonyConnectionServiceUtil.getInstance()
                .selectPhoneBySpecialEccRule(request.getAccountHandle(), number);
        // Get the right phone object from the account data passed in.
        if (phone == null) {
            phone = getPhoneForAccount(request.getAccountHandle(), true);
        }
        /// M: ECC Retry @{
        if (!TelephonyConnectionServiceUtil.getInstance().isEccRetryOn()) {
            Log.d(this, "ECC Retry : set param with Intial ECC.");
