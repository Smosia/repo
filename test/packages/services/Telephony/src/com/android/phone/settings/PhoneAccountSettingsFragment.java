...
public class PhoneAccountSettingsFragment extends PreferenceFragment
    ...
    private static final String BUTTON_AUTO_CALL_RECORD = "button_auto_call_record";
    ...
    public void onResume() {
        ...
        */
        ...
        if (shouldShowConnectionServiceList(allNonSimAccounts)) {
            ...
            //modify DWLEBM-118 songqingming 20160317 (start)
            //mDefaultOutgoingAccount.setListener(this);
            if (mDefaultOutgoingAccount != null) {
                mDefaultOutgoingAccount.setListener(this);
            }
            //modify DWLEBM-118 songqingming 20160317 (end)
        ...
        //add DWYSLM-711 songqingming 20160420 (start)
        boolean sipEnable = getActivity().getResources().getBoolean(R.bool.sip_call_enabled);
        //add DWYSLM-711 songqingming 20160420 (end)
        if (SipUtil.isVoipSupported(getActivity()) && sipEnable) { //add: && sipEnable
            ...
            Preference sipCatogary = getPreferenceScreen().
                    findPreference(SIP_SETTINGS_CATEGORY_PREF_KEY);
            if (sipCatogary != null) {
                getPreferenceScreen().removePreference(sipCatogary);
            }
        }
        ...
        //add DWYQLSSMY-70 songqingming 20160324 (start)
        mAutoCallRecord = (CheckBoxPreference) findPreference(BUTTON_AUTO_CALL_RECORD);
        if (!getActivity().getResources().getBoolean(R.bool.auto_call_record_support)) {
            if (mAutoCallRecord != null) {
                getPreferenceScreen().removePreference(mAutoCallRecord);
            }
        } else {
            boolean enabled = getAutoCallRecordValue() == 1;
    ...
    private int getAutoCallRecordValue() {
        int autoRecordValue = Settings.Global.getInt(getActivity().getContentResolver(),
                    Settings.Global.TELEPHONY_AUTO_CALL_RECORD_CONFIG, 0);
        android.util.Log.d("sqm", "getAutoCallRecordValue(): autoRecordValue="+autoRecordValue);
    ...
    public boolean onPreferenceChange(Preference pref, Object objValue) {
        ...
        //add DWYQLSSMY-70 songqingming 20160324 (start)
        else if (pref == mAutoCallRecord) {
            boolean autoCallRecordEnable = (Boolean) objValue;
            setAutoCallRecordValue(autoCallRecordEnable);
            return true;
        }
        //add DWYQLSSMY-70 songqingming 20160324 (end)
		
        //add DWYBL-2577 songqingming 20151023 (start)
        else if (pref == mFloatWindow) {
            boolean enabled = (Boolean) objValue;
            setFloatWindowValue(enabled);
            return true;
        }
    ...
    private void initAccountList(List<PhoneAccountHandle> enabledAccounts) {
        ...
        Collections.sort(accounts, new Comparator<PhoneAccount>() {
            ...
            public int compare(PhoneAccount account1, PhoneAccount account2) {
                ...
                //add DWYQLSSMY-398 songqingming 20160330 (start)
                int subId1 = mTelephonyManager.getSubIdForPhoneAccount(account1);
                int slot1 = SubscriptionManager.getSlotId(subId1);
                int subId2 = mTelephonyManager.getSubIdForPhoneAccount(account2);
                int slot2 = SubscriptionManager.getSlotId(subId2);
                retval = slot1 < slot2 ? -1 : 1;
                //add DWYQLSSMY-398 songqingming 20160330 (end)
    ...
    //add DWYBL-2577 songqingming 20151023 (start)
    private static final String FLOAT_WINDOW_WHEN_CALLING_KEY = "float_window_when_calling_key";
    private CheckBoxPreference mFloatWindow;
    //add DWYBL-2577 songqingming 20151023 (end)
    ...
    private void configCallFeatures() {
        ...
        //add DWYBL-2577 songqingming 20151023 (start)
        mFloatWindow = (CheckBoxPreference) getPreferenceScreen().findPreference(FLOAT_WINDOW_WHEN_CALLING_KEY);
        if (getActivity().getResources().getBoolean(R.bool.float_window_when_calling_support)) {
            boolean enabled = getFloatWindowValue() == 1;
            mFloatWindow.setChecked(enabled);
            mFloatWindow.setOnPreferenceChangeListener(this);
        } else {
            getPreferenceScreen().removePreference(mFloatWindow);
        }
        //add DWYBL-2577 songqingming 20151023 (end)
    ...
    private void setFloatWindowValue(boolean enabled) {
        Settings.System.putInt(getActivity().getContentResolver(), 
                Settings.System.FLOAT_WINDOW_WHEN_CALLING, enabled ? 1 : 0);
    }
    //add DWYBL-2577 songqingming 20151023 (end)
    ...
    private void removeOldAccounts(PreferenceCategory accountList) {
        for (; mAccountNumber > 0 ; mAccountNumber --) {
            Preference account = accountList.findPreference("account" + mAccountNumber);
            if (account != null) {
                accountList.removePreference(account);
            }
        }
    }
    /// @}
    ...
    private void updateDefaultOutgoingAccountPreference() {
        if (mDefaultOutgoingAccount != null) {
            ...
            if (mayCauseSimSwitch) {
                ...
                if (isAirplaneModeEnabled || isInSwitching) {
                    if (mDefaultOutgoingAccount.getDialog() != null &&
