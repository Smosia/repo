...
public class PhoneGlobals extends ContextWrapper {
    ...
    private boolean isTurnToSilence = false;
    private boolean isAnswerBySwing = false;
    private boolean isPhoneStateFirstChanged = true;
    private SensorListener mSensorListener;
    private final String DB_SMART_ANSWER = "smart_answer";
    private final String DB_ANSWER_BY_SWING = "answer_by_swing";
    private final String DB_TURN_TO_SILENCE = "turn_to_silence";
    private final SensorCallback mSensorCallback = new SensorCallback(){
        public void onUnlock() {}

        public void onSwing() {
            //if(getAnswerBySwingState()){
            if(isAnswerBySwing){
                Log.d(LOG_TAG,"onSwing - Answer by Swing is enable in SmartMotion.");
                answerIncomingCall();
            }
		}
        public void onCalling() {
            if(getSmartAnswerState()){
                Log.d(LOG_TAG,"onCalling - Smart Answer is enable in SmartMotion.");
                answerIncomingCall();
            }
		}
        public void onRotate() {
            //if(getTurnToSilenceState()){
            if(isTurnToSilence){
                Log.d(LOG_TAG,"onRotate - Turn to Silence is enable in SmartMotion.");
                keepIncomingCallSilence();
            }
		}
    };

    private boolean getSmartAnswerState() {
        return Settings.System.getInt(getContentResolver(), DB_SMART_ANSWER, -1) == 1;
    }

    private boolean getAnswerBySwingState() {
        return Settings.System.getInt(getContentResolver(), DB_ANSWER_BY_SWING, -1) == 1;
    }

    private boolean getTurnToSilenceState() {
        return Settings.System.getInt(getContentResolver(), DB_TURN_TO_SILENCE, -1) == 1;
    }

    private void answerIncomingCall() {
        Log.d(LOG_TAG,"answerIncomingCall - Answer the incoming call.");
        if (mCM != null) {
            boolean hasRingingCall = mCM.hasActiveRingingCall();
            Log.d(LOG_TAG,"hasRingingCall = " + hasRingingCall);
            if (!hasRingingCall) {
                Log.d(LOG_TAG,"answerIncomingCall - Has not ringing : return.");
                return;
            }
            Phone phone = mCM.getRingingPhone();
            Call ringing = mCM.getFirstActiveRingingCall();
            Log.d(LOG_TAG,"answerIncomingCall - Call string is " + ringing.toString() + ".");
            int phoneType = phone.getPhoneType();
            Log.d(LOG_TAG,"phoneType = " + phoneType + ", PHONE_TYPE_GSM = " + PhoneConstants.PHONE_TYPE_GSM);
            if (phoneType == PhoneConstants.PHONE_TYPE_GSM) {
                final boolean hasActiveCall = mCM.hasActiveFgCall();
                final boolean hasHoldingCall = mCM.hasActiveBgCall();
                Log.d(LOG_TAG,"hasActiveCall = " + hasActiveCall + ", hasHoldingCall = " + hasHoldingCall);
    ...
    private void keepIncomingCallSilence() {
        Log.d(LOG_TAG,"keepIncomingCallSilence - isRinging:" + isRinging);
    ...
    public void onCreate() {
        ...
        //A: kuangyunfeng 20160112(start)
        mSensorListener = new SensorListener(this, mSensorCallback);
        //A: kuangyunfeng 20160112(end)
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //A: kuangyunfeng 20160112(start)
                ...
                if (tm.getCallState() == TelephonyManager.CALL_STATE_RINGING) {
                    isPhoneStateFirstChanged = false;
                    isRinging = true;
                    Log.d(LOG_TAG,"mReceiver:TelephonyManager.CALL_STATE_RINGING");
                } else if (tm.getCallState() == TelephonyManager.CALL_STATE_IDLE) {
                    isPhoneStateFirstChanged = true;
                    isRinging = false;
                    Log.d(LOG_TAG,"mReceiver:TelephonyManager.CALL_STATE_IDLE");
                }

                if (mSensorListener != null && (tm.getCallState() == TelephonyManager.CALL_STATE_OFFHOOK) && !isPhoneStateFirstChanged) {
                    Log.d(LOG_TAG,"mReceiver:TelephonyManager.CALL_STATE_OFFHOOK");
                    isRinging = false;
                } else if (mSensorListener != null){          
                    Log.d(LOG_TAG,"mReceiver: tm.getCallState() != TelephonyManager.CALL_STATE_OFFHOOK");
                    //mOrientation = SensorListener.ORIENTATION_UNKNOWN;
                    mSensorListener.enable(tm.getCallState() != TelephonyManager.CALL_STATE_IDLE);
                	if(tm.getCallState() != TelephonyManager.CALL_STATE_IDLE) {
                        isSmartAnswer = getSmartAnswerState();
                        isAnswerBySwing = getAnswerBySwingState();                
                        isTurnToSilence = getTurnToSilenceState();
                        Log.d(LOG_TAG, "mReceiver: isSmartAnswer = " + isSmartAnswer + ", isAnswerBySwing = " + isAnswerBySwing
                                                + /*", isSpeakerOff = " + isSpeakerOff + */", isTurnToSilence = " + isTurnToSilence);
                    } else {
                        //isSpeakerOff = false;
                        isSmartAnswer = false;
                        isAnswerBySwing = false;
                        isTurnToSilence = false;
                    }
                } 
            } 
            //A: kuangyunfeng 20160112(end)
