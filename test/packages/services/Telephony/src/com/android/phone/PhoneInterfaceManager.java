...
public class PhoneInterfaceManager extends ITelephony.Stub {
            ...
            switch (msg.what) {
               ...
               case EVENT_SIM_IO_DONE:
                ...
                case CMD_SET_PREFERRED_NETWORK_TYPE:
                    ...
                    Phone activePhone = getPhoneFromRequest(request);
                    if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                        if (TelephonyUtilsEx.isCDMAPhone(activePhone)) {
                            SvltePhoneProxy svltePhoneProxy = (SvltePhoneProxy) activePhone;
                            int setvalue = SvlteRatController.RAT_MODE_SVLTE_2G
                                    | SvlteRatController.RAT_MODE_SVLTE_3G
                                    | SvlteRatController.RAT_MODE_SVLTE_4G;
                            AsyncResult.forMessage(onCompleted, null, null);
                            svltePhoneProxy.getSvlteRatController().
                                    setRadioTechnology(setvalue, onCompleted);
                            log("setPreferredNetworkType svlte");
                        } else {
                            activePhone.setPreferredNetworkType(networkType, onCompleted);
                            log("setPreferredNetworkType default");
                        }
                    } else {
                        activePhone.setPreferredNetworkType(networkType, onCompleted);
                    }
    ...
    public boolean setPreferredNetworkType(int subId, int networkType) {
        ...
        Phone activePhone = getPhone(subId);
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport() &&
                TelephonyUtilsEx.isCDMAPhone(activePhone)) {
            int ratMode = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
                    SvlteUtils.getCdmaRatModeKey(subId),
                    SvlteRatController.SvlteRatMode.SVLTE_RAT_MODE_4G.ordinal());
            if (ratMode == SvlteRatController.SvlteRatMode.SVLTE_RAT_MODE_4G.ordinal()) {
                //just return if svlte mode alreayd 4G
                log("setPreferredNetworkType just return");
                return true;
            }
        }
            ...
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                if (TelephonyUtilsEx.isCDMAPhone(activePhone)) {
                    Settings.Global.putInt(mPhone.getContext().getContentResolver(),
                            SvlteUtils.getCdmaRatModeKey(subId),
                            SvlteRatController.SvlteRatMode.SVLTE_RAT_MODE_4G.ordinal());
                    log("setPreferredNetworkType svlte 4G");
                } else {
                    Settings.Global.putInt(mPhone.getContext().getContentResolver(),
