...
import java.util.HashSet;
import java.util.Iterator;
import android.provider.Telephony.Sms.Inbox; //add JBLW-482 zhangzixiao 20131216
import android.os.SystemProperties;//add BUG_ID:JBLW-482 zhangyuwen 20141216
...
//            + Mms.THREAD_ID + "; " +
    ...
    //Modify JBLW-482 zhangzixiao 20131216 start
    //static final int DATABASE_VERSION = 601020;
	//static final int DATABASE_VERSION = 601050;
    //static final int DATABASE_VERSION = 601040;
	static final int DATABASE_VERSION = 601060;
	//static final int DATABASE_VERSION = 601030;
    //Modify JBLW-482 zhangzixiao 20131216 end
...
//                                            + " LIMIT 1";
    ...
    static public void deleteSimSmsOnDB(SQLiteDatabase db, String selection, String[] selectionArgs){
        int thread_id = -1;
        // Find the thread ID that the specified SMS belongs to.
        Cursor c = db.query("sms", new String[] { "thread_id" },
    ...
    private void createSmsTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE sms (" +
                   ...
                   /// M: @}
                   //A: RGK-MMS-Feature: SyncSimMessages (START) @{
                   (SmsProvider.supportSyncSimMessage() ? ",index_on_icc TEXT" : "") +
                   //A: RGK-MMS-Feature: SyncSimMessages (END) }@
                   ");");
    ...
    private void createCommonTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE threads (" +
                   ...
                   (supportEncryptedMessage() ? ",encrypted INTEGER DEFAULT 0" : "") + ");");//modify JWYYL-83 chenshu 20141215
...
//                   "END;");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
       ...
       //add DWYSLM-75 chenshu 20160111 start
        case 601020:
            if (currentVersion <= 601020) {
				return;
			}
			...
			if (supportBlacklist()) {
				db.beginTransaction();
				try {
					upgradeDatabaseToVersion601030(db);					
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG, ex.getMessage(), ex);
					break;
        ...
        //add DWYSLM-75 chenshu 20160111 end
        //A: RGK-MMS-Feature: SyncSimMessages (START) @{
		case 601030:
			if (currentVersion <= 601030) {
				return;
			}
			if (SmsProvider.supportSyncSimMessage()) {
				db.beginTransaction();
				try {
					upgradeDatabaseToVersion601040(db);
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG, ex.getMessage(), ex);
					break;
	   ...
	   case 601040:
			if (currentVersion <= 601040) {
				return;
			}
			if (supportEncryptedMessage()) {
				db.beginTransaction();
				try {
					upgradeDatabaseToVersion601050(db);
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG, ex.getMessage(), ex);
					break;
        ...
        //Add, JWLW-1141, zhangzixiao, 20131124, Start
        case 601050:
	        if (currentVersion <= 601050) {
                    return;
                }
	            ...
	            if(supportDualRingtone()) {
		            db.beginTransaction();
                	try {
                        upgradeDatabaseToVersion601060(db);
                        db.setTransactionSuccessful();
                	} catch (Throwable ex) {
        ...
        //Add, JWLW-1141, zhangzixiao, 20131124, End
            return;
        }
    ...
    private void upgradeDatabaseToVersion601050(SQLiteDatabase db) {
        db.execSQL("ALTER TABLE threads add COLUMN encrypted INTEGER DEFAULT 0");
    }

    //Add, JWLW-1141, zhangzixiao, 20131124, Start
    private void upgradeDatabaseToVersion601060(SQLiteDatabase db) {
    	db.execSQL("ALTER TABLE thread_settings ADD COLUMN " + ThreadSettings.RINGTONE2 + " TEXT");
	    db.execSQL("DROP TRIGGER IF EXISTS insert_thread_settings_when_insert_threads");
        // Updates threads table whenever a message in sms is updated.
        db.execSQL("CREATE TRIGGER insert_thread_settings_when_insert_threads " +
                   "AFTER INSERT ON threads " +
	...
	//add JBLW-482 zhangzixiao 20131216 start
	if(supportBlacklist()) {
	    db.execSQL("DROP TABLE IF EXISTS blacktext");   
	    db.execSQL("DROP TABLE IF EXISTS black_messages");   
	}
	//add JBLW-482 zhangzixiao 20131216 end
        /// M: Code analyze 008, new feature, support for wappush. @{
        if (MTK_WAPPUSH_SUPPORT) {
...
//        createRCSTable(db);
	...
	// add JBLW-482 zhangzixiao 20131216 start
    private void upgradeDatabaseToVersion601030(SQLiteDatabase db) {
    	createBlackText(db);
		createBlackMessages(db);
    }
	// add JBLW-482 zhangzixiao 20131216 end
    //Add, JWLW-1141, zhangzixiao, 20131124, End
    private void upgradeDatabaseToVersion601040(SQLiteDatabase db) {
		db.execSQL("ALTER TABLE sms add COLUMN index_on_icc TEXT");
    }
    //A: RGK-MMS-Feature: SyncSimMessages (END) }@
    ...
    /// @}
    //add JBLW-482 zhangzixiao 20131216 start
    private void createBlackText(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE blacktext (" +
                "_id INTEGER PRIMARY KEY AUTOINCREMENT," +
		"message_count INTEGER DEFAULT 0," +
                "number TEXT);");
    }
    private void createBlackMessages(SQLiteDatabase db) {
	db.execSQL("CREATE TABLE black_messages (" +
    		"_id INTEGER PRIMARY KEY AUTOINCREMENT," +
    		"blacktext_id INTEGER," +
		"black_msg_type INTEGER DEFAULT 1," +
    		Inbox.ADDRESS + " TEXT," +
    		Inbox.DATE + " INTEGER," +  //==Mms.DATE
    		Inbox.DATE_SENT + " INTEGER DEFAULT 0," +  //==Mms.DATA_SENT
    		Inbox.PROTOCOL + " INTEGER," +
    		Inbox.READ + " INTEGER DEFAULT 0," +
    		Inbox.SEEN + " INTEGER DEFAULT 0," +
    		Inbox.SUBJECT + " TEXT," +
		Inbox.BODY + " TEXT," +
    		Inbox.REPLY_PATH_PRESENT + " INTEGER," +
    		Inbox.SERVICE_CENTER + " TEXT," +
    		Inbox.ERROR_CODE+ " INTEGER DEFAULT 0," +
    		"sub_id LONG DEFAULT -1," +  //the following is Mms
    		Mms.RETRIEVE_TEXT + " TEXT," +  //ENCODED_STRING_COLUMN_NAME_MAP
    		Mms.SUBJECT + " TEXT," +
		Mms.SUBJECT_CHARSET + " INTEGER," +
		Mms.RETRIEVE_TEXT_CHARSET + " INTEGER," +
    		Mms.CONTENT_LOCATION + " TEXT," + //TEXT_STRING_COLUMN_NAME_MAP
    		Mms.CONTENT_TYPE + " TEXT," +
    		Mms.MESSAGE_CLASS + " TEXT," +
    		Mms.MESSAGE_ID + " TEXT," +
    		Mms.RESPONSE_TEXT + " TEXT," +
    		Mms.TRANSACTION_ID + " TEXT," +
    		Mms.CONTENT_CLASS + " INTEGER," + //ENCODED_STRING_COLUMN_NAME_MAP
    		Mms.DELIVERY_REPORT + " INTEGER," +
    		Mms.MESSAGE_TYPE + " INTEGER," +
    		Mms.MMS_VERSION + " INTEGER," +
    		Mms.PRIORITY + " INTEGER," +
    		...
    		Mms.RETRIEVE_STATUS + " INTEGER," +
    		Mms.STATUS + " INTEGER," +
    		Mms.DELIVERY_TIME + " INTEGER," + //LONG_COLUMN_NAME_MAP
    		Mms.EXPIRY + " INTEGER," +
    		Mms.MESSAGE_SIZE + " INTEGER," +
    		"flag1 INTEGER," + //
    		"flag2 INTEGER," + //
    		"flag3 INTEGER);");
	db.execSQL("DROP TRIGGER IF EXISTS delete_black_messages_when_delete_blacklist");
	db.execSQL("CREATE TRIGGER delete_black_messages_when_delete_blacklist " +
                   "BEFORE DELETE ON blacktext " +
                   "BEGIN " +
                   "  DELETE FROM black_messages " +
                   "    WHERE blacktext_id=old._id; " +
                   "END;");
	db.execSQL("DROP TRIGGER IF EXISTS delete_black_messages_when_update_blacklist");
	db.execSQL("CREATE TRIGGER delete_black_messages_when_update_blacklist " +
                   "BEFORE UPDATE OF number ON blacktext " +
                   "BEGIN " +
                   "  DELETE FROM black_messages " +
                   "    WHERE blacktext_id=old._id; " +
                   "END;");
	db.execSQL("DROP TRIGGER IF EXISTS update_blacklist_count_when_delete");
	db.execSQL("CREATE TRIGGER update_blacklist_count_when_delete " +
		   "AFTER DELETE ON black_messages " +
		   "BEGIN " +
		   "  UPDATE blacktext SET message_count = " +
		   "    (SELECT count(_id) FROM black_messages WHERE blacktext_id=old.blacktext_id) " +
		   "  WHERE _id=old.blacktext_id; " +
		   "END;");
	db.execSQL("DROP TRIGGER IF EXISTS update_blacklist_count_when_insert");
	db.execSQL("CREATE TRIGGER update_blacklist_count_when_insert " +
		   "AFTER INSERT ON black_messages " +
		   "BEGIN " +
		   "  UPDATE blacktext SET message_count = " +
		   "    (SELECT count(_id) FROM black_messages WHERE blacktext_id=new.blacktext_id) " +
		   "  WHERE _id=new.blacktext_id; " +
		   "END;");
    }
    //add JBLW-482 zhangzixiao 20131216 end
    /// M: Code analyze 005, new feature, create table cellbroadcast. @{
