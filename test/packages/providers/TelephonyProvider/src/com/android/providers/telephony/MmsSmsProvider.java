...
public class MmsSmsProvider extends ContentProvider {
    ...
    private static final int URI_BLACK_TEXT			   = 52;
    private static final int URI_BLACK_MESSAGES			   = 53;
    //add JBLW-482 zhangzixiao 20131216 end
    //add chenshu
    private static final int URI_ALL_THREAD = 54;
    //add chenshu
    /**
    ...
    //add JBLW-482 zhangzixiao 20131216 start
    private static final String TABLE_BLACK_TEXT = "blacktext";
    private static final String TABLE_BLACK_MESSAGES = "black_messages";
    //add JBLW-482 zhangzixiao 20131216 end
    /// @}
    /// M: Code analyze 003, new feature, support for cellbroadcast.
    ...
    static {
        if(!SmsProvider.supportSyncSimMessage()) {
            SMS_ONLY_COLUMNS = new String[] {
            "address", "body", "person", "reply_path_present",
                    "service_center", "status", "subject", "type", "error_code", "ipmsg_id"};
        } else {
            SMS_ONLY_COLUMNS = new String[] {
            "address", "body", "person", "reply_path_present",
                    "service_center", "status", "subject", "type", "error_code", "ipmsg_id", "index_on_icc"};
        }
    }
...
//            "snippet FROM words WHERE index_text MATCH ? ORDER BY snippet LIMIT 50;";
  ...
  /// M: add for op09 of new feature cc @{
    ...
    static {
        ...
        //chenshu
        URI_MATCHER.addURI(AUTHORITY, "all_threads", URI_ALL_THREAD);
        //chenshu
        /// M: Add for ip message
	...
	//add JBLW-482 zhangzixiao 20131216 start
	if(supportBlacklist()) {
	    URI_MATCHER.addURI(AUTHORITY, "blacktext", URI_BLACK_TEXT); 
	    URI_MATCHER.addURI(AUTHORITY, "black-messages", URI_BLACK_MESSAGES);  
	}
	//add JBLW-482 zhangzixiao 20131216 end
        /// M: Code analyze 003, new feature, support for cellbroadcast.
    ...
    public Cursor query(Uri uri, String[] projection,
	    ...
	    //add JBLW-482 zhangzixiao 20131216 start
	    case URI_BLACK_TEXT: {
		cursor = db.query(TABLE_BLACK_TEXT, projection,
                        selection, selectionArgs, null, null, sortOrder);
                break;
	    }
	    case URI_BLACK_MESSAGES: {
		cursor = db.query(TABLE_BLACK_MESSAGES, projection,
			selection, selectionArgs, null, null, sortOrder);
		break;
	    }
	    //add JBLW-482 zhangzixiao 20131216 end
...
//            Collections.sort(recipients);
    ...
    public int delete(Uri uri, String selection,
	    ...
	    //add JBLW-482 zhangzixiao 20131216 start
	    case URI_BLACK_TEXT: 
                affectedRows = db.delete(TABLE_BLACK_TEXT, selection, selectionArgs);
                break;
	    case URI_BLACK_MESSAGES:
		affectedRows = db.delete(TABLE_BLACK_MESSAGES, selection, selectionArgs);
		break;
	    //add JBLW-482 zhangzixiao 20131216 end
            /// M: Code analyze 003, new feature, support for cellbroadcast.
    ...
    private int deleteConversation(SQLiteDatabase db,
        ...
        if(SmsProvider.supportSyncSimMessage()) {
            return MmsProvider.deleteMessages(getContext(), db,
                    finalMmsSelection, selectionArgs, uri, false)
                    + SmsProvider.deleteMessages(getContext(), db, finalSmsSelection, selectionArgs)
                    /// M: Add for OP01 RCS Message @{
                    + count
                    /// @}
                    + db.delete("cellbroadcast", finalSelection, selectionArgs);
    ...
    public Uri insert(Uri uri, ContentValues values) {
	    ...
	    //add JBLW-482 zhangzixiao 20131216 start
	    case URI_BLACK_TEXT:
                long bt_id = db.insert(TABLE_BLACK_TEXT, null, values);
                return Uri.parse(uri + "/" + bt_id);
	    case URI_BLACK_MESSAGES:
		long bm_id = db.insert(TABLE_BLACK_MESSAGES, null, values);
		notifyBlackMessagesChange();
                return Uri.parse(uri + "/" + bm_id);
	    //add JBLW-482 zhangzixiao 20131216 end
            case URI_PENDING_MSG:
    ...
    public int update(Uri uri, ContentValues values,
        ...
        switch(URI_MATCHER.match(uri)) {
            //chenshu
            case URI_ALL_THREAD:
                affectedRows = db.update(TABLE_THREADS, values, selection, selectionArgs);
                android.util.Log.e("chenshu","affectedRows:"+affectedRows);
                break;
            //chenshu
            case URI_CONVERSATIONS_MESSAGES:
	    ...
	    //add JBLW-482 zhangzixiao 20131216 start
	    case URI_BLACK_TEXT: 
                affectedRows = db.update(TABLE_BLACK_TEXT, values, 
                        selection, selectionArgs);
                break;
	    case URI_BLACK_MESSAGES: 
                affectedRows = db.update(TABLE_BLACK_MESSAGES, values, 
                        selection, selectionArgs);
                break;
	    //add JBLW-482 zhangzixiao 20131216 end
            /// M: Code analyze 005, fix bug ALPS00091288, ANR after send mms to self.
        ...
        if (affectedRows > 0) {
            notifyChange();
	    //add JBLW-482 zhangzixiao 20131216 start
	    if(supportBlacklist() && URI_MATCHER.match(uri)==URI_BLACK_MESSAGES) {
		notifyBlackMessagesChange();
	    }
	    //add JBLW-482 zhangzixiao 20131216 end
        }
...
//                Set<String> recipients = searchRecipients(cursor.getString(1));
                   ...
                   // Log.d(LOG_TAG, "searchContacts cursor recipient " + recipient);
                    ...
                    if (name == null) {
                        ...
                        for (String addr : addresses) {
                            ...
                            boolean isBrazilConfig = getContext().getResources().getBoolean(com.android.internal.R.bool.config_use_brazil_number_format);
                            if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
                                if (PhoneNumberUtils.compareWithAreaCode(addr, recipient)) {
                                    name = (String) contactRes.get(addr);
                                    break;
                                }
                            } else {
  ...
  /// M: For op09 new feature of cc. @{
    ...
    /// @}
    //add JBLW-482 zhangzixiao 20131216 start
    private void notifyBlackMessagesChange() {
	    Uri blackMessagesUri = Uri.parse("content://mms-sms/black-messages");
            getContext().getContentResolver().notifyChange(blackMessagesUri, null);
    }
    public static boolean supportBlacklist() {
	    return android.os.SystemProperties.getBoolean("ro.rgk.support.msg.blacklist", true);
    }
    //add JBLW-482 zhangzixiao 20131216 end
}
