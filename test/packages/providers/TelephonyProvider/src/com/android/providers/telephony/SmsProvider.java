...
//    private static final Uri ICC_URI_FOUR = Uri.parse("content://sms/icc4");
    ...
    */
    ...
    //A: RGK-MMS-Feature: SyncSimMessages (START) @{
    private static SmsProvider sSmsProvider = null;
    //A: RGK-MMS-Feature: SyncSimMessages (END) }@
    @Override
...
//                return getAllMessagesFromIccInternational(uri, PhoneConstants.SIM_ID_1);
    ...
    //chenshu modify about security exception com.android.mms does not belong as 1001 20160116
    private Uri insertInner(Uri url, ContentValues initialValues, int callerUid, String callerPkg) {
        ...
        switch (match) {
		    ...
		    //A: RGK-MMS-Feature: SyncSimMessages (START) @{
	        case COPY_ALL_SIMSMS_TO_DB:
				Log.d(TAG, "insertInner COPY_ALL_SIMSMS_TO_DB");
				if(supportSyncSimMessage()) {
				    loadAllMessagesFromIcc();   
				}
	            return null;
				
		    case COPY_SIM1SMS_TO_DB:
				Log.d(TAG, "insertInner COPY_SIM1SMS_TO_DB");
				if(supportSyncSimMessage()) {
				    //Modify, DWYYL-829, DaiHongyi, 20150603, Start
				    int subId1 = getSubIdFromUri(url);
				    loadAllMessagesFromIccExt(subId1,callerUid, callerPkg);
				    //loadAllMessagesFromIccExt(SUB1);
				    //Modify, DWYYL-829, DaiHongyi, 20150603, End
				}
				return null;
				
		    case COPY_SIM2SMS_TO_DB:
				Log.d(TAG, "insertInner COPY_SIM2SMS_TO_DB");
				if(supportSyncSimMessage()) {
				    //Modify, DWYYL-829, DaiHongyi, 20150603, Start
				    int subId2 = getSubIdFromUri(url);
				    loadAllMessagesFromIccExt(subId2,callerUid, callerPkg);
				    //loadAllMessagesFromIccExt(SUB2);
				    //Modify, DWYYL-829, DaiHongyi, 20150603, End
				}
				return null;
	        //A: RGK-MMS-Feature: SyncSimMessages (END) }@
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        switch (match) {
            case SMS_ALL:
				...
				//A: RGK-MMS-Feature: SyncSimMessages (START) @{
                } else if(supportSyncSimMessage()) {
					deleteSimSms(db, where, whereArgs);
		    	//A: RGK-MMS-Feature: SyncSimMessages (END) }@
                } else {
...
//                        }
            ...
            case SMS_ICC:
                 ...
                 */
			    if(supportSyncSimMessage()) {
					return deleteMessageFromIcc(db, messageIndexString, getSubIdFromUri(url));
			    } else {
					return deleteMessageFromIcc(messageIndexString, getSubIdFromUri(url));
			    }
            ...
            case SMS_ALL_ICC:
                ...
                break;
		    //A: RGK-MMS-Feature: SyncSimMessages (START) @{
		    case REMOVE_ALL_SIMSMS_IN_DB: {
				if(supportSyncSimMessage()) {
				    String selection = "index_on_icc is not null";
				    count = db.delete(TABLE_SMS, selection, null);
				    if(count > 0) {
		    ...
		    case REMOVE_SIM1SMS_IN_DB: {
				if(supportSyncSimMessage()) {
				    //EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getContext(), SUB1);
		        	//    if (null == si) {
		            //	    	Log.e(TAG, "REMOVE_SIM1SMS_IN_DB:SIMInfo is null for slot " + SUB1);
		        	//    } else {
                    //Modify chenshu 201616 modify sub id (start)
				    //int simId1 = SUB1;
				    //String selection = "index_on_icc is not null AND sub_id = " + simId1;
                    int subId1 = getSubIdFromUri(url);
                    String selection = "index_on_icc is not null AND sub_id = " + subId1;
                    //Modify chenshu 201616 modify sub id (end)
				    count = db.delete(TABLE_SMS, selection, null);
				    if(count > 0) {
				    	MmsSmsDatabaseHelper.updateAllThreads(db, null, null);
				    }   
	     ...
	     case REMOVE_SIM2SMS_IN_DB: {
			 if(supportSyncSimMessage()) {
			    //EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getContext(), SUB2);
	        	//    if (null == si) {
	            // 	    	Log.e(TAG, "REMOVE_SIM1SMS_IN_DB:SIMInfo is null for slot " + SUB2);
	        	//    } else {
			    //int simId2 = (int)si.getSimId();
                //Modify chenshu 201616 modify sub id (start)
				//int simId2 = SUB2;
			    //String selection = "index_on_icc is not null AND sub_id = " + simId2;
                int subId2 = getSubIdFromUri(url);
                String selection = "index_on_icc is not null AND sub_id = " + subId2;
                //Modify chenshu 201616 modify sub id (end)
			    count = db.delete(TABLE_SMS, selection, null);
			    if(count > 0) {
			    	MmsSmsDatabaseHelper.updateAllThreads(db, null, null);
...
//    private static final int SMS_ALL_ICC_FOUR = 30;
    ...
    //A: RGK-MMS-Feature: SyncSimMessages (START) @{
    private static final int COPY_ALL_SIMSMS_TO_DB = 41;
    private static final int COPY_SIM1SMS_TO_DB = 42;
    private static final int COPY_SIM2SMS_TO_DB = 43;
    private static final int REMOVE_ALL_SIMSMS_IN_DB = 44;
    private static final int REMOVE_SIM1SMS_IN_DB = 45;
    private static final int REMOVE_SIM2SMS_IN_DB = 46;
    //A: RGK-MMS-Feature: SyncSimMessages (END) }@
...
//        sURLMatcher.addURI("sms", "sim4", SMS_ALL_ICC_FOUR);
		...
		//A: RGK-MMS-Feature: SyncSimMessages (START) @{
        sURLMatcher.addURI("sms", "copyallsimsmstodb", COPY_ALL_SIMSMS_TO_DB);
		sURLMatcher.addURI("sms", "copysim1smstodb", COPY_SIM1SMS_TO_DB);
		sURLMatcher.addURI("sms", "copysim2smstodb", COPY_SIM2SMS_TO_DB);
		sURLMatcher.addURI("sms", "removeallsimsmsindb", REMOVE_ALL_SIMSMS_IN_DB);
		sURLMatcher.addURI("sms", "removesim1smsindb", REMOVE_SIM1SMS_IN_DB);
		sURLMatcher.addURI("sms", "removesim2smsindb", REMOVE_SIM2SMS_IN_DB);
		//A: RGK-MMS-Feature: SyncSimMessages (END) }@
    }
    /// M: Code analyze 007, fix bug ALPS00042403, should show the sender's number
    /// in manage SIM card. show concatenation sms in one bubble, set incoming sms
...
//        }
            ...
            if (candidates != null) {
                for (int i = 0; i < candidates.size(); i++) {
                    ...
                    if (isPhoneNumber && (refinedAddress != null
                        ...
                        */
                        boolean isBrazilConfig = getContext().getResources().getBoolean(com.android.internal.R.bool.config_use_brazil_number_format);
                        if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
                        	if (PhoneNumberUtils.compareWithAreaCode(refinedAddress, addressValue)) {
                                retVal = addressesMap.get(addressValue);
                                break;
                            }
                        } else {
    ...
    private static final int SUB4 = 3;

    synchronized public static SmsProvider getSmsProvider() {
        return sSmsProvider;
    }
    
    private static Uri getIccUri(int subscription) { 
        switch (subscription) {
            case SUB1:
                return ICC_URI_INTERNATIONAL;
            case SUB2:
                return ICC_URI_GEMINI_INTERNATIONAL;
	     //case SUB3:
		 // return ICC_URI_THREE;
	     //case SUB4:
		 // return ICC_URI_FOUR;
            default:
	...
	return MSimSmsManager.isMultiSimEnabled();
    }*/
    private int loadAllMessagesFromIcc() {
    		//Modify, DWYYL-829, DaiHongyi, 20150603, Start
		//if(TelephonyManager.getDefault().getSimState(SUB1) == TelephonyManager.SIM_STATE_READY) {
		//	loadAllMessagesFromIcc(SUB1);
		//}
		//if(TelephonyManager.getDefault().getSimState(SUB2) == TelephonyManager.SIM_STATE_READY) {
		//	loadAllMessagesFromIcc(SUB2);
		//}
		Log.d(TAG, "loadAllMessagesFromIcc->");
		int [] subIdList1 = SubscriptionManager.getSubId(SUB1);
		loadAllMessagesFromIcc(subIdList1[0],0,null);
		int [] subIdList2 = SubscriptionManager.getSubId(SUB2);
		loadAllMessagesFromIcc(subIdList2[0],0,null);
		//Modify, DWYYL-829, DaiHongyi, 20150603, End

		return 1;
    }

    private int loadAllMessagesFromIccExt(int subscription,int callerUid, String callerPkg) {
		//Modify, DWYYL-829, DaiHongyi, 20150603, Start
		//if(TelephonyManager.getDefault().getSimState(subscription) == TelephonyManager.SIM_STATE_READY) {
		//    loadAllMessagesFromIcc(subscription);
		//}
		Log.d(TAG, "loadAllMessagesFromIccExt-> subscription->" + subscription);
		loadAllMessagesFromIcc(subscription,callerUid, callerPkg);
		//Modify, DWYYL-829, DaiHongyi, 20150603, End
		return 1;
    }
    private int getMessageType(SmsMessage message) {
    ...
    private void loadAllMessagesFromIcc(int subscription,int callerUid, String callerPkg) {
		ArrayList<SmsMessage> messages = null;
		//if(isMultiSim()) {
		//    messages = MSimSmsManager.getDefault().getAllMessagesFromIcc(subscription);
		//} else {
		//    messages = SmsManager.getDefault().getAllMessagesFromIcc();
		//}
		long token = Binder.clearCallingIdentity();
		try {
		    //messages  = EncapsulatedGeminiSmsManager.getAllMessagesFromIccGemini(subscription);
			messages  = SmsManager.getSmsManagerForSubscriptionId(subscription).getAllMessagesFromIcc();
		} catch (NullPointerException ne) {
			Log.e(TAG, "loadAllMessagesFromIcc:get messages null: " + ne);
		} finally {
		    Binder.restoreCallingIdentity(token);
		}
	

		//EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getContext(), subscription);
	    //    if (null == si) {
	    //        Log.e(TAG, "insertMessage:SIMInfo is null for slot " + subscription);
	    //        return;
	    //    }
		//int simId = (int)si.getSimId();
		int simId = subscription;
		boolean showInOne = true;
		ArrayList<String> concatSmsIndexAndBody = null;
		//Modify, JSLEL-1272, DaiHongyi, 20141010, Start
		//int count = messages.size();
		int count;
		if(messages != null) {
			count = messages.size();
		...
		//Modify, JSLEL-1272, DaiHongyi, 20141010, End
	for (int i = 0; i < count; i++) {
	    concatSmsIndexAndBody = null;
	    SmsMessage message = messages.get(i);
	    if (message != null && !message.isStatusReportMessage()) {
		    ...
		    /* message body */
		    String messageBody = concatSmsBody == null ? message.getDisplayMessageBody() : concatSmsBody;
		    /* index on icc */
		    ...
		    if (mIsInternationalCardNotActivate) {
            	...
            	indexOnIcc = concatSmsIndex == null ? ""+message.getIndexOnIcc() : concatSmsIndex;
        	}
		    /* message address */
		    String messageAddress = null;
		    if ((message.getStatusOnIcc() == SmsManager.STATUS_ON_ICC_READ) ||
               			(message.getStatusOnIcc() == SmsManager.STATUS_ON_ICC_UNREAD)) {
            	messageAddress = message.getDisplayOriginatingAddress();
        	} else {
            	messageAddress = message.getDestinationAddress();
        	}
		    /* thread id */
		    long threadId = -1;
	            threadId = Threads.getOrCreateThreadId(getContext(), messageAddress);
		    /* type */
		    int type = getMessageType(message);
		    if (type == Sms.MESSAGE_TYPE_INBOX) {
	            	/*Sms.Inbox.addSIMMessage(getContext().getContentResolver(), 
	            	    messageAddress, messageBody, null, message.getTimestampMillis(), 
			    true,threadId, simId, indexOnIcc);*/
				addSIMMessageToInbox(
	            	    messageAddress, messageBody, null, message.getTimestampMillis(), 
			    		true /* read */,threadId, simId, indexOnIcc, callerUid, callerPkg);
	        } else {
	            	/*
	            	Sms.Sent.addSIMMessage(getContext().getContentResolver(), 
	            	    messageAddress, messageBody, null, message.getTimestampMillis(), threadId, simId, indexOnIcc);*/
			addSIMMessageToSent(
	            	    messageAddress, messageBody, null, message.getTimestampMillis(), threadId, simId, indexOnIcc, callerUid, callerPkg);
	            }
    	...
    	SQLiteDatabase db = mOpenHelper.getWritableDatabase();
    	if(db==null) {
	    Log.e(TAG,"getWritableDatabase null after loadAllMessagesFromIcc");
    	} else {
	    	MmsSmsDatabaseHelper.updateAllThreads(db, null, null);
		}
    }

    public Uri addSIMMessageToInbox(
                    String address, String body, String subject, Long date,
                    boolean read, long threadId, int subId, String indexOnIcc,int callerUid, String callerPkg) {
		Uri inboxUri = Uri.parse("content://sms/inbox");
		return addSIMMessageToDb(inboxUri, address, body,
	                        subject, date, read, false, threadId, subId, indexOnIcc,callerUid, callerPkg);
    }

    public Uri addSIMMessageToSent(
                    String address, String body, String subject, Long date, long threadId, int subId, String indexOnIcc, 
int callerUid, String callerPkg) {
		Uri sentUri = Uri.parse("content://sms/sent");
		return addSIMMessageToDbSent(sentUri, address, body,
	                        subject, date, true, false, threadId, subId, indexOnIcc,callerUid, callerPkg);
    }

    //chenshu add start
    public Uri addSIMMessageToDbSent(Uri uri, String address, String body, String subject,
            Long date, boolean read, boolean deliveryReport, long threadId, int subId, String indexOnIcc,int callerUid, String callerPkg) {
        android.util.Log.i("TAG", "addSIMMessageToDb: uri="+uri+",address="+address+",body="+body+",subId="+subId+",indexOnIcc="+indexOnIcc+"callerUid:"+callerUid+" callerPkg:"+callerPkg);
        ...
        if (deliveryReport) {
        	values.put("status", 32 /*STATUS_PENDING*/ );
        }
        if (threadId != -1L) {
        ...
        values.put("index_on_icc", indexOnIcc);
        int type = Sms.MESSAGE_TYPE_SENT;
        long rowID = 0;
        boolean importSms = false;
        ContentValues initialValues = values;
        String table = TABLE_SMS;
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        db.beginTransaction();
        try {
            if (table.equals(TABLE_SMS)) {
                ...
                /// after restored. set the date as the right value when import.
                     ...
                     date = values.getAsLong(Sms.DATE);
                    values.put(Sms.DATE, date);
                    Log.d(TAG, "insert sms date " + date);
                /// @}
                }
                ...
                // thread_id
                Long threadId2 = values.getAsLong(Sms.THREAD_ID);
                address = values.getAsString(Sms.ADDRESS);

                if (((threadId2 == null) || (threadId2 == 0)) && (!TextUtils.isEmpty(address))) {
                    /// M: Code analyze 005, fix bug ALPS00245352, it cost long time
                    /// to restore messages. remove useless operation and add transaction
                    /// while import sms. @{
                    Log.d(TAG, "insert sms getThreadId start");
...
//                setThreadStatus(db, values, 0);
    ...
    public Uri addSIMMessageToDb(
        ...
        ContentValues initialValues = values;
        String table = TABLE_SMS;
        int type = Sms.MESSAGE_TYPE_INBOX;
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        db.beginTransaction();
        try {
            if (table.equals(TABLE_SMS)) {
...
//                setThreadStatus(db, values, 0);
    ...
    private int deleteOneSimSms(SQLiteDatabase db, int message_id) {
        int simId = -1;
        String indexOnIcc = null;
        // Find the thread ID that the specified SMS belongs to.
        Cursor c = db.query("sms", new String[] { "sub_id", "index_on_icc" },
                            		"_id=" + message_id, null, null, null, null);
        if (c != null) {
            if (c.moveToFirst()) {
                simId = c.getInt(0);
                indexOnIcc = c.getString(1);
            }
            c.close();
        }
        deleteSimMessage(indexOnIcc, simId);
        return 1;
    }

    private int deleteSimMessage(String indexOnIcc, int simId) {
		return deleteSimMessage(getContext(), indexOnIcc, simId);
    }

    private static int deleteSimMessage(Context context, String indexOnIcc, int simId) {
		int count = 0;
		if(indexOnIcc != null) {
		    String[] index = indexOnIcc.split(";");
		    String message_id = "";
		    long slotId = simId;//EncapsulatedSimInfoManager.getSlotById(context, simId);
		    for(int i=0;i<index.length;i++) {
				message_id = index[i];
			    count += getSmsProvider().deleteMessageFromIcc(message_id, (int)slotId);
		    }
		}
		return count;
    }

    private int deleteSimSms(SQLiteDatabase db, String selection, String[] selectionArgs) {
		return deleteSimSms(getContext(), db, selection, selectionArgs);
    }

    public static int deleteSimSms(Context context, SQLiteDatabase db, String selection, String[] selectionArgs){//8888888
        int simId = -1;
        String indexOnIcc = null;
	/*Cursor c = null;
	if (selection != null && selection.equals(FOR_MULTIDELETE)) {
            String selectids = getSmsIdsFromArgs(whereArgs);
            String sql = String.format("SELECT sim_id,index_on_icc FROM sms WHERE _id IN %s", selectids);
            c = db.rawQuery(sql, null);
	} else {
	    c = db.query("sms", new String[] {"sim_id", "index_on_icc"}, selection, selectionArgs, null, null, null);    
        }*/
        ...
        if (c != null) {
            try {
                while (c.moveToNext()) {
                    simId = c.getInt(0);
                    indexOnIcc = c.getString(1);
                    deleteSimMessage(context, indexOnIcc, simId);
                }
            } finally {
                c.close();
            }
    ...
    /*private static int deleteMessageFromIcc(int simindex, int subscription, ContentResolver cr) {
        try {
            return EncapsulatedGeminiSmsManager.deleteMessageFromIccGemini(simindex, subscription) ? 1 : 0;
        } catch (NumberFormatException exception) {
            throw new IllegalArgumentException("Bad SMS ICC ID: " + simindex);
        } finally {
            Uri iccUri = getIccUri(subscription);
            cr.notifyChange(iccUri, null);
        }
    }*/


	private int deleteMessageFromIcc(SQLiteDatabase db, String messageIndexString, long slotId) {

        long token = Binder.clearCallingIdentity();
        try {
	    //boolean res = EncapsulatedGeminiSmsManager.deleteMessageFromIccGemini(
        //            Integer.parseInt(messageIndexString), slotId);
		boolean res = SmsManager.getSmsManagerForSubscriptionId((int)slotId).deleteMessageFromIcc(Integer.parseInt(messageIndexString));
	    if(res) {
		//EncapsulatedSimInfoManager si = EncapsulatedSimInfoManager.getSimInfoBySlot(getContext(), slotId);
        //	if (null == si) {
        //    	    Log.e(TAG, "insertMessage:SIMInfo is null for slot " + slotId);
        //	} else {
		    int simId = (int)slotId;//(int)si.getSimId();
		    String selectionIcc = "index_on_icc=? and sub_id=" + simId;
		    String[] selectionArgs = new String[] { messageIndexString };
		    MmsSmsDatabaseHelper.deleteSimSmsOnDB(db, selectionIcc, selectionArgs);
		//	}
			return 1;
	    } else {
			return 0;
    ...
    private int deleteMessageFromIcc(SQLiteDatabase db, String messageIndexString) {
        SmsManager smsManager = SmsManager.getDefault();

        long token = Binder.clearCallingIdentity();
        try {
	    boolean res = smsManager.deleteMessageFromIcc(Integer.parseInt(messageIndexString));
	    if(res) {
		String selectionIcc = "index_on_icc=?";
		String[] selectionArgs = new String[] { messageIndexString };
		MmsSmsDatabaseHelper.deleteSimSmsOnDB(db, selectionIcc, selectionArgs);
		return 1;
	    } else {
    ...
    static int deleteMessages(Context context, SQLiteDatabase db,
        ...
        int count = 0;
        while (deleteCount > 0) {
	    deleteSimSms(context, db, selection, selectionArgs);
            deleteCount = db.delete(TABLE_SMS, selection, selectionArgs);
            count += deleteCount;
            Log.d(TAG, "deleteMessages, delete " + deleteCount + " sms");
        }
