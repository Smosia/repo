    ...
    private boolean isEquals(List<String> a,
        ...
        while (aTemp.size() > 0) {
            ...
            for (int i = 0; i < bTemp.size(); i++) {
                ...
                */
            	boolean isBrazilConfig = sContext.getResources().getBoolean(com.android.internal.R.bool.config_use_brazil_number_format);
            	if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
            		if (aTemp.get(0).equals(bTemp.get(i)) || PhoneNumberUtils.compareWithAreaCode(
                            aTemp.get(0), bTemp.get(i))) {
                        bTemp.remove(i);
                        aTemp.remove(0);
                        result = true;
                        break;
