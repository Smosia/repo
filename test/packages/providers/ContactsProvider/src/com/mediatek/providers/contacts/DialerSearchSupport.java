...
//    }
    ...
    private static String createCacheOffsetsTableSelect(String filterNum) {
        String offsetsSelect = " SELECT "
                ...
                + DialerSearchLookupColumns.NAME_TYPE + ",'"
                + filterNum + "'" + ")"
                //add jingyi dialer search for russia start
                + getExtraTable(filterNum, Tables.DIALER_SEARCH, " LEFT JOIN ("
                + "SELECT _id as raw_id, contact_id," + RawContacts.SORT_KEY_PRIMARY
                + " FROM " + Tables.RAW_CONTACTS + ") AS raw_contact_info ON "
                + "raw_contact_info.raw_id=" + DialerSearchLookupColumns.RAW_CONTACT_ID)
                //add jingyi dialer search for russia end
	...
	//add jingyi dialer search for russia start
	private static String getExtraTable(String filterParam, String table, String join) {
        String extraWhere = null;
        String searchContent = null;
        String extraTable = null;
        String extraTableColumns = null;
        if (filterParam.startsWith("8")) {
            searchContent = "+7"+filterParam.substring(1);
        } else if (filterParam.startsWith("+7")) {
            searchContent = "8"+filterParam.substring(2);
        }
                    	        
        if (searchContent != null && android.os.SystemProperties.getInt("ro.rgk_russia_number_match", 0) == 1) {
            extraWhere = "DIALER_SEARCH_MATCH_FILTER("
                    + DialerSearchLookupColumns.NORMALIZED_NAME + ","
                    + DialerSearchLookupColumns.SEARCH_DATA_OFFSETS + ","
                    + DialerSearchLookupColumns.NAME_TYPE + ",'"
                    + searchContent + "'" + ")";
            extraTableColumns = getOffsetsTempTableColumns(ContactsContract.Preferences.DISPLAY_ORDER_PRIMARY, searchContent)+ "," + RawContacts.CONTACT_ID;
            extraTable = " UNION SELECT " + extraTableColumns  + " FROM " + table +join+ " WHERE " + extraWhere;
        } else {
            extraTable = "";
        }
            ...
            if (firstNum >= 0 && firstNum <= 9 && mIsCached
                ...
                if (filterParam.length() == 1) {
                    ...
                    offsetsSelect = " SELECT "
                        ...
                        + filterParam + "'" + ")"
                        //add jingyi dialer search for russia start
                        +getExtraTable(filterParam, cachedOffsetsTable," JOIN " + "(select "
                        + DialerSearchLookupColumns._ID + ","
                        + DialerSearchLookupColumns.NORMALIZED_NAME + ","
                        + DialerSearchLookupColumns.SEARCH_DATA_OFFSETS + " from "
                        + Tables.DIALER_SEARCH + ") AS ds_info " + " ON (_id = ds_id)")
                ...
                String dsOffsetsTable = " SELECT "
                        ...
                        + filterParam + "'" + ")"
                        //add jingyi dialer search for russia start
                        +getExtraTable(filterParam, Tables.DIALER_SEARCH, " LEFT JOIN ("
                        + "SELECT _id as raw_id, contact_id,"
                        + ((mSortOrder == ContactsContract.Preferences.SORT_ORDER_ALTERNATIVE)
