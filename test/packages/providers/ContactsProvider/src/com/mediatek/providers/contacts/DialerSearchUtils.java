...
public class DialerSearchUtils {
    ...
    public static Cursor queryPhoneLookupByNumber(SQLiteDatabase db,
            ...
            String[] selectionArgs, String groupBy, String having, String sortOrder, String limit, boolean isBrazilConfig) {
        ...
        //modify for positivo JWLW-84 songqingming 20131023 (start)
        /*
        ...
        */
        if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
            return getCustomMatchedCursor(qb.query(db, projection, selection, selectionArgs, groupBy, having,
                    sortOrder, limit), normalizedNumber);
        } else {
            return qb.query(db, projection, selection, selectionArgs, groupBy, having,
                    sortOrder, limit);
        }
        //modify for positivo JWLW-84 songqingming 20131023 (end)
    }

    //add for positivo JWLW-84 songqingming 20131023 (start)
    public static Cursor getCustomMatchedCursor(Cursor cursor, String normalizedNumber) {
        ...
        while (cursor.moveToNext()) {
            String oldNumber = cursor.getString(cursor.getColumnIndexOrThrow(Phone.NUMBER));
            boolean equal = PhoneNumberUtils.compareWithAreaCode(normalizedNumber, oldNumber);
            Log.d("sqm", "DialerSearchUtils [getCustomMatchedCursor] number="+normalizedNumber+",oldNumber="+oldNumber+",equal="+equal);
            if (equal) {
                int length = columnNames.length;
                String[] contactsValues = new String[length];
                for (int i = 0; i < length; i++) {
                    contactsValues[i] = cursor.getString(i);
                }
                matrixCursor.addRow(contactsValues);
                break;
            }
        }
        cursor.close();
    ...
    //add for positivo JWLW-84 songqingming 20131023 (end)
