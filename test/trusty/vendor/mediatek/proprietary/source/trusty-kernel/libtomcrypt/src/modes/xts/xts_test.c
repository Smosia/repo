...
/* #21, 32 byte key, 31 byte PTX */
{
   ...
   pxts = (symmetric_xts *)malloc(sizeof(symmetric_xts));
#else
   pxts = &xts;
#endif
   for (i = 0; i < (int)(sizeof(tests)/sizeof(tests[0])); i++) {
       err = xts_start(idx, tests[i].key1, tests[i].key2, tests[i].keylen/2, 0, pxts);
       if (err != CRYPT_OK) {
#ifdef MTK_TEE_SUPPORT
          free(pxts);
#endif
       ...
       err = xts_encrypt(tests[i].PTX, tests[i].PTLEN, OUT, T, pxts);
       if (err != CRYPT_OK) {
          xts_done(pxts);
#ifdef MTK_TEE_SUPPORT
          free(pxts);
       ...
       err = xts_decrypt(tests[i].CTX, tests[i].PTLEN, OUT, T, pxts);
       if (err != CRYPT_OK) {
          xts_done(pxts);
#ifdef MTK_TEE_SUPPORT
          free(pxts);
