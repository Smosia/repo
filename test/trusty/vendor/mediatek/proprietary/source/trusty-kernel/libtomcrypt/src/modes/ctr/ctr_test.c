...
/* 128-bit key, 36-byte pt */
{
  ...
  pctr = (symmetric_CTR *)malloc(sizeof(symmetric_CTR));
#else
  pctr = &ctr;
#endif
  for (x = 0; x < (int)(sizeof(tests)/sizeof(tests[0])); x++) {
     if ((err = ctr_start(idx, tests[x].IV, tests[x].key, tests[x].keylen, 0, CTR_COUNTER_BIG_ENDIAN|LTC_CTR_RFC3686, pctr)) != CRYPT_OK) {
#ifdef MTK_TEE_SUPPORT
        free(pctr);
#endif
        return err;
     }
     if ((err = ctr_encrypt(tests[x].pt, buf, tests[x].msglen, pctr)) != CRYPT_OK) {
#ifdef MTK_TEE_SUPPORT
        free(pctr);
#endif
        return err;
