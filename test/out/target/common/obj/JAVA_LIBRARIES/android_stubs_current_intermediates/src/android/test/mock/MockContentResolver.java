package android.test.mock;
public class MockContentResolver
  extends android.content.ContentResolver
{
public  MockContentResolver() { super((android.content.Context)null); throw new RuntimeException("Stub!"); }
public  MockContentResolver(android.content.Context context) { super((android.content.Context)null); throw new RuntimeException("Stub!"); }
public  void addProvider(java.lang.String name, android.content.ContentProvider provider) { throw new RuntimeException("Stub!"); }
public  void notifyChange(android.net.Uri uri, android.database.ContentObserver observer, boolean syncToNetwork) { throw new RuntimeException("Stub!"); }
}
