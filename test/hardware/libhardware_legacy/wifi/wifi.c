...
/*
 * Fix multi-thread sync bug in wifi_send_command.
 * ex. FWK call wifi_send_command in two threads for wlan0 and p2p0 respectively,
 *     it may have chance to get wrong return value from socket due to thread preemption.
 */
#include <pthread.h>
...
#ifdef CONFIG_MEDIATEK_DEBUG
int sync_change_wlan_driver_state(char *status)
{
    char driver_status[PROPERTY_VALUE_MAX] = {'\0'};
    const prop_info *pi;
    unsigned serial = 0, i;
    int count = 20; /* wait at most 20 seconds for completion */
    pi = __system_property_find(DRIVER_PROP_NAME);
    if (pi != NULL)
        serial = __system_property_serial(pi);

    property_set(DRIVER_PROP_NAME, status);
    sched_yield();
    ...
    while (count-- > 0) {
        ...
        if (pi != NULL && __system_property_serial(pi) != serial) {
            /*
...
#endif
#ifdef CONFIG_MEDIATEK_DEBUG
    return sync_change_wlan_driver_state("ok");
#endif
    property_set(DRIVER_PROP_NAME, "ok");
    return 0;
#endif
...
#endif
#ifdef CONFIG_MEDIATEK_DEBUG
    return sync_change_wlan_driver_state("unloaded");
#endif
    property_set(DRIVER_PROP_NAME, "unloaded");
    return 0;
#endif
