...
public class KeyguardSecurityContainer extends FrameLayout implements KeyguardSecurityView {
    ...
    private final String DB_ACTION_UNLOCK = "action_unlock";
    private final SensorCallback mSensorCallback = new SensorCallback(){
        public void onUnlock() {
            if(mCallback != null){
                Log.d(TAG, "Set keyguard dismiss.");
    ...
    public KeyguardSecurityContainer(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (Settings.System.getInt(mContentResolver, DB_ACTION_UNLOCK, -1) == 1) {
            Log.d(TAG, "Action unlock is enable in SmartMotion.");
            mSensorListener = new SensorListener(mContext, mSensorCallback);
            if(mSensorListener != null){
                Log.d(TAG, "Set sensor enable for Action Unlock.");
    ...
    @Override
    public void onPause() {
        if (DEBUG) Log.d(TAG, "onPause() setEnable = " + setEnable) ;
        //A: kuangyunfeng 20160112(start)
        ContentResolver mContentResolver = mContext.getContentResolver();
        boolean switchState = Settings.System.getInt(mContentResolver, DB_ACTION_UNLOCK, -1) == 1;
        Log.d(TAG, "onPause() switchState = " + switchState) ;
        if(!switchState) setEnable = false;

        if (mSensorListener != null && !setEnable) {
            Log.d(TAG,"Set the proximity listener enable(false) in onPause().");
