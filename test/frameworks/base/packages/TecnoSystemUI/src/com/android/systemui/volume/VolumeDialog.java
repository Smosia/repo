 ...
 */
 /**********************************
 *History
 *1.lei.zhou@ragentek.com 20160318 BUG_ID:DELSLMY-856
 *Description:Drag the progress bar displays the default ringtone
 *
 *
 *
 *
...
//add BUG_ID:DELSLMY-856 by zhoulei 20160318 (start)
import android.media.Ringtone;
import android.media.RingtoneManager;
import android.net.Uri;
import com.mediatek.audioprofile.AudioProfileManager;
import android.provider.Settings;
//add BUG_ID:DELSLMY-856 by zhoulei 20160318 (end)
...
public class VolumeDialog {
    ...
    private final class VolumeSeekBarChangeListener implements OnSeekBarChangeListener {
        ...
        private final int MSG_SAMPLE_START = 0;
        private final int MSG_SAMPLE_STOP = 1;
        ...
        private AudioManager mAudioManager;
       	private Handler ringHandler = new Handler() {
					public void handleMessage(android.os.Message msg) {
						switch (msg.what) {
						case MSG_SAMPLE_START:
						//int i =1;
							onStartSample();
							break;
						case MSG_SAMPLE_STOP:
						//int i =1;
							onStopSample();
							break;
						default:
        ...
        private VolumeSeekBarChangeListener(VolumeRow row) {
            mRow = row;
            //add BUG_ID:DELSLMY-856 by zhoulei 20160318 (start)
            mProfileManager = (AudioProfileManager) mContext
                .getSystemService(Context.AUDIO_PROFILE_SERVICE);
            mAudioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);   
           /*if (mRow.stream == AudioProfileManager.STREAM_RING) {
                defaultUri = mProfileManager.getRingtoneUri(KEY_RING_VOLUME,AudioProfileManager.TYPE_RINGTONE);
            } else */if (mRow.stream == AudioProfileManager.STREAM_ALARM) {
            		defaultUri = Settings.System.DEFAULT_ALARM_ALERT_URI;
                //defaultUri = mProfileManager.getRingtoneUri(KEY_NOTIFICATION_VOLUME,AudioProfileManager.TYPE_NOTIFICATION);
            } else {
                mRingtone = null;
                return;
            }
            ...
            if (mRingtone != null) {
            	mRingtone.setStreamType(mRow.stream);
        		}
        		//add BUG_ID:DELSLMY-856 by zhoulei 20160318 (end)
        }
        ...
        public void onStopTrackingTouch(SeekBar seekBar) {
        	...
        	//add BUG_ID:DELSLMY-856 by zhoulei 20160318 (start)
			    private void postStartSample() {
			        ringHandler.removeMessages(MSG_SAMPLE_START);
			        ringHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_SAMPLE_START),
			               (mRingtone != null && mRingtone.isPlaying()) ? CHECK_RINGTONE_PLAYBACK_DELAY_MS : 0);
			    }
			    
			    private void onStartSample() {
			        //add DELSLM-197 wanglin 20160216 start
			        if (mAudioManager != null) {
			            if (AudioManager.AUDIOFOCUS_REQUEST_FAILED == mAudioManager.requestAudioFocus(null, AudioManager.STREAM_MUSIC,AudioManager.AUDIOFOCUS_GAIN_TRANSIENT)) {
			                return;
			            }
			        }
			        //add DELSLM-197 wanglin 20160216 end
			        if (!(mRingtone != null && mRingtone.isPlaying())) {
			            ...
			            if (mRingtone != null) {
			                try {
			                    //add DWYSLM-34 guowen 201600201 start
			                    if (mRow.stream == AudioProfileManager.STREAM_RING) {
			               				 defaultUri = mProfileManager.getRingtoneUri(KEY_RING_VOLUME,AudioProfileManager.TYPE_RINGTONE);
			            				} else if (mRow.stream == AudioProfileManager.STREAM_NOTIFICATION) {
			               				 defaultUri = mProfileManager.getRingtoneUri(KEY_NOTIFICATION_VOLUME,AudioProfileManager.TYPE_NOTIFICATION);
			            				} else {
			               				 defaultUri = Settings.System.DEFAULT_ALARM_ALERT_URI;
			            				}
			                    mRingtone.setUri(defaultUri);
			                    //add DWYSLM-34 guowen 201600201 end
			                    mRingtone.play();
			                } catch (Throwable e) {
			                    Log.w(TAG, "Error playing ringtone, stream " + mRow.stream, e);
			                }
			            }
			        }
			    }
			    ...
			    void postStopSample() {
			        ...
			        mHandler.removeMessages(MSG_SAMPLE_START);
			        mHandler.removeMessages(MSG_SAMPLE_STOP);
			        mHandler.sendMessage(mHandler.obtainMessage(MSG_SAMPLE_STOP));
			    }
