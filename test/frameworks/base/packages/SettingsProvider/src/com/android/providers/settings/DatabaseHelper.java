...
class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String SET_LOSSLESSBT_ENABLED = "LosslessBT_Status=1";
    private static final String SET_LOSSLESSBT_DISABLED = "LosslessBT_Status=0";
    //add guowen 20160304 end
 //A: JWBLL-2424 liuhao 20150717
       ...
       if (upgradeVersion == 51) {
        ...
        if (upgradeVersion == 55) {
            ...
            try {
                stmt = db.compileStatement("INSERT INTO system(name,value)"
                        + " VALUES(?,?);");
                loadSetting(stmt, Global.SET_INSTALL_LOCATION,R.integer.def_set_install_location); // A: DWLEBM-42 lihuachun 20160310
    ...
    private void loadVibrateWhenRingingSetting(SQLiteDatabase db) {
        ...
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO system(name,value)"
                    + " VALUES(?,?);");
            //modify DWLJE-574 songqingming 20150825 (start)
            //loadSetting(stmt, Settings.System.VIBRATE_WHEN_RINGING, vibrateWhenRinging ? 1 : 0);
            if (mContext.getResources().getBoolean(R.bool.def_vibrate_when_ring_enable)) {
        	loadSetting(stmt, Settings.System.VIBRATE_WHEN_RINGING, 1);
            } else {
                loadSetting(stmt, Settings.System.VIBRATE_WHEN_RINGING, vibrateWhenRinging ? 1 : 0);
            }
            //modify DWLJE-574 songqingming 20150825 (end)
        } finally {
            if (stmt != null) stmt.close();
        }
    }
 ...
 //A: JWBLL-2424 liuhao 20150717
            boolean batteryPercentage = mContext.getResources().getBoolean(com.android.internal.R.bool.config_battery_percentage_display);
            if(batteryPercentage){
                loadIntegerSetting(stmt, BATTERY_PERCENTAGE_ENABLE, R.integer.def_battery_percentage_display);
			}else{
                loadIntegerSetting(stmt, BATTERY_PERCENTAGE_ENABLE, R.integer.def_battery_percentage_enable);
			}
            //A: }
            ...
            boolean boldefplus = mContext.getResources().getBoolean(com.android.internal.R.bool.def_music_plus_enabled);
            if(boldefplus){
                mAudioManager.setParameters(SET_MUSIC_PLUS_ENABLED);
            }else{
                mAudioManager.setParameters(SET_MUSIC_PLUS_DISABLED);
            }
            boolean besplus = mContext.getResources().getBoolean(com.android.internal.R.bool.def_bes_loudness_enabled);
            if(besplus) {
                mAudioManager.setParameters(SET_BESLOUDNESS_ENABLED);
            } else {
                mAudioManager.setParameters(SET_BESLOUDNESS_DISABLED);
            }
            boolean Losslessplus = mContext.getResources().getBoolean(com.android.internal.R.bool.def_Lossless_BT_enabled);
            if(Losslessplus) {
                mAudioManager.setParameters(SET_LOSSLESSBT_ENABLED);
            } else {
                mAudioManager.setParameters(SET_LOSSLESSBT_DISABLED);
            }
            //add guowen 20160304 end
			
			
            //add DWYBL-2577 songqingming 20151023 (start)       
            loadIntegerSetting(stmt, Settings.System.FLOAT_WINDOW_WHEN_CALLING,
                    R.integer.def_float_window_when_calling);
            //add DWYBL-2577 songqingming 20151023 (end)
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            loadIntegerSetting(stmt, Settings.Secure.ACCESSIBILITY_CAPTIONING_ENABLED,
                    R.integer.def_accessibility_captioning_enabled);
            //add BUG_ID:DELSLMY-181 dushuaihui 20160303(end)
            loadStringSetting(stmt, Settings.Secure.DEFAULT_INPUT_METHOD, R.string.def_ime);// Add DWYQLSSMY-364 by yeyu for default IME
            // Add by sunjie for enabled IME begin
            String enabledIme = "";
            ...
            for (android.content.pm.ResolveInfo resolveInfo : services) {
                ...
                String imeId = new ComponentName(serviceInfo.packageName, serviceInfo.name).flattenToShortString();
                Log.i(TAG, "serviceInfo.name:" + serviceInfo.name + " imeId:" + imeId);
                if (enabledIme.isEmpty()) {
                    enabledIme = enabledIme + imeId;
                } else {
                    enabledIme = enabledIme + ":" + imeId;
                }
            }
            if (!enabledIme.isEmpty()) {
                loadSetting(stmt, Settings.Secure.ENABLED_INPUT_METHODS, enabledIme);
            }
            // Add by sunjie for enabled IME end
            /*
             * IMPORTANT: Do not add any more upgrade steps here as the global,
             * secure, and system settings are no longer stored in a database
             * but are kept in memory and persisted to XML.
    ...
    private void loadGlobalSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            //A:DELSLM-116 liuhuaizhi 20160203 (start)
...
//                    SystemProperties.get("ro.mtk_gemini_support").equals("1") ? 0 : 1));
            ...
            loadSetting(stmt, Settings.Global.MOBILE_DATA,
                    mUtils.getValue(Settings.Global.MOBILE_DATA, mContext.getResources().getInteger(R.integer.def_mobile_data_default_off)));
            //A:DELSLM-116 liuhuaizhi 20160203 (end)
            ...
            loadSetting(stmt, Settings.Global.SET_INSTALL_LOCATION,R.integer.def_set_install_location); // A: DWLEBM-42 lihuachun 20160310
            loadSetting(stmt, Settings.Global.DEFAULT_INSTALL_LOCATION,
                    PackageHelper.APP_INSTALL_AUTO);
 loadIntegerSetting(stmt, Settings.Global.DEVELOPMENT_SETTINGS_ENABLED,
                    R.integer.def_develop_options_header_switch_bar);  // A:DWLEBM-45 lihuachun 20160311
			...
			//modify   BUG_ID:JLLB-148  xieshanlin  20140518(start)
            if("1".equals(SystemProperties.get("ro.rgk_app_wake_gesture"))){
            loadIntegerSetting(stmt, Settings.Global.BLACK_GESTURE_WAKE,
                    R.integer.def_black_gesture_wake);}
			//modify   BUG_ID:JLLB-148  xieshanlin  20140518(end)
            ...
            loadIntegerSetting(stmt, Settings.Global.TELEPHONY_AUTO_CALL_RECORD_CONFIG,
                    R.integer.def_auto_call_record_config);
