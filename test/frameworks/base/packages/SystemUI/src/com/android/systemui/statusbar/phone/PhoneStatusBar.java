...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    //add BUG_ID:DWYQLSSMY-476 dushuaihui 20160406(start)
    private TextView mCarrierLabel;
    private boolean mShowCarrierInPanel = false;
    private boolean mCarrierLabelVisible = false;
    private int mCarrierLabelHeight;
    private int mStatusBarHeaderHeight;
    //add BUG_ID:DWYQLSSMY-476 dushuaihui 20160406(end)
    ...
    //A:JWBLB-2271 guoshuai 20160222(start)
    private boolean mHasNavigationBar;
    AlertDialog mNavibarNoticeDialog;
    private NavigationBarView mRecordNavigationBarView = null;
    private boolean mIsNavibarEnabled;
    //A:JWBLB-2271 guoshuai 20160222(end)
    ...
    //A:DELSLM-56 guoshuai 20160203(start)
    final private ContentObserver mBatteryPercentageObserver = new ContentObserver(new Handler()) {
        public void onChange(boolean selfChange) {
            final boolean enable = Settings.System.getInt(mContext.getContentResolver(),
                    BATTERY_PERCENTAGE_ENABLE, 0) != 0;
            mBatteryLevel.setVisibility(enable ? View.VISIBLE : View.GONE);
            mHeader.updateBatteryLevelVisible(enable);
            mKeyguardStatusBar.updateBatteryLevelVisible(enable);
        };
    };
    //A:DELSLM-56 guoshuai 20160203(end)
    ...
    public ImageView mBackdropFront, mBackdropBack;
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        try {
            ...
            //add BUG_ID:JBLW-93 daizhiyi 20131123 (start)
            mHasNavigationBar = showNav;
            //add BUG_ID:JBLW-93 daizhiyi 20131123 (end)
            ...
            if (showNav) {
                ...
                //add BUG_ID:JBLW-93 daizhiyi 20131123 (start)
                mRecordNavigationBarView = mNavigationBarView;
                //add BUG_ID:JBLW-93 daizhiyi 20131123 (end)
                Settings.System.putInt(mContext.getContentResolver(), "is_navibar_shown",1);//A:JWBLB-2271 guoshuai 20160222
            }
        ...
        //add BUG_ID:DELSLMY-375 dushuaihui 20160303(start)
        mHeader.setService(this);
        //add BUG_ID:DELSLMY-375 dushuaihui 20160303(end)
        ...
        //A:DELSLMY-686 guoshuai 20160308(start)
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(BATTERY_PERCENTAGE_ENABLE), true, mBatteryPercentageObserver);
        ...
        //add BUG_ID:DWYQLSSMY-476 dushuaihui 20160406(start)
        mCarrierLabel = (TextView)mNotificationPanel.findViewById(R.id.carrier_label);
        ...
        //A:DWQBE-104 guoshuai 20151012(start)
        if (mHasNavigationBar && mNavigationBarView != null) {
            mContext.getContentResolver().registerContentObserver(
                    Settings.Secure.getUriFor("accessibility_navibar_enabled"),
                    false,
                    mNavibarSettingsObserver);
            mContext.getContentResolver().registerContentObserver(
                    Settings.Secure.getUriFor("accessibility_navibar_direction"), 
                    false, 
                    mNavibarSettingsObserver);
            mIsNavibarEnabled = Settings.Secure.getInt(
                    mContext.getContentResolver(),
                    "accessibility_navibar_enabled",
                    mContext.getResources().getInteger(R.integer.config_navbar_show_default)
                    ) == 1;
            mNavigationBarView.setNavibarEnabled(mIsNavibarEnabled);
            mNavigationBarView.getNavigationBarButton().setVisibility(
                mIsNavibarEnabled ? View.VISIBLE : View.INVISIBLE);
        }
        //A:DWQBE-104 guoshuai 20151012(end)
    ...
    private ContentObserver mNavibarSettingsObserver = new ContentObserver(new Handler()){
        public void onChange(boolean selfChange, android.net.Uri uri) {
            ...
            if(lastPathSegment.equals("accessibility_navibar_direction")) {
                handleNaviDirectionChange();
            } else if (lastPathSegment.equals("accessibility_navibar_enabled")) {
                handleNavibarHide();
            }
        }

        private void handleNavibarHide(){
        ...
        private void handleNaviDirectionChange(){
            final boolean mNormalDirection = Settings.Secure.getInt(
                    mContext.getContentResolver(), "accessibility_navibar_direction", 0) == 0;
            if(mNavigationBarView != null) {
    ...
    //A:JWBLB-2271 guoshuai 20160222(start)
    private void dismissNavibarNoticeDialog() {
        if (mNavibarNoticeDialog != null) {
            mNavibarNoticeDialog.dismiss();
        }
    }

    private void showNavibarNoticeDialog() {
        if (mNavibarNoticeDialog == null) {
            AlertDialog.Builder b = new AlertDialog.Builder(mContext);
                final LayoutInflater dialogInflater = LayoutInflater.from(b.getContext());
                final View view = dialogInflater.inflate(R.layout.navigation_bar_notice, null, false);
                b.setCancelable(false);
                b.setTitle(R.string.show_remove_naviagtaionbar_title);
                b.setView(view);
                b.setPositiveButton(com.android.internal.R.string.ok, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface arg0, int arg1) {
                        CheckBox neverNotice = (CheckBox) view.findViewById(R.id.never_notice);
                        if (neverNotice.isChecked()) {
                            Settings.System.putInt(mContext.getContentResolver(), "show_navibar_notice", 1);
                        }
                        dismissNavibarNoticeDialog();
                    }
                });
                b.setNegativeButton(com.android.internal.R.string.cancel, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface arg0, int arg1) {
                        dismissNavibarNoticeDialog();
                    }
                });
            ...
            d.setOnDismissListener(new DialogInterface.OnDismissListener() {
                ...
                public void onDismiss(DialogInterface dialog) {
                    mNavibarNoticeDialog = null;
                }
            });
            d.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
            d.show();
            mNavibarNoticeDialog = d;
        }
    }

    private View.OnClickListener mNavigationBarClickListener = new View.OnClickListener() {
        public void onClick(View v) {
            mIsNaviButtonClicked = true;
        }
    };
    private boolean mIsNaviButtonClicked;
    //A:JWBLB-2271 guoshuai 20160222(end)
    ...
    private void prepareNavigationBarView() {
        ...
        //A:JWBLB-2271 guoshuai 20160222(start)
        mNavigationBarView.getNavigationBarButton().setOnClickListener(mNavigationBarClickListener);
        ((KeyButtonView)mNavigationBarView.getNavigationBarButton())
            .getKeyButtonRipple().setOnAnimationEndListener(new KeyButtonRipple.OnAnimationEndListener() {

                @Override
                public void onAnimationEnd() {
                    if(!mIsNaviButtonClicked) return;
                    if((Settings.System.getInt(mContext.getContentResolver(), "show_navibar_notice", 0) == 0)
                            && mNavibarNoticeDialog == null ) {
                        showNavibarNoticeDialog();
                    }
                    removeNavigationBar();
                    mIsNaviButtonClicked = false;
                }
        });
        //A:JWBLB-2271 guoshuai 20160222(end)
        mAssistManager.onConfigurationChanged();
    ...
    public void reloadNavigationBar() {
        if (DEBUG) Log.d(TAG, "reloadNavigationBar: about to reload " + mNavigationBarView);
        if (mNavigationBarView != null) return;

        if (mHasNavigationBar) {
            try {
                mNavigationBarView = mRecordNavigationBarView;
                mNavigationBarView.setDisabledFlags(mDisabled1);
                //A: JWBLL-908 daizhiyi 20150312 {
                ...
                mWindowManager.addView(mNavigationBarView, getNavigationBarLayoutParams());
                checkBarModes();
                Settings.System.putInt(mContext.getContentResolver(), "is_navibar_shown", 1);
    ...
    public void removeNavigationBar() {
        ...
        if (mHasNavigationBar) {
            try {
                ...
                mNavigationBarView = null;
                Settings.System.putInt(mContext.getContentResolver(), "is_navibar_shown", 0);
            } catch (IllegalArgumentException ex) {
                mNavigationBarView = null;
                Log.w(TAG, "WindowManager remove NavigationBarView has a IllegalArgumentException");
            }
        }
    }
    //add BUG_ID:JBLW-93 daizhiyi 20131123 (end)
    ...
    public void addNotification(StatusBarNotification notification, RankingMap ranking,
        ...
        //A:DELSLMY-377 guoshuai 20160408(start)
        boolean flagCollapsePanels = isHeadsUped;
        isHeadsUped = false;
        //A:DELSLMY-377 guoshuai 20160408(end)
        if (isHeadsUped) {
    ...
    //Add by zhaopenglin for keyguardwallpaper 20160303 start
    private boolean hasArtwork(){
        ...
        return artworkBitmap != null;
    }
    //Add by zhaopenglin for keyguardwallpaper 20160303 end
    ...
    //Add by zhaopenglin for keyguardwallpaper 20160303 start
     private void setKeyguardWallpaper(){
        if(!hasArtwork()){
            ...
            Drawable d = wallpaperManager.peekDrawableForKeyguard();
            if (d != null) {
                mBackdrop.setVisibility(View.VISIBLE);
                mBackdropBack.setImageDrawable(d);
            } else {
                Resources sysRes = Resources.getSystem();
                int res_id = sysRes.getIdentifier("keyguard_wallpaper", "drawable", "android");
                mBackdropBack.setImageResource(res_id);
            }
            // add by wangjian for keyguard wallpaper end
        }else {
            mBackdrop.setVisibility(View.GONE);
        }
    }
    ...
    //modify BUG_ID:DWYQLSSMY-476 dushuaihui 20160406(start)
    protected void updateCarrierLabelVisibility(boolean force) {
        ...
        if(mContext.getResources().getBoolean(R.bool.config_show_carrier_lable)){
            makeVisible = mState != StatusBarState.KEYGUARD;
        } else {
            makeVisible =
    ...
    //modify BUG_ID:DWYQLSSMY-476 dushuaihui 20160406(end)
    /// M: Support "Operator plugin - Customize Carrier Label for PLMN". @}
    ...
    private View.OnClickListener mFloatClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            toggleFloatApps();
            if(mIsSplitModeEnable){
                setFloatModeButtonVisibility(View.VISIBLE);
                setSplitModeButtonVisibility(View.VISIBLE);
                updateFloatModeButton(!mIsSplitModeOn);
                updateSpilitModeButton(mIsSplitModeOn);
            }
        }
