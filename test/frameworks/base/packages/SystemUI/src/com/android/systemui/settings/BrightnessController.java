...
public class BrightnessController implements ToggleSlider.Listener {
    ...
    public BrightnessController(Context context, ImageView icon, ToggleSlider control) {
        ...
        mUserTracker = new CurrentUserTracker(mContext) {
            ...
            public void onUserSwitched(int newUserId) {
                updateMode();
                updateSlider();
                //A:DWYQLSSMY-94 wanglin 20160329(start)
                if(mControl != null){
                	mControl.setChecked(mAutomatic);
                }
                //A:DWYQLSSMY-94 wanglin 20160329(end)
            }
        };
        ...
        if (!mAutomatic) {
            ...
            if (!tracking) {
                AsyncTask.execute(new Runnable() {
                        public void run() {
                            //M:DWLEBM-403 guoshuai 20160419(start)
                            try {
                            ...
                            //M:DWLEBM-403 guoshuai 20160419(end)
                        }
                    });
            }
        } else {
    ...
    //A:DWYQLSSMY-94 wanglin 20160329(start)
    @Override
    public void onModeChanged(ToggleSlider v, boolean tracking,
            boolean automatic, int value) {
    ...
    private void updateIcon(boolean automatic) {
        //M:DWYQLSSMY-94 wanglin 20160329(start)
        /*
        if (mIcon != null) {
        ...
        */
        if(mControl != null){
            int color = mContext.getResources().getColor(automatic 
            	    ? R.color.qs_brightness_label_focus_color
            	    	    : R.color.qs_brightness_label_normal_color);
            mControl.setTextColor(color);
        }
        //M:DWYQLSSMY-94 wanglin 20160329(end)
    }
    ...
    private void updateSlider() {
        if (mAutomatic) {
			...
			//A:DWYQLSSMY-94 wanglin 20160329(start)
        	if(mContext.getResources().getBoolean(R.bool.show_mControl)) {
            int value;
