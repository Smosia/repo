...
public class SignalStrength implements Parcelable {
    ...
    public int getLevel() {
        ...
        //add infinix standard by dushuaihui 20160303(start)
        if("1".equals(SystemProperties.get("ro.rgk_infinix_signal_support"))){
            // 4G network
            if(isCampOnLte()){
                ...
                log("TAG - 4G lteDbm = " + lteDbm + ", level = " + level);
                return level;
            }
            if (mGsmRscpQdbm < 0) {
                // 3G network
                int dBm = mGsmRscpQdbm / 4;
                if (dBm >= -89) {
                    level = SignalStrength.SIGNAL_STRENGTH_GREAT;
                } else if (dBm >= -97) {
                    level = SignalStrength.SIGNAL_STRENGTH_GOOD;
                ...
                log("TAG - 3G mGsmRscpQdbm = " + mGsmRscpQdbm + ", dBm = " + dBm + ", level = " + level);
            } else {
                // 2G network
                ...
                log("TAG - 2G mGsmRscpQdbm = " + mGsmRscpQdbm + ", asu = " + asu + ", dBm = " + dBm + ", level = " + level);
            }
            return level;
        }
        //add infinix standard by dushuaihui 20160303(end)
		
		
        //add MOBIISTAR standard BUG_ID:DWYQLSSMY-482 dushuaihui 20160405(start)
        if("1".equals(SystemProperties.get("ro.rgk_mobiistar_signal"))){
            int dbm = getDbm();
            if (dbm >= -89) {
            ...
            log("TAG - 2G/3G/4G dbm = " + dbm + ", level = " + level);
            return level;
        }
        //add MOBIISTAR standard BUG_ID:DWYQLSSMY-482 dushuaihui 20160405(end)
		

        //add blu standard by dushuaihui 20160419(start)
        if("1".equals(SystemProperties.get("ro.rgk_blu_signal_support"))){
            // 4G network
            if(isCampOnLte()){
                int lteDbm = getLteDbm();
                if(lteDbm >= -85){
        ...
        //add blu standard by dushuaihui 20160419(end)
