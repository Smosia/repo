...
public class PhoneNumberFormattingTextWatcher implements TextWatcher {
    ...
    public synchronized void afterTextChanged(Editable s) {
		...
		//modify by chenshuyue 20160302 start DWYQLSQ-56
        //String formatted = reformat(s, Selection.getSelectionEnd(s));
		//boolean isBrazilConfig = android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1;		
		 String formatted = null;
        if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 ) {
        	formatted = reformatForBrazil(s);
        } else {
        	formatted = reformat(s, Selection.getSelectionEnd(s));
        }
		//modify by chenshuyue 20160302 end DWYQLSQ-56
        if (formatted != null) {
	...
	//add by chenshuyue 20160302 start DWYQLSQ-56
	private String reformatForBrazil(CharSequence s) {
    	mFormatter.clear();
    	StringBuffer sb = new StringBuffer(s.toString().replaceAll("[\\s-]*", ""));
