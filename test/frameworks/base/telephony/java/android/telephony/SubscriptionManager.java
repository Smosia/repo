...
public class SubscriptionManager {
    ...
    //add BUG_ID:DWYQLSSMY-1005 dushuaihui 20160509(start)
    private void disableDataForOtherSubscriptions(int currentSirID) {
        final List<SubscriptionInfo> subInfoList = getActiveSubscriptionInfoList();
        if (subInfoList != null) {
            for (SubscriptionInfo subInfo : subInfoList) {
                if (subInfo.getSubscriptionId() != currentSirID) {
                	TelephonyManager.getDefault().setDataEnabled(subInfo.getSubscriptionId(), false);
                }
            }
        }
    }
    //add BUG_ID:DWYQLSSMY-1005 dushuaihui 20160509(end)
