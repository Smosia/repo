...
public class PhoneNumberUtils
{
    ...
    compareLoosely(String a, String b) {
        ...
        if (sIsCtaSupport) {
            ...
            //modify DWYLL-94 songqingming 20150408 (start)
            // minMatchLen = sPhoneNumberExt.getMinMatch();
            minMatchLen = MIN_MATCH;
            //modify DWYLL-94 songqingming 20150408 (end)
        }
        ...
        if (android.os.SystemProperties.getInt("ro.rgk_russia_number_match", 0) == 1) {
        	if (ia < 4 && ib < 4 && (a.startsWith("+") || b.startsWith("+"))
    ...
    static final int MIN_MATCH = SystemProperties.getInt("ro.rgk_number_min_match", 7);
    //modify DWYLL-94 songqingming 20150408 (end)
    ...
    */
    ...
    public static boolean isEmergencyNumberExt(String number, int phoneType) {
        ...
        if (PhoneConstants.PHONE_TYPE_CDMA == phoneType) {
            ...
            Rlog.d(LOG_TAG, "CDMA subId:" + subId + ", slotId:" + slotId
    ...
    private static void parseEccList() {
        ...
        try {
            ...
            //modify DELSLMY-752 songqingming 20160317 (start)
            //if (sIsCtaSet) {
            boolean isRmEcc120 = ("1").equals(SystemProperties.get("ro.rm_ecc_120_config"));
            if (sIsCtaSet && !isRmEcc120) {
            //modify DELSLMY-752 songqingming 20160317 (end)
     ...
     * @hide
     */
	public static boolean compareWithAreaCode(String newNumber, String oldNumber) {
		if (newNumber == null || oldNumber == null) {
			return false;
		}

		newNumber = newNumber.replaceAll("[^\\d]*", "");
		oldNumber = oldNumber.replaceAll("[^\\d]*", "");

		if (newNumber.equals(oldNumber)) {
			return true;
		}
		Log.i("Positivo_number_match", "newNumber:" + newNumber + "|oldNumber:" + oldNumber);

		String realNewNumber = getRealNumber(newNumber);
		String realOldNumber = getRealNumber(oldNumber);
		Log.i("Positivo_number_match", "realNewNumber:" + realNewNumber + "|realOldNumber:" + realOldNumber);

		String newAreaCode = getAreaCode(newNumber);
		String oldAreaCode = getAreaCode(oldNumber);
		Log.i("Positivo_number_match", "newAreaCode:" + newAreaCode + "|oldAreaCode:" + oldAreaCode);

		String localAreaCode = getAreaCode(getLocalNumber());
		Log.i("Positivo_number_match", "localAreaCode:" + localAreaCode);

		if (realNewNumber != null && realNewNumber.equals(realOldNumber)) {
			if (newAreaCode == null) {
				Log.i("Positivo_number_match", "newAreaCode is null, return true!");
				return true;
			} else {
				if (oldAreaCode == null) {
					if (localAreaCode == null) {
						Log.i("Positivo_number_match", "oldAreaCode is null and localAreaCode is null, return true!");
						return true;
					} else if (newAreaCode.equals(localAreaCode)) {
						Log.i("Positivo_number_match", "newAreaCode is the same with localAreaCode, return true!");
						return true;
					}
				} else if (newAreaCode.equals(oldAreaCode)) {
					Log.i("Positivo_number_match", "newAreaCode is the same with oldAreaCode, return true!");
					return true;
				}
			}
		}
		Log.i("Positivo_number_match", "return false!");
		return false;
	}
