...
public class AudioService extends IAudioService.Stub {
    ...
    private void sendVolumeUpdate(int streamType, int oldIndex, int index, int flags) {
        ...
        if ((mStreamVolumeAlias[streamType] == AudioSystem.STREAM_RING)||(mStreamVolumeAlias[streamType] == AudioSystem.STREAM_NOTIFICATION)) {
            notifyRingerVolumeChanged(oldIndex, index, Integer.toString(mStreamVolumeAlias[streamType]));
        }
        ...
        synchronized (mSafeMediaVolumeState) {
            ...
            if ((mMcc != mcc) || ((mMcc == 0) && force)) {
                ...
                if (safeMediaVolumeEnabled && !safeMediaVolumeBypass) {
                    ...
                    //add guowen 20160408 start
                    int volumeMusicSpeaker = Settings.System.getIntForUser(mContentResolver, "volume_music_speaker", AudioSystem.DEFAULT_STREAM_VOLUME[3], UserHandle.USER_CURRENT);
                    int volumeMusicHeadset = Settings.System.getIntForUser(mContentResolver, "volume_music_headset", AudioSystem.DEFAULT_STREAM_VOLUME[3], UserHandle.USER_CURRENT);
                    int safeMediaVolume = mContext.getResources().getInteger(com.android.internal.R.integer.config_safe_media_volume_index);
                    Log.v(TAG, "volumeMusicSpeaker=" + volumeMusicSpeaker + ";volumeMusicHeadset=" + volumeMusicHeadset + ";safeMediaVolume=" + safeMediaVolume);
                    if(volumeMusicSpeaker > safeMediaVolume){
                        Settings.System.putIntForUser(mContentResolver,"volume_music_speaker", safeMediaVolume, UserHandle.USER_CURRENT);
                    }
                    if(volumeMusicHeadset > safeMediaVolume){
                        Settings.System.putIntForUser(mContentResolver,"volume_music_headset", safeMediaVolume, UserHandle.USER_CURRENT);
                    }
                    mSafeMediaVolumeState = SAFE_MEDIA_VOLUME_ACTIVE;
                    enforceSafeMediaVolume(caller);
                    //add guowen 20160408 end
                } else {
