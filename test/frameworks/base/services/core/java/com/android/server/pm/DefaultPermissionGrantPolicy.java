...
final class DefaultPermissionGrantPolicy {
    ...
    /// M: Default grant permissions policy for multi-user when MoMs is on
    public void grantPermissionsToApps(int userId, boolean onlySystem) {
        Log.i(TAG, "Granting app permissions for user " + userId + " onlySystem " + onlySystem);

        synchronized (mService.mPackages) {
            for (PackageParser.Package pkg : mService.mPackages.values()) {
                if (!doesPackageSupportRuntimePermissions(pkg)
                        || pkg.requestedPermissions.isEmpty()
                        || (onlySystem && !pkg.isSystemApp())) {
    ...
    private void grantDefaultSystemHandlerPermissions(int userId) {
        ...
        synchronized (mService.mPackages) {
            ...
            //A: DWYQLSSMY-949, open system app permissions default, kui.li, 20160506 start {
            ...
            for(ApplicationInfo app: appInfoList) {
                ...
                if (pkgName != null && doesPackageSupportRuntimePermissions(pkgName)) {
                    try{
                        /*
                        PHONE_PERMISSIONS
                        ...
                        STORAGE_PERMISSIONS
                        */
                        grantRuntimePermissionsLPw(pkgName, PHONE_PERMISSIONS, userId);
                        grantRuntimePermissionsLPw(pkgName, CONTACTS_PERMISSIONS, userId);
                        grantRuntimePermissionsLPw(pkgName, LOCATION_PERMISSIONS, userId);
                        grantRuntimePermissionsLPw(pkgName, CALENDAR_PERMISSIONS, userId);
                        //Modify no bug id chenshu 20160524 start
                        //Because when open the SMS_PERMISSIONS default, google messaging app will insert a record to MMS Database
                        //So when first open MTK MMS , In ConversationListActivity will show it every time after reboot
                        //Here we change the logic to ignore this operation in google messageing app.
                        //Log.i(TAG,"app.packageName:"+app.packageName);
                        //Log.i(TAG,"app.packageName.equals(com.google.android.apps.messaging):"+
                        //app.packageName.equals("com.google.android.apps.messaging"));
                        if (app.packageName != null && !app.packageName.equals("com.google.android.apps.messaging")) {
                            grantRuntimePermissionsLPw(pkgName, SMS_PERMISSIONS, userId);
                        }
                        //grantRuntimePermissionsLPw(pkgName, SMS_PERMISSIONS, userId);
                        //Modify no bug id chenshu 20160524 end
                        grantRuntimePermissionsLPw(pkgName, MICROPHONE_PERMISSIONS, userId);
                        grantRuntimePermissionsLPw(pkgName, CAMERA_PERMISSIONS, userId);
                        grantRuntimePermissionsLPw(pkgName, SENSORS_PERMISSIONS, userId);
    ...
    private boolean isSysComponentOrPersistentPlatformSignedPrivAppLPr(PackageParser.Package pkg) {
        ...
        //A: Gmobi fota(start)
        if(pkg.packageName.equals("com.rock.gota")){
                return true;
        }
        //A: Gmobi fota(end)
