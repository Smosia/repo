...
public class WallpaperManagerService extends IWallpaperManager.Stub {
        ...
        synchronized (mLock) {
            ...
            try {
                if (outParams != null) {
                    ...
                    //outParams.putInt("width", wallpaper.width);
                    //outParams.putInt("height", wallpaper.height);
                    outParams.putInt("width", realSize.x);
                    outParams.putInt("height", realSize.y);
                }
                wallpaper.callbacks.register(cb);

                //f = new File(getWallpaperDir(wallpaperUserId), mWallpaperFileName);
                File f = new File(getWallpaperDir(wallpaperUserId), "rgk_lockpaper");
 ...
 public ParcelFileDescriptor setWallpaper(String name){//, String callingPackage) {     //Delete by zhaopenglin for keyguardwallpaper 20160303 

        checkPermission(android.Manifest.permission.SET_WALLPAPER);
        //Delete by zhaopenglin for keyguardwallpaper 20160303 start
        /*
        if (!isWallpaperSupported(callingPackage)) {
            return null;
        }*/
        //Delete by zhaopenglin for keyguardwallpaper 20160303 end
        synchronized (mLock) {
            ...
            final long ident = Binder.clearCallingIdentity();
            try {
                //Modify by zhaopenglin for keyguardwallpaper 20160303 start
                //ParcelFileDescriptor pfd = updateWallpaperBitmapLocked(name, wallpaper);
                ParcelFileDescriptor pfd;
                if ("rgk_lockpaper".equals(name)){
                    pfd = updateKeyguardWallpaperBitmapLocked(name, wallpaper);
                } else{
                    pfd = updateWallpaperBitmapLocked(name, wallpaper);
                }
                //Modify by zhaopenglin for keyguardwallpaper 20160303 end
                if (pfd != null) {
    ...
    //Add by zhaopenglin for keyguardwallpaper 20160303 start
    ParcelFileDescriptor updateKeyguardWallpaperBitmapLocked(String name, WallpaperData wallpaper) {
        if (name == null) name = "";
        try {
            File dir = getWallpaperDir(wallpaper.userId);
            Slog.i("updateWallpaperBitmapLocked", "File dir = "+ dir);
            if (!dir.exists()) {
                dir.mkdir();
            Slog.e("updateWallpaperBitmapLocked","File dir = getWallpaperDir(wallpaper.userId)====>>"+dir.getPath());
            ...
            Slog.e("updateWallpaperBitmapLocked","File file = new File(dir, mWallpaperFileName)===>>"+file.getPath()+"[mWallpaperFileName ==>>"+name+"]");
