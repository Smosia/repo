...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    //add BUG_ID:DSYYLM-74 dushuaihui 20160311(start)
    GlobalActionsDialog mGlobalActionsDialog;
    int previousOrientation;
    //add BUG_ID:DSYYLM-74 dushuaihui 20160311(end)
    ...
    class MyOrientationListener extends WindowOrientationListener {
        ...
        public void onProposedRotationChanged(int rotation) {
            ...
            updateRotation(false);
            //add BUG_ID:DSYYLM-74 dushuaihui 20160311(start)
            boolean rgkGlobalSupport = "1".equals(SystemProperties.get("ro.show_rgk_globalactions"));
            if(rgkGlobalSupport && mGlobalActionsDialog != null && mGlobalActionsDialog.isShowing()){
                if(previousOrientation != mContext.getResources().getConfiguration().orientation){
                    mGlobalActionsDialog.dismiss();
    ...
    private void interceptScreenshotChord() {
        ...
        // A BUG:DWYLL-491 by zyn 20150525 {
              boolean ultraPowerOn = android.provider.Settings.Global.getInt(mContext.getContentResolver(),
               android.provider.Settings.Global.ULTRA_POWER_MODE,0) == 1;
              if (ultraPowerOn) {
                return;
            }
          // A }
    ...
    void showGlobalActionsInternal() {
        ...
        //modify BUG_ID:DSYYLM-74 dushuaihui 20160311(start)
        if(!"1".equals(SystemProperties.get("ro.show_rgk_globalactions"))){
		    ...
		    if(mGlobalActionsDialog != null){
                if(mGlobalActionsDialog.isShowing()){
                    mGlobalActionsDialog.dismiss();
                }
                mGlobalActionsDialog = null;
            }
            mGlobalActionsDialog = new GlobalActionsDialog(mContext, mWindowManagerFuncs);
            mGlobalActionsDialog.show();
        ...
        //modify BUG_ID:DSYYLM-74 dushuaihui 20160311(end)
    }
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
			//A:DWYSLM-38 xuemingwei {
            /// M: [ALPS00054781]Dispatch the home key to the application @{
            if (win != null && win.getAttrs() != null) {
                final int privateFlags = win.getAttrs().privateFlags;
                if ((privateFlags & WindowManager.LayoutParams.FLAG_HOMEKEY_DISPATCHED) != 0) {
    ...
    private boolean isVolumeUPKeyUp = false, isVolumeDownKeyUp = false
        , isVolumeUPKeyDown = false, isVolumeDownKeyDown = false;
    private int volumeUpKeyDownCount = 0, volumeDownKeyDownCount = 0;
    private static final int KEYDOWN_REPEAT_COUNT = 60; // 3 seconds delay
    private static final int SEND_REPEAT_COUNT = 20; //
    // private static final boolean SEND_MESSAGE_FOREVER = true;
    private Handler handler = null;
    private Runnable runnable = null;
    
    private static int currentRepeatSendMsgCount = 0;
    private static boolean shouldResend = false;
    private boolean isDelay3Seconds = false;
    private boolean isAssistantOpened = false;
    
    private long volumeDownKeyDownTime = 0;
    private long volumeUpKeyDownTime = 0;
    // private boolean inOnceTime = false;
    private int interval = 1;

    // if opened return true, else false
    public boolean isAssistantOpened(){ 
        int assistantState = Settings.Global.getInt(mContext.getContentResolver(), 
                        Settings.Global.EMERGENCY_ASSISTANT_STATE, 0);
        return assistantState == 1; 
    }
    // get the interval between two messages sent
    public int getInterval() {
        return Settings.Global.getInt(mContext.getContentResolver(),
                        Settings.Global.EMERGENCY_ASSISTANT_INTERVAL, 1);
    } 

    //Add, DaiHongyi, JWBLB-54, emergency assistant, 20150520, End
	//add      BUG_ID:JWLWEL-123     xieshanlin  20150204(start)
    private  int   isConfigurationApplication(int keyCode){
             int   flag=0;
			 ContentResolver res = mContext.getContentResolver();
			 if(keyCode==KeyEvent.KEYCODE_C){
			 	
			 	flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_C, 1);
				
			 	}else if(keyCode==KeyEvent.KEYCODE_E){
			 	
			 	flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_E, 1);

				}else if(keyCode==KeyEvent.KEYCODE_M){

                flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_M, 1);
				
				}else if(keyCode==KeyEvent.KEYCODE_O){
				
                flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_O, 1);
					
				}else if(keyCode==KeyEvent.KEYCODE_W){

				flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_W, 1);
				
				}else if(keyCode==KeyEvent.KEYCODE_F){

				flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_F, 1);
				
				}else if(keyCode==KeyEvent.KEYCODE_DPAD_UP){

				flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_DPAD_UP, 1);
				
				}else if(keyCode==KeyEvent.KEYCODE_DPAD_DOWN){

				flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_DPAD_DOWN, 1);
				
				}else if(keyCode==KeyEvent.KEYCODE_DPAD_LEFT){

				flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_DPAD_LEFT, 1);
				
				}else if(keyCode==KeyEvent.KEYCODE_DPAD_RIGHT){

				flag=Settings.Global.getInt(
                    res,Settings.Global.BLACK_GESTURE_WAKE_DPAD_RIGHT, 1);
				
				}
				return  flag;
	}

	private boolean  isUnlock(int keyCode){	
		
		return  keyCode!=KeyEvent.KEYCODE_DPAD_LEFT && keyCode!=KeyEvent.KEYCODE_DPAD_RIGHT				
			    && keyCode!=KeyEvent.KEYCODE_F;
		}
	//add      BUG_ID:JWLWEL-123     xieshanlin  20150204( end )
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        switch (keyCode) {
			...
			case KeyEvent.KEYCODE_W:{
				 if(!mPowerManager.isScreenOn() && down && "1".equals(SystemProperties.get("ro.rgk_app_wake_gesture")) 
				 	&& (isConfigurationApplication(keyCode)==1)){
					...
					Settings.Global.putInt(mContext.getContentResolver(),Settings.Global.BLACK_GESTURE_WAKE_KEYEVENT, keyCode);					    
					Log.e("WakeGesture","FeatureOption----Wake--other"); 				    
					if(isUnlock(keyCode)){				    
					   Log.e("WakeGesture","FeatureOption----Wake--keygurardDone");										
					   if(!mKeyguardDelegate.isSecure()){ 					
										mKeyguardDelegate.keyguardDone(true,true);					
										Log.e("WakeGesture","FeatureOption----Wake--keygurardDone---isSecure");					        
										} 					     
									}	 										
					   Log.e("WakeGesture","FeatureOption----Wake--LauncherInto");                    
					   Intent intent = new Intent();                    
					   intent.setComponent(new ComponentName("com.ragentek.wakegesture",                    
					   "com.ragentek.wakegesture.MainActivity"));                    
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                ...
                if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
                    if (down) {
                        ...
                        Log.d("guocl","KEYCODE_VOLUME_DOWN > volumeDownKeyDownTime:"+volumeDownKeyDownTime);
                        //Add, DaiHongyi, JWBLB-54, emergency assistant, 20150520, End
                        ...
                        //Add, DaiHongyi, JWBLB-54, emergency assistant, 20150520, Start
                        long volumeDownKeyUpTime = System.currentTimeMillis();
                        long volumeDownKeyDeltaTime = volumeDownKeyUpTime - volumeDownKeyDownTime;
                        Log.d("guocl","KEYCODE_VOLUME_DOWN > volumeDownKeyUpTime:"+volumeDownKeyUpTime+", volumeDownKeyDeltaTime:"+volumeDownKeyDeltaTime+", isVolumeUPKeyDown:"+isVolumeUPKeyDown);
                        
                        long volumeUpKeyDeltaTime = volumeDownKeyUpTime - volumeUpKeyDownTime;
                        long timeCheck = Math.abs(volumeDownKeyDeltaTime - volumeUpKeyDeltaTime);
                        Log.d("guocl","volumeUpKeyDownTime:"+volumeUpKeyDownTime+", volumeUpKeyDeltaTime:"+volumeUpKeyDeltaTime+", timeCheck:"+timeCheck);
                        boolean go = (volumeDownKeyDeltaTime > 3000) && (isVolumeUPKeyDown || (timeCheck < 1000));
                        if (go && SystemProperties.get("ro.rgk_em_assistant_screenoff").equals("1")) {
                            if (handler != null) {
                                handler.removeCallbacks(runnable);
                            }
                            currentRepeatSendMsgCount = 0;
                            final Intent intent = new Intent("com.android.settings.emergencyassistant.START");
                            isAssistantOpened = isAssistantOpened();
                            
                            interval = getInterval();
                            shouldResend = interval != 1;
                            Log.d("guocl","KEYCODE_VOLUME_DOWN, isAssistantOpened:"+isAssistantOpened+", interval:"+interval+", shouldResend:"+shouldResend);
                            if (isAssistantOpened) {
                                if (!shouldResend) {
                                    intent.putExtra("only_once", true);
                                    ...
                                    if (runnable == null) {
                                        runnable = new Runnable() {
                                            ...
                                            public void run() {
                                                mContext.sendBroadcast(intent);
                                                if ((currentRepeatSendMsgCount > SEND_REPEAT_COUNT -1) 
                                                || (getInterval() == 1)) {
                                                    currentRepeatSendMsgCount = 0;
                                                    ...
                                                    handler = null;
                                                    runnable =null;
                                                } else if (!isAssistantOpened()) {
                                                    currentRepeatSendMsgCount = 0;
                                                    intent.putExtra("clean_before_send", true);
                                                    mContext.sendBroadcast(intent);
                                                    handler = null;
                                                    runnable =null;
                                                }else {
                                                    currentRepeatSendMsgCount ++;
                    ...
                    if (down) {
                        //Add, DaiHongyi, JWBLB-54, emergency assistant, 20150520, Start
                        Log.d("guocl","KEYCODE_VOLUME_UP");  
                        isVolumeUPKeyDown = true;
                        volumeUpKeyDownTime = System.currentTimeMillis();
                        Log.d("guocl","KEYCODE_VOLUME_UP > volumeUpKeyDownTime:"+volumeUpKeyDownTime);
                        //Add, DaiHongyi, JWBLB-54, emergency assistant, 20150520, End
			...
			ActivityManager activityManager = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
                	try{
				if (activityManager.getRunningTasks(1).get(0).topActivity.getPackageName().equals("com.rgk.factory")){
