...
public class AudioProfileService extends IAudioProfileService.Stub {
    ...
    private final AudioProfileListener mRingerVolumeListener = new AudioProfileListener() {
        ...
        public void onRingerVolumeChanged(int oldVolume, int newVolume, String extra) {
            ...
            String etraInfo = extra;
            int streamType = Integer.valueOf(etraInfo);
            ...
            switch (activeScenario) {
                case OUTDOOR:
                    ...
                    if ((newVolume > minVolume) && (newVolume != maxVolume)
                        ...
                        //modify guowen 20160304 start
                        //syncRingerVolumeToProfile(generalProfilekey, newVolume);
                        if((etraInfo!=null)&&(streamType ==AudioProfileManager.STREAM_NOTIFICATION)){
                            syncNotificationVolumeToProfile(generalProfilekey, newVolume);
                        } else {
                            syncRingerVolumeToProfile(generalProfilekey, newVolume);
                        }
                        //modify guowen 20160304 end
                ...
                case CUSTOM:
                    ...
                    if((etraInfo!=null)&&(streamType ==AudioProfileManager.STREAM_NOTIFICATION)&&(activeProfileState.mRingerVolume != newVolume)){
                        syncNotificationVolumeToProfile(mActiveProfileKey, newVolume);
                        Log.d(TAG, "onRingerVolumeChanged: notification volume changed");
                    } else if (activeProfileState.mRingerVolume != newVolume) {
                        // If active profile is general or custom and volume changed,
                        // sync to active profile.
                        notifyRingerVolumeChanged(oldVolume, newVolume, mActiveProfileKey);
                        syncRingerVolumeToProfile(mActiveProfileKey, newVolume);
                ...
                default:
                    ...
                    if (oldVolume > 0 && newVolume == 0) {
                        ...
                        if((extra!=null)&&(streamType ==AudioProfileManager.STREAM_NOTIFICATION)){
                            syncNotificationVolumeToProfile(mLastActiveProfileKey, newVolume);
                            Log.d(TAG, "onRingerVolumeChanged: sync notification volume 1->0 to last active ");
                        }else{
                            notifyRingerVolumeChanged(oldVolume, newVolume, mLastActiveProfileKey);
                            syncRingerVolumeToProfile(mLastActiveProfileKey, newVolume);
                            Log.d(TAG, "onRingerVolumeChanged: sync volume 1->0 to last active "
            ...
            if (Intent.ACTION_MEDIA_SCANNER_FINISHED.equals(action)) {
                ...
                if (mShouldGetDefaultRingtoneAfterScanFinish) {
                    ...
                    synchronized (mLock) {
                        for (String profileKey : mKeys) {
                            ...
                            || SILENT_NOTIFICATION_URI == profileState.mNotificationStream) {
                ...
                for (String profileKey : allKeys) {
                    ...
                    if (SystemProperties.get("ro.mtk_multisim_ringtone").equals("1")) {
                        ...
                        final int numSlots =
                          SubscriptionManager.from(mContext).getActiveSubscriptionInfoCount();
                        long simId = -1;
                        for (int i = 0; i < numSlots; i++) {
                            simId = SubscriptionManager.getSubId(i)[0];;
    ...
    public AudioProfileService(Context context) {
        ...
        mContext.registerReceiverAsUser(mUpgradeReceiver,
                                        UserHandle.ALL,
                                        scanerFilter,
                                        null,
                                        null);
        ...
        mContext.registerReceiverAsUser(mBootCompleteReceiver,
                                        UserHandle.ALL,
                                        bootFilter,
                                        null,
                                        null);
    ...
    private boolean setActiveKey(String profileKey) {
	 ...
	 //Add, JWLW-1141, zhangzixiao, 20140326, Start
	 onChangeProfile(profileKey); 
	 //Add, JWLW-1141, zhangzixiao, 20140326, End
    ...
    public String addProfile() {
        ...
        AudioProfileState defaultState = AudioProfileManager.getDefaultState(mContext , newKey);
        //modify guowen 20160304 end
    ...
    public void reset() {
        ...
        synchronized (mLock) {
            ...
            //add guowen 20160304 start
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_STATE, "-1");
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_MODE, "-1");
            initBesSurroundStatus();
            //add guowen 20160304 end
			...
			//Add, JWLW-1141, zhangzixiao, 20140326, Start
	     	onResetProfile(isGeneralActive);  
	    	//Add, JWLW-1141, zhangzixiao, 20140326, End
            Log.d(TAG, "reset end!");
        }
        mResetFlag = false;
    ...
    private void restoreToDefaultValues(String profileKey) {
        ...
        AudioProfileState defaultState = AudioProfileManager.getDefaultState(mContext , profileKey);
        //modify guowen 20160304 end
        AudioProfileState profileState = mProfileStates.get(profileKey);
        synchronized (mLock) {
    ...
    public Uri getRingtoneUri(String profileKey, int type) {
        ...
        switch (type) {
	     ...
	     case AudioProfileManager.TYPE_MESSAGE_NOTIFICATION:
                uri = AudioProfileManager.SYSTEM_NOTIFICATION_SOUND;
                break;
    ...
    public Uri getRingtoneUriWithSIM(String profileKey, int type, long simId) {
        ...
        switch (type) {
	     ...
	     //Add, JWLW-1141, zhangzixiao, 20131118, Start
            case AudioProfileManager.TYPE_MESSAGE_NOTIFICATION:
		        uri = readPersistedMessageRingtone(profileKey, simId);
                Log.e(TAG, "getRingtoneUriWithSIM:profileKey---"+profileKey);
                if (uri != null) {
                    Log.e(TAG, "getRingtoneUriWithSIM:profileKey---"+uri);
                }
                break;
                //Add, JWLW-1141, zhangzixiao, 20131118, End
    ...
    public void setRingtoneUri(String profileKey, int type, long simId, Uri ringtoneUri) {
	 ...
	 //Add, JWLW-1141, zhangzixiao, 20131118, Start
	 if (AudioProfileManager.TYPE_MESSAGE_NOTIFICATION == type) {
	     if (isActiveProfile(profileKey)) {
		  changeMessageNotification(simId, newRingtoneUri);
	     }
	     if (null == newRingtoneUri) {
                newRingtoneUri = SILENT_NOTIFICATION_URI;
            } else if(AudioProfileManager.SYSTEM_NOTIFICATION_SOUND.equals(newRingtoneUri) || AudioProfileManager.SYSTEM_MESSAGE_SOUND.equals(newRingtoneUri)) {
          Log.d(TAG, "newRingtoneUri = "+newRingtoneUri);
		  newRingtoneUri = Uri.parse("default_ringtone_for_message");
          Log.d(TAG, "newRingtoneUri = "+newRingtoneUri);
	     }
	 }
	 //Add, JWLW-1141, zhangzixiao, 20131118, End
    ...
    private boolean isValidRingtoneType(int type) {
		...
		return (AudioProfileManager.TYPE_RINGTONE == type)
                ...
                || (AudioProfileManager.TYPE_VIDEO_CALL == type)
                || (AudioProfileManager.TYPE_SIP_CALL == type)
				|| (AudioProfileManager.TYPE_MESSAGE_NOTIFICATION == type);
    ...
    public Uri getDefaultRingtone(int type) {
        ...
        switch (type) {
            ...
            case AudioProfileManager.TYPE_SIP_CALL:
                ...
                break;
            //add  chenshu 20151116 (start)
            case AudioProfileManager.TYPE_MESSAGE_NOTIFICATION:
                //uri = AudioProfileManager.SYSTEM_NOTIFICATION_SOUND;
                uri = mDefaultRingtone.get(DEFAULT_NOTIFICATION_INDEX);
                break;
            //add  chenshu 20151116 (end)
            default:
                Log.e(TAG, "getRingtoneUri with unsupport type!");
                return null;
        }
    ...
    private void readDefaultRingtones() {
        Log.d(TAG, "readDefaultRingtones: mUserId = " + mUserId);
    ...
    //add guowen 20160304 start
    public void syncNotificationVolumeToProfile(String profileKey, int volume) {
        if (getProfileState(profileKey).mNotificationVolume != volume) {
            mAudioProfileHandler.removeMessages(MSG_PERSIST_NOTIFICATION_VOLUME_TO_DATABASE);
            persistStreamVolumeToDatabase(profileKey, AudioProfileManager.STREAM_NOTIFICATION, volume);
            getProfileState(profileKey).mNotificationVolume = volume;
            Log.d(TAG, "syncNotificationVolumeToProfile: profileKey = " + profileKey + ", volume = " + volume);
        }
    }
    //add guowen 20160304 end
    ...
    private String getExternalUriData(String uriString) {
        ...
        Log.d(TAG, "getExternalUriData for uriString" + uriString);
        String data = null;
        Cursor cursor = mContentResolver.query(
                Uri.parse(uriString),
    ...
    private void initBesSurroundStatus() {
        ...
        if (BesSurroundCode == -1) {
            ...
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_STATE,
                                 String.valueOf(mContext.getResources().getBoolean(com.android.internal.R.bool.def_bes_surround_enabled) ?
                AudioProfileManager.KEY_SURROUND_ON : AudioProfileManager.KEY_SURROUND_OFF));
            //modify guowen 20160304 end
        }
        ...
        if (BesSurroundMode == -1) {
            ...
            mProfileSettings.put(AudioProfileManager.KEY_BESSUROUND_MODE,
                                 String.valueOf(mContext.getResources().getBoolean(com.android.internal.R.bool.def_is_movie_mode_code) ?
                AudioProfileManager.KEY_MOVIE_MODE_CODE : AudioProfileManager.KEY_MUSIC_MODE_CODE));
            //modify guowen 20160304 end
        }
        ...
        //add guowen 20160304 start
        if (BesSurroundCode == -1 && BesSurroundMode == -1){
            initBesSurroundStatus();
        }
        //add guowen 20160304 end
    }
    ...
    //Add, JWLW-1141, zhangzixiao, 20140326, Start
    private void onAddProfile(String newKey) {
    }

    private void onChangeProfile(String profileKey) {
	changeMessageNotification(profileKey);
    }

    private void onResetProfile(boolean isGeneralActive) {
	deleteMessageNotification(mKeys);
	resetMessageNotification();
    }

    private void onDeleteProfile(String profileKey) {
	deleteMessageNotification(profileKey);
    }

    private void resetMessageNotification() {
	if(AudioProfileManager.isSmsDualRingtoneSupport()) {
	    Intent intent = new Intent("rgk.intent.action.MESSAGE_RINGTONE_CHANGE");
            String ringStr1 = AudioProfileManager.getDefaultMessageRingtone(mContext, 0);
            String ringStr2 = AudioProfileManager.getDefaultMessageRingtone(mContext, 1);
	    intent.putExtra("ringtone1", ringStr1);
            intent.putExtra("ringtone2", ringStr2);
            mContext.sendBroadcast(intent);
	} else if(AudioProfileManager.isSmsSingleRingtoneSupport()) {
	    Intent intent = new Intent("rgk.intent.action.MESSAGE_RINGTONE_CHANGE");
            String ringStr = AudioProfileManager.getDefaultMessageRingtone(mContext, -1);
	    intent.putExtra("ringtone", ringStr);
            mContext.sendBroadcast(intent);
	}
    }

    private void changeMessageNotification(String profileKey) {
	if(AudioProfileManager.isSmsDualRingtoneSupport()) {
	    Intent intent = new Intent("rgk.intent.action.MESSAGE_RINGTONE_CHANGE");
	    Uri ringtone1 = getRingtoneUriWithSIM(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 0);
            Uri ringtone2 = getRingtoneUriWithSIM(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 1);
            String ringStr1 = ringtone1==null ? "" : ringtone1.toString();
            String ringStr2 = ringtone2==null ? "" : ringtone2.toString();
	    intent.putExtra("ringtone1", ringStr1);
            intent.putExtra("ringtone2", ringStr2);
            mContext.sendBroadcast(intent);
	} else if(AudioProfileManager.isSmsSingleRingtoneSupport()) {
	    Intent intent = new Intent("rgk.intent.action.MESSAGE_RINGTONE_CHANGE");
	    Uri ringtone = getRingtoneUriWithSIM(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, -1);
            String ringStr = ringtone==null ? "" : ringtone.toString();
	    intent.putExtra("ringtone", ringStr);
            mContext.sendBroadcast(intent);
	}
    }

    private void changeMessageNotification(long simId, Uri newUri) {
	if(AudioProfileManager.isSmsDualRingtoneSupport() && (simId==0 || simId==1)) {
	    Intent intent = new Intent("rgk.intent.action.MESSAGE_RINGTONE_CHANGE");
	    if(simId==0) {
		String ringStr1 = newUri==null ? "" : newUri.toString();
		intent.putExtra("ringtone1", ringStr1);
	    } else if(simId==1) {
		String ringStr2 = newUri==null ? "" : newUri.toString();
		intent.putExtra("ringtone2", ringStr2);
	    }
            mContext.sendBroadcast(intent);
	} else if(AudioProfileManager.isSmsSingleRingtoneSupport() && simId==-1) {
	    Intent intent = new Intent("rgk.intent.action.MESSAGE_RINGTONE_CHANGE");
            String ringStr = newUri==null ? "" : newUri.toString();
	    intent.putExtra("ringtone", ringStr);
            mContext.sendBroadcast(intent);
	}
    }

    private void deleteMessageNotification(List<String> allKeys) {
	...
	if(AudioProfileManager.isSmsDualRingtoneSupport()) {
        for(int i=0;i<size;i++) {
            String profileKey = allKeys.get(i);
            String mSim0profileKey=AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 0);
            String mSim1profileKey=AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 1);
            mList.add(mSim0profileKey);
            mList.add(mSim1profileKey);
            boolean success = mProfileSettings.remove(mList);
            Log.e("chenshu", "deleteMessageNotification(List<String> allKeys):"+success);
        }
	    /*selectionArgs = new String[size*2];
	    for(int i=0;i<size;i++) {
		String profileKey = allKeys.get(i);
		if(i == size-1) {
		    sb.append("?,?)");
		...
		selectionArgs[i*2] = AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 0);
		selectionArgs[i*2+1] = AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 1);
	    }
	    mContentResolver.delete(Settings.System.CONTENT_URI, sb.toString(), selectionArgs);*/
        
	} else if(AudioProfileManager.isSmsSingleRingtoneSupport()) {
        for(int i=0;i<size;i++) {
            String profileKey = allKeys.get(i);
            String mProfileKey=AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION);
            mList.add(mProfileKey);
            boolean success2 = mProfileSettings.remove(mList);
            Log.e("chenshu", "deleteMessageNotification(List<String> allKeys):"+success2);
        }
		...
		selectionArgs[i] = AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION);
	    }
	    mContentResolver.delete(Settings.System.CONTENT_URI, sb.toString(), selectionArgs);*/
	}
    }

    private void deleteMessageNotification(String profileKey) {
	//String where = null;
	//String[] selectionArgs = null;
    List<String> mList = new ArrayList<String>();
	if(AudioProfileManager.isSmsDualRingtoneSupport()) {
        ...
        mList.add(mSim1profileKey);
        boolean success = mProfileSettings.remove(mList);
        Log.e("chenshu", "deleteMessageNotification(String profileKey):"+success);
	    /*(where = Settings.System.NAME + " in (?,?)";
	    selectionArgs = new String[] {
            	AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 0),
            	AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, 1)
            };
	    mContentResolver.delete(Settings.System.CONTENT_URI, where, selectionArgs);*/
	} else if(AudioProfileManager.isSmsSingleRingtoneSupport()) {
	    /*where = Settings.System.NAME + "=?";
	    selectionArgs = new String[] {
		AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION)
	    };
	    mContentResolver.delete(Settings.System.CONTENT_URI, where, selectionArgs);*/
        String mProfileKey=AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION);
        mList.add(mProfileKey);
        boolean success2 = mProfileSettings.remove(mList);
        Log.e("chenshu", "deleteMessageNotification(String profileKey):"+success2);
	}
    }

    private Uri readPersistedMessageRingtone(String profileKey, long simId) {
    	Uri defaultUri = Uri.parse(AudioProfileManager.getDefaultMessageRingtone(mContext, (int)simId));
    	if (profileKey == null) {
            Log.e(TAG, "readPersistedMessageRingtone---profileKey == null");
            return defaultUri;
        }
    	String name = AudioProfileManager.getStreamUriKey(profileKey, AudioProfileManager.TYPE_MESSAGE_NOTIFICATION, simId);
        String getFromSettingsINI = mProfileSettings.get(name,defaultUri.toString());
    	if(getFromSettingsINI != null) {
            Log.e(TAG,"getFromSettingsINI:"+getFromSettingsINI);
            //if (!RingtoneManager.isRingtoneExist(mContext, Uri.parse(getFromSettingsINI))){
            //replace this method sometime failed
            if ((RingtoneManager.validRingtoneUri(mContext, Uri.parse(getFromSettingsINI), 2) == -1) 
                    && !getFromSettingsINI.equals("com.mediatek.uri.silent_notificaton")) {
                mProfileSettings.put(name,"default_ringtone_for_message");
                getFromSettingsINI = "default_ringtone_for_message";
            }
            if("default_ringtone_for_message".equals(getFromSettingsINI)) {
                if (isSmsCustomRingtoneSupport()) {
                    Log.e(TAG,"getFromSettingsINI:AudioProfileManager.SYSTEM_MESSAGE_SOUND");
    		        return AudioProfileManager.SYSTEM_MESSAGE_SOUND;
                } else {
                    Log.e(TAG,"getFromSettingsINI:SYSTEM_NOTIFICATION_SOUND");
                    return AudioProfileManager.SYSTEM_NOTIFICATION_SOUND;
                }
    	    } else {
                Log.e(TAG,"Uri.parse(getFromSettingsINI)");
    		    return Uri.parse(getFromSettingsINI);
    	    }
    	} else {
            Log.e(TAG, "readPersistedMessageRingtone---getFromSettingsINI == null");
        }
    	/*Cursor cursor = null;
	try {
	    String[] projection = new String[] { Settings.System.VALUE };
    	    String selection = Settings.System.NAME + "=?";
	    String[] selectionArgs = new String[] {  name };
	    cursor = mContentResolver.query(Settings.System.CONTENT_URI, projection, selection, selectionArgs, null);
    	    if(cursor != null && cursor.getCount()==1 && cursor.moveToFirst()) {
    	    	String uriString = cursor.getString(0);
    	    	if(uriString != null) {
    	            if("default_ringtone_for_message".equals(uriString)) {
    	    		return AudioProfileManager.SYSTEM_NOTIFICATION_SOUND;   
    	    	    } else {
