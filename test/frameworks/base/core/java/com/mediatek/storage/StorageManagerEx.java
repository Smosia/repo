...
public class StorageManagerEx {
	...
	public static final String PROP_HAVE_SD = "persist.sys.have.sd";//A by shubin for default sd
        ...
        if (path.equals("")
            ...
            try {
                ...
                for (int i = 0; i < volumeInfos.length; ++i) {
					...
					*/
					if(vol.isVisibleForWrite(userId) && vol.getType() == 0){
						path = vol.getPathForUser(userId).getAbsolutePath();
						break;
					}else if (vol.isVisibleForWrite(userId) && vol.isPrimary()){
						path = vol.getPathForUser(userId).getAbsolutePath();
                        Log.i(TAG, "Find primary and visible volumeInfo, "
                        + "path=" + path + ", volumeInfo:" + vol);
					} 
     ...
     * @hide
     */
    public static boolean isSetPrimaryStorageUuidFinish() {
        Log.d(TAG, "isSetPrimaryStorageUuidFinish");
        boolean result = true;
        try {
            IMountService mountService =
              IMountService.Stub.asInterface(ServiceManager.getService("mount"));
            ...
            result = mountService.isSetPrimaryStorageUuidFinish();
        } catch (RemoteException e) {
            Log.e(TAG, "RemoteException when invoke findPhoneStorage VolumeInfo:" + e);
        }
        Log.d(TAG, "isSetPrimaryStorageUuidFinish return " + result);
        return result;
    }
}
