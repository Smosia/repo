...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    private final static int CAN_DISPATCH_TOUCH_DISTANCE = 60;
    private final static int MOVE_DISTANCE = 25;
    private final static int MOVE_OFFSET = 15;
    private boolean mCanDispatch = true;
    ...
    public PhoneWindow(Context context) {
        ...
        //A:JWBLB-2271 guoshuai 20160222(start)
        try {
            mHasNavigationBar = WindowManagerHolder.sWindowManager.hasNavigationBar();
        } catch (RemoteException ex) {
            Log.e(TAG, "Couldn't get the value of mHasNavigationBar", ex);
        }
        //A:JWBLB-2271 guoshuai 20160222(end)
    }
    ...
    //A:JWBLB-2271 guoshuai 20160222(start)
    private boolean canDispatchTouchEvent(MotionEvent ev) {
        final WindowManager wm = getWindowManager();
        wm.getDefaultDisplay().getRealSize(mRealSize);
        int width = mRealSize.x;
        int height = mRealSize.y;
        ...
        if (rotation == Surface.ROTATION_0) {
            if (action == MotionEvent.ACTION_DOWN) {
                if (y >= height - CAN_DISPATCH_TOUCH_DISTANCE) {
                    mIsMove = false;
                    mCanDispatch = false;
                    mTouchDownX = x;
                    mTouchDownY = y;
                ...
                if (!mCanDispatch) {
                    if (!mIsMove) {
                        int deltaX = Math.abs(mTouchDownX - x);
                        int deltaY = Math.abs(mTouchDownY - y);
                        if (deltaX >= MOVE_OFFSET && deltaY <= MOVE_OFFSET) {
                            mCanDispatch = true;
                            mIsMove = true;
                            sendEvent(ev, MotionEvent.ACTION_DOWN);
                            sendEvent(ev, MotionEvent.ACTION_MOVE);
                        }
                    }
                }
            } else if (action == MotionEvent.ACTION_UP) {
                if (!mCanDispatch) {
                    if (mTouchDownY - y >= MOVE_DISTANCE) {
                        StatusBarManager statusBar =
                            (StatusBarManager) getContext().getSystemService(Context.STATUS_BAR_SERVICE);
                        if (statusBar != null) {
                            statusBar.reloadNavigationBar();
                        }
                    } else {
                        mIsClick = true;
                        sendEvent(ev, MotionEvent.ACTION_DOWN);
                        sendEvent(ev, MotionEvent.ACTION_MOVE);
                        sendEvent(ev, MotionEvent.ACTION_UP);
                    }
                }
            }
        } else {
            if (action == MotionEvent.ACTION_DOWN) {
                if (x >= width - CAN_DISPATCH_TOUCH_DISTANCE) {
                    mIsMove = false;
                    mCanDispatch = false;
                    mTouchDownX = x;
                    mTouchDownY = y;
                ...
                if (!mCanDispatch) {
                    if (!mIsMove) {
                        int deltaX = Math.abs(mTouchDownX - x);
                        int deltaY = Math.abs(mTouchDownY - y);
                        if (deltaY >= MOVE_OFFSET && deltaX <= MOVE_OFFSET) {
                            mCanDispatch = true;
                            mIsMove = true;
                            sendEvent(ev, MotionEvent.ACTION_DOWN);
                            sendEvent(ev, MotionEvent.ACTION_MOVE);
                ...
                if (!mCanDispatch) {
                    if (mTouchDownX - x >= MOVE_DISTANCE) {
                        StatusBarManager statusBar =
                            (StatusBarManager) getContext().getSystemService(Context.STATUS_BAR_SERVICE);
                        if (statusBar != null) {
                            statusBar.reloadNavigationBar();
    ...
    void sendEvent(MotionEvent ev, int action) {
                ...
                switch (keyCode) {
                    case KeyEvent.KEYCODE_VOLUME_UP:
                        ...
                        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
                            //Log.d("zhouzhuobin","KeyEvent.KEYCODE_VOLUME_UP");
                            isVolumeUPKeyDown = true;
                            isVolumeUPKeyUp = false;
                            volumeUpKeyDownCount = event.getRepeatCount();
                        }
                        //Add, DaiHongyi, JWBLB-54, emergency assistant, 20150520, End
                        break;
                    case KeyEvent.KEYCODE_VOLUME_DOWN:
                        ...
                        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
                            //Log.d("zhouzhuobin","KeyEvent.KEYCODE_VOLUME_DOWN");
                            isVolumeDownKeyDown = true;
                            isVolumeDownKeyUp = false;
                            volumeDownKeyDownCount = event.getRepeatCount();
                        }
                        //Log.d("zhouzhuobin","isAssistantOpened = " + isAssistantOpened() );
                        isDelay3Seconds = isVolumeUPKeyDown && volumeDownKeyDownCount == KEYDOWN_REPEAT_COUNT + 1;
                        //Log.d("zhouzhuobin","isVolumeUPKeyDown = " + isVolumeUPKeyDown + " ,volumeDownKeyDownCount = " + volumeDownKeyDownCount);
                        if (isDelay3Seconds) {
                            //Log.d("zhouzhuobin","isDelay3Seconds");
                            if (handler != null) {
                            ...
                            if (isAssistantOpened) {
                                //Log.d("zhouzhuobin","isAssistantOpened");
                                if (!shouldResend) {
                                        ...
                                        runnable = new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                || (getInterval() == 1)) {
                                                    ...
                                                    //Log.d("zhouzhuobin","handler.postDelayed(runnable, interval)");
        ...
        public boolean dispatchTouchEvent(MotionEvent ev) {
            ...
            if (mHasNavigationBar) {
                if (mIsMove) {
                    ...
                    mCount++;
                    //The judgement of mCount is 3, Why?
                    //Because we send the event of ACTION_DOWN, ACTION_MOVE, ACTION_UP,
                    //we must wait they are dispatched
                    if (mCount >= 3) {
                        mCount = 0;
                        mIsClick = false;
                    }
                } else if (!canDispatchTouchEvent(ev)) {
                    return true;
                }
            }
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        if (mIsFloating) {
            /// M: BMW @{
            if (MultiWindowProxy.isSupported() && isMultiWindow) {
