...
public class LockPatternUtils {
    ...
    public void updateEmergencyCallButtonState(Button button, boolean shown, boolean showIcon) {
     ...
     //add erhui DELSLMY-15 20160304 start{
     public void clearLock() {
        clearLock(false, 0);
    }
    public void setLockScreenDisabled(boolean disable) {
        setBoolean(DISABLE_LOCKSCREEN_KEY, disable, 0);
    }
    public boolean isLockScreenDisabled() {
        return !isSecure(0) &&
                getBoolean(DISABLE_LOCKSCREEN_KEY, false, 0);
    }
    public boolean isSecure() {
      int mode = getKeyguardStoredPasswordQuality(0);
        return isLockPatternEnabled(mode, 0) || isLockPasswordEnabled(mode, 0);
    }
     public void clearLock(boolean isFallback) {
        clearLock(isFallback, 0);
    }
     //end}
}
