...
/************************************************************************
History:
1. chuiguo.zeng@ragentek.com 2014.05.13 BUG_ID:JWYY-180
   Description: Add the support to show PTP label as custom's name.
************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
...
void MtpServer::addStorage(MtpStorage* storage) {
    ...
    ALOGD("addStorage, storageID = 0x%x, size = %d",
...
MtpResponseCode MtpServer::doGetDeviceInfo() {
    ...
    //modify bug_id:JWYY-180 zengchuiguo 20140512 (start)
    //property_get("ro.product.model", prop_value, "MTP Device");
    if(property_get("ro.ptp.label", prop_value, "") > 0) {
	ALOGI("[zcg] doGetDeviceInfo property_get, ro.ptp.label, property_get = %s, \n",prop_value);
    } else {
    	property_get("ro.product.model", prop_value, "MTP Device");
	ALOGI("[zcg] doGetDeviceInfo property_get, ro.product.model, property_get = %s, \n",prop_value);
    }
    //modify bug_id:JWYY-180 zengchuiguo 20140512 (end)	
    string.set(prop_value);
...
MtpResponseCode MtpServer::doGetPartialObject(MtpOperationCode operation) {
    ...
    // transfer the file
    int ret = ioctl(mFD, MTP_SEND_FILE_WITH_HEADER, (unsigned long)&mfr);
    ALOGD("MTP_SEND_FILE_WITH_HEADER returned %d\n", ret);
    close(mfr.fd);
    if (ret < 0) {
        if (errno == ECANCELED)
            return MTP_RESPONSE_TRANSACTION_CANCELLED;
...
MtpResponseCode MtpServer::doSendObjectInfo() {
    ...
    ALOGD("name: %s , mSendObjectFileSize = 0x%x, maxSize: 0x%llx, \n",
