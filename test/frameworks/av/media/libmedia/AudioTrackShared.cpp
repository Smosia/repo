        ...
        struct timespec *elapsed)
{
    ...
    for (;;) {
        ...
        ALOGV("front(%d), mIsOut %d, avail %d, mFrameCount %d, filled %d",
        ...
        if (!(old & CBLK_FUTEX_WAKE)) {
            ...
            errno = 0;
#if defined(CONFIG_MT_ENG_BUILD)
            ALOGV("front(%d), mIsOut %d, obtainBuffer() FUTEX_WAIT", cblk->u.mStreaming.mFront, mIsOut);
#endif
#ifdef MTK_AOSP_ENHANCEMENT
...
void ClientProxy::binderDied()
{
    ...
    if (!(android_atomic_or(CBLK_INVALID, &cblk->mFlags) & CBLK_INVALID)) {
        ALOGD("front(%d), mIsOut %d, binderDied() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
...
void ClientProxy::interrupt()
{
    ...
    if (!(android_atomic_or(CBLK_INTERRUPT, &cblk->mFlags) & CBLK_INTERRUPT)) {
        ALOGD("front(%d), mIsOut %d, interrupt() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
        android_atomic_or(CBLK_FUTEX_WAKE, &cblk->mFutex);
        (void) syscall(__NR_futex, &cblk->mFutex, mClientInServer ? FUTEX_WAKE_PRIVATE : FUTEX_WAKE,
                1);
    }
...
status_t AudioTrackClientProxy::waitStreamEndDone(const struct timespec *requested)
{
    ...
    for (;;) {
        ...
        if (!(old & CBLK_FUTEX_WAKE)) {
            errno = 0;
            ALOGD("front(%d), mIsOut %d, waitStreamEndDone() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
            (void) syscall(__NR_futex, &cblk->mFutex,
                    mClientInServer ? FUTEX_WAIT_PRIVATE : FUTEX_WAIT, old & ~CBLK_FUTEX_WAKE, ts);
            switch (errno) {
            case 0:            // normal wakeup by server, or by binderDied()
...
void ServerProxy::releaseBuffer(Buffer* buffer)
{
    ...
    if (!mIsOut || (mAvailToClient + stepCount >= minimum)) {
        /// M: Reduce userload log for Camera @{
#if defined(CONFIG_MT_USER_BUILD)
        if (mIsOut)
#endif
        /// @}
        ...
        ALOGV("front(%d), mIsOut(%d), mAvailToClient=%zu stepCount=%zu minimum=%zu",
                cblk->u.mStreaming.mFront, mIsOut, mAvailToClient, stepCount, minimum);
    }
...
bool  AudioTrackServerProxy::setStreamEndDone() {
    ...
    if (!old) {
        ALOGD("front(%d), mIsOut %d, setStreamEndDone() FUTEX_WAKE", cblk->u.mStreaming.mFront, mIsOut);
        (void) syscall(__NR_futex, &cblk->mFutex, mClientInServer ? FUTEX_WAKE_PRIVATE : FUTEX_WAKE,
                1);
    }
    return old;
...
void StaticAudioTrackServerProxy::releaseBuffer(Buffer* buffer)
{
    ...
    ALOGV("StaticAudioTrackServerProxy::releaseBuffer: stepCount, position, newPosition, %zu, %zu, %zu",
            stepCount, position, newPosition);
