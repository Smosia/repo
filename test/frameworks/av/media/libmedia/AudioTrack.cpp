...
status_t AudioTrack::start()
{
    ...
    if (t != 0) {
        if (previousState == STATE_STOPPING) {
            ALOGD("start(): previousState == STATE_STOPPING: interrupt AudioTrackThread");
            mProxy->interrupt();
        } else {
            ALOGD("start(): resume AudioTrackThread");
            t->resume();
        }
    } else {
...
status_t AudioTrack::createTrack_l()
{
   ...
   // forbit fast track for voice feature
    ...
    ALOGD("createTrack_l: mLatency = %d, mAfLatency = %d, frameCount = %zu, mSampleRate = %d",
            mLatency, mAfLatency, frameCount, mSampleRate);
...
{
    ALOGD("%s: userSize = %zu, blocking = %d", __FUNCTION__, userSize, blocking);
    ...
    ALOGV("%s: written = %zu", __FUNCTION__, written);
    return written;
}
...
nsecs_t AudioTrack::processAudioBuffer()
{
    ...
    /// M: ALPS02471859: Don't callback after stop for CTS
#ifdef MTK_AOSP_ENHANCEMENT
    bool isNeedCallbackMarkReached = false;
#endif
    ...
    if (!mMarkerReached && (markerPosition > 0) && (position >= markerPosition)) {
        ...
        /// M: ALPS02471859: Don't callback after stop for CTS @{
#ifdef MTK_AOSP_ENHANCEMENT
        if ((STATE_ACTIVE == mState) && markerReached) {
            isNeedCallbackMarkReached = true;
        }
#endif
        /// @}
    }
...
uint32_t AudioTrack::updateAndGetPosition_l()
{
    ...
    ALOGV("updateAndGetPosition_l: mPosition, mServer, delta = %d, %d, %d"
            , mPosition, mServer, delta);
...
status_t AudioTrack::getTimestamp(AudioTimestamp& timestamp)
{
    ...
    if (mFlags & AUDIO_OUTPUT_FLAG_FAST) {
        ALOGD("getTimestamp: retun INVALID_OPERATION(%d): AUDIO_OUTPUT_FLAG_FAST", INVALID_OPERATION);
        return INVALID_OPERATION;
    }
    ...
    if (isOffloadedOrDirect_l()) {
        ...
        if ((uint32_t) (mServer - timestamp.mPosition) > mPosition) {
            ALOGD("getTimestamp: retun INVALID_OPERATION(%d): (mServer(%d) - timestamp.mPosition(%d)) > mPosition(%d)",
                    INVALID_OPERATION, mServer, timestamp.mPosition, mPosition);
            return INVALID_OPERATION;
        }
