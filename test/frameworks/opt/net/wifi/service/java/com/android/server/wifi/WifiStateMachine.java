...
public class WifiStateMachine extends StateMachine implements WifiNative.WifiPnoEventHandler,
    ...
    */
    ...
    private static final int M_CMD_DO_CTIA_TEST_ON           = BASE + 173;
    private static final int M_CMD_DO_CTIA_TEST_OFF          = BASE + 174;
    private static final int M_CMD_DO_CTIA_TEST_RATE         = BASE + 175;
    private static final int M_CMD_GET_CONNECTING_NETWORK_ID = BASE + 176;
    private static final int M_CMD_UPDATE_RSSI               = BASE + 177;
    private static final int M_CMD_SET_TX_POWER_ENABLED      = BASE + 178;
    private static final int M_CMD_SET_TX_POWER              = BASE + 179;
    private static final int M_CMD_BLOCK_CLIENT              = BASE + 180;
    private static final int M_CMD_UNBLOCK_CLIENT            = BASE + 181;
    private static final int M_CMD_START_AP_WPS              = BASE + 182;
    private static final int M_CMD_UPDATE_BGSCAN             = BASE + 183;
    private static final int M_CMD_SET_AP_PROBE_REQUEST_ENABLED = BASE + 184;

    //* M: For stop scan after screen off in disconnected state feature */
    private static final int M_CMD_SLEEP_POLICY_STOP_SCAN    = BASE + 185;
    private static final int M_CMD_NOTIFY_SCREEN_OFF         = BASE + 186;
    private static final int M_CMD_NOTIFY_SCREEN_ON          = BASE + 187;
    ...
    private static final int M_CMD_GET_WIFI_STATUS           = BASE + 191;
    private static final int M_CMD_SET_POWER_SAVING_MODE     = BASE + 192;

    /** M: NFC Float II @{ */
    private static final int M_CMD_START_WPS_NFC_TAG_READ    = BASE + 193;
    private static final int M_CMD_HS_RECEIVED               = BASE + 194;
    public static final int M_CMD_HR_RECEIVED                = BASE + 195;
    public static final int M_CMD_CLEAE_HR_WAIT_FLAG         = BASE + 196;
    /** @} */

    /** M: Set Driver Wowlan Mode @{ */
    private static final int M_CMD_SET_WOWLAN_NORMAL_MODE    = BASE + 197;
    private static final int M_CMD_SET_WOWLAN_MAGIC_MODE     = BASE + 198;
    /** @} */

    private static final int M_CMD_FLUSH_BSS                 = BASE + 202;
    ///M: for poor link st value
    private static final int M_CMD_SET_POORLINK_RSSI         = BASE + 228;
    private static final int M_CMD_SET_POORLINK_LINKSPEED    = BASE + 229;
    /// @}
    private static final int M_CMD_DHCP_V6_SUCCESS           = BASE + 230;

    private static final int M_CMD_START_AUTOJOIN_PROFILING  = BASE + 231;
    private static final int M_CMD_GET_TEST_ENV              = BASE + 232;
    ///M: ALPS02279279 [Google Issue] Cannot remove configured networks when factory reset
    private static final int M_CMD_FACTORY_RESET             = BASE + 233;
    ///M: ALPS01986276 should enable EAP SIM in state machine
    private static final int M_CMD_ENABLE_EAP_SIM_CONFIG_NETWORK       = BASE + 243;
    
    ///M: ALPS02550356 IpReachabilityMonitor refine
    private static final int M_CMD_CHECK_RX_PACKETS                    = BASE + 245;
...
//                                WIFI_INDIDE_SOURCE);
                ...
                case M_CMD_FACTORY_RESET:
                    ...
                    if (networks != null) {
                        for (WifiConfiguration c : networks) {
                            mWifiConfigStore.removeNetwork(c.networkId);
                        }
    ...
    class L2ConnectedState extends State {
        ///M: ALPS02550356 IpReachabilityMonitor refine @{
        long oldRxPackets = 0;
        final int CHECK_RX_INTERVAL = 3000;
        ///@}
                    ...
                    mIpReachabilityMonitor = new IpReachabilityMonitor(
                            ...
                            new IpReachabilityMonitor.Callback() {
                                ...
                                public void notifyLost(InetAddress ip, String logMsg) {
                                    ...
                                    ///M: ALPS02550356 IpReachabilityMonitor refine @{
                                    oldRxPackets = TrafficStats.getRxPackets(mInterfaceName);
                                    sendMessageDelayed(obtainMessage(M_CMD_CHECK_RX_PACKETS, logMsg),
                                        CHECK_RX_INTERVAL);
                                    //sendMessage(CMD_IP_REACHABILITY_LOST, logMsg);
                                    ///@}
                                }
            ...
            switch (message.what) {
              ...
              case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                ...
                case EVENT_PPPOE_SUCCEEDED:
                    ...
                    break;
                ///M: ALPS02550356 IpReachabilityMonitor refine @{
                case M_CMD_CHECK_RX_PACKETS:
                    long newRxPackets = TrafficStats.getRxPackets(mInterfaceName);
                    long diffRx = newRxPackets - oldRxPackets;
                    log("oldRxPackets: " + oldRxPackets + ", newRxPackets: " + newRxPackets
                            + ", diff: " + (diffRx>0 ? ">0 ignore" : "=0 send ") + "CMD_IP_REACHABILITY_LOST");
                    if (diffRx == 0) {
                        sendMessage(CMD_IP_REACHABILITY_LOST, message.obj);
                    }
                    break;
                ///@}
                default:
    ...
    public String syncGetTestEnv(AsyncChannel channel, int wifiChannel) {
     ...
     public void factoryReset() {
         sendMessage(M_CMD_FACTORY_RESET);
     }
