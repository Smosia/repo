...
///M: add
    ...
    public boolean mUtf8LikeGbkEncoding = false;
    ...
    public String waitForEvent() {
        ...
        return parseSsidInByteArray(event);
        //@}
    }
    ...
    public String scanResults(int sid) {
        ...
        byte[] byteArray = doByteArrayCommand("BSS RANGE=" + sid + "- MASK=0x29d87");
...
//            auth = 0;
    ...
    private boolean isGBK(byte[] byteArray, int ssidStartPos,
        ...
        if (isNotUtf8(byteArray, ssidStartPos, ssidEndPos)) {
            ...
            if (mUtf8LikeGbkEncoding) {
                ...
                if (isUtf8LikeGbk(byteArray, ssidStartPos, ssidEndPos)) {
    ...
    private boolean isNotUtf8(byte[] input, int ssidStartPos, int ssidEndPos) {
    ...
    private boolean isUtf8LikeGbk(byte[] chBytes, int ssidStartPos, int ssidEndPos) {
        boolean isExistGBK = false;
        boolean isExistASCII = false;
        for (int i = ssidStartPos; i < ssidEndPos && i < chBytes.length;) {
            if (i < chBytes.length && i + 1 < chBytes.length
                && isWordUtf8LikeGBK(chBytes[i],
                                     chBytes[i + 1])) {
                isExistGBK = true;
                i += 2;
    ...
    private int getUtf8CharLen(byte firstByte) {
        int nBytes = 0;
    ...
    private boolean isGBKChar(byte b0, byte b1) {
        if ((b0 >= (byte) 0xA1 && b0 <= (byte) 0xA9 &&
             b1 >= (byte) 0xA1 && b1 <= (byte) 0xFE) ||
            (b0 >= (byte) 0xB0 && b0 <= (byte) 0xF7 &&
             b1 >= (byte) 0xA1 && b1 <= (byte) 0xFE) ||
            (b0 >= (byte) 0x81 && b0 <= (byte) 0xA0 &&
             b1 >= (byte) 0x40 && b1 <= (byte) 0xFE) ||
            (b0 >= (byte) 0xAA && b0 <= (byte) 0xFE &&
             b1 >= (byte) 0x40 && b1 <= (byte) 0xA0 &&
             b1 != (byte) 0x7F) ||
            (b0 >= (byte) 0xA8 && b0 <= (byte) 0xA9 &&
             b1 >= (byte) 0x40 && b1 <= (byte) 0xA0 && b1 != (byte) 0x7F) ||
            (b0 >= (byte) 0xAA && b0 <= (byte) 0xAF &&
             b1 >= (byte) 0xA1 && b1 <= (byte) 0xFE && b1 != (byte) 0x7F) ||
            (b0 >= (byte) 0xF8 && b0 <= (byte) 0xFE &&
             b1 >= (byte) 0xA1 && b1 <= (byte) 0xFE) ||
            (b0 >= (byte) 0xA1 && b0 <= (byte) 0xA7 &&
             b1 >= (byte) 0x40 && b1 <= (byte) 0xA0 && b1 != (byte) 0x7F)) {
            return true;
        }
        return false;
