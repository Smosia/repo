...
public class SvlteModeController extends Handler {
    ...
    */
        ...
        while (true) {
            if (OPERATOR_OP09.equals(mOperatorSpec) && SPEC_OP09_A.equals(mOP09Spec)) {
                ...
                if (isGsmCard(cardType1) && isGsmCard(cardType2)) {
                    // slot1 GSM, slot2 GSM
                    modemStatus = 0;
                    remoteSimProtocol = mLastProtocol != -1 ? mLastProtocol : 2;
                    break;
                    ...
                    // slot1 N/A, slot2 N/A
                    // For SS project, MD3 is on MD1
                    int phoneCount = TelephonyManager.getDefault().getSimCount();
                    logicLog("phoneCount: " + phoneCount);
                    if (phoneCount == 1) {
                        // SS
                        modemStatus = 2;
                        remoteSimProtocol = mLastProtocol != -1 ?
                                mLastProtocol : 1;
                     } else {
                        modemStatus = 1;
                        remoteSimProtocol = mLastProtocol != -1 ?
    ...
    private boolean blockBySimSwitch() {
       boolean blockBySimSwitch = SystemProperties.get("ril.cdma.switching", "0").equals("1");
       logicLog("[blockBySimSwitch] "  + blockBySimSwitch);
       return blockBySimSwitch;
    }
