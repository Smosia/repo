    ...
    private boolean matchNumber(String number, List<String> numbers) {
        if (number != null && numbers != null) {
            for (String s : numbers) {
                ...
                */
            	boolean isBrazilConfig = getBaseContext().getResources().getBoolean(com.android.internal.R.bool.config_use_brazil_number_format);
            	if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
            		if (PhoneNumberUtils.compareWithAreaCode(s, number)) {
                        return true;
                    }
            	} else {
            		if (PhoneNumberUtils.compare(s, number)) {
