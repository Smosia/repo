...
public class SvlteRatController extends Handler {
    ...
    private boolean isLteOn() {
        // radio power
        boolean isCtaCase = RadioManager.getInstance().checkForCTACase();
        if (isCtaCase) {
            return true;
        }
        if (SvlteUiccUtils.getInstance().isCt3gDualMode(mSlotId) &&
                    (mNewRoamingMode == RoamingMode.ROAMING_MODE_HOME)) {
            return false;
        }
    ...
    private boolean isCdmaOn() {
        /// M: add for eng set IR setting to LWG mode
        if (mEngMode == ENGINEER_MODE_CSFB) {
            return false;
        }
        boolean isCtaCase = RadioManager.getInstance().checkForCTACase();
        if (isCtaCase) {
        ...
        if ((cardType[mSlotId] == UiccController.CARD_TYPE_NONE) &&
                    (SvlteModeController.getRadioTechnologyMode(mSlotId) ==
        ...
        if (CdmaFeatureOptionUtils.getC2KOMNetworkSelectionType() == 0) {
            isGmssOn = isCdma4GSim() && (mNewSvlteRatMode == SvlteRatMode.SVLTE_RAT_MODE_4G);
        }

        logd("cdmaOn = " + cdmaOn + ", isGmsson=" + isGmssOn);
        return cdmaOn || isGmssOn;
    }
    ...
    public boolean allowRadioPowerOn(int phoneId) {
        ...
        boolean lwgOn = isLteOn();
        boolean cdmaOn = isCdmaOn();

        logd("allowRadioPowerOn lwgOn=" + lwgOn + ", cdmaOn=" + cdmaOn);
