    ...
    private SvlteRatModeChangedListener mModeChangedListener = new SvlteRatModeChangedListener() {
            ...
            public void onSvlteRatModeChangeStarted(SvlteRatMode curMode, SvlteRatMode newMode) {
                ...
                if (newMode == SvlteRatMode.SVLTE_RAT_MODE_3G
                        || SystemProperties.getInt("ro.mtk_c2k_om_nw_sel_type", 0) == 1) {
            ...
            switch (msg.what) {
                ...
                case EVENT_SERVICE_STATE_CHANGED:
                    if (getRoamingMode() == RoamingMode.ROAMING_MODE_HOME
                            || getRoamingMode() == RoamingMode.ROAMING_MODE_JPKR_CDMA) {
                        logdForController(" LWG service state change at home, do nothing.");
                        return;
                    }
                    ...
                    if (mPreVoiceState == regState && mPreDataState == regDataState) {
                        logdForController(" Voice state and Data state not change, do nothing.");
                        return;
                    }
        ...
        protected void defaultMessageHandler(Message msg) {
            ...
            switch (msg.what) {
                case EVENT_DUAL_PHONE_POWER_ON:
                    ...
                    break;
                case EVENT_SERVICE_STATE_CHANGED:
                    if (getRoamingMode() == RoamingMode.ROAMING_MODE_NORMAL_ROAMING) {
                        logdForController(" CDMA service state change at roaming, do nothing.");
                        return;
                    }
                    ServiceState serviceState = (ServiceState) ar.result;
