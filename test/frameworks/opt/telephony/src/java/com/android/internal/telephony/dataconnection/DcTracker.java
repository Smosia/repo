...
//            attachedState = true;
    ...
    protected boolean cleanUpAllConnections(boolean tearDown, String reason) {
        ...
        if (!TextUtils.isEmpty(reason)) {
            ...
            // /Ignore IMS PDN deactivation when Radio turned off @{
    ...
    private void onRecordsLoaded() {
        ...
        if (!SubscriptionManager.isUsableSubIdValue(subId)) {
            if (DBG) {
                log("onRecordsLoaded: subInfo updater isn't ready," +
                        " waiting for subinfo record updated intent");
            }
            mIsSubInfoNotReadyWhenRecordsLoaded = true;
            return;
        }
        ...
        // M: update initial attach APN for SVLTE since SVLTE use specific
        // APN for initial attach in case CDMA records loaded after LTE records.
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
            updateInitialAttachApnForSvlte();
        }
        setInitialAttachApn();
...
*/
    ...
    private void onIratDataRatChanged(int sourceRat, int targetRat) {
        log("[IRAT_DcTracker] onIratDataRatChanged: mIsDuringIrat = "
        ...
        if (mIsDuringIrat) {
            log("[IRAT_DcTracker] Ignore RAT change during IRAT.");
            return;
        }

        if (targetRat == ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN) {
            log("[MD_IRAT_DcTracker] De-register on C2K/LTE network, do thing.");
        } else if (sourceRat == ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN) {
            log("[MD_IRAT_DcTracker] First register on C2K/LTE network.");
                ...
                if (cursor.getCount() > 0) {
                    ...
                    if (apnSettings != null && apnSettings.size() > 0) {
                        ...
                        if (iaApnSetting == null) {
                            if (mPreferredApn != null
                                    && operatorNumericMatches(
                                            mPreferredApn.apn,
                                            mPreferredApn.numeric, operator)) {
    ...
    // Check if operator numeric matched, for SVLTE check the pair(both CDMA and
    // LTE numeric) for user defined APN.
    private boolean operatorNumericMatches(String apnName, String operNumeric,
            String otherOperNumeric) {
        boolean matched = false;
        if (TextUtils.isEmpty(operNumeric)
                || TextUtils.isEmpty(otherOperNumeric)) {
            matched = false;
        } else if (TextUtils.equals(operNumeric, otherOperNumeric)) {
            matched = true;
        } else if (!APN_CTNET.equals(apnName)) {
            // Not predefined APN, only check for user define APN.
            if (OPERATOR_NUMERIC_CT_LTE.equals(operNumeric)
                    || OPERATOR_NUMERIC_CT_CDMA.equals(operNumeric)) {
                matched = (OPERATOR_NUMERIC_CT_LTE.equals(otherOperNumeric)
                        || OPERATOR_NUMERIC_CT_CDMA.equals(otherOperNumeric));
            }
            if (OPERATOR_NUMERIC_MACAO_LTE.equals(operNumeric)
                    || OPERATOR_NUMERIC_MACAO_CDMA.equals(operNumeric)) {
                matched = (OPERATOR_NUMERIC_MACAO_LTE.equals(otherOperNumeric)
                        || OPERATOR_NUMERIC_MACAO_CDMA.equals(otherOperNumeric));
            }
        }
        log("[IRAT_DcTracker] operatorNumericMatches: matched = " + matched);
        return matched;
    }
