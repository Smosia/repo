...
int initialize_directories() {
    ...
    //A: kuangyunfeng 20160201(start)
    bool isInternalStorageInMedia = false;
    // /data/media/Notifications
    char media_notifications_dir[PATH_MAX];
    char media_android_dir[PATH_MAX];
    char media_O_dir[PATH_MAX];
    //A: kuangyunfeng 20160201(end)
    ...
    //A: kuangyunfeng 20160201(start)
    snprintf(media_notifications_dir, PATH_MAX, "%sNotifications", android_media_dir.path);
    snprintf(media_android_dir, PATH_MAX, "%sAndroid", android_media_dir.path);
    snprintf(media_O_dir, PATH_MAX, "%s0", android_media_dir.path);
    if (!access(media_notifications_dir, F_OK) || !access(media_android_dir, F_OK) || access(media_O_dir, F_OK)) {
       ALOGD("The internal storage still is /data/media.");
       isInternalStorageInMedia = true;

       if (!access(media_O_dir, F_OK)) {
           char cmd[PATH_MAX];
           snprintf(cmd, sizeof(cmd), "/system/bin/cp -p -R -P -d %s/* %s", media_O_dir, android_media_dir.path);
           system(cmd);
           ALOGD("%s", cmd);

           snprintf(cmd, sizeof(cmd), "/system/bin/rm -f -R %s", media_O_dir);
           system(cmd);
           ALOGD("%s", cmd);
       }
    }
    ...
    //M: kuangyunfeng 20160201(start)
    //if (version == 0) {
    if (version == 0 || isInternalStorageInMedia) {
    //M: kuangyunfeng 20160201(end)
