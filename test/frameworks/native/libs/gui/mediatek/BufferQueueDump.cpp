...
void BufferQueueDump::setName(const String8& name) {
    ...
    int iLen = property_get(PROP_DUMP_NAME, value, DEFAULT_DUMP_NAME);
    BQD_LOGD("this:%p, value:%p, iLen:%d", this, value, iLen);
...
void BufferQueueDump::dump(String8& result, const char* prefix) {
    ...
    result.appendFormat("%s*BufferQueueDump mIsBackupBufInited=%d, m%sBufs(size=%zu), mMode=%s\n",
    ...
    for (size_t i = 0; i < mObtainedBufs.size(); i++) {
        const sp<DumpBuffer>& buffer = mObtainedBufs[i];
        result.appendFormat("%s [%02zu] handle=%p, fence=%p, time=%#" PRIx64 "\n",
...
void BufferQueueDump::addBuffer(const int& slot,
    ...
    sp<DumpBuffer> v = mObtainedBufs.valueFor(static_cast<const unsigned int>(slot));
    if (v == NULL) {
...
status_t BufferQueueDump::kickDump(String8& result, const char* prefix) {
    ...
    if (bq != NULL) {
        ...
        ALOGE("kickDump() failed because BufferQueue(%p) is dead", mBq.unsafe_get());
        return DEAD_OBJECT;
    }
}
// ----------------------------------------------------------------------------
bool BackupBufPusher::push(const sp<BackupBuffer>& in) {
    ...
    if ((buffer == NULL) || (buffer->mGraphicBuffer == NULL)) {
        ...
        if ((buffer->mGraphicBuffer->width != in->mGraphicBuffer->width) ||
            ...
            needCreate = true;
            ALOGD("[%s] geometry changed, backup=(%d, %d, %d) => active=(%d, %d, %d)",
    ...
    memcpy(dst, src, (static_cast<unsigned int>(stride) * static_cast<unsigned int>(height) * bits) >> 3);
