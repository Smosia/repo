...
bool MetadataExtractor::populateVarNameMetadata(
...
#ifdef RS2CL_ENABLE
  // [RSCL] Export all global variable hack.
  if (VarNameMetadata) {
    mExportVarCount = VarNameMetadata->getNumOperands();
  }

  llvm::Module *module = const_cast<llvm::Module*>(mModule);
  ...
  char **TmpNameList = new char *[mExportVarCount];

  size_t i = 0;
  if (VarNameMetadata && VarNameMetadata->getNumOperands()) {
    for (; i < VarNameMetadata->getNumOperands(); ++i) {
      llvm::MDNode *Name = VarNameMetadata->getOperand(i);
      if (Name != nullptr && Name->getNumOperands() > 1) {
        TmpNameList[i] = const_cast<char*>(createStringFromValue(Name->getOperand(0)));
      }
    }
  }
  ...
  for (auto &GV : module->getGlobalList()) {
    if (!GV.isConstant() && GV.hasInternalLinkage() &&
      ...
      TmpNameList[i] = new char[Name.size() + 1];
      std::memcpy(TmpNameList[i], Name.data(), Name.size());
      TmpNameList[i][Name.size()] = '\0';
      ++i;
    }
  }

  mExportVarNameList = const_cast<const char**>(TmpNameList);

#else
  // [RSCL] Disable for exporting global variable hack.
  if (!VarNameMetadata) {
    return true;
  }
