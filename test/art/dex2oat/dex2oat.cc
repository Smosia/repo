...
class Dex2Oat FINAL {
 public:
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    // add by chenweihua start
    if ((oat_filename_.find("facebook") != std::string::npos) || (oat_filename_.find("google") != std::string::npos)
      || (oat_filename_.find("gms") != std::string::npos) || (oat_filename_.find("systemui") != std::string::npos)) {

      compiler_filter_string = "interpret-only";

      LOG(INFO) <<" This apk is in whitelist from property so set interpret-only ";
    }
    // add by chenweihua end
