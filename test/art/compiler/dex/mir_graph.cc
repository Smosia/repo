...
MIRGraph::MIRGraph(CompilationUnit* cu, ArenaAllocator* arena)
    : reg_location_(nullptr),
      ...
      rev_dfs_order_(arena->Adapter(kArenaAllocDfsPreOrder)),
      rev_dfs_post_order_(arena->Adapter(kArenaAllocDfsPostOrder)),
      rev_dom_post_order_traversal_(arena->Adapter(kArenaAllocDomPostOrder)),
      p_i_dom_list_(nullptr),
#endif
      temp_scoped_alloc_(),
...
BasicBlock* MIRGraph::NewMemBB(BBType block_type, int block_id) {
  ...
  #ifdef MTK_ART_COMMON
  bb->rev_predecessors.reserve((block_type == kEntryBlock) ? 2048 : 2);
  #endif
...
void BasicBlock::EraseReversePredecessor(BasicBlockId old_pred) {
  auto pos = std::find(rev_predecessors.begin(), rev_predecessors.end(), old_pred);
  DCHECK(pos != rev_predecessors.end());
  // It's faster to move the back() to *pos than erase(pos).
  *pos = rev_predecessors.back();
  rev_predecessors.pop_back();
}

bool BasicBlock::IsPredecessor(BasicBlockId pred) {
  auto pos = std::find(predecessors.begin(), predecessors.end(), pred);
  return pos != predecessors.end();
}
#endif
