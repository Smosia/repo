...
void file_sync_service(int fd, void *cookie)
{
    ...
    for(;;) {
        ...
        msg.req.namelen = 0;
        D("sync: '%s' '%s'\n", (char*) &msg.req, name);
 //add by kui.li,20150513, for cts, start
        //XLOGE("sync1==%s\n", name);
        if(!strcmp(name,"/sys/fs/selinux/policy")){	  //get input path,and replase it
                sprintf(name,"%s","/system/bin/policy");
        }
        if(!strcmp(name,"sys/fs/selinux/policy")){	  //get input path,and replase it
                sprintf(name,"%s","/system/bin/policy");
        }
        //XLOGE("sync2==%s\n", for cts, name);
        //add by kui.li,20150513,  end  
        switch(msg.req.id) {
        case ID_STAT:
