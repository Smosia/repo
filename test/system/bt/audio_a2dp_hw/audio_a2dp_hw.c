...
static const uint8_t buffer_black_list_addr_prefix[][3] = {
    {0x00, 0x54, 0xAF} // Auto BLuetooth
};

static bool is_in_buffer_black_list = false;

static bool string_is_bdaddr(const char *string);
static bool string_to_bdaddr(const char *string, bt_bdaddr_t *addr);
#endif
...
#if (MTK_STACK_CONFIG_BL == TRUE)
    if (is_in_buffer_black_list)
        common->buffer_sz = AUDIO_STREAM_OUTPUT_BUFFER_SZ * 2;
    else
#endif
...

{
    ...
    int i;
#if (MTK_STACK_CONFIG_BL == TRUE)
    if (string_is_bdaddr(address)) {
        bt_bdaddr_t peer_dev;
        string_to_bdaddr(address, &peer_dev);
        int whitelist_size = sizeof(buffer_black_list_addr_prefix)/sizeof(buffer_black_list_addr_prefix[0]);
        for (int j = 0; j < whitelist_size; j++) {
            if (buffer_black_list_addr_prefix[j][0] == peer_dev.address[0] &&
                    buffer_black_list_addr_prefix[j][1] == peer_dev.address[1] &&
                    buffer_black_list_addr_prefix[j][2] == peer_dev.address[2]) {
                INFO("black list absolute volume for %02x:%02x:%02x",
                        peer_dev.address[0], peer_dev.address[1], peer_dev.address[2]);
                 is_in_buffer_black_list = true;
                 break;
            }
        }
    }
