...
UINT16 L2CA_UcdDataWrite (UINT16 psm, BD_ADDR rem_bda, BT_HDR *p_buf, UINT16 flags)
{
    ...
    if (p_ccb->cong_sent)
    {
        L2CAP_TRACE_ERROR ("L2CAP - Handle: 0x%04x UCD cannot be sent, already congested._count: %u  buff_quota: %u",
                            p_lcb->handle,
                            (p_ccb->xmit_hold_q._count + p_lcb->ucd_out_sec_pending_q._count),
                            p_ccb->buff_quota);
...
void l2c_ucd_delete_sec_pending_q(tL2C_LCB  *p_lcb)
{
    ...
    while (p_lcb->ucd_out_sec_pending_q._p_first)
        GKI_freebuf (GKI_dequeue (&p_lcb->ucd_out_sec_pending_q));

    while (p_lcb->ucd_in_sec_pending_q._p_first)
...
BOOLEAN l2c_ucd_check_pending_out_sec_q(tL2C_CCB  *p_ccb)
{
    ...
    if ( p_ccb->p_lcb->ucd_out_sec_pending_q._count )
    {
        p_buf = (BT_HDR*)(p_ccb->p_lcb->ucd_out_sec_pending_q._p_first);
        p = (UINT8 *)(p_buf + 1) + p_buf->offset;
        STREAM_TO_UINT16(psm, p)
...
BOOLEAN l2c_ucd_check_pending_in_sec_q(tL2C_CCB  *p_ccb)
{
    ...
    if ( p_ccb->p_lcb->ucd_in_sec_pending_q._count )
    {
        p_buf = (BT_HDR*)(p_ccb->p_lcb->ucd_in_sec_pending_q._p_first);
        p = (UINT8 *)(p_buf + 1) + p_buf->offset;
        STREAM_TO_UINT16(psm, p)
