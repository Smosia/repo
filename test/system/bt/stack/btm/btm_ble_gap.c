...
BOOLEAN btm_ble_update_inq_result(tINQ_DB_ENT *p_i, UINT8 addr_type, UINT8 evt_type, UINT8 *p)
{
    ...
    else
    {
     ...
     if (p_le_inq_cb->adv_len != 0)
    {
        p_remote_name = BTM_CheckAdvData(p_le_inq_cb->adv_data_cache, BTM_BLE_AD_TYPE_NAME_CMPL, &p_i->inq_info.remote_name_len);
        if (p_remote_name == NULL)
            p_remote_name = BTM_CheckAdvData(p_le_inq_cb->adv_data_cache, BTM_BLE_AD_TYPE_NAME_SHORT, &p_i->inq_info.remote_name_len);
        if(p_remote_name)
        {
            p_i->inq_info.remote_name_len = p_i->inq_info.remote_name_len>BTM_MAX_REM_BD_NAME_LEN ? BTM_MAX_REM_BD_NAME_LEN : p_i->inq_info.remote_name_len;
            memcpy(p_i->inq_info.remote_name, p_remote_name, p_i->inq_info.remote_name_len);
        }
    }
#endif
    return to_report;
