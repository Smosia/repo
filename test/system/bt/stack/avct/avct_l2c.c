...
void avct_l2c_br_connect_ind_cback(BD_ADDR bd_addr, UINT16 lcid, UINT16 psm, UINT8 id)
{
    ...
    if ((p_lcb = avct_lcb_by_bd(bd_addr)) == NULL || avct_bcb_by_lcb(p_lcb) != NULL || (index = avct_lcb_to_idx(p_lcb)) >= AVCT_NUM_LINKS) {
        ...
        if (i == AVCT_NUM_CONN) {
            ...
            ertm_cfg.user_rx_pool_id = MTK_AVCT_BR_USER_RX_POOL_ID;
            ertm_cfg.user_tx_pool_id = MTK_AVCT_BR_USER_TX_POOL_ID;
            ertm_cfg.fcr_rx_pool_id = MTK_AVCT_BR_FCR_RX_POOL_ID;
            ertm_cfg.fcr_tx_pool_id = MTK_AVCT_BR_FCR_TX_POOL_ID;
        }
    }
