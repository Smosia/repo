...
static int load_blueangel_dev_info(config_t *config)
{
    ...
    if(fd < 0 || size == 0)
    {
        LOG_ERROR("open_file_map fail, fd:%d, path:%s, size:%d", fd, path, size);
        if (fd >= 0)
            close(fd);
        return FALSE;
    }
    ...
    for (int i =0; i < num_paired_dev; i++)
    {
        ...
        else if (paired_dev_cache[i]->device_type == BTBM_DEVICE_TYPE_BR_EDR)
        {
            ...
            {
                HidRemoteInfoType HidRemoteInfo;
                load_blueangel_cod_info (config, paired_dev_cache[i]->cod, bdstr);
        ...
        while((dptr = readdir(dirp)) != NULL)
        {
            if(strcmp(dptr->d_name, bdaddr_str) == 0)
            {
                ...
                while (pos_cur < pos_file_end)
                {
                     ...
                     */
                    service_size -= ((5 + gattservice->uuid_size) + (2 + gattservice->include_size));
                     ...
                     */
                    map = map + (7 + gattservice->uuid_size) + (2 + gattservice->include_size);
                    while (service_size > 0)
                    {
                        ...
                        while (char_size > 0)
                        {
                                    ...
                                    (remote_desc->desc_size > 0))
                            {
                                ...
                                memcpy(hid_rpt->rpt_value, remote_desc->desc_value, remote_desc->desc_size);
...
config_t *load_blueangel_cfg()
{
    ...
    if (load_blueangel_host_info(config))
    {
        if (FALSE == load_blueangel_dev_info(config))
            LOG_WARN("%s: load_blueangel_dev_info fail.", __func__);
