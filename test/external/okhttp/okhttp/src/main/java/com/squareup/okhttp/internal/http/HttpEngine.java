...
public final class HttpEngine {
  ...
  private Request networkRequest(Request request) throws IOException {
    ...
    if (request.header("Accept-Encoding") == null && 
            !"true".equals(System.getProperty("xcap.req"))) {
  ...
  public Request followUpRequest() throws IOException {
    ...
    switch (responseCode) {
      ...
      case HTTP_UNAUTHORIZED:
        if (++authenticateCount > MAX_AUTHENTICTORS) {
          throw new ProtocolException("Too many authentication: " + authenticateCount);
        }
        System.setProperty("http.method", networkRequest.method());

        String urlPath = networkRequest.url().getFile();
        if (!urlPath.substring(0, 1).equals("/")) {
            urlPath = "/" + urlPath;
        }
        System.setProperty("http.urlpath", urlPath);
        System.out.println("processResponseHeaders");
