...
#ifdef MTK_AOSP_ENHANCEMENT
    /*@ For MIDI seek table code, allocate MST */
...
EAS_RESULT SMF_Event (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode)
{
    ...
    if (parserMode == eParserModeMetaData) {
        ...
        pSMFData->pMST->MST_UpdateParsingEventTable(pSMFData->pMST, pSMFData->time >>8,  pSMFData->nextStream->ticks,
                                                    (int)pSMFData->nextTrackNum, (int)curTrackOffset,
    ...
    if (pSMFData->nextStream)
    {
        ...
        if (parserMode == eParserModeMetaData)
        {
            // if parserMode == eParserModeMetaData: get total duration, time = total duration
...
#endif
            {
                ...
                if(pSMFData->time_TmpMsgs != 0)
                    ALOGV("SMF_Event SMF_UpdateTime done: time = %lld time_TmpMsgs = %lld new ticks = %ld old ticks = %ld",
            ...
            /*@ For MIDI seek table code */
            pSMFData->pMST->MST_UpdateParsingEventTable(pSMFData->pMST, pSMFData->time >>8, pSMFData->nextStream->ticks,
                                                        (int)curTrackNum, (int)curTrackOffset,
                                                        pSMFData->nextStream->midiStream.runningStatus);
        ...
        else
        {
            ...
            ALOGV("SMF_Event SMF_UpdateTime done: pSMFData->time = %lld, new ticks = %ld, old ticks = %ld", pSMFData->time, pSMFData->nextStream->ticks - ticks, ticks);
        }
    }
    else
    {
        ALOGV("SMF_Event: stop condition: no next streams,pSMFData->state = EAS_STATE_STOPPING");
    ...
    if ((parserMode != eParserModeMetaData) && (pSMFData->state != EAS_STATE_STOPPING)) {
        if ((pEASData->durationTime > 0) && ((pSMFData->time >> 8) > pEASData->durationTime)) {
            ALOGV("SMF_Event: stop condition: pSMFData->state = EAS_STATE_STOPPING, time=%lld > pEASData->durationTime= %ld", pSMFData->time >> 8, pEASData->durationTime);   
            pSMFData->state = EAS_STATE_STOPPING;
            VMReleaseAllVoices(pEASData->pVoiceMgr, pSMFData->pSynth);
        }
    }
...
#ifdef MTK_AOSP_ENHANCEMENT
    /*@ For MIDI seek table code, free MST */
    if (pSMFData->pMST) {
...
EAS_RESULT SMF_SetData (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_I32 param, EAS_I32 value)
{
    ...
    switch (param)
    {
        ...
        {
            EAS_I32 i, trackNum;
            EAS_U32 deltaTime, ticks, timeMs;
            EventOffset *tracksOffset;
            EAS_RESULT  result;

            ALOGV("SMF_SetData:  PARSER_DATA_LOCATE, seek time is %d", (int)value);
            ...
            for (i = 0; i < pSMFData->numStreams; i++)
            {
                ...
                result = EAS_HWFileSeek(pEASData->hwInstData, pSMFData->streams[trackNum].fileHandle, tracksOffset[i].offset);
                if (EAS_SUCCESS != result) {
                    ALOGV("SMF_SetData:  EAS_HWFileSeek stream[%d] track[%d] return %d", i, trackNum, result);
                }
                ...
                if (EAS_SUCCESS != result) {
                    ALOGV("SMF_SetData:  PARSER_DATA_LOCATE, track[%d] call SMF_HandleSeekTableEvent return %d", trackNum, result);
                }
...
static EAS_RESULT SMF_ParseSysEx (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_U8 f0, EAS_INT parserMode)
{
    ...
    while (len--)
    {
        ...
        if (pSMFStream->midiStream.flags & MIDI_FLAG_GM_ON) {
            ...
            ALOGV("set SMF_FLAGS_HAS_GM_ON, pSMFData->flags=0x%x", pSMFData->flags);
        }
    }
...
static EAS_RESULT SMF_ParseEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_INT parserMode)
{
    ...
    EAS_U8 c;
#ifdef MTK_AOSP_ENHANCEMENT
    ALOGV("+ wt SMF_ParseEvent pSMFData->flags=0x%x, pSMFStream->midiStream.flags=0x%x", pSMFData->flags, pSMFStream->midiStream.flags);
#endif
    /* get the event type */
    if ((result = EAS_HWGetByte(pEASData->hwInstData, pSMFStream->fileHandle, &c)) != EAS_SUCCESS)
        return result;
    ...
    else
    {
        ...
        if (parserMode == eParserModeMetaData) {
            ...
            data = EAS_HWMalloc(0, dataSize);
            if (!data)
                return EAS_ERROR_MALLOC_FAILED;

            count = 0;
            pData = data;
            EAS_HWMemSet(data, 0, dataSize);
        }

        if ((result = EAS_ParseMIDIStream(pEASData, pSMFData->pSynth, &pSMFStream->midiStream, c, parserMode)) != EAS_SUCCESS) {
            if (data) EAS_HWFree(0, data);
            return result;
        }
    ...
    if (pSMFData->time == 0 && pSMFData->time_TmpMsgs == 0)
    {
        ...
        {
            ALOGV("SET chase mode pSMFData->flags=0x%x", pSMFData->flags);
            pSMFData->flags = (pSMFData->flags & ~SMF_FLAGS_SETUP_BAR) | SMF_FLAGS_CHASE_MODE;
        }
    }
    ALOGV("- wt SMF_ParseEvent pSMFData->flags=0x%x", pSMFData->flags);
#else
    if (pSMFData->time == 0)
    {
        if (pSMFData->flags & SMF_FLAGS_CHASE_MODE)
...
#ifdef MTK_AOSP_ENHANCEMENT
    ALOGV("division=%d, pSMFData->ppqn=%d, ,pSMFData->tickConv=%d, ", division,pSMFData->ppqn,pSMFData->tickConv);
    /*@ For MIDI seek table code */
...
#ifdef MTK_AOSP_ENHANCEMENT
    if (pSMFData->nextStream) {
        ALOGV("Enter wt SMF_ParseHeader: SMF_UpdateTime, ticks=%ld", pSMFData->nextStream->ticks);
...
static void SMF_UpdateTime (S_SMF_DATA *pSMFData, EAS_U32 ticks)
{
#ifdef MTK_AOSP_ENHANCEMENT
    EAS_I64 temp1, temp2;
    ALOGV("+ wt SMF_UpdateTimeorigin pSMFData->time =%lld, pSMFData->flags=0x%x", pSMFData->time, pSMFData->flags);
    if (pSMFData->flags & SMF_FLAGS_CHASE_MODE)
        return;
    ...
    ALOGV("- wt SMF_UpdateTime: pSMFData->time =%lld, ticks=%ld, temp1=%lld, temp2=%lld", pSMFData->time, ticks, temp1, temp2);
#else
    EAS_U32 temp1, temp2;
...
{
    EAS_I64 temp1, temp2;
    ALOGV("+ wt SMF_UpdateTimeMsg origin *ticks_Msgs =%lld, new ticks=%ld, pSMFData->flags=0x%x", *ticks_Msgs, ticks, pSMFData->flags);
    if (pSMFData->flags & SMF_FLAGS_CHASE_MODE)
        return;
    ...
    *ticks_Msgs += (temp1<<8) + (temp2>>2);
    ALOGV("- wt SMF_UpdateTimeMsg: *ticks_Msgs =%lld, temp1=%lld, temp2=%lld", *ticks_Msgs, temp1, temp2);
}
...
static EAS_RESULT SMF_HandleSeekTableEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, EAS_INT trackNum, EAS_INT parserMode, EAS_U32 position)
{
    ...
    ALOGV("-----------Enter wt SMF_HandleSeekTableEvent ----------------------------------------");
    ...
    if (!eventList || !eventCount) {
        ALOGW("SMF_HandleSeekTableEvent[0x%x] eventList: 0x%x eventCount: %u", pSMFData, eventList, eventCount);
        return EAS_ERROR_INVALID_PARAMETER;
    }
...
void SMF_DumpStreamInfo(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode)
{
    ...
    for (i = 0; i < pSMFData->numStreams; i++)
    {
        ALOGV("SMF_DumpStreamInfo: pSMFData->streams[%d].ticks = %d",i,pSMFData->streams[i].ticks);
        EAS_HWFilePos(pEASData->hwInstData, pSMFData->streams[i].fileHandle,(EAS_I32*)&filePos);

        if(pSMFData->nextStream == (&pSMFData->streams[i])) {
            ALOGV("SMF_DumpStreamInfo: pSMFData->streams[%d] is the next stream",i);
        }
    }
}
#endif
