...
static EAS_RESULT IMY_Event (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode)
{
    ...
    pData = (S_IMELODY_DATA*) pInstData;
#ifdef MTK_AOSP_ENHANCEMENT
    ALOGV("IMY_Event pData->state =%d", pData->state);
    ...
    {
        ALOGV("IMY_Event set EAS_STATE_PRESTOPPING -> EAS_STATE_STOPPING");
        pData->state = EAS_STATE_STOPPING;
        ALOGV("IMY_Event VMReleaseAllVoices");
...
#ifdef MTK_AOSP_ENHANCEMENT
        c = ToLower(c); /* change character to lower case */
#endif
        switch (c)
        {
            /* start repeat */
            ...
            case ')':
...
#endif
#ifdef MTK_AOSP_ENHANCEMENT
                ALOGV("IMY_Event:  pData->repeatTime = %lld, pData->time = %lld", pData->repeatTime, pData->time);
#endif
                ...
                if (pData->repeatTime == pData->time) {
                    ...
                    else // the case is actually infinite loop
                    {
                        ...
                        ALOGV("IMY_Event infinite loop, pData->time = %lld", pData->time);
                    }
#endif
...
//3 We need a different mode for metadata parsing where we don't loop at all
            ...
            case '@':
                ...
                {
                    ALOGV("IMY_Event @ set EAS_STATE_PRESTOPPING");
            ...
            case 'v':
                if (!IMY_GetVibeState(pEASData, pData))
                {
                    ...
                    {
                        ALOGV("IMY_Event V set EAS_STATE_PRESTOPPING");
                        pData->state = EAS_STATE_PRESTOPPING;
                        eof = EAS_TRUE;
                    }
                }
            ...
            case 'b':
                ...
                else
                {
                    ...
                    ALOGV("IMY_Event IMY_PlayNote result = %d", result);
                    if (result == EAS_SUCCESS)
                        return EAS_SUCCESS;
                    else if(result == EAS_TOKENEND)
                    {
                        ALOGV("IMY_Event B set EAS_STATE_PRESTOPPING");
                        pData->state = EAS_STATE_PRESTOPPING;
                    }
#else
    ...
    if(pData->state != EAS_STATE_PRESTOPPING)
    {
        ALOGV("IMY_Event set EAS_STATE_STOPPING");
        pData->state = EAS_STATE_STOPPING;
        VMReleaseAllVoices(pEASData->pVoiceMgr, pData->pSynth);
    }
#else
...
#endif
{
    ...
    if(pData->noteModifier != 0)
    {
        if(pData->noteModifier == 1)
        {
            if(( note == 'b')||( note == 'e'))
             {
                ALOGV("IMY_PlayNote: set # pData->noteModifier = 0, note =%d", note);
    ...
    if (parserMode == eParserModePlay)
    {
        ALOGV("IMY_PlayNote: VMStartNote pData->note=%d", pData->note);
        VMStartNote(pEASData->pVoiceMgr, pData->pSynth, IMELODY_CHANNEL, pData->note, velocity);
    }
#endif
...
static EAS_BOOL IMY_PlayRest (S_EAS_DATA *pEASData, S_IMELODY_DATA *pData)
{
    ...
    pData->time += duration;
#ifdef MTK_AOSP_ENHANCEMENT
    ALOGV("IMY_PlayRest: pData->time=%lld, duration=%d,", pData->time, duration);
    return result;
#else
    return EAS_TRUE;
#endif
...
static EAS_RESULT IMY_GetNumber (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_INT *temp, EAS_BOOL inHeader)
{
    ...
    *temp = 0;
    ...
    for (;;)
    {
        ...
        if (IsDigit(c))
        {
            // add for restricting maximum number to 9999, only for beat/repeat count
            if(*temp < 9999)
            {
                *temp = *temp * 10 + c - '0';
                if(*temp > 9999)
