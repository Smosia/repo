...
static EAS_RESULT EAS_ParseEvents (S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_I64 endTime, EAS_INT parseMode)
{
    ...
    ALOGV("EAS_ParseEvents start: pStream->time %lld, endTime %lld", pStream->time, endTime);
    ...
    while (!done)
    {
        ...
        if ((result = (*pParserModule->pfState)(pEASData, pStream->handle, &parserState)) != EAS_SUCCESS)
        {
            ALOGV("EAS_ParseEvents Stop done: State result!= EAS_SUCCESS");
            return result;
        }
        ...
        if ((parserState > EAS_STATE_PLAY)&&(parserState != EAS_STATE_PRESTOPPING))
        {
            ...
            ALOGV("EAS_ParseEvents Stop done: parserState %d > EAS_STATE_PLAY %d", parserState, EAS_STATE_PLAY);
            break;
        }
        ...
        if ((parseMode == eParserModeMetaData)&&(pEASData->mForceStopDuration))
        {
            ...
            ALOGV("EAS_ParseEvents Stop done: pEASData->mForceStopDuration =%d, parserState %d ", pEASData->mForceStopDuration, parserState);
            return EAS_ERROR_ALREADY_STOPPED;
        }
        /* get the next event time */
        if (pParserModule->pfTime)
        {
            //ALOGV("EAS_ParseEvents call *pParserModule->pfTime");
repeat:
            if ((result = (*pParserModule->pfTime)(pEASData, pStream->handle, (EAS_U32 *)&time)) != EAS_SUCCESS)
            {
                return result;
            }

            //ALOGV("EAS_ParseEvents: before event time = %lld, pStream->time = %lld", time, pStream->time);
            ...
            if (time < (endTime >> 8))
            {
                ...
                if (pParserModule->pfEvent) {
                    ...
                    {
                        ALOGV("EAS_ParseEvents Stop done: Event result!= EAS_SUCCESS");
                        return result;
                    }
                }
            }
            ...
            else
            {
                ...
                if(parseMode == eParserModeLocate)
                    ...
                    pStream->time = endTime;
#endif
                ALOGV("EAS_ParseEvents Stop done: pStream->time %lld = endTime = %lld", pStream->time, endTime);
                /*@ For MIDI seek table code */
                if(parseMode == eParserModeLocate) {
                    ALOGD("EAS_ParseEvents Stop done: pStream->time %lld = endTime = %lld", pStream->time, endTime);
...
//                  SMF_DumpStreamInfo(pEASData, pStream->handle, parseMode);
        ...
        if (VMCheckWorkload(pEASData->pVoiceMgr))
        {
             ...
             */
            ALOGV("EAS_ParseEvents Stop done: max workload exceeded");
            break;
        }
...
EAS_PUBLIC EAS_RESULT EAS_ParseMetaData (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *playLength)
{
    ...
    if ((result = EAS_ParseEvents(pEASData, pStream, 0x7fffffffff, eParserModeMetaData)) != EAS_SUCCESS)
    {
        ...
        ALOGV("EAS_ParseMetaData: ERROR result from EAS_ParseEvents = %d,", result);
        return result;
    }
    /* get the parser time */
    if ((result = EAS_GetLocation(pEASData, pStream, playLength)) != EAS_SUCCESS)
    {
        ALOGV("EAS_ParseMetaData: ERROR duration from EAS_GetLocation = %ld,", *playLength);
        return result;
    }
    else
    {
        ALOGV("EAS_ParseMetaData: SUCCESS duration from EAS_GetLocation = %ld,", *playLength);
    }
#else
...
EAS_PUBLIC EAS_RESULT EAS_Locate (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 milliseconds, EAS_BOOL offset)
{
    ...
    ALOGD("EAS_Locate: Reverb effect Init and set EAS_PARAM_REVERB_BYPASS = EAS_FALSE...");
    if (pEASData->effectsModules[EAS_MODULE_REVERB].effectData)
    {
        ...
        {
            ALOGE("EAS_Locate: Reverb effect Init Fail !!!");
            return result;
        }
    }
...
EAS_PUBLIC EAS_RESULT EAS_SetForceStopDuration (EAS_DATA_HANDLE pEASData, EAS_BOOL mForceStopDuration)
{
    if (pEASData != NULL)
    {
        ...
        ALOGV("mForceStopDuration=%ld, pEASData->mForceStopDuration=%ld, ", mForceStopDuration, pEASData->mForceStopDuration);
        return EAS_SUCCESS;
    }
    else
    {
...
EAS_PUBLIC EAS_RESULT EAS_GetLocation (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *pTime)
{
...
#ifdef MTK_AOSP_ENHANCEMENT
    {
        ...
        ALOGV("EAS_GetLocation: *pTime = %ld, temp = %lld, pStream->time = %lld", *pTime, temp, pStream->time);
    }
#else
