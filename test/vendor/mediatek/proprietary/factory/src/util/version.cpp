...
bool get_hardware_info(const char* read_dir,char* out_put,const char* def_info)
{
	FILE *fp=NULL;
	...
	if ((fp = fopen(read_dir, "r")) == NULL) {
        LOGD("[rgk][%s]""can not open file == %s\n",__FUNCTION__,read_dir);
		strcpy(out_put,def_info);
		return false;
    }
    
	file_size = fread(buf_temp,sizeof(buf_temp),1,fp);
	LOGD("[rgk][%s]""buf_temp == %s\n",__FUNCTION__,buf_temp);
	
	if(0 == sizeof(buf_temp)) //why the file_size is always 0 whether the fread successed or failed?
	{
		LOGD("[rgk][%s]""the file == %s is null\n",__FUNCTION__,read_dir);
		strcpy(out_put,def_info);
		if(NULL != fp)
		{
			fclose(fp);
...
static int create_hw_verinfo(char *info, int *len)
{
    char val[128] = {0};                                                                                     
    ...
    property_get("ro.internal.version", val, "unknown");
    print_verinfo(info, len,  "SW Ver.     ", val);                                                          

	property_get("ro.build.display.id",val,"unknown");                                                     
	print_verinfo(info, len,  "Custom Build Verno.", val);                                                     
    if(strlen(val) > 17){
        val[17] = '\0';
    }
    extern void get_bt_addr(char* ,const char*);
    get_bt_addr(val,"unknown");
    print_verinfo(info, len,  "BT Address.", val);

    extern void get_wifi_addr(char* ,const char*);
    get_wifi_addr(val,"unknown");
    print_verinfo(info, len,  "Wifi Address.", val);

	get_hardware_info(CAMERA_PATH,val,"unknown");
	//********************************************************************
	{
		char *toStart = NULL;
		char *toEnd = NULL;
		char buf_temp[128] = {0};
		strcpy(buf_temp,val);
		if(strstr(buf_temp,"NoBackCamera"))
		{
			if(strstr(buf_temp,"NoFrontCamera"))
			{
				strcpy(val,"BackCamera:NoBackCamera or not initialized,");
				strcat(val,"FrontCamera:NoFrontCamera or not initialized");
			}
			else
			{
				toStart = strstr(buf_temp,"FrontCamera:");
				toEnd = strstr(toStart,":");
				toStart = toEnd + 1;
				strcpy(val,"BackCamera:NoBackCamera or not initialized,FrontCamera:");
				strcat(val,toStart);
			}
		}
		else
		{
			...
			{
				toStart = &buf_temp[0];
				toEnd = strstr(buf_temp,"FrontCamera");
				if(NULL != toEnd)
					*toEnd = '\0';
				strcpy(val,buf_temp);
				strcat(val,"FrontCamera:NoFrontCamera or not initialized");
	...
	//********************************************************************
	print_verinfo(info, len,  "Camera.", val);

	get_hardware_info(MEMORY_PATH,val,"unknown");
	print_verinfo(info, len,  "Memory.", val);

	get_hardware_info(TP_VERSION_PATH,val,"unknown");
LOGD("---->>>>  TP info: %s\n",val);
	print_verinfo(info, len,  "TP Version.", val);

	get_hardware_info(LCD_PATH,val,"unknown");
LOGD("---->>>> LCD info: %s\n",val);
	print_verinfo(info, len,  "Lcd.", val);

#ifdef FEATURE_FTM_CALI_EXAM
	if(barcode == NULL)
	{
		print_verinfo(info, len,  "RF Cali     ", "unknown");
	}
	else
	{
		print_verinfo(info, len,  "RF Cali     ", (strstr(barcode,"10p") ||  strstr(barcode,"10P"))? "RF calibrat success !":"RF calibrat fail !");
	    is_RF_cali  = (strstr(barcode,"10p") ||  strstr(barcode,"10P"))? true :false;
	}
#ifndef FEATURE_FTM_WITH_CRYSTAL
	get_GPS_cali(val,"unknown");
	print_verinfo(info, len,  "GPS Cali    ", val);
#endif
#endif
...
int create_board(char *info, int size)                                                              
{                                                                                                            
...
char* display_board_version(int index, char* result, bool autoreturn)                                              
{                                                                                                            
	char *buf = NULL;                                                                                          
	struct textview vi;	 /* board info */                                                                    
	text_t vi_title;                                                                                           
	int nr_line;                                                                                               
	text_t info;                                                                                               
	int avail_lines = 0;                                                                                       
	...
	init_text(&vi_title, uistr_board_version, COLOR_YELLOW);                                                         
	init_text(&info, buf, COLOR_YELLOW);                                                                       
	init_text(&info, buf, COLOR_YELLOW);                                                                       
	...
	ui_init_textview(&vi, textview_key_handler, &vi);                                                          
	vi.set_btn(&vi, NULL, NULL, &rbtn);                                                                        
  create_board(buf, BUFSZ);                                                                                
	LOGD(TAG "after create_board");                                                                          
	vi.set_title(&vi, &vi_title);                                                                              
	vi.set_text(&vi, &info);                                                                                   
	vi.m_pstr = trans_verinfo(info.string, &nr_line);                                                          
	vi.m_nr_lines = g_nr_lines;                                                                                
