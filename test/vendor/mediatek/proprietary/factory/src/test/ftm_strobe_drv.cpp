...
int FlashSimpleDrv::setDuty()
{	
    int err = 0;
	ioctl(m_fdSTROBE,FLASH_IOC_SET_DUTY,0);	
    return err;
}

int FlashSimpleDrv::noTimeOut()
{	
    int err = 0;
    err = ioctlEx(m_fdSTROBE,FLASH_IOC_SET_TIME_OUT_TIME_MS, mSensorDev, 0);
	//err = ioctl(m_fdSTROBE,FLASH_IOC_SET_TIME_OUT_TIME_MS,0);
    return err;
}
