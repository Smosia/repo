...
static ftm_cmd_hdlr other_cmd_hdlr[]={
        ...
        {12,ITEM_FMRADIO,    "AT+FMRadio.CH1", (hdlr)ftm_set_property},
...
//    int at_cmd_type;
    ...
    else if(at_cmd_struct->string_ptr[at_cmd_struct->cmd_index] == '=')
    {
        ...
        LOGD(TAG "==>>      at_cmd_string is: %s\n", at_cmd_struct->at_cmd_string);
        LOGD(TAG "==>>         string_ptr is: %s\n", at_cmd_struct->at_cmd_string);
        if(!strcmp((const char *)at_cmd_struct->at_cmd_string, "AT+FMRadio.CH1"))
        {
            *at_command_type = 1;
            at_cmd_struct->param = "FMRadio.CH1,1077";
            return 0;
        }
		...
		//M:wxd for compatible camond  STOP and CLOSE 20160316 start
        else if(at_cmd_struct->string_ptr[at_cmd_struct->cmd_index] == 'S' ||at_cmd_struct->string_ptr[at_cmd_struct->cmd_index] == 'C')// For Vibretor and LED item stop
        {
		//M:wxd for compatible camond  STOP and CLOSE 20160316 end	
            *at_command_type = 0;
            at_cmd_struct->test_type = 0;
            return 0;
        }
