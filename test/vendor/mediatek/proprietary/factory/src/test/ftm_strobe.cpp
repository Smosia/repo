...
void subFlash_setOnOff(char val) {
	char *value=&val;
	int fd = open("/sys/class/sub_flashlight/sub_flashlight/sub_flashlight_enable", O_RDWR, 0777);
	int result = write(fd, value, sizeof(char));
	close(fd);
}
#endif
// wpf add for ATA test
extern "C" void strobe_test_thread_for_ata()
{
	FlashSimpleDrv*  pStrobe;
	pStrobe = FlashSimpleDrv::getInstance();
	pStrobe->init(1);
	...
	pStrobe->noTimeOut(); // keep strobe bright
	pStrobe->setOnOff(1); // turn ON strobe
#ifdef FEATURE_FTM_SUB_STROBE
	subFlash_setOnOff('1');
#endif
	while(1) {
		usleep(30000);
		if(keypadled_test_exit || led_test_exit) {
            // trigger by LED OFF command
			LOGD("STROBE" "EXIT STROBE ATA TEST");
			break;
		}
	}
	pStrobe->setOnOff(0); // turn off strobe
#ifdef FEATURE_FTM_SUB_STROBE
	subFlash_setOnOff('0');
#endif
	usleep(30000);
