...
#endif
#ifdef FEATURE_FTM_SWCHR_I_220mohm
			batt->current_charging = ((temp-(batt->bat_voltage))*1000)/220;
#endif
			if((batt->current_charging > 600) || (batt->current_charging < 300))
			  batt->current_charging = 420;
		} else {
...
#endif
#ifdef FEATURE_FTM_SWCHR_I_220mohm
    temp_charging_current = ((temp_i_sen-temp_bat_sen)*1000)/220;
#endif
  }
	else
...
int battery_entry(struct ftm_param *param, void *priv)
{
  ...
  {
    //modify BUG_ID:TRACKSYS-27 zengchuiguo 20130418 (start)
    /*
    ...
    */
    int wait_counter = 5;
    //Wait to be charged and expire after 5 seconds.
    while(wait_counter-- > 0){
	usleep(1000*1000);
	if(temp_v_chr >= AUTO_TEST_VCHR_VALUE){
		break;
	}
	LOGD(TAG "[zcg] wait charger: %d \n",wait_counter);		
    }
    if(wait_counter == 0){
    	LOGD(TAG "[FTM_BAT 0] %d < %d => no charger, return fail\n", temp_v_chr, AUTO_TEST_VCHR_VALUE);		
    	batt->mod->test_result = FTM_TEST_FAIL;
    ...
    //modify BUG_ID:TRACKSYS-27 zengchuiguo 20130418 (end)
  }

	//auto test - V_bat
	temp_v_bat = get_v_bat_sen();
	return_data.battery.voltage = temp_v_bat;
	//add BUG_ID:TRACKSYS-27 zengchuiguo 20130418 (start)
	battery_update_info(batt, batt->info);
	iv->redraw(iv);
	usleep(1000);
	//add BUG_ID:TRACKSYS-27 zengchuiguo 20130418 (end)
	...
	for(i=0 ; i<i_loop_time ; i++)
	{
		...
		if((temp_chr_cuttent > AUTO_TEST_CHR_CURRENT_VALUE) && (temp_chr_cuttent < AUTO_TEST_CHR_CURRENT_VALUE + 300) && (temp_chr_cuttent > AUTO_TEST_CHR_CURRENT_VALUE + 100))
			break;
	}
	//add BUG_ID:TRACKSYS-27 zengchuiguo 20130418 (start)
	battery_update_info(batt, batt->info);
