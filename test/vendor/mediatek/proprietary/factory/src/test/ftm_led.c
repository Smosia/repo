...
extern void strobe_test_thread_for_ata();
static pthread_t strobe_flash_thread;
...
// wpf add for ATA test
static void *update_strobe_flash_thread(void *priv){
    LOGD("Entry %s\n",__FUNCTION__);
    strobe_test_thread_for_ata();
    LOGD("Leave %s\n",__FUNCTION__);
    return NULL;
}
...
{
LOGD("ATA test for LED\n");
//	iv->start_menu(iv, 0);
//	iv->redraw(iv);

// wpf add for ATA test
#ifdef FEATURE_FTM_STROBE
    // 1. LED & STROBE are independent of each other
    // 2. AT+LED test cover strobe light, because we have no AT+STROBE Command !
    pthread_create(&strobe_flash_thread, NULL, update_strobe_flash_thread, priv);
#endif
...
// wpf add for ATA test
	pthread_join(strobe_flash_thread, NULL);
	return 0;
}
