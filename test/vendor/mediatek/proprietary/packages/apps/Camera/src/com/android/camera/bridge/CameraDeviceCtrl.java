...
import com.android.camera.animation.AnimationControler;
//end
...
public class CameraDeviceCtrl implements SurfaceHolder.Callback {
	 ...
	 * Add for animation when switch camera by songqingming
	 */
	private static final int MSG_SWITCH_CAMERA_START_ANIMATION = 100;
	private boolean mIsSwitchCamera = false;
	private AnimationControler mAnimationControler;
	private AnimationControler.Listener mAcListener = new AnimationControler.Listener() {

		@Override
		public void onAnimationStarted(int nextCameraId) {
			android.util.Log.d("sqm", "mAcListener: onAnimationStarted------nextCameraId="+nextCameraId);
			
			switchCameraExt(nextCameraId);
			
		}

		@Override
		public void onAnimationEnd() {
			android.util.Log.d("sqm", "mAcListener: onAnimationEnd------");
     ...
     * Add for animation when switch camera by songqingming
     */
    public void initAnimationControler() {
        mAnimationControler = new AnimationControler(
    		(ImageView) (mCameraActivity.findViewById(R.id.switchAnimationImageView)),
    		mCameraActivity.findViewById(R.id.cameraViewOverlay),
    		mCameraActivity);
    	mAnimationControler.setListener(mAcListener);
    }
    //end
     ...
     * Add for animation when switch camera by songqingming
     */
    public void switchCameraExt(int cameraId) {
        ...
        //closeCamera();
        if (!mCameraActivity.getResources().getBoolean(R.bool.switch_camera_animation_enabled)) {
            closeCamera();
        }
        ...
        mIsSwitchCamera = true; // add by songqingming
    }
    ...
    public void switchCamera(int cameraId) {
        if (mCameraActivity.getResources().getBoolean(R.bool.switch_camera_animation_enabled)) {
            boolean isFrontCamera = (getCameraHolder().getBackCameraId() == cameraId);
    	    Camera camera = getCameraDevice().getCamera().getInstance();
    	    android.util.Log.d("sqm", "switchCamera: cameraId="+cameraId
                    +", isFrontCamera="+isFrontCamera+", camera="+camera);
    	
	    if (camera != null) {
		mAnimationControler.prepareAnimationIcon(camera, isFrontCamera, 
                        cameraId, this, mSurfaceView.getWidth(), mSurfaceView.getHeight());
	    }
        } else {
            switchCameraExt(cameraId);
        }
    }
    //end
  ...
  //A: DWYQLSSMY-161  anxiang.xiao  for shutter sound switch start 20160323
    public void enableShutterSound(boolean enable) {
        if(enable){
        	 //modify bug:DSLLE-560 yangdan 20150609 start
    		/*if(mCameraActivity.getPreferences().getString(SettingConstants.KEY_SHUTTER_SOUND,"on").equals("on")){
    		    if(mCameraActivity.getString(R.string.set_shutter_sound_flag).equals("true"))
    			    mCurCameraDevice.enableShutterSound(false); // wpf modfiy
    		    else
    			    mCurCameraDevice.enableShutterSound(true);
    		} else 
    			mCurCameraDevice.enableShutterSound(false);
            } else {
    		    if(mCameraActivity.getString(R.string.set_shutter_sound_flag).equals("true"))
    			    mCurCameraDevice.enableShutterSound(false); // wpf modfiy
    		    else
            	...
            	if(mCameraActivity.getPreferences().getString(SettingConstants.KEY_SHUTTER_SOUND,"on").equals("on")){
            		ParametersHelper.enableRecordingSound(mCurCameraDevice.getParameters(), "0");
        		    if(mCameraActivity.getString(R.string.set_shutter_sound_flag).equals("true")){
        			    mCurCameraDevice.enableShutterSound(false); // wpf modfiy
        		    }
        		    else{
        			...
        			mCurCameraDevice.enableShutterSound(false);
        		  	ParametersHelper.enableRecordingSound(mCurCameraDevice.getParameters(), "1");//"1--false for video shutter sound"
        		 }
             } else {
            	    ParametersHelper.enableRecordingSound(mCurCameraDevice.getParameters(), "1");
        		    if(mCameraActivity.getString(R.string.set_shutter_sound_flag).equals("true")){
        			    mCurCameraDevice.enableShutterSound(false); // wpf modfiy
        		    }
        		    else{
           ...
           //modify bug:DSLLE-560 yangdan 20150609 end
        }
    }
  //A: DWYQLSSMY-161  anxiang.xiao  for shutter sound switch end 20160323
    ...
    public void applyParameterForCapture(final SaveRequest request) {
        ...
        //A:DELSLMY-735  wxd for contrl  shutsound  start 20160315
		AudioManager am = (AudioManager)mCameraActivity.getSystemService(Context.AUDIO_SERVICE);
        if(am.isMusicActive()){
		         Log.i(TAG,"isMusicActive");
		         mCurCameraDevice.enableShutterSound(false);
		}else{
		        Log.i(TAG,"else isMusicActive");
		          mCurCameraDevice.enableShutterSound(true);
		}
		//A:DELSLMY-735  wxd for contrl  shutsound  start 20160315
    ...
    private void prepareParameter(boolean needRestart, boolean isPreviewRatioChanged,
        ...
        //modify for animation when switch camera by songqingming
    ...
    private class MainHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_SWITCH_CAMERA_START_ANIMATION:
                //mAnimationControler.startAnimation();
                mAnimationControler.setCameraSwitchState(true);
                break;
            //end
    ...
    private void applySecondParameters() {
		...
		AudioManager am = (AudioManager)mCameraActivity.getSystemService(Context.AUDIO_SERVICE);
        if(am.isMusicActive()){
		         Log.i(TAG,"applySecondParameters -----isMusicActive");
		         mCurCameraDevice.enableShutterSound(false);
		}else{
		        Log.i(TAG,"applySecondParameters-------else isMusicActive");
		          mCurCameraDevice.enableShutterSound(true);
		}
		//A:DELSLMY-735  wxd for contrl  shutsound  start 20160315
        CameraPerformanceTracker.onEvent(TAG,
