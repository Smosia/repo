...
public class SettingCtrl implements ISettingCtrl {
    ...
    private boolean mergePicSize = false;
    //end
    ...
    public SettingCtrl(ICameraContext cameraContext) {
        ...
        // merge picture size 20160509 by cxd
        mergePicSize = mContext.getResources().getBoolean(R.bool.merge_picture_size);
        //end
    ...
    public void onSettingChanged(String settingKey, String value) {
        ...
        // merge picture size 20160509 by cxd
        if(mergePicSize) {
            if(SettingConstants.KEY_PICTURE_SIZE.equals(settingKey)) {
                String ratios = getRatiosByPictureSize(value);
                Log.i("cxd", "KEY_PICTURE_SIZE,ratios"+ratios);
                onSettingChanged(parameters, currentCameraId, SettingConstants.KEY_PICTURE_RATIO, ratios);
            }
        }
        //end
    ...
    private void onSettingChanged(Parameters parameters, int currentCameraId, String key,
        ...
        if (!isExecutedByRule) {
            if (SettingConstants.KEY_PICTURE_RATIO.equals(key)) {
                ...
                //merge picture size 20160509 by cxd
                if(mergePicSize) {
                    SharedPreferences ratiosPre = ((CameraActivity)mContext).getSharePreferences();
                    Editor ratiosEditor = ratiosPre.edit();
                    ratiosEditor.putString(SettingConstants.KEY_PICTURE_RATIO, value);
                    ratiosEditor.apply();
                }
                //end
            } else {
    ...
    //merge picture size 20160509 by cxd
    public String getRatiosByPictureSize(String picturesize) {
        if(checkValueInArray(SettingDataBase.getPictureSizes_16_9(), picturesize)) {
            return SettingUtils.PICTURE_RATIO_16_9;
        }
        if(checkValueInArray(SettingDataBase.getPictureSizes_4_3(), picturesize)) {
            return SettingUtils.PICTURE_RATIO_4_3;
        }
        if(checkValueInArray(SettingDataBase.getPictureSize_5_3(), picturesize)) {
            return SettingUtils.PICTURE_RATIO_5_3;
        }
        if(checkValueInArray(SettingDataBase.getPictureSize_3_2(), picturesize)) {
            return SettingUtils.PICTURE_RATIO_3_2;
        }
        return null;
    }
    public boolean checkValueInArray(String[] picturesizes, String picturesize) {
        for(int i=0; i<picturesizes.length; i++) {
            if(picturesizes[i].equals(picturesize)) {
                return true;
            }
        }
        return false;
    ...
    private void executeRule(Parameters parameters, int currentCameraId, String conditionKey) {
        //merge picture size by cxd
        if(mergePicSize) {
            if(SettingConstants.KEY_PICTURE_RATIO.equals(conditionKey)) {
                return;
            }
        }
        //end
    ...
    private void resetSettings(SharedPreferences sharedPreference) {
		...
		//bug_id:DELSLMY-672 add by wangzhenfeng make white balance save native when onpressbackkey start		
		if(key.equals("pref_camera_whitebalance_key")){
			continue;
		}
		//bug_id:DELSLMY-672 add by wangzhenfeng make white balance save native when onpressbackkey end
