...
//add for faster thumb by songqingming (start)
import android.view.animation.Animation;
import android.view.animation.DecelerateInterpolator;
import android.view.animation.ScaleAnimation;
//add for faster thumb by songqingming (end)
...
public class ThumbnailViewManager extends ViewManager implements OnClickListener,
    ...
    private ScaleAnimation mThumbnailAnim; //add for faster thumb by songqingming
    ...
    public ThumbnailViewManager(CameraActivity context) {
        ...
        createThumbnailAnimation();//add for faster thumb by songqingming
    }

    //add for faster thumb by songqingming (start)
    private void createThumbnailAnimation() {
	mThumbnailAnim = new ScaleAnimation(0.0f, 1, 0.0f, 1,  
    		Animation.RELATIVE_TO_SELF, 1.0f,  Animation.RELATIVE_TO_SELF, 1.0f);
	mThumbnailAnim.setDuration(20);
	mThumbnailAnim.setFillAfter(true);
	mThumbnailAnim.setInterpolator(new DecelerateInterpolator());
    }
    //add for faster thumb by songqingming (end)
        ...
        if (mThumbnailView != null && !mIsUpdatingThumbnail) {
            if (super.isShowing()) {
                ...
                if (mThumbnail != null && mThumbnail.getBitmap() != null) {
                    ...
                    mThumbnailView.setBitmap(null);
                    //modify DWYQA-251 songqingming 20160427 (start)
                    //mThumbnailView.setVisibility(View.INVISIBLE);
                    mThumbnailView.setVisibility(View.GONE);
                    //modify DWYQA-251 songqingming 20160427 (end)
                }
            } else {
    ...
    //add for faster thumb by songqingming (start)
    public void updateThumbnailViewExt() {
        Log.i(TAG, "[updateThumbnailViewExt]this = " + this
                + ", mIsUpdatingThumbnail = " + mIsUpdatingThumbnail);
        if (mThumbnailView != null && !mIsUpdatingThumbnail) {
            if (super.isShowing()) {
                Log.i(TAG, "[updateThumbnailViewExt]showing is true");
                if (mThumbnail != null && mThumbnail.getBitmap() != null) {
                    // here set bitmap null to avoid show last thumbnail in a
                    // moment.
                    Log.i(TAG, "[updateThumbnailViewExt]showing is true,set VISIBLE.");
                    mThumbnailView.setBitmap(null);
                    mThumbnailView.setBitmap(mThumbnail.getBitmap());
                    mThumbnailView.setVisibility(View.VISIBLE);
                    mThumbnailView.startAnimation(mThumbnailAnim);//add by songqingming
                } else {
                    Log.i(TAG, "[updateThumbnailViewExt]thumbnail is null,set INVISIBLE!");
                    mThumbnailView.setBitmap(null);
                    mThumbnailView.setVisibility(View.INVISIBLE);
                }
            } else {
                Log.i(TAG, "[updateThumbnailViewExt]showing is false,set INVISIBLE.");
                mThumbnailView.setVisibility(View.INVISIBLE);
            }
        }
    }
    ...
    private Handler mMainHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case MSG_UPDATE_THUMBNAIL:
                //add for faster thumb by songqingming (start)
                if (getContext().getResources().getBoolean(R.bool.faster_thumb_config)) {
                    mIsUpdatingThumbnail = false;
            	    updateThumbnailViewExt();
                    return;
            	}
                //add for faster thumb by songqingming (end)
