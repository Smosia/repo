        ...
        if (pictureRatio == null) {
            ...
            if (supportedRatios != null && supportedRatios.size() > 0) {
               ...
               //  String ratioString = supportedRatios.get(supportedRatios.size() - 1); 
                ...
                if(mActivity.getResources().getBoolean(R.bool.is_set_frontcamera_ratios_separate)){
                	if(mCameraId == 0){
                   	 if(mActivity.getResources().getBoolean(R.bool.camera_ratios_flag)) {
                   		 Log.i(TAG, "WPF_CAM ... 4:3");
                   		 ratioString = supportedRatios.get(0);
                   	 } else {
                   		 Log.i(TAG, "WPF_CAM ... FullScreen");
                   		 Log.d(TAG, "WPF_CAM ... @@@FullScreen");
                   		 ratioString = supportedRatios.get(supportedRatios.size() - 1);
                   	  }
                   }else if(mCameraId == 1){
                   		Log.i(TAG, "WPF_CAM ... 4:3");
                   		Log.d(TAG, "WPF_CAM ... @@4:3");
                   		ratioString = supportedRatios.get(0);
                   }
                }else{
                	if(mActivity.getResources().getBoolean(R.bool.camera_ratios_flag)) {
