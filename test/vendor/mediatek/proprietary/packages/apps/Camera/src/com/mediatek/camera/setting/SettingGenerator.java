...
public class SettingGenerator {
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_ZSD:
        	//M :bug-id:null support ZSD even low raw start by anxiang.xiao 20160324 
            if(mContext.getResources().getBoolean(R.bool.show_zsd_func)) 
            {
          ...
          //M :bug-id:null support ZSD even low raw end by anxiang.xiao 20160324 
            break;
        ...
        case SettingConstants.ROW_SETTING_RECORD_LOCATION:
            break;
         // BUG_ID:null  add mirror view  by anxiang.xiao start (start)
        case SettingConstants.ROW_SETTING_CAMERA_MIRROR_VIEW:
            if(!mContext.getResources().getBoolean(R.bool.show_mirror_view)) {
               removePreference = true;
            }
            int currentCameraId = mICameraDeviceManager.getCurrentCameraId();
            Log.i(TAG, "currentCameraId:" + currentCameraId + ", frontCameraId:" + mICameraDeviceManager.getFrontCameraId());
            if (!removePreference && currentCameraId != mICameraDeviceManager.getFrontCameraId()) {
                removePreference = true;
            }
            break;
           // BUG_ID:null  add mirror view  by anxiang.xiao start (end)
...
//                    settingId);
        ...
        case SettingConstants.ROW_SETTING_VOLUME:
            if(!mContext.getResources().getBoolean(R.bool.show_volume_control)) 
                removePreference = true;
        ...
        case SettingConstants.ROW_SETTING_DNG:
            ...
            break;
        //add by cxd 20160406 start
        case SettingConstants.ROW_SETTING_FOCUS_SOUND:
        	if (!mContext.getResources().getBoolean(R.bool.show_touch_focus_sound)) {
        		removePreference = true;
        	}
        	break;
        //end
        //add by xiaodong.cheng for font flash
        case SettingConstants.ROW_SETTING_FONT_FLASH:
      	  if(mICameraContext.getCameraActivity().getCameraDeviceCtrl().getCameraId() == CameraHolder.instance().getBackCameraId()) {
      		  removePreference = true;
      	  }
      	  if(!mContext.getResources().getBoolean(R.bool.font_lcd_flash_on)) {
      		  removePreference = true;
      	  }
        break;
        //end
