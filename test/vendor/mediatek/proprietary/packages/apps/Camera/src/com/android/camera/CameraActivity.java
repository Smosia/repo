...
import android.content.SharedPreferences.Editor;  //add BUG_ID:DWLEBM-35 anxiang.xiao 20160316 
...
public class CameraActivity extends ActivityBase implements
    ...
    //A:DELSLMY-731  wxd for contrl volume key start 20160310
    private int volumeControlType = -1;
    boolean volumeUp = false;
    boolean volumeDown = false;
    boolean isDownTogether = false;
    //A:DELSLMY-731  wxd for contrl volume key end 20160310
  ...
  //add DELSLMY-613 anxiang.xiao 20160309 start
    private boolean mBluetoothCapture = false;		
  //add DELSLMY-613 anxiang.xiao 20160309 end
    ...
    public void onCreate(Bundle icicle) {
        sendBroadcast(new Intent("com.android.camera_led"));//add  DELSLM-457 huangerhui 20160222
     ...
     //add DELSLMY-613 anxiang.xiao 20160309 start 
       mBluetoothCapture = this.getResources().getBoolean(R.bool.bluetooth_capture);	
       Log.i(TAG, "--");
    //add DELSLMY-613 anxiang.xiao 20160309 end
        ...
        //add by xiaodong.cheng 20160329 start
        if(getResources().getBoolean(R.bool.make_navigation_bar_transparent)) {
...
//            View rootView = findViewById(R.id.camera_root);
...
//            rootView.setSystemUiVisibility(flag);
	 ...
	 */
        mCameraDeviceCtrl.initAnimationControler();
		...
		CameraPerformanceTracker.onEvent(TAG,
                ...
                CameraPerformanceTracker.ISBEGIN);
      //A:DELSLMY-735  wxd for contrl  shutsound  start 20160315
        /*mModuleManager = new ModuleManager(this,fileSaver, mCameraAppUi, featureConfig,
                deviceManager, moduleCtrl, mISelfTimeManager);*/
        mModuleManager = new ModuleManager(this, this,fileSaver, mCameraAppUi, featureConfig,
                deviceManager, moduleCtrl, mISelfTimeManager);
      //A:DELSLMY-735  wxd for contrl  shutsound  end 20160315
        ...
        //modify bug: DWYSLM-122 anxiang.xiao 20160329 start
//       SettingUtils.upgradeGlobalPreferences(mPreferences.getGlobal(),
//        CameraHolder.instance().getNumberOfCameras());
        
        SettingUtils.upgradeGlobalPreferences(this,mPreferences.getGlobal(), CameraHolder.instance()
              .getNumberOfCameras());
       //modify bug: DWYSLM-122 anxiang.xiao 20160329 end
    ...
    protected void onResume() {
        ...
        CameraPerformanceTracker.onEvent(TAG,
                ...
                CameraPerformanceTracker.ISBEGIN);
        super.onResume();
        // A : DWYQLSSMY-261, chengxiaodong , 2016-03-22,for brightness control, start
...
//            rootView.setSystemUiVisibility(View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);
    ...
    protected void onPause() {
        ...
        CameraPerformanceTracker.onEvent(TAG,
                ...
                CameraPerformanceTracker.ISBEGIN);
        super.onPause();
        //add  DWYQLSSMY-261, chengxiaodong 2016-03-22,for brightness control, start
	...
	//bug_id:DELSLMY-733  DELSLMY-735 save camera native setting start 20160310		
        if(getResources().getBoolean(R.bool.isSaveCameraSetting)){
           	//do nothing
          }else{
          	 clearUserSettings();
          }
...
//A:DELSLMY-731  wxd for contrl volume key start 20160310
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        Log.e("hrl","volumeUp ="+volumeUp+" volumeDown= "+volumeDown+" isDownTogether ="+isDownTogether+"volumeControlType="+volumeControlType);
        if((volumeControlType == 0) || (volumeControlType == 1)){
            if (KeyEvent.KEYCODE_VOLUME_UP == keyCode && event.getAction()==KeyEvent.ACTION_UP 
			&& !volumeDown && !isDownTogether){		
                if(volumeControlType == 1){
                    if (mCameraAppUi.getShutterManager() != null) 
                        mCameraAppUi.getZoomManager().zoomOut();
                }
                return true;
            }
            if (KeyEvent.KEYCODE_VOLUME_DOWN == keyCode&& event.getAction()==KeyEvent.ACTION_UP
			&& !volumeUp && !isDownTogether){
                if(volumeControlType == 1){
                    if (mCameraAppUi.getShutterManager() != null) 
                        mCameraAppUi.getZoomManager().zoomIn();
                }
                return true;
            }
            if(isDownTogether && (keyCode == KeyEvent.KEYCODE_VOLUME_UP ||
                     keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) &&
                      event.getAction() == KeyEvent.ACTION_DOWN){
                    if(volumeControlType == 1){
                        if (mCameraAppUi.getShutterManager() != null && !isVideoMode()) {
                            mCameraAppUi.getShutterManager().performPhotoShutter();
                        }
                    }
                    isDownTogether = false;
                    return true;
                }
            }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (mPaused) {
          ...
          //@{add yangdan bluetooth key action  DWLELL-109
        if (mBluetoothCapture) {
            if (isFullScreen() && keyCode == KeyEvent.KEYCODE_ENTER) {
                if (mCameraAppUi.getShutterManager()!= null && !isVideoMode()) {
                    mCameraAppUi.getShutterManager().performPhotoShutter();
                }
                return true;
    ...
    public void gotoGallery() {
        ...
        // gulincheng 20160217 DWYSLMA-211 start
        try {
            if ( this.getPackageManager().getApplicationInfo("com.android.gallery3d", 0) != null) {
        ...
        // gulincheng 20160217 DWYSLMA-211 end
    ...
    private void notifyParametersReady() {
      ...
      //A:DELSLMY-731  wxd for contrl volume key start 20160310
	 String volumeShoot = mPreferences.getString(SettingConstants.KEY_VOLUME, 
				getString(R.string.pref_camera_volume_default));
        if(this.getResources().getBoolean(R.bool.show_volume_control)) {
            if("shoot".equals(volumeShoot)){
                volumeControlType = 0;
            }else if("zoom".equals(volumeShoot)){
                volumeControlType = 1;	
            }else if("volume".equals(volumeShoot)){
                volumeControlType = 2; 
            }
        } else {
            volumeControlType = 2; 
    ...
    public void disableOrientationListener() {
     ...
     public boolean getConfigBool(int id) {
...
//                    CameraHolder.instance().getNumberOfCameras());
                   ...
                   SettingUtils.upgradeGlobalPreferences(CameraActivity.this,mPreferences.getGlobal(), CameraHolder.instance()
                         .getNumberOfCameras());
                  //modify bug: DWYSLM-122 anxiang.xiao 20160329 end
					...
					//add by xiaodong.cheng
                    if(getResources().getBoolean(R.bool.is_set_frontcamera_video_quality_separate))
                    {
                    	Log.i(TAG,"video_quality");
                        SharedPreferences sp_front_video =  getSharePreferences(frontCameraId);
                        Editor editor_front_video = sp_front_video.edit();
                        editor_front_video.putString(SettingConstants.KEY_VIDEO_QUALITY,"108");
                        editor_front_video.apply();
                        mISettingCtrl.updateSetting(mPreferences.getLocal());
                    }
                    //end
                    
                   //add BUG_ID:DWLEBM-35 anxiang.xiao 20160316 start 
                    if(getConfigBool(R.bool.is_set_frontcamera_ratios_separate)){
                    	 SharedPreferences sp_front =  getSharePreferences(frontCameraId);
                         Editor editor_front = sp_front.edit();
                         editor_front.putString(SettingConstants.KEY_PICTURE_RATIO,SettingUtils.PICTURE_RATIO_4_3);
                         editor_front.apply();
                         SharedPreferences sp_back =  getSharePreferences(backCameraId);
                         Editor editor_back = sp_back.edit();
                         editor_back.putString(SettingConstants.KEY_PICTURE_RATIO, SettingUtils.PICTURE_RATIO_16_9);
                         editor_back.apply();
                        mCameraDeviceCtrl.getCurCameraDevice().setPreviewSize();
                        mISettingCtrl.updateSetting(mPreferences.getLocal());
                    }
                    // merge picture size 20160509 by cxd
                    if(getResources().getBoolean(R.bool.merge_picture_size)) {
                         SharedPreferences prefPic =  getSharePreferences();
                         Editor pictureSizeEditor = prefPic.edit();
                         List<String> picSizes = SettingUtils.buildSupportedPictureSizeByRatio(mCameraDeviceCtrl.getParametersExt(), 
                             prefPic.getString(SettingConstants.KEY_PICTURE_RATIO, SettingUtils.PICTURE_RATIO_16_9));
                         Log.i("cxd", "set Max Picture size"+picSizes);
                         pictureSizeEditor.putString(SettingConstants.KEY_PICTURE_SIZE, picSizes.get(picSizes.size()-1));
                         pictureSizeEditor.apply();
                         mISettingCtrl.updateSetting(mPreferences.getLocal());
                    }
                    //end
                   //add BUG_ID:DWLEBM-35 anxiang.xiao 20160316 end
    ...
    private PickerManager.PickerListener mPickerListener = new PickerManager.PickerListener() {
        ...
        //add by xiaodong.cheng for font flash
        @Override
        public boolean onFontFlashPiceked(String fontflashMode) {
        	mISettingCtrl.onSettingChanged(SettingConstants.KEY_FONT_FLASH, fontflashMode);
        	//mMainHandler.sendEmptyMessage(MSG_APPLY_PARAMETERS_WHEN_IDEL);
        	return true;
        }
        //end
    ...
    // A: DWYQLSSMY-275 chengxiaodong 2016-03-22,for brightness control, start
    ...
    private void setBrightness(int brightness){
    	...
    	boolean permission = (PackageManager.PERMISSION_GRANTED == getPackageManager().checkPermission("android.permission.WRITE_SETTINGS", "com.mediatek.camera"));
    	if(permission) {
            Settings.System.putInt(getContentResolver(), Settings.System.SCREEN_BRIGHTNESS, brightness);
        Log.d("cxd", "permission");
    	}else {
    		//getPackageManager().getPackageInfo("com.mediatek.camera", PackageManager.GET_PERMISSIONS).requestedPermissions;
        	Log.d("cxd", "dispermission");
        	String[] permissionStr = new String[]{"Manifest.permission.WRITE_SETTINGS"};
        	requestPermissions(permissionStr , 123); 
        	Settings.System.putInt(getContentResolver(), Settings.System.SCREEN_BRIGHTNESS, brightness);
        }
    }
    /**
    ...
    */
    ...
    private void setBrightnessAutoMode(int mode) {
    	Log.i(TAG, "setBrightnessAutoMode");
    	boolean permission = (PackageManager.PERMISSION_GRANTED == getPackageManager().checkPermission("android.permission.WRITE_SETTINGS", "com.mediatek.camera"));
        if(permission) {
    ...
    */
    ...
    //end
    // add by xiaodong.cheng 10260330 start
    private void adjustLayoutForNavi() {
    	int naviHeight = this.getResources().getDimensionPixelSize(
				com.android.internal.R.dimen.navigation_bar_height);
		View layerNormalView = findViewById(R.id.view_layer_normal);
		View layerShutterView = findViewById(R.id.view_layer_shutter);
		LayoutParams lp = (LayoutParams) layerNormalView.getLayoutParams();
		if(this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT)
			lp.bottomMargin = naviHeight;
		else
			lp.rightMargin = naviHeight;
		layerNormalView.setLayoutParams(lp);
		layerShutterView.setLayoutParams(lp);
	}
    //end
}
