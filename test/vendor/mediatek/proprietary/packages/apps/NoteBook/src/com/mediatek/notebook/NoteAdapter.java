...
public class NoteAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
		...
		//modify BUG_ID:DWLJE-865 yeyu 20150928(start)
		String ctime = setHour(item.create_time);
		// view.mCreateTime.setText(item.create_time);
		view.mCreateTime.setText(ctime);
		//modify BUG_ID:DWLJE-865 yeyu 20150928(end)
	...
	//add BUG_ID:DWLJE-865 yeyu 20150928(start)
	private String setHour(String create_time) {
		...
		String[] ht = ct[0].split(" ");
		int h = Integer.parseInt(ht[ht.length - 1]);
		int t = ht[ht.length - 1].length();

		ct[0].substring(0, t);
		if (ct[0].length() == ht[0].length())
			ct[0] = "";
		if (h == 0) {
			h = h + 12;
			create_time = ct[0] + " " + h + ":" + ct[1] + " "+mContext.getResources().getString(R.string.date_time_am);
		} else if (h == 12) {
			create_time = ct[0] + " " + h + ":" + ct[1] + " "+mContext.getResources().getString(R.string.date_time_pm);
		} else if (h >= 12) {
			h = h - 12;
			create_time = ct[0] + " " + h + ":" + ct[1] + " "+mContext.getResources().getString(R.string.date_time_pm);
		} else {
	...
	//add BUG_ID:DWLJE-865 yeyu 20150928(end)
