...
//add BUG_ID:DELSLMY-719 liqiang 20160311 end
...
public class ShutdownActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        sCountDownTimer = new CountDownTimer(mSecondsCountdown * millisSeconds, millisSeconds) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                Log.d("@M_" + TAG, "showDialog time = " + millisUntilFinished / millisSeconds);
                Log.d("@M_" + TAG, "isFinishing() = " + isFinishing());
                if (!isFinishing()) {
                    showDialog(DIALOG);
                }
            }
            ...
            public void onFinish() {
                if (mTelephonyManager.getCallState() != TelephonyManager.CALL_STATE_IDLE) {
                    Log.d("@M_" + TAG, "phone is incall, countdown end");
                    SchPwrWakeLock.releaseCpuWakeLock();
                  //add BUG_ID:DELSLMY-719 liqiang 20160311 start
                    NotificationManager nm = (NotificationManager)ShutdownActivity.this.getSystemService(Context.NOTIFICATION_SERVICE);
                    Notification mNotification = new Notification(R.drawable.ic_settings_schpwroff,getString(R.string.schedule_power_on_off_settings_title),System.currentTimeMillis());
                    Intent mIntent = new Intent(ShutdownActivity.this,SetAlarm.class);
                    final Bundle bundle = new Bundle();
                    bundle.putInt(Alarms.ALARM_ID, 2);
                    mIntent.putExtras(bundle);
                    PendingIntent pi = PendingIntent.getActivity(ShutdownActivity.this,0,mIntent,0);
                    mNotification.flags = Notification.FLAG_AUTO_CANCEL|Notification.FLAG_HIGH_PRIORITY;
                    mNotification.setLatestEventInfo(ShutdownActivity.this,getString(R.string.schedule_power_on_off_settings_title),getString(R.string.shutdown_cancel),pi);
                    nm.notify(0,mNotification);
                  //add BUG_ID:DELSLMY-719 liqiang 20160311 end
                    finish();
                } else {
                    Log.d("@M_" + TAG, "count down timer arrived, shutdown phone");
                    fireShutDown();
        ...
        Log.d("@M_" + TAG, "ShutdonwActivity onCreate");
    ...
    private void cancelCountDownTimer() {
        if (sCountDownTimer != null) {
            Log.d("@M_" + TAG, "cancel sCountDownTimer");
