...
public final class MountPointManager {
    ...
    public long getMountPointTotalSpace(String path) {
        ...
        long totalSpace = 0;
        //A:DWYQB-175 guoshuai 20160420(start)
        long mUnitConversion = 1024 * 1024 * 512;
        long mUnitConversionLess = 1024 * 1024 * 461;
        boolean mConditionsMet = false;
        boolean mIsExternal = false;
        //A:DWYQB-175 guoshuai 20160420(end)
        for (MountPoint mp : mMountPathList) {
            if (mp.mPath.equalsIgnoreCase(path)) {
                totalSpace = mp.mTotalSpace;
                mIsExternal = mp.mIsExternal;
                if (totalSpace == 0) {
        ...
        //A:DWYQB-175 guoshuai 20160420(start)
        LogUtils.d(TAG, "totalSpace01 " + totalSpace);
        if(!mIsExternal){
            do {
                if (totalSpace <= mUnitConversionLess) {
                    totalSpace = mUnitConversion;
                    mConditionsMet = true;
                } else {
                    mUnitConversion = mUnitConversion * 2;
                    mUnitConversionLess = mUnitConversionLess * 2;
                }
            } while (mConditionsMet == false);
        }
        LogUtils.d(TAG, "totalSpace02 " + totalSpace);
        //A:DWYQB-175 guoshuai 20160420(end)
