...
public class HotKnotSendActivity extends Activity implements
    ...
    private void checkSpecailTypes(Uri[] mUris) {
        for (int i = 0; i < mUris.length; i++) {
            if (mUris[i].toString().startsWith(GOOGLE_APP_PHOTO)) {
                Uri newUrl = handleGooglePhoto(mUris[i]);
                if (newUrl != null) {
                    mUris[i] = newUrl;
                }
            }
    ...
    private Uri handleGooglePhoto(Uri uriExtra) {
        ...
        try {
            ...
            if (startIndex < 0) {
                ...
                int endIndex = encodingPath.indexOf("/ACTUAL");
                if (endIndex < 0) {
    ...
    private boolean handleShareIntent(Intent intent, String type) {
        ...
        if (textExtra == null) {
            ...
            if (uriExtra.toString().startsWith(GOOGLE_APP_PHOTO)) {
                Uri newUrl = handleGooglePhoto(uriExtra);
                if (newUrl != null) {
                    mUris[0] = newUrl;
                } else {
                    return false;
