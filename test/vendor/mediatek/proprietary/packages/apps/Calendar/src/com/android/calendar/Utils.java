...
public class Utils {
    ...
    **/
	...
	//delete BUG_ID:DELSLMY-682 liujia  20160309 (start)
			/**
    ...
    public static void setEventInfoTodayIcon(LayerDrawable icon, Context c, String timezone) {
        DayOfCalendarDrawable today;

        // Reuse current drawable if possible
        Drawable currentDrawable = icon.findDrawableByLayerId(R.id.today_icon_day);
        if (currentDrawable != null && currentDrawable instanceof DayOfMonthDrawable) {
            today = (DayOfCalendarDrawable)currentDrawable;
        } else {
            today = new DayOfCalendarDrawable(c);
        }
        // Set the day and update the icon
        Time now =  new Time(timezone);
        now.setToNow();
        now.normalize(false);
        today.setDayOfMonth(now.monthDay);
        String[] weekStrings=c.getResources().getStringArray(R.array.weeks_array);
        String weekString  = weekStrings[now.weekDay];
        today.setDayOfWeek(weekString);
        icon.mutate();
        icon.setDrawableByLayerId(R.id.today_icon_day, today);
    }
    ...
    public static Time getValidTimeInCalendar(Context context, Time t) {
      ...
      //  if (time < first || time > last) { 
       if (time < first || time > last || t.year>=2037) {
            t.setToNow();
        }
        return t;
