...
//add BUG_ID:DELSLMY-682 liujia  20160309 (start)
import android.provider.Settings;
//add BUG_ID:DELSLMY-682 liujia  20160309 (end)
...
public class GeneralPreferences extends PreferenceFragment implements
    ...
    //modify BUG_ID:DELSLMY-680 liujia  20160311 (start)
    //public static final int DEFAULT_START_VIEW = CalendarController.ViewType.WEEK;
    public static  int DEFAULT_START_VIEW = CalendarController.ViewType.WEEK;
    //modify BUG_ID:DELSLMY-680 liujia  20160311 (end)
	...
	//modify BUG_ID:DELSLMY-682 liujia  20160309 (start)
	...
	*/
 	 ...
 	 String mRingToneUri = Utils.getSharedPreference(activity, KEY_ALERTS_RINGTONE, "");
        String ringToneUri = Utils.getRingTonePreference(activity);
        String ringtoneDisplayString = getRingtoneTitleFromUri(activity, ringToneUri);
        if(RingtoneManager.isRingtoneExist(activity, Uri.parse(mRingToneUri))) {
        // case ringTone exist
            if(Settings.AUTHORITY.equals(Uri.parse(mRingToneUri).getAuthority())) {
                // fix bug: Setting STYLE exception  "default ringtone (xxx)"
                //mRingtone.setSummary(activity.getResources().getString(R.string.default_ringtone));
                mRingtone.setSummary(ringtoneDisplayString);
            } else {
                mRingtone.setSummary(ringtoneDisplayString == null ? "":ringtoneDisplayString);
            }
        } else {
             //modify BUG_ID:DELSLMY-798 liujia 20160321 (start)
             // Uri defaultUri = RingtoneManager.getDefaultRingtoneUri(activity,RingtoneManager.TYPE_NOTIFICATION);
             //RingtoneManager.setActualDefaultRingtoneUri(activity,RingtoneManager.TYPE_NOTIFICATION,defaultUri);
			  ...
			  //modify BUG_ID:DELSLMY-798 liujia 20160321 (end)
		
        }
	//modify BUG_ID:DELSLMY-682 liujia  20160309 (end)
    ...
    public String getRingtoneTitleFromUri(Context context, String uri) {
        ...
        //M:DWYSLM-575 guoshuai 20160413(start)
        //Ringtone ring = RingtoneManager.getRingtone(getActivity(), Uri.parse(uri));
        if(-1 == RingtoneManager.validRingtoneUri(context,Uri.parse(uri),RingtoneManager.TYPE_NOTIFICATION)){
        	ring = RingtoneManager.getRingtone(getActivity(), Settings.System.DEFAULT_NOTIFICATION_URI);
        } else {
        	ring = RingtoneManager.getRingtone(getActivity(), Uri.parse(uri));
        }
        //M:DWYSLM-575 guoshuai 20160413(end)
        if (ring != null) {
