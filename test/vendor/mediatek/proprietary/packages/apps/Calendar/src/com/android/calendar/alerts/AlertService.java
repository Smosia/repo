...
public class AlertService extends Service {
    ...
    private volatile ServiceHandler mServiceHandler;
	 private static final String EXTERNAL_RINGTONE_PATH = "content://media/external/audio/media/";// A:BUGID JLLBLS-434 huangerhui 20151023
...
//                    cr.update(alertUri, values, null, null);
    ...
    private static void addNotificationOptions(NotificationWrapper nw, boolean quietUpdate,
            String tickerText, boolean defaultVibrate, String ringtoneUri,
            boolean showLights,Context context) {
		
        // A:BUGID JLLBLS-434 huangerhui 20151023{
    	Log.d("ringtoneUri", ringtoneUri);
    	  if (!TextUtils.isEmpty(ringtoneUri)
                  && ringtoneUri.equals(GeneralPreferences.DEFAULT_RINGTONE)) {
              ...
              if (uri != null) {
                  ...
                  if (!TextUtils.isEmpty(ringtoneUri) && ringtoneUri.startsWith(EXTERNAL_RINGTONE_PATH)) {
                  	boolean isRingtoneExist = RingtoneManager.isRingtoneExist(context, Uri.parse(ringtoneUri));
                      if (!isRingtoneExist) {
                  		Log.d(TAG, "isRingtoneExist:"+isRingtoneExist);
                          ...
                          Utils.setRingTonePreference(context,ringtoneUri);
                  	}
                  }
              }
          }else if(ringtoneUri.startsWith(EXTERNAL_RINGTONE_PATH)){
        	  boolean isRingtoneExist = RingtoneManager.isRingtoneExist(context, Uri.parse(ringtoneUri));
        	  if(!isRingtoneExist){
        		  ringtoneUri= GeneralPreferences.DEFAULT_RINGTONE;
        		  Utils.setRingTonePreference(context,ringtoneUri);
        	  }
          }
        //A:}		
        ...
        if (!quietUpdate) {
            ...
            notification.sound = TextUtils.isEmpty(ringtoneUri) ? null : Uri
                    .parse(ringtoneUri);
        }
    }
