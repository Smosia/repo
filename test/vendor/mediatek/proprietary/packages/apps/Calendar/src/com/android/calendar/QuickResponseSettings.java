...
import android.preference.Preference.OnPreferenceClickListener;//add DELSLMY-220 huangerhui 20160307

/**
 * Fragment to facilitate editing of quick responses when emailing guests
 *
 */
//add DELSLMY-220 huangerhui 20160307
public class QuickResponseSettings extends PreferenceFragment implements OnPreferenceChangeListener,OnPreferenceClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mResponses != null) {
            ...
            for (String response : mResponses) {
                ...
                et.setOnPreferenceClickListener(this);//add DELSLMY-220 huangerhui 20160307
                mEditTextPrefs[i++] = et;
                ps.addPreference(et);
            }
        } else {
	...
	//add DELSLMY-220 huangerhui 20160307(start)
	@Override
    public boolean onPreferenceClick(Preference preference) {
        if (preference instanceof EditTextPreference) {
            EditTextPreference editTextPref = (EditTextPreference) preference;
            if (!(editTextPref.getText() == null || editTextPref.getText().equals(""))) {
               //editTextPref.getEditText().setSelected(false);
               editTextPref.getEditText().setSelection(editTextPref.getEditText().getText().length());
            }
        }
         return true;
    }
//add DELSLMY-220 huangerhui 20160307(end)
