...
public class SystemUpdateReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.i("@M_" + TAG, "onReceive: action = " + action);
        ...
        if (action.equalsIgnoreCase(ConnectivityManager.CONNECTIVITY_ACTION)) {
            ...
            if (status == DownloadInfo.STATE_QUERYNEWVERSION && getQueryRequest()) {
                Log.i("@M_" + TAG, "query new version");
                ...
                Util.setAlarm(context, AlarmManager.RTC, getNextAlarmTime(),
                        Util.Action.ACTION_AUTO_QUERY_NEWVERSION);
                Log.i("@M_" + TAG, "status = " + status + ". Need not query, set next query alarm");
            }
             ...
             * DownloadInfo.STATE_QUERYNEWVERSION; Log.i("@M_" + TAG,
             * "update complete"); }
             */
            ...
            if (status == DownloadInfo.STATE_DOWNLOADING) {
                ...
                Log.i("@M_" + TAG, "Abnormal stop during downloading.");
                showDlReminderNotification(context, true);
                ...
                Log.i("@M_" + TAG, "ACTION_BOOT_COMPLETED, download complete");
            ...
            if (status == DownloadInfo.STATE_QUERYNEWVERSION && getQueryRequest()) {
                Log.i("@M_" + TAG, "ACTION_BOOT_COMPLETED: query day to query new version");
                Util.setAlarm(context, AlarmManager.RTC, Calendar.getInstance().getTimeInMillis()
                        + QUERY_TIME_DELAY,
                        Util.Action.ACTION_AUTO_QUERY_NEWVERSION);
            } else {
            ...
            Log.i("@M_" + TAG, "ACTION_UPDATE_REPORT");
            startReportActivity();
            Intent i = new Intent("com.mediatek.systemupdate.UPDATE_SUCCESSFUL");
            mContext.sendBroadcast(i);

        } else if (action.equalsIgnoreCase(Util.Action.ACTION_AUTO_DL_TIME_OUT)) {
            Log.i("@M_" + TAG, "ACTION_AUTO_DL_TIME_OUT");
    ...
    private long getNextAlarmTime() {
        ...
        Log.i("@M_" + TAG, "current time:" + format.format(calendar.getTime()));
        ...
        Log.i("@M_" + TAG, "alarm time:" + format.format(calendar.getTime()));
    ...
    private boolean getUpdateStatus() {
        ...
        if (upgradeStarted == false) {
            boolean updated = getUpdateResult();
            Log.i("@M_" + TAG, "getUpdateStatus, updated from recovery" + updated);
            upgradeStarted = updated;
        }

        Log.i("@M_" + TAG, "upgradeStarted: " + upgradeStarted);
        ...
        if ((upgradeStarted) && (pkgPath != null)
            ...
            Log.i("@M_" + TAG, "getUpdateStatus, update finished");
    ...
    private String getFullPathResultFileName() {
        ...
        Log.i("@M_" + TAG, "mNandResultFilePath = " + mNandResultFilePath);
        return mNandResultFilePath;

    }

    private boolean getUpdateResult() {
        Log.i("@M_" + TAG, "getUpdateResult");
        ...
        if (Util.isEmmcSupport()) {
            ...
            if (strCurrentVersion != null) {
                ...
                Log.i("@M_" + TAG, "strCurrentVersion = " + strCurrentVersion);
            }
            ...
            if (strTargetVersion != null) {
                ...
                Log.i("@M_" + TAG, "strTargetVersion = " + strTargetVersion);
            }
    ...
    private void startReportActivity() {
        ...
        Log.i("@M_" + TAG, "getUpdateResult=" + result);
    ...
    private void deleteUpdatePkg() {
        ...
        if (strPkgPath != null) {
            ...
            if (imgf.exists()) {
                Log.i("@M_" + TAG, "resetDescriptionInfo, image exist, delete it");
    ...
    private void resetUpdateResult() {
        Log.i("@M_" + TAG, "deleteUpdateResult");
        DownloadInfo downloadStatus = DownloadInfo.getInstance(mContext.getApplicationContext());
        ...
        try {
              Log.i("@M_" + TAG, "clear update result");
            IBinder binder = ServiceManager.getService("GoogleOtaBinder");
            SystemUpdateBinder agent = SystemUpdateBinder.Stub.asInterface(binder);

            if (agent == null) {
                Log.i("@M_" + TAG, "agent is null");
            }

            if (Util.isEmmcSupport()) {
                if (!agent.clearUpdateResult()) {
                    Log.i("@M_" + TAG, "clearUpdateResult() false");
                }
            }
