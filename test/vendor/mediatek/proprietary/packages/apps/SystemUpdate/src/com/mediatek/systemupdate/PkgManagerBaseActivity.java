...
class PkgManagerBaseActivity extends Activity {
    ...
    private boolean setInstallInfo(String strPkgPath, String strTarVer) {
        ...
        try {
            ...
            if (Util.getPackageFileName(getApplicationContext()).startsWith("/data/")) {
                //using internal storage.
                intent.putExtra(COMMAND_PART2, Util.getPackageFileName(getApplicationContext()));
            } else {
                //using SD card path
    ...
    class InstallPkgThread extends Thread {
        ...
        public void run() {
            if (checkUpgradePackage() && setInstallInfo(mPkgPath, mTarVer)) {
                ...
                if (Util.getPackageFileName(getApplicationContext()).startsWith("/data/")) {
                    //Using internal storage (use power manager)
                    intent.putExtra(REBOOT_METHOD, PMS_REBOOT);
                } else {
                    //Using external storage(normal boot)
                    intent.putExtra(REBOOT_METHOD, NORMAL_REBOOT);
                }
