...
public class SystemUpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Util.Action.ACTION_AUTO_QUERY_NEWVERSION.equals(action)) {
            ...
            boolean isOtaOnly = Util.getUpdateType().equals(
                    Util.UPDATE_TYPES.OTA_UPDATE_ONLY);
            DownloadInfo downloadInfo = DownloadInfo.getInstance(this
            ...
            if (isOtaOnly) {
                if (downloadInfo.getActivityID() >= 0) {
                    ...
                    // Now delete previous path and set new path
                    Util.deleteFile(Util.getAvailablePath(this.getApplicationContext()));
                    Util.resetAvailablePath();
                }
            } else {
                downloadInfo = DownloadInfo.getInstance(this.getApplicationContext());
                if (downloadInfo.getActivityID() >= 0) {
                    Log.v("@M_" + TAG,
                            "[onStartCommand] mounted,in downloading/installing process, ignore");
                } else if (isQuerying()) {
...
//            Log.i("@M_" + TAG, "startForeground");
            ...
            if (order >= 0) {
                ...
                String path = (info == null) ? DownloadInfo.getInstance(this)
                              .getDownloadPath() : info.path;
                String availablePath = DownloadInfo.getInstance(this).getDownloadPath();
                ...
                if (Util.getUpdateType().equals(Util.UPDATE_TYPES.OTA_UPDATE_ONLY)) {
                    Log.v("@M_" + TAG, "[onStartCommand],Reset path,"
                                + "will switch to internal storage");
                    Util.resetAvailablePath();
                    Log.v("@M_" + TAG,
