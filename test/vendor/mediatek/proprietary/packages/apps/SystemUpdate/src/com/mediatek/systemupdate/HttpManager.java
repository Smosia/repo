...
final class HttpManager {
    ...
    static HttpManager getInstance(Context context) {
        Log.i("@M_" + TAG, "sHttpManager = " + sHttpManager);
    ...
    private HttpManager(Context context) {
        ...
        new Thread() {
            public void run() {
                Log.v("@M_" + TAG, "thread run " + Thread.currentThread().getName());
                Looper.prepare();
    ...
    void setMessageHandler(Handler handler) {
        ...
        Log.i("@M_" + TAG, "setMessageHandler, mHandler = " + mHandler);
    }

    void resetMessageHandler(Handler handler) {
        Log.i("@M_" + TAG, "resetMessageHandler");
        if (mHandler == handler) {
            mHandler = null;
            Log.i("@M_" + TAG, "resetMessageHandler");
    ...
    void queryNewVersion() {
        ...
        Log.i("@M_" + TAG, "onQueryNewVersion, hasNewVersion");
        notifyNewVersion();
    }
    ...
    void notifyNewVersion() {
        ...
        if (Util.getUpdateType() == Util.UPDATE_TYPES.OTA_UPDATE_ONLY) {
            ...
            if (mHandler != null) {
                ...
                Log.i("@M_" + TAG, "onQueryNewVersion, Send new version founded message.");
            } else {
    ...
    void resetDescriptionInfo() {
        Log.i("@M_" + TAG, "resetDescriptionInfo");
    ...
    private boolean handsakeAuthentication() {
        ...
        if (deviceInfo == null) {
            Log.i("@M_" + TAG, "onHandsakeAuthentication, deviceInfo is null");
            return false;
        }
        Log.i("@M_" + TAG, "onHandsakeAuthentication, imei = " + deviceInfo.mImei + ", sn = "
        ...
        if (response == null) {
            ...
            Log.i("@M_" + TAG, "onHandsakeAuthentication, response = null");
            return false;
        }
        StatusLine status = response.getStatusLine();
        if (status.getStatusCode() != HttpStatus.SC_OK) {
            Log.i("@M_" + TAG, "onHandsakeAuthentication, ReasonPhrase = " + status.getReasonPhrase());
        ...
        try {
            ...
            Log.i("@M_" + TAG, "onHandsakeAuthentication, response content = " + content);
            ...
            if (mCookies == null) {
                Log.e("@M_" + TAG, "onHandsakeAuthentication: mCookies = null");
                mErrorCode = HTTP_RESPONSE_NETWORK_ERROR;
                return false;
            }
            mCookies.addCookie(cookie);
            Log.i("@M_" + TAG, "mCookies size = " + mCookies.getCookies().size());
            Log.i("@M_" + TAG, "onHandsakeAuthentication, rand = " + res.mRand + ", sessionId = "
                    + res.mSessionId);
        } catch (IOException e) {
            e.printStackTrace();
            return false;
    ...
    private boolean checkNewVersion() {
        ...
        try {
            ...
            HttpResponse response = doPost(url, null, bnvpa);
            if (response == null) {
                Log.i("@M_" + TAG, "onCheckNewVersion: response = null");
                mErrorCode = HTTP_UNKNOWN_ERROR;
                return false;
            }
            StatusLine status = response.getStatusLine();
            if (status.getStatusCode() != HttpStatus.SC_OK) {
                Log.i("@M_" + TAG, "onCheckNewVersion, ReasonPhrase = " + status.getReasonPhrase());
                mCookies = null;
                mErrorCode = HTTP_RESPONSE_AUTHEN_ERROR;
                return false;
            }
            String content = getChunkedContent(response.getEntity());
            Log.i("@M_" + TAG, "onCheckNewVersion, response content = " + content);
            ...
            if (res.mFileSize <= 0 || res.mPkgId < 0) {
                ...
                Log.i("@M_" + TAG, "onCheckNewVersion, fileSize = " + res.mFileSize + ", deltaId = "
                        + res.mPkgId);
                return false;
            }
            ...
            mDownloadInfo.setVersionNote(Html.fromHtml(res.mReleaseNote).toString());
    ...
    private boolean checkFingerPrint(String fingerPrintPkg) {
        ...
        Log.i("@M_" + TAG, "fingerPrintPkg = " + fingerPrintPkg);
        ...
        if (fingerPrintLocal != null) {
            ...
            Log.i("@M_" + TAG, "fingerPrintLocal = " + fingerPrintLocal);
    ...
    /** The default maximum number of connections allowed per host */
        Log.d("@M_" + TAG, " -----------set the max socket connect number 20--------------");
    ...
    private HttpResponse doPost(String url, Map<String, String> headers,
        ...
        Log.i("@M_" + TAG, "doPost, url = " + url + ", mCookies = " + mCookies);
        ...
        try {
            ...
            try {
                if (url.contains("https")) {
                    Log.i("@M_" + TAG, "doPost, https");
                    response = httpClient.execute(host, httpPost);
                } else {
                    Log.i("@M_" + TAG, "doPost, http");
                    Log.i("@M_" + TAG, "mHttpClient =" + httpClient + "httpPost = " + httpPost
    ...
    private String getChunkedContent(HttpEntity entity) throws IOException {
        ...
        Log.i("@M_" + TAG, "getChunkedContent, isChunked = "
        ...
        try {
            ...
            if (mDownloadInfo.getDLSessionStatus() == DownloadInfo.STATE_DOWNLOADING) {
                ...
                Log.e("@M_" + TAG, "getChunkedContent, exception to set pause state");
            }
            mErrorCode = HTTP_RESPONSE_NETWORK_ERROR;
            return null;
        }
    ...
    private HttpResponseContent parseAuthenInfo(String result) {
        ...
        if (result == null) {
            mErrorCode = HTTP_RESPONSE_AUTHEN_ERROR;
            Log.e("@M_" + TAG, "parseAuthenInfo error: param is null");
            return null;
        }
        try {
            ...
            if (jo.getInt("status") == HTTP_RESPONSE_SUCCESS) {
                ...
                Log.e("@M_" + TAG, "parseAuthenInfo, error info = " + info);
                return null;
            }
    ...
    private HttpResponseContent parseCheckVersionInfo(String result) {
        Log.i("@M_" + TAG, "parseCheckVersionInfo");
        try {
            JSONObject jo = new JSONObject(result);
            HttpResponseContent res = new HttpResponseContent();
            if (jo.getInt("status") == HTTP_RESPONSE_SUCCESS) {
                Log.i("@M_" + TAG, "HTTP_RESPONSE_SUCCESS");
                res.mVersionName = jo.getString("name");
                Log.i("@M_" + TAG, "res.mVersionName = " + res.mVersionName);
                res.mFileSize = jo.getLong("size");
                Log.i("@M_" + TAG, "res.fileSize = " + res.mFileSize);
                res.mReleaseNote = jo.getString("release_notes");
                Log.i("@M_" + TAG, "res.releaseNote = " + res.mReleaseNote);
                if (jo.has("versionId")) {
                    res.mPkgId = jo.getInt("versionId");
                    res.mIsFullPkg = true;
                    Log.i("@M_" + TAG, "full package: res.packageId = " + res.mPkgId);
                } else if (jo.has("deltaId")) {
                    res.mPkgId = jo.getInt("deltaId");
                    res.mIsFullPkg = false;
                    Log.i("@M_" + TAG, "delta package: res.packageId = " + res.mPkgId);
                    res.mFingerprint = jo.getString("fingerprint");
                }
                Log.i("@M_" + TAG, "res.packageId = " + res.mPkgId + "res.isFullPkg = " + res.mIsFullPkg);
                res.mAndroidNum = "Android " + jo.getString("android_version");
                Log.i("@M_" + TAG, "res.mAndroidNum = " + res.mAndroidNum);
    ...
    private String getRandCookieStr() {
        ...
        if (mCookies != null) {
            ...
            for (int n = 0; n < nSize; n++) {
                ...
                Log.i("@M_" + TAG, "index:" + n + "cookieRand.getName = " + cookie.getName());
                if ((cookie != null) && (COOKIE_TAG.equals(cookie.getName()))) {
                    cookieRand = cookie;
                    Log.i("@M_" + TAG, "cookieRand = " + cookieRand);
                }
            }

            if (cookieRand != null) {
                Log.i("@M_" + TAG, "cookieRand getName = " + cookieRand.getName());

                strCookie = "15811375356" + cookieRand.getValue();
                Log.i("@M_" + TAG, "getToken, strCookie = " + strCookie);

            }
        }
        Log.i("@M_" + TAG, "strCookie = " + strCookie);
        return strCookie;

    }

    private String getToken() {
        Log.i("@M_" + TAG, "getToken");
        ...
        try {
            ...
            if (strCookie == null) {
                ...
                Log.i("@M_" + TAG, "could not get cookie value");
                return null;
    ...
    int writeFile(HttpResponse response, long currSize) {
        ...
        try {
            ...
            while ((rc = in.read(buff, 0, 4096)) > 0) {
                ...
                try {
                    if (fPkg.exists()) {
                        ...
                        Log.e("@M_" + TAG, "file not exist during downloading ");
                        setPauseState();
                        out.close();
                        onShutdownConn();
                ...
                if (status == DownloadInfo.STATE_PAUSEDOWNLOAD
                    ...
                    Log.i("@M_" + TAG, "writeFile, DownloadInfo = " + status);
                    mCookies = null;
                    finish = false;
                    out.close();
                    onShutdownConn();
            ...
            Log.i("@M_" + TAG, "writeFile, finish, rc = " + rc + "bytes" + ". finish = " + finish);
            if (finish) {
                onTransferRatio();
                onDownloadProcessUpdate();
            }
        ...
        if (mDownloadInfo.getDLSessionStatus() == DownloadInfo.STATE_DOWNLOADING) {
            setPauseState();
            Log.e("@M_" + TAG, "writeFile, exception to set pause state");
    ...
    private void sendErrorMessage() {
        Log.i("@M_" + TAG, "sendErrorMessage, mErrorCode = " + mErrorCode);
    ...
    void onDownloadImage() {
        ...
        Log.i("@M_" + TAG, "onDownloadImage pkgid = " + packageid);
        ...
        if (mDownloadInfo.getFullPkgFlag()) {
            ...
            Log.v("@M_" + TAG, "download full url = " + url);
        }
        mDownloadInfo.setDownloadPath(Util.getPackageFileName(mContext));
        HttpResponse response = doPost(url, null, bnvpa);

        if (mDownloadInfo.getDLSessionStatus() != DownloadInfo.STATE_DOWNLOADING) {
            Log.i("@M_" + TAG, "onDownloadImage: status not right");
            setNotDownload();
            return;
        }
        if (response == null) {
            Log.i("@M_" + TAG, "onDownloadImage: response = null");
            mErrorCode = HTTP_UNKNOWN_ERROR;
        ...
        if (status.getStatusCode() != HttpStatus.SC_OK
            ...
            Log.i("@M_" + TAG, "onDownloadImage, ReasonPhrase = " + status.getReasonPhrase()
        ...
        Log.i("@M_" + TAG, "onDownloadImage, download result = " + ret);
    ...
    private void checkIfAutoDl() {
        ...
        if ((status == DownloadInfo.STATE_PAUSEDOWNLOAD) && (mDownloadInfo.getOtaAutoDlStatus())) {
            ...
            if ((networkStatus == Util.NETWORK_STATUS.STATE_WIFI)
                ...
                Log.i("@M_" + TAG, "showDlReminderNotification");
                mNotification.resetBuilder();
                //mNotification.clearNotification(NotifyManager.NOTIFY_DOWNLOADING);
                mNotification.showDownloadingNotificaton(mDownloadInfo.getVerNum(),
                        (int) (((double) Util.getFileSize(Util
    ...
    void onDownloadComplete() {
        Log.i("@M_" + TAG, "onDownloadComplete");
    ...
    class IncomingHandler extends Handler {
        @Override
        public void handleMessage(Message msg) {
            Log.i("@M_" + TAG, "handleMessage, msg.what=" + msg.what);
            switch (msg.what) {

            case MSG_DELETE_COMMANDFILE:
                Log.i("@M_" + TAG, "MSG_DELETE_COMMANDFILE: arg1=" + msg.arg1);
                break;
            default:
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            Log.i("@M_" + TAG, "onServiceConnected");
    ...
    private boolean doBindService(Context context) {
        ...
        Log.i("@M_" + TAG, "dobindService, isbound=" + mIsBound);
        return mIsBound;
    }

    private void doUnbindService(Context context) {
        Log.i("@M_" + TAG, "doUnbindService");
        if (mIsBound) {
            mService = null;
            if (context != null) {
