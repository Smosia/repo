...
public class PlatformManager {
    ...
    public String[] getComplexImsi(Context context, int slotId, int masterSlotId) {
        ...
        // For master slot, need both CDMA/LTE IMSI. If non-CT, only set LTE IMSI
        if (slotId == masterSlotId) {

            Intent imsiIntent = null;
            if (SvlteUiccUtils.getInstance().isUsimWithCsim(slotId)) {
                imsiIntent = context.registerReceiver(null,
                        new IntentFilter(TelephonyIntents.ACTION_CDMA_CARD_IMSI));
                Log.i(TAG, "CT 4G, previous intent " + imsiIntent);
            } else {
                Log.i(TAG, "Not CT 4G, keep intent null");
            }
