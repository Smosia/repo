...
public class RegisterMessage {
    ...
    private static final String VALUE_REG_VERSION = "3.0";

    private RegisterService mRegisterService;
    private PlatformManager mPlatformManager;

    private int mSlotMaster = Const.SLOT_ID_0;
    private int mSlotSlave = Const.SLOT_ID_1;

    /**
     * Constructor method.
     * @param service The service which invokes this class.
     */
    public RegisterMessage(RegisterService service, int slotMaster) {
        mRegisterService = service;
        mPlatformManager = mRegisterService.getPlatformManager();

        if (mSlotMaster != slotMaster) {
            mSlotMaster = Const.SLOT_ID_1;
            mSlotSlave = Const.SLOT_ID_0;
        }
        Log.i(TAG, "Use " + mSlotMaster + " as master slot");
    }
    ...
    private String generateMessageData() {
        ...
        try {
            ...
            appendImeiInfo(data);
            appendMasterSlotInfo(data);
            appendLocationInfo(data);
            appendSlaveSlotInfo(data);
    ...
    private void appendMasterSlotInfo(JSONObject data) throws JSONException {
        if (mPlatformManager.hasIccCard(mSlotMaster)) {
            data.put(FIELD_SIM1_CDMA_IMSI, getCdmaImsi(mSlotMaster));
            data.put(FIELD_SIM1_ICCID, getIccId(mSlotMaster));
            data.put(FIELD_SIM1_LTE_IMSI, getLteImsi(mSlotMaster));
            data.put(FIELD_SIM1_TYPE, getSimType(mSlotMaster));
        } else {
    ...
    private void appendSlaveSlotInfo(JSONObject data) throws JSONException {
        if (!mPlatformManager.isSingleLoad()
                && mPlatformManager.hasIccCard(mSlotSlave)) {
            data.put(FIELD_SIM2_IMSI, getLteImsi(mSlotSlave));
