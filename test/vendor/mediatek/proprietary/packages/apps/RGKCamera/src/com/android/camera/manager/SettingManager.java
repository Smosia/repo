...
public class SettingManager extends ViewManager implements View.OnClickListener,
    ...
    protected RotateImageView mTestIndicator;
    ...
    // add for normal use control panel by anxiang.xiao 20160316 start 
    public static final int MODE_NUM_ALL = 9;
    protected ViewGroup mControlPanelLayout;
    ...
    protected View getView() {
        ...
        mTestIndicator = (RotateImageView) view.findViewById(R.id.box_show_indicator);
        mTestIndicator.setOnClickListener(this);
    ...
    public void onClick(View view) {
        ...
        if (view == mTestIndicator) {
            if (!mShowingContainer) {
            	showBoxTest();
            } else {
               // collapse(true);
                mControlPanelLayout.setVisibility(View.GONE);
    ...
    public void showBoxTest() {
        ...
        if (getContext().isFullScreen()) {
            if (!mShowingContainer && getContext().getCameraAppUI().isNormalViewState()) {
                ...
                mShowingContainer = true;
                mListener.onSettingContainerShowing(mShowingContainer);
                initializeControlPanelSettings();
                refresh();
                highlightCurrentSetting(mPager.getCurrentItem());
                mControlPanelLayout.setVisibility(View.VISIBLE);
                if (mControlPanelLayout.getParent() == null) {
                    getContext().addView(mControlPanelLayout, SETTING_PAGE_LAYER);
                }
                getContext().getCameraAppUI().setViewState(ViewState.VIEW_STATE_SETTING);
                startFadeInAnimation(mControlPanelLayout);
                mTestIndicator.setImageResource(R.drawable.ic_setting_focus);
            }
            setChildrenClickable(true);
        }
        */
       ...
       // if (mControlPanelLayout.getParent() == null) {
        	initializeControlPanelSettings();
        	  mShowingContainer = true;
            getContext().addView(mControlPanelLayout, VIEW_LAYER_OVERLAY);
       // }
    }
    ...
    // add by anxiang.xiao start 20160316 start 
    private void initializeControlPanelSettings() {
    	/*
        if (mControlPanelLayout == null && mSettingController.getPreferenceGroup() != null) {
        	mControlPanelLayout = (ViewGroup) getContext().inflate(R.layout.normal_operation_control_panel,
                    SETTING_PAGE_LAYER);
        }
        Util.setOrientation(mControlPanelLayout, getOrientation(), false);*/
        ...
        mModeViews[MODE_PHOTO] = (RotateImageView) mControlPanelLayout.findViewById(R.id.mode_photo);
        mModeViews[MODE_PHOTO_PIP] = (RotateImageView) mControlPanelLayout.findViewById(R.id.mode_photo_pip);
        mModeViews[MODE_STEREO_CAMERA] = (RotateImageView) mControlPanelLayout.findViewById(R.id.mode_stereo_camera);
        mModeViews[MODE_FACE_BEAUTY] = (RotateImageView) mControlPanelLayout.findViewById(R.id.mode_face_beauty);
        mModeViews[MODE_PANORAMA] = (RotateImageView) mControlPanelLayout.findViewById(R.id.mode_panorama);
        
        
        mModeViews[MODE_PHOTO].setImageResource(MODE_ICONS_NORMAL[MODE_PHOTO]);
        mModeViews[MODE_PHOTO_PIP].setImageResource(MODE_ICONS_NORMAL[MODE_PHOTO_PIP]);
        mModeViews[MODE_STEREO_CAMERA].setImageResource(MODE_ICONS_NORMAL[MODE_STEREO_CAMERA]);
        mModeViews[MODE_FACE_BEAUTY].setImageResource(MODE_ICONS_NORMAL[MODE_FACE_BEAUTY]);
        mModeViews[MODE_PANORAMA].setImageResource(MODE_ICONS_NORMAL[MODE_PANORAMA]);
    	
    }
    // add by anxiang.xiao start 20160316 end 
