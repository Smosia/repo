...
// modify BUG_ID:DWLEL-336 chenchunyong 20141204 (start)
import com.mediatek.storage.StorageManagerEx;
// modify BUG_ID:DWLEL-336 chenchunyong 20141204 (end)
...
public class DetailDialog extends AlertDialog implements DialogInterface.OnClickListener {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        if (mView != null) {
            ...
            // modify BUG_ID:DWLEL-336 chenchunyong 20141204 (start)
            mPathView.setText(mContext.getString(R.string.detail_path, getStoragepath(mContext, mHolder.mData.toString())));
	...
	// add BUG_ID:DWLEL-336 chenchunyong 20141204 (start)
	public static String getStoragepath(Context context, String path) {
		...
		if (!StorageManagerEx.getSdSwapState()) {
			...
			if (path.contains("sdcard0")) {
				Filepath = path.replace("sdcard0", "phone");
			}
			
		} else {

			if (path.contains("sdcard0")) {
				Filepath = path.replace("sdcard0", "sdcard");
			} else if (path.contains("sdcard1")) {
				Filepath = path.replace("sdcard1", "phone");
			}

		}
		return Filepath;
	}
	// add BUG_ID:DWLEL-336 chenchunyong 20141204 (end)
