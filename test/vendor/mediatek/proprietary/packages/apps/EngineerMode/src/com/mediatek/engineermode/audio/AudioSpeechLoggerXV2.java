...
*/
                    ...
                    if (checked) {
                        ...
                        String ret = mAudioManager.getParameters(CMD_GET_DEBUG_INFO);
                        Elog.i(Audio.TAG, "getParameters " + ret);
                        if (ret != null && ret.length() > CMD_PREFIX.length()) {
                            String[] entries = ret.substring(CMD_PREFIX.length()).split(",");
                            if (entries.length >= 1) {
                                entries[0] = "0x3";
                                ...
                                Elog.i(Audio.TAG, "setParameters " + cmd);
                        ...
                        ret = mAudioManager.getParameters(CMD_GET_COMMON_PARAMETER);
                        Elog.i(Audio.TAG, "getParameters " + ret);
                        if (ret != null && ret.length() > CMD_PREFIX.length()) {
                            String[] entries = ret.substring(CMD_PREFIX.length()).split(",");
                            if (entries.length >= 1) {
...
*/
        ...
        mCbVoipLogger.setOnCheckedChangeListener(null);
        if (getVOIP() == 0) {
