...
public class FastDormancy extends Activity implements OnClickListener {
    ...
    private void initFdMgr() {
        ...
        if (mPhone instanceof LteDcPhoneProxy) {
            Elog.d(TAG, "mPhone instanceof LteDcPhoneProxy");
            Phone phone = ((LteDcPhoneProxy) mPhone).getLtePhone();
            if (phone instanceof PhoneBase) {
                mFdMgr = FdManager.getInstance((PhoneBase) phone);
            }
        } else if (mPhone instanceof PhoneProxy){
            Elog.d(TAG, "mPhone instanceof PhoneProxy");
            Phone phone = ((PhoneProxy) mPhone).getActivePhone();
            if (phone instanceof PhoneBase) {
                mFdMgr = FdManager.getInstance((PhoneBase) phone);
            }
        } else {
            Elog.d(TAG, "mPhone instanceof others");
        }
    }
