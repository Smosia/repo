...
public class BandSelect extends Activity implements OnClickListener {
    ...
    private static int getModemType(int simType) {
        int mode;
        final int mask = WorldPhoneUtil.get3GDivisionDuplexMode();
        if (1 == mask) {
            mode = WCDMA;
        } else if (2 == mask) {
            mode = TDSCDMA;
    ...
    private static boolean isCapabilitySim(int i) {
        try {
            ...
            if (iTelEx != null) {
                ...
                Log.i("@M_" + LOG_TAG, "getMainCapabilityPhoneId() = " + phoneId);
                return phoneId == i;
            } else {
                Log.e("@M_" + LOG_TAG, "ITelephonyEx iTelEx = null");
                return false;
            }
        } catch (RemoteException e) {
            Log.e("@M_" + LOG_TAG, "ITelephonyEx RemoteException");
            e.printStackTrace();
            return false;
        }
    }
