...
public class DrmUtilsEx {
    ...
    public static boolean checkHasDrmContent(Context context, String src, Uri uri) {
        if (FeatureOption.MTK_DRM_APP) {
            DrmCacheItem cacheItem = DrmCacheManager.get(uri.toString());
            if (cacheItem != null) {
                return cacheItem.isDrm();
            }

            boolean result = (src != null) ? isDrm(src) : checkUriContainsDrm(context, uri);
            boolean hasRight = false;
            if (result) {
                // check right
                hasRight = checkHasDrmRight(context, result, uri);
            }
            DrmCacheManager.add(uri.toString(), src, result, hasRight);
            return result;
        }
        return false;
    }
    ...
    public static Bitmap getDrmBitmapWithLockIcon(Context context, Uri uri, boolean isVideo) {
        if (!FeatureOption.MTK_DRM_APP) return null;
        // has DRM content
        boolean hasRight = checkHasDrmRight(context, true, uri);

        // lock icon
        Drawable front = null;
        if (hasRight) {
            front = context.getResources().getDrawable(MessageResource.drawable.drm_green_lock);
        } else {
            front = context.getResources().getDrawable(MessageResource.drawable.drm_red_lock);
        }
