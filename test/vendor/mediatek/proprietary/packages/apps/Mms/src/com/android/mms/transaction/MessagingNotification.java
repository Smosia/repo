...
//                | Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    private static void updateNotification(
        ...
        //Add chenshu DELSLMY-1210 20160328 start
        /*PowerManager powerManager = (PowerManager)(context.getSystemService(Service.POWER_SERVICE));
        android.util.Log.d("chenshu","powerManager.isScreenOn():"+powerManager.isScreenOn());
        if (!powerManager.isScreenOn()) {
            ...
            /// M: Save NotificationPlayer instance so that we can stop it when composer is pausing
            if (sNotificationPlayer == null) {
                sNotificationPlayer = new NotificationPlayer(TAG);
                sNotificationPlayer.stop();
                sNotificationPlayer.play(context, ringtoneUri, false, AudioManager.STREAM_NOTIFICATION,
                    1.0f);
                //android.util.Log.d("chenshu","play playInConversationNotificationSound2");
            } else {
                sNotificationPlayer.stop();
                sNotificationPlayer.play(context, ringtoneUri, false, AudioManager.STREAM_NOTIFICATION,
                    1.0f);
            ...
            //android.util.Log.d("chenshu","play playInConversationNotificationSound3");
        }*/
        //Add chenshu DELSLMY-1210 20160328 end
    }
    ...
    //Modify, JWLW-1141, zhangzixiao, 20131118, End
        ...
        if (noisy) {
	    	...
	    	if(supportDualRingtone()) {
                if(slotId == 1) {
		    	     ringtoneStr = sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE2, null);
	    	    } else {
...
//        }
    ...
    public static NotificationProfile getNotificationProfile(Context context) {
	    ...
	    //Modify, JWLW-1141, zhangzixiao, 20131118, End
	     //Add, JWLW-1141, zhangzixiao, 20131118, Start
	     if(supportDualRingtone()) {
	          np.appRing2 = prefs.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE2, NotificationPreferenceActivity.DEFAULT_RINGTONE);
	     }
	     //Add, JWLW-1141, zhangzixiao, 20131118, End
        //Modify, JWLW-1141, zhangzixiao, 20131118, Start
    ...
    public static final String checkRingtone(Context context, String ringtoneUri) {
        ...
        if (!TextUtils.isEmpty(ringtoneUri)
			...
			if (isSmsCustomRingtoneSupport()){
				uri = RingtoneManager.getActualDefaultRingtoneUri(context, RingtoneManager.TYPE_MESSAGE_RINGTONE);
			} else {
            ...
            if (uri != null) {
                if (RingtoneManager.isRingtoneExist(context, uri)) {
					...
					if (isSmsCustomRingtoneSupport()){
					    ringtoneUri = RingtoneManager.getDefaultRingtoneUri(context, RingtoneManager.TYPE_MESSAGE_RINGTONE).toString();
					} else {					 	
                        ringtoneUri = RingtoneManager.getDefaultRingtoneUri(context, RingtoneManager.TYPE_NOTIFICATION).toString();
					}
					//Modify DWYQLSSMY-57 chenshu 20160328 end
