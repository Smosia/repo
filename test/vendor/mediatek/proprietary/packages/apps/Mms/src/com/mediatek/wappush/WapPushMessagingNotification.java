...
public class WapPushMessagingNotification {
    ...
    private static final String[] WAPPUSH_STATUS_PROJECTION;
    static {
	if(supportDualRingtone()) {
	    WAPPUSH_STATUS_PROJECTION = new String[] {
        	WapPush.THREAD_ID, WapPush.DATE,
        	WapPush._ID, WapPush.URL, WapPush.TEXT, WapPush.ADDR, WapPush.SUBSCRIPTION_ID};
	} else {
	    WAPPUSH_STATUS_PROJECTION = new String[] {
            	WapPush.THREAD_ID, WapPush.DATE,
            	WapPush._ID, WapPush.URL, WapPush.TEXT, WapPush.ADDR};
    ...
    //Add, JWLW-1141, zhangzixiao, 20131118, Start
    private static final int COLUMN_WAPPUSH_SIM_ID		= 6;
    //Add, JWLW-1141, zhangzixiao, 20131118, End
    ...
    private static void playInConversationNotificationSound(Context context) {
        ...
        int slotId = 0;
        if(supportDualRingtone()) {
            SortedSet<WapPushNotificationInfo> infos = new TreeSet<WapPushNotificationInfo>(INFO_COMPARATOR);
                Set<Long> threads = new HashSet<Long>(4);
                addWapPushNotificationInfos(context, threads, infos);
            if(!infos.isEmpty()) {
	            WapPushNotificationInfo info = infos.first();
	            slotId = SubscriptionManager.getSlotId(info.simId);
            }   
        }
        //Add, JWLW-1141, zhangzixiao, 20131118, End
    ...
    private static void addWapPushNotificationInfos(
    	...
    	addWapPushNotificationInfos(context, threads, sNotificationSet);
    }
    //Add, JWLW-1141, zhangzixiao, 20131118, End
    ...
    //        Context context, Set<Long> threads) {
    private static void addWapPushNotificationInfos(
            Context context, Set<Long> threads, SortedSet<WapPushNotificationInfo> notificationSet) {
    //Add, JWLW-1141, zhangzixiao, 20131118, End
        ...
        try {
		 ...
		 //Add, JWLW-1141, zhangzixiao, 20131118, Start
		 if(supportDualRingtone()) {
		     int simId = cursor.getInt(COLUMN_WAPPUSH_SIM_ID);
                   info.simId = simId;   
		 }
               //Add, JWLW-1141, zhangzixiao, 20131118, End
                /*
    ...
    private static void updateNotification(
        ...
        if (isNew) {
            ...
            */
            ...
            String ringtoneStr = null;
	        if(supportDualRingtone()) {
		        int slotId = SubscriptionManager.getSlotId(mostRecentNotification.simId);
                	if(slotId == 1) {
                	    ringtoneStr = sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE2, null);
                	} else {
                	    ringtoneStr = sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE, null);
            ...
            ringtoneStr = notiProf.needMute() ? null : ringtoneStr;
            //Add, JWLW-1141, zhangzixiao, 20131118, end
            ringtoneStr = MessagingNotification.checkRingtone(context, ringtoneStr);
