    ...
    public static byte[] getResizedImageData(int width, int height,
        ...
        try {
            ...
            do {
                ...
                /*if (context.getResources().getBoolean(R.bool.improve_picture_quality)){
                    if (MmsApp.getApplication().takePictureMethod != 0) {
                        options.inSampleSize = scaleFactor *2 /3;
                    } else {
                        options.inSampleSize = scaleFactor ;
                    }
                    Log.v(TAG, "takePictureMethod:"+MmsApp.getApplication().takePictureMethod);
                } else {
                    options.inSampleSize = scaleFactor;
                }*/
                if (context.getResources().getBoolean(R.bool.improve_picture_quality)){
                    if (MmsApp.getApplication().takePictureMethod == 2) {
                        options.inSampleSize = scaleFactor *2 /3;
                    } else if (MmsApp.getApplication().takePictureMethod == 1) {
                        options.inSampleSize = scaleFactor *2 /3;;
                    }
                    Log.v("chenshu", "takePictureMethod:"+MmsApp.getApplication().takePictureMethod);
                } else {
                    options.inSampleSize = scaleFactor;
                }
                //options.inSampleSize = scaleFactor ;
        ...
        /// @}
                    ...
                    if (needReduce || isMustResize) {
                        ...
                        if (context.getResources().getBoolean(R.bool.improve_picture_quality)) {
                            if (MmsApp.getApplication().takePictureMethod == 1) {
                                b = Bitmap.createScaledBitmap(b, (outWidth / scaleFactor) *3/2,
                                    (outHeight / scaleFactor)*3/2, false);
                            } else if (MmsApp.getApplication().takePictureMethod == 2) {
                                b = Bitmap.createScaledBitmap(b, (outWidth / scaleFactor) *3/2,
                                    (outHeight / scaleFactor)*3/2, false);
                            } else {
                                b = Bitmap.createScaledBitmap(b, outWidth / scaleFactor,
