...
public class CommandProxyAsyncTask extends AsyncTask<CommandProxyAction, Integer, Integer> {
    ...
    public CommandProxyAsyncTask(MonitorCmdProxy cmdproxy, ConnectionState connectState, HashMap<Integer , WeakReference<CommandProxyListener>> listener_map){
        m_cmdProxy = cmdproxy;
    ...
    protected Integer doInBackground(CommandProxyAction... params) {
        ...
        for(int commands_i = 0 ; commands_i < commands_count ; commands_i++) {
            Log.d(TAG, "doInBackground() :  Operate command , sid = " + m_connectState.m_sid);
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        Log.d(TAG, "onProgressUpdate() : Update progress value = " + values[0]);
    ...
    protected void onPostExecute(Integer result) {
        ...
        Log.d(TAG, "onPostExecute() : sid = " + m_connectState.m_sid);
    ...
    private int DoCommandProxyOperation(CommandProxyAction action){
        switch (action.actionType){
            ...
            case ACTION_TYPE_SUBSCRIBE_TRAP: {
                ...
                MONITOR_CMD_RESP ret = m_cmdProxy.onSubscribeTrap(m_connectState.m_sid, action.trapType, action.msgID);
                // error check
                if (MONITOR_CMD_RESP_SUCCESS != ret) {
            ...
            case ACTION_TYPE_UNSUBSCRIBE_TRAP: {
                ...
                MONITOR_CMD_RESP ret = m_cmdProxy.onUnsubscribeTrap(m_connectState.m_sid, action.trapType, action.msgID);
                // error check
                if (MONITOR_CMD_RESP_SUCCESS != ret) {
