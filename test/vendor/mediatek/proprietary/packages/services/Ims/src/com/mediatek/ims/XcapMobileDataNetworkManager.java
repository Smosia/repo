...
public class XcapMobileDataNetworkManager {
    ...
    private boolean isNotPermitAcquireXcapNetwork(int phoneId) {
        if (MMTelSSUtils.isOp08IccCard(phoneId)) {
            Rlog.d(LOG_TAG, "isNotPermitAcquireXcapNetwork: true");
            return true;
        }

        Rlog.d(LOG_TAG, "isNotPermitAcquireXcapNetwork = false");
        return false;
    }

    // Retire, but keep it to track the op
    // which need to use dedicate network.
    ...
    public Network acquireNetwork(int phoneId) {
        ...
        if (isNotPermitAcquireXcapNetwork(phoneId)) {
