...
public class ImsCallSessionProxy extends IImsCallSession.Stub {
    ...
    public void close() {
        ...
        /// M: Call might terminate before VT stack ready, then VT stack
        /// can not awareness the call is terminated.
        /// Send this terminate to VT provider.
    ...
    private class MyHandler extends Handler {
        ...
        /// M: ALPS02519352, parse tel/sip field to keep the effective value.
        /// eg: <tel:1234;bala> => <tel:1234>
        /// eg: <sip:jacky chen@mtk.com> => <sip:jacky chen> @{
        private String handlePau(String pau) {
            pau = parseField(pau, PAU_SIP_NUMBER_FIELD);
            pau = parseField(pau, PAU_NUMBER_FIELD);
            Rlog.d(LOG_TAG, "updated pau: " + pau);

            String displayName = getDisplayNameFromPau(pau);
            if (displayName.trim().length() > 0) {
                /// M: ALPS02492264, add <name:> flag for telecom.
                pau = "<name:" + displayName + ">" + pau;
                Rlog.d(LOG_TAG, "Add display name to pau: " + pau);
            }

            return pau;
        }

        private String parseField(String pau, String targetField) {
            String val = "";
            if (TextUtils.isEmpty(pau) || !pau.contains(targetField)) {
                Rlog.d(LOG_TAG, "parseTelTagFromPau()... pau or field is null !");
                return pau;
            }
            String field = getFieldValueFromPau(pau, targetField);
            // use ; and @ as separator
            String[] split = field.split("[;@]+");
            val = split[0];

            if (val != "") {
                pau = pau.replace(field, val);
            }
            return pau;
        }
        /// @}
