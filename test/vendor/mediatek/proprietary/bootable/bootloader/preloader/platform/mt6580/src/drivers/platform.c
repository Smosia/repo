...
void platform_set_boot_args_by_atag(unsigned int *ptr)
{
  ...
  for(i=0;i<4;i++)
   ...
   //add *#*#9375#*#* info  2015-03-16 start
    tags = ptr;
    tags->hdr.size = boot_tag_size(boot_tag_mem_info);
    tags->hdr.tag  = BOOT_TAG_MEM_INFO;
    extern int rgk_save_memory_info_boot_arg(char *mem_info);
    rgk_save_memory_info_boot_arg(tags->u.rgk_mem.rgk_mem_name);
    ptr += tags->hdr.size; 
    //add *#*#9375#*#* info  2015-03-16 end
...
/*In FPGA phase, dram related function should by pass*/
    ...
    //add *#*#9375#*#* info  2015-03-16 start
    extern int rgk_save_memory_info_boot_arg(boot_arg_t *boot_arg);
    rgk_save_memory_info_boot_arg(&bootarg);
    //add *#*#9375#*#* info  2015-03-16 end
