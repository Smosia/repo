...
int rgk_save_memory_info_boot_arg(char *mem_info)
{
    int index;
    char *dest;
    char *str;
#pragma message("add by 9375")
    index = mt_get_mdl_number();

    dest = mem_info;
    str = RGK_MEMORY_NAME[index];

    if(31 < sizeof(RGK_MEMORY_NAME[index]))
        return -1;

    memset((void*)mem_info, 0, 32);
