    ...
    stream_attribute_t *stream_attribute_target)
{
    ...
    stream_attribute_target->BesRecord_Info.besrecord_bypass_dualmicprocess = mBypassDualMICProcessUL;  // bypass dual MIC preprocess in K_2 E1 chip
...
status_t AudioALSAStreamManager::routingOutputDevice(AudioALSAStreamOut *pAudioALSAStreamOut, const audio_devices_t current_output_devices, audio_devices_t output_devices)
{
    ...
    if (isModeInPhoneCall() == true)
    {
        ...
        //if (pAudioALSAStreamOut->getStreamAttribute()->output_devices == current_output_devices) // TODO(Harvey): or add group?
        {
