...
status_t AudioALSAHardwareResourceManager::startInputDevice(const audio_devices_t new_device)
{
    ...
    if (new_device == AUDIO_DEVICE_IN_BUILTIN_MIC)
    {
        setMIC1Mode(false);
        setMIC2Mode(false);
        if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC2_ONLY)   //use ref mic  //(BUILTIN_MIC_MIC3_ONLY)
        {
            #ifdef ENABLE_XMLCONFIG
        ...
        else if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC1_ONLY)  //use main mic
        {
            #ifdef ENABLE_XMLCONFIG
...
status_t AudioALSAHardwareResourceManager::setMicType(void)
{
    ...
    int ret, bflag;
    //modification for Rainier
    mPhoneMicMode = GetMicDeviceMode(0);  //only support AUDIO_MIC_MODE_ACC on Rainier
