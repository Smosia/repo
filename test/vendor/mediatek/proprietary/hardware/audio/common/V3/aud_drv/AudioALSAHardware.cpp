...
//3in1 xiangkezhu 20160304 start
static String8 keySET_VIBSPK_HZ = String8("SET_VIBSPK_HZ");
static String8 keySET_VIBSPK_GAIN = String8("SET_VIBSPK_GAIN");
//3in1 xiangkezhu 20160304 end
// Voice Wake up
...
status_t AudioALSAHardware::setParameters(const String8 &keyValuePairs)
{
    ...
    if (IsAudioSupportFeature(AUDIO_SUPPORT_VIBRATION_SPEAKER))
    {
        ...
        //3in1 xiangkezhu 20160304 start
        if (param.getInt(keySET_VIBSPK_HZ, value) == NO_ERROR)
        {
            param.remove(keySET_VIBSPK_HZ);
            AudioVIBSPKControl::getInstance()->setVibSpkHz(value);
            ALOGD("setParameters SET_VIBSPK_HZ!!, %x", value);
        }
        if (param.getInt(keySET_VIBSPK_GAIN, value) == NO_ERROR)
        {
            param.remove(keySET_VIBSPK_GAIN);
            AudioVIBSPKControl::getInstance()->setVibSpkGain(15,4,value);
            ALOGD("setParameters SET_VIBSPK_GAIN!!, %x", value);
        }
        //3in1 xiangkezhu 20160304 end
    }
