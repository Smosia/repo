...
static int write_block(MtdWriteContext *ctx, const char *data)
{
    ...
    while (pos + size <= (int) partition->size) {
        ...
        if (ioctl(fd, MEMGETBADBLOCK, &bpos) > 0) {
            ...
            fprintf(stderr, "mtd: not writing bad block at 0x%016llx\n", pos);
            pos += partition->erase_size;
        ...
        for (retry = 0; retry < 2; ++retry) {
            if (ioctl(fd, MEMERASE64, &erase_info) < 0) {
                fprintf(stderr, "mtd: erase failure at 0x%016llx (%s)\n",
                        pos, strerror(errno));
                continue;
            }
            if (lseek64(fd, pos, SEEK_SET) != pos ||
                write(fd, data, size) != size) {
                fprintf(stderr, "mtd: write error at 0x%016llx (%s)\n",
                        pos, strerror(errno));
            }
            ...
            if (lseek64(fd, pos, SEEK_SET) != pos ||
                ...
                fprintf(stderr, "mtd: re-read error at 0x%016llx (%s)\n",
                        pos, strerror(errno));
                continue;
            }
            if (memcmp(data, verify, size) != 0) {
                fprintf(stderr, "mtd: verification error at 0x%016llx (%s)\n",
                        pos, strerror(errno));
                continue;
            }
        ...
        add_bad_block_offset(ctx, pos);
        fprintf(stderr, "mtd: skipping write block at 0x%016llx\n", pos);
...
off64_t mtd_erase_blocks(MtdWriteContext *ctx, int blocks)
{
    ...
    while (blocks-- > 0) {
        ...
        if (ioctl(ctx->fd, MEMGETBADBLOCK, &bpos) > 0) {
            fprintf(stderr, "mtd: not erasing bad block at 0x%016llx\n", pos);
        ...
        if (ioctl(ctx->fd, MEMERASE64, &erase_info) < 0) {
            fprintf(stderr, "mtd: erase failure at 0x%016llx\n", pos);
        }
        pos += ctx->partition->erase_size;
    }
