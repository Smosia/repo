...
Hal3A::addCommandQ(ECmd_T const &r3ACmd)
{
    ...
    MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "mCmdQ size(%d) + cmd(%d)", mCmdQ.size(), r3ACmd);
...
Hal3A::getCommand(ECmd_T &rCmd)
{
    ...
    MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "CmdQ-size(%d), todo(%d)", mCmdQ.size(), *mCmdQ.begin());
...
Hal3A::onThreadLoop(MVOID *arg)
{
    ...
    {
        MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "rCmd(%d)+", rCmd);
        //
        switch (rCmd)
        {
            case ECmd_Update:
                ...
                if ( ! _this->isReadyToCapture())
                {
                    ...
                    _this->waitVSirq();
                    MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::onThreadLoop] waitVSirq done.");
