...
init(MINT32 i4SensorDev)
{
    ...
    MY_LOG_IF(m_bDebugEnable,"IState::sm_3APvLogEnable = %d\n", IState::sm_3APvLogEnable);
...
*/
   ...
   MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"[%s()] AE done\n", __FUNCTION__);
   ...
   MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"[%s()] AF done\n", __FUNCTION__);
   ...
   MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"[%s()] AWB done\n", __FUNCTION__);
   ...
   MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[%s()] ISP Tuning done\n", __FUNCTION__);
   ...
   MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[%s()] Flash done\n", __FUNCTION__);

   ERROR_CHECK(FlickerHalBase::getInstance().setFlickerMode(m_i4SensorDev,rParamConvert.u4AntiBandingMode))

   MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[%s()] Flicker done\n", __FUNCTION__);
...
MVOID* Hal3A::AFNotifyThreadLoop(void *arg)
{
    ...
    while (_this->bEnAFNotifyThreadLoop)
    {
        ...
        MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"[%s] mSemAFNotify wait done\n", __FUNCTION__);
...
{
    MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"Hal3A::sendAFCallbacks, (%d)\n", bAfNeedSingleNotify);
    IAfMgr::getInstance().DoCallback(m_i4SensorDev);

    if (bAfNeedSingleNotify)
    {
        MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"Hal3A::SingleAF_CallbackNotify, before\n");
        IAfMgr::getInstance().SingleAF_CallbackNotify(m_i4SensorDev);
        MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG,"Hal3A::SingleAF_CallbackNotify, after\n");
        bAfNeedSingleNotify = MFALSE;
    }
}
