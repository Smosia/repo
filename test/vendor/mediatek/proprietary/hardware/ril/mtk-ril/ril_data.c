...
extern int queryEpdgRat(RIL_SOCKET_ID rid, reqSetupConf_t *pConfig, RIL_Default_Bearer_VA_Config_Struct* pDefaultBearerVaConfig);
...
extern int rildata_isOp01Volte();
...
[%s][%s]", iaParameter, iaProperty);
        ...
        if (strlen(iaProperty) == 0 && isApnDiff) {
            int isOp01 = 0;
            int isOp01Volte = rildata_isOp01Volte();
            int check = 0;
