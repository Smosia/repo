 ...
 _NORETURN void drIpchLoop( void )
{
    ...
    for (;;)
    {
         ...
         */
        if (DRAPI_OK != (ret = drApiIpcCallToIPCH(&ipcClient, &ipcMsg, &ipcData)))
        {
            drDbgPrintLnf("[Driver DrAsyncExample] drIpchLoop(): drApiIpcCallToIPCH failed 0x%08X",ret);
            continue;
        }
            ...
            case MSG_RQ_EX:
                ...
                if (pMarshal) {
                    ...
                    case FID_DR_OPEN_SESSION:
                        ...
                        pMarshal->header.retVal = TLAPI_OK;;
                    ...
                    case FID_DR_EXECUTE:
                        ...
                        if ((drSession != NULL)&&
                            (SESSION_STATE_ACTIVE == drSession->state)) {
                            taskid_t task= drApiGetTaskid();
                            ret = drApiIpcSignal(drApiTaskidGetThreadid(task,DRIVER_THREAD_NO_IRQH));
                            drDbgPrintLnf("[Driver DrAsyncExample] send signal return value: 0x%08X",ret);
                            pMarshal->header.retVal = ret;
                        }
                        else {
                            drDbgPrintLnf("[Driver DrAsyncExample] Session not open ");
                            pMarshal->header.retVal = E_TLAPI_CR_HANDLE_INVALID;
                        }
                        break;
                    ...
                    case FID_DR_ASK_NOT:
                        ...
                        pMarshal->header.retVal= tmp - testSession[sid].rCounter;
...
drApiResult_t drIpchInit(void){
    ...
    if (DRAPI_OK != ret)
    {
        drDbgPrintLnf("[Driver DrAsyncExample] drIpchInit(): drApiStartThread failed 0x%08X",ret);
    }
    return ret;
}
