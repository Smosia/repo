...
static tciReturnCode_t encrypt( tciMessage_t* message )
{
    ...
    memset(&crSession, 0, sizeof(crSession));
    ...
    size_t dstBufLen = message->cmdAes.len;
...
static tciReturnCode_t decrypt( tciMessage_t* message )
{
    ...
    uint32_t srcLen = message->cmdAes.len;
    uint8_t* dst = (uint8_t*) message->cipherData;
    size_t dstBufLen = message->rspAes.len;
...
_TLAPI_ENTRY void tlMain( const addr_t tciBuffer, const uint32_t tciBufferLen )
{
    ...
    dbgSN("AES Trusted Application is starting");
    ...
    dbgSN("AES Trusted Application is processing the command");
    tciMessage_t* message = (tciMessage_t*) tciBuffer;

    for (;;)
    {

        dbgSN("AES Trusted Application is waiting for a notification to arrive");
        ...
        dbgSN("AES Trusted Application received command");
      ...
      dbgSDN("AES Trusted Application is exiting with status ", ret);
