...
static mcResult_t tlcNotifyAndWait(mcSessionHandle_t *pSessionHandle,
                                const char *name);
static void tlcInitCryptoBuffer(uint8_t* cryptoBuffer, uint32_t size);

/* TA binary should be in the same directory instead of GP TA  which
 * should be in the directory "/data/app/mcRegistry"
 */
#define TRUSTLET_PATH "."

#define CHECK_TCI_RES(name, pTci, exp, err)	{						        \
        LOG_I("%s => response_id=%x, result=%x",    				        \
                name,                                                       \
...
int main(int argc, char *args[]) {
    ...
    uint8_t *pCryptoBuffer[4];
    mcBulkMap_t mapInfo[4];
    mcResult_t mcRet;
    int err = 1;
    int mode = 0;
    int i;

    LOG_i("Copyright (c) Trustonic Limited 2013-2014");
    ...
    do {
        //put wrong device id to remember that tl session is not opened
        tlSessionHandle.deviceId = deviceId + 1;
        for (i = 0; i < 4; i++) {
            mapInfo[i].sVirtualLen = 0;
            pCryptoBuffer[i] = NULL;
        }
         ...
         * Open TA SESSION
         */
        mcRet = tlcOpenSession("07120000000000000000000000000000.tlbin",
                            deviceId,
                            &tlSessionHandle,
                            (uint8_t**) &pTci,
                            sizeof(tci_t));
        ...
        /*
         * Allocate Buffers to exchange encrypted data and Map it to TA
         */
        for (i = 0; i< 4; i++) {
            uint32_t size;
            if (mode == 0) {
                ...
                size = (i == 0 ? 4096+16 : 1000*1024);
                /* FIXME
                 * Originally we want to test with big buffers.
                 * But We must allocate Driver Outbuffer for that.
                 * As only tplatKernelHandle.ko can make this, by default
                 * (mode == 0) we allocate with size of 2000.
                 * IMPORTANT:
                 * Normally size of 4096 should work but it fails.
                 * This has to be investigated and fixed.
                 */
            }
            if ((pCryptoBuffer[i] = (uint8_t*) malloc(size)) == NULL) {
            //if (mcMallocWsm(MC_DEVICE_ID_DEFAULT, 0, size, &pCryptoBuffer[i], 0) != MC_DRV_OK) {
                LOG_E("ERROR: Malloc cryptoBuffer #%d", i);
                break;
            }
            if ((mcRet = mcMap(&tlSessionHandle,
                               ...
                               size,
                               &mapInfo[i])) != MC_DRV_OK) {
                LOG_E("mcMap(cryptoBuffer #%d) returned: %d", mcRet, i);
                break;
            }
            LOG_I("mcMap#%d(ptr=%p,sec_ptr=%p,size=%d) returned: %d",
                  i,
                  pCryptoBuffer[i],
                  mapInfo[i].sVirtualAddr,
                  mapInfo[i].sVirtualLen,
                  mcRet);

            /* Fill crypto buffer with iv and data */
            tlcInitCryptoBuffer(pCryptoBuffer[i], size);
        }
        ...
        for (i = 0; i < 4; i++) {
            char strMsg[1024];
            snprintf(strMsg, 1024, "drm ProcessContent #%d", i);
            pTci->message.command.header.commandId
                    = CMD_ID_RUN_DRM_TEST_PROCESS_DRM_CONTENT;
            pTci->message.command.len = mapInfo[i].sVirtualLen;
            pTci->message.command.data = (uint32_t) mapInfo[i].sVirtualAddr;

            /* notify, check result */
            mcRet = tlcNotifyAndWait(&tlSessionHandle, strMsg);
            if (MC_DRV_OK != mcRet) {
                break;
            }
            CHECK_TCI_RES(strMsg, pTci, RET_OK, err);
        }
    ...
    /* Unmap cryptoBuffer */
    if (tlSessionHandle.deviceId == deviceId) {
        for (i = 0; i < 4; i++) {
            if (mapInfo[i].sVirtualLen != 0) {
                mcRet = mcUnmap(&tlSessionHandle, pCryptoBuffer[i], &mapInfo[i]);
                if (MC_DRV_OK != mcRet) {
                    LOG_E("mcUnmap#%d returned: %d", i, mcRet);
                }
            }
            if (pCryptoBuffer[i] != NULL) {
                free(pCryptoBuffer[i]);
            }
        }
    }
...
static void tlcInitCryptoBuffer(uint8_t* cryptoBuffer, uint32_t size) {
    ...
    memset(cryptoBuffer, 0x5A, size);

    /* return if no space to insert IV */
    if (size < IV_SIZE) {
        return ;
    }
