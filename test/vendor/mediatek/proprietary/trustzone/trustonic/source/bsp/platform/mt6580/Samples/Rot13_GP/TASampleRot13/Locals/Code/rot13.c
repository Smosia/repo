 ...
 */
static void rot13(char *txt, char* out, uint32_t len) {
                                                ...
                                                TEE_Param pParams[4])
{
    ...
    switch(nCommandID)
    {
        ...
        TEE_DbgPrintLnf(TATAG "%s pInput     =0x%p", __func__, pInput);
        TEE_DbgPrintLnf(TATAG "%s nInputSize =%d",   __func__, nInputSize);
        TEE_DbgPrintLnf(TATAG "%s pOutput    =0x%p", __func__, pOutput);
        TEE_DbgPrintLnf(TATAG "%s nOutputSize=%d",   __func__, nOutputSize);
        ...
        if (pOutput == NULL)
        {
            pParams[1].memref.size = nInputSize;
            return ret;
        }
        ...
        rot13((char*)pInput, (char*)pOutput, nInputSize);

        clientSession->nEncrypted += nInputSize;
        pParams[1].memref.size = nInputSize;
        ...
        if ((pOutput != NULL) && (nOutputSize != 0))
        {
            ...
            if (nOutputSize < sizeof(msg))
            {
                return TEE_ERROR_SHORT_BUFFER;
            }

            nOutputSize = sizeof(msg);
