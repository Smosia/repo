...
{
    timestamp_t uptime = 0;
    if(!get_milliseconds_since_boot(&uptime))
    {
        return TEE_ERR_INTERNAL_ERROR;
    }
    pTci->message.uptime = uptime;
    return TEE_OK;
}
...
TEE_ReturnCode_t tlCryptoHandler_FailureRecordEncode( tci_t* pTci )
{
    ...
    // TODO: this might be not needed (to be double checked)
    if(0==sw_failure_record.last_access_timestamp)
    {
        if(!get_milliseconds_since_boot(&timestamp))
        {
            return TEE_ERR_INTERNAL_ERROR;
        }
        sw_failure_record.last_access_timestamp = timestamp;
    }
...
TEE_ReturnCode_t tlCryptoHandler_FailureRecordDecode( tci_t* pTci )
{
    ...
    do
    {
        ...
        // SEC: don't change anything in failure_record object until SUID
        //      is verified
