...
{
    /*drUnMap(DR_VA_HDL,SIZE_4KB);
    drUnMap(DR_VA_TABLE,SIZE_4KB);
    drUnMap(DR_VA_OUT,SIZE_1MB * 2);*/
}
    ...
    for (;;)
    {
        ...
        switch(mr1)
        {
            ...
            case TRAP_SEGMENTATION:
                drDbgPrintLnf("[DRM Driver] drExchLoop(): thread #%d caused TRAP_SEGMENTATION", faultedThread);
            ...
            case TRAP_UNDEF_INSTR:
                drDbgPrintLnf("[DRM Driver] drExchLoop(): thread #%d caused TRAP_ALIGNMENT/TRAP_UNDEF_INSTR", faultedThread);
                /**
                ...
                if (E_OK != drApiStopThread(DRIVER_THREAD_NO_DCIH))
                {
                    dbgSN("[DRM Driver] drExchLoop(): Unable to stop DCI handler thread");
                }
