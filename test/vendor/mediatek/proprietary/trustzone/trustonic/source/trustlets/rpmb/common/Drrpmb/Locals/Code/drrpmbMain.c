...
{
    drApiLogPrintf("[<t Driver Drrpmb] %d.%d, Build " __DATE__ ", " __TIME__ EOL,
                        DRIVER_VERSION_MAJOR, DRIVER_VERSION_MINOR);
    
    /* Initialization */
    if (DRAPI_OK != InitDriver())
    {
        drDbgPrintLnf("[Driver Drrpmb] drMain(): Initialization failed");

        /* No need to proceed further. Shutdown the main thread */
        if (DRAPI_OK != drApiStopThread(NILTHREAD))
        {
            drDbgPrintLnf("[Driver Drrpmb] drMain(): Unable to stop main thread");
        }
    }
