...
_NORETURN void drExchLoop(
    ...
    drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): Exception handler thread is running");

    for(;;)
    {
        drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): drApiIpcWaitForMessage start!!");
                     ...
                     &mr2) )
        {
            ...
            drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): drApiIpcWaitForMessage failed");
            continue;
        }
        ...
        drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): drApiIpcWaitForMessage done. mr1=%d, faultedThread=%d", mr1, faultedThread);
        ...
        switch(mr1)
        {
            ...
            case TRAP_SEGMENTATION:
                ...
                switch(faultedThread)
                {
                    ...
                    case DRIVER_THREAD_NO_IPCH:
                        drDbgPrintLnf("[Driver Drrpmb] drExchLoop():DRIVER_THREAD_NO_IPCH");
                        ...
                        if (DRAPI_OK != drUtilsRestartThread(
                                        ...
                                        sp))
                        {
                            drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): restarting thread failed");
                        }
                    ...
                    //--------------------------------------
                    default:
                        drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): Unknown thread. This should never happen");
                        break;
                }
            ...
            case TRAP_UNDEF_INSTR:
                ...
                {
                    drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): Unable to stop DCI handler thread");
                }
            #endif // DR_FEATURE_DCI_HANDLER
                ...
                {
                    drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): Unable to stop IPC handler thread");
                }

                /* Stop main thread */
                if (DRAPI_OK != drApiStopThread(NILTHREAD))
                {
                    drDbgPrintLnf("[Driver Drrpmb] drExchLoop(): Unable to stop main thread");
                }
