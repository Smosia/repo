...
/**
 * Derive the key used to encrypt key data to blobs for passing to NWd.
 *
 * @param key empty key structure (with memory allocated and length set)
 * @return KM_ERROR_OK or error
 */
static keymaster_error_t derive_wrapping_key(
    tlApiSymKey_t *key)
{
    ...
    ret = get_device_state(state_data);
    if (ret != KM_ERROR_OK) {
        return ret;
    }
    ...
    uint32_t *blob_len)
{
    ...
    tlApiSymKey_t key = {keydata, KEY_LEN};
    keymaster_error_t ret = derive_wrapping_key(&key);
    if (ret != KM_ERROR_OK) {
        return ret;
    }
    ...
    uint32_t ct_len = *blob_len - NONCE_LEN - TAG_LEN;

    // 3. generate random nonce
    size_t randomLen = NONCE_LEN;
    ...
    if (TLAPI_OK != aesAEInit(&aesOpCtx, nonce, NONCE_LEN, TAG_LEN, 0, 0)) {
        return KM_ERROR_UNKNOWN_ERROR;
    }
    uint32_t tagLen = TAG_LEN;
    if ((TLAPI_OK != aesAEEncryptFinal(&aesOpCtx,
        data, data_len,
        blob + NONCE_LEN, &ct_len,
        tag, &tagLen)) ||
        (tagLen != TAG_LEN))
    {
        return KM_ERROR_UNKNOWN_ERROR;
    ...
    assert(NONCE_LEN + ct_len + TAG_LEN <= *blob_len);
    memcpy(blob + NONCE_LEN + ct_len, tag, TAG_LEN);

    // 7. set blob_len
    *blob_len = NONCE_LEN + ct_len + TAG_LEN;
    ...
    uint32_t *data_len)
{
    ...
    uint32_t ct_len = blob_len - NONCE_LEN - TAG_LEN;
    memcpy(nonce, blob, NONCE_LEN);
    memcpy(tag, blob + NONCE_LEN + ct_len, TAG_LEN);
