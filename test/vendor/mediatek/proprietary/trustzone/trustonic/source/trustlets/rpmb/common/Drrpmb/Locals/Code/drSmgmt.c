 ...
 */
uint32_t drSmgmtOpenSession(uint32_t uid, threadid_t threadid)
{
    ...
    do
    {
        for (uint32_t i = 0; i < MAX_DR_SESSIONS; i++)
        {
            ...
            if (pRegEntry->state == SESSION_STATE_UNUSED)
            {
                ...
                pRegEntry->mData.uid = uid;
                break;
            }
            else if (pRegEntry->mData.uid == uid) {
                break;
            }
        }
    }while(0);
        ...
        tlApiRpmb_ptr pData)
{
    ...
    for (uint32_t i = 0; i < MAX_DR_SESSIONS; i++)
    {
        ...
        {
            /* Copy data */
            memcpy(&(pRegEntry->mData), pData, sizeof(tlApiRpmb_t));
            ret = E_DR_SMGMT_OK;
            break;
        }
    }
