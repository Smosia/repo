 ...
 *
 * Note that Gatekeeper must derive the same key.
 *
 * @param[out] key per-boot HMAC key
 *
    ...
    hw_auth_token_t *auth_token)
{
    ...
    /* Verify the HMAC */
    return (KM_ERROR_OK == get_hmac_key(key))
        && (KM_ERROR_OK == hmac_begin(KM_PURPOSE_VERIFY, NULL, 0, 256, key, 32,
            KM_DIGEST_SHA_2_256, 256, &tag_len, &hmacOpCtx))
        && (tag_len == 32)
