 ...
 class CheckScreenOn extends CheckItemBase {
     ...
     public boolean onCheck() {
         ...
         if (getKey().equals(CheckItemKeySet.CI_SCREEN_ON_SLEEP)
             ...
             int currentValue = Settings.System.getInt(cc.getContentResolver(),
             ...
             int isOn = Settings.Global.getInt(cc.getContentResolver(),
     ...
     public boolean onReset() {
         ...
         if (getKey().equals(CheckItemKeySet.CI_SCREEN_ON_SLEEP)) {
            setValue(R.string.value_sleep_max);
            Settings.System.putInt(cc.getContentResolver(),
                                   Settings.System.SCREEN_OFF_TIMEOUT, TIME_30_MIN);
         } else if (getKey().equals(CheckItemKeySet.CI_SCREEN_ON_SLEEP_15SEC)) {
             setValue(R.string.value_sleep_15s);
             Settings.System.putInt(cc.getContentResolver(),
                                    Settings.System.SCREEN_OFF_TIMEOUT, TIME_15_SEC);
         } else if (getKey().equals(CheckItemKeySet.CI_SCREEN_ON_UNLOCK)) {
             setValue(R.string.value_unlockscreen);
...
class CheckUrl extends CheckItemBase {
    ...
    static int getIdByNameOrUrl(String name, String url, long parentId) {
        ...
        Cursor cursor = cc.getContentResolver().query(BrowserContract.Bookmarks.CONTENT_URI,
                projection, where,
...
class CheckDefaultStorage extends CheckItemBase {
    ...
    public boolean onCheck() {
        ...
        CTSCLog.d(TAG, " Current default path=" + defaultpath + "; external path ="
                  + externalStoragePath);
    ...
    public boolean onReset() {
        ...
        List<StorageVolume> storageVolumes = new ArrayList<StorageVolume>();
          ...
          for (int i = 0; i < mPathList.length; i++) {
              CTSCLog.d(TAG, "Volume: " + volumes[i].getDescription(cc) + " ,state is: "
                      + mStorageManager.getVolumeState(mPathList[i]) + " ,emulated is: "
                      + volumes[i].isEmulated()
        ...
        String externalStoragePath = StorageManagerEx.getExternalStoragePath();
        CTSCLog.d(TAG, "default path group length = " + length + " external patch "
                + externalStoragePath);
...
class CheckShortcut extends CheckItemBase {
    ...
    private static final String CAMERANAME = "com.mediatek.camera"; //L1
    ...
    private static final String NOTEBOOKNAME = "com.mediatek.notebook";
    private static final String CONTACTNAME = "com.android.contacts";
    ...
    public boolean onReset() {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                deleteShortcut(CALENDERAPNAME, "com.android.calendar.AllInOneActivity");

                deleteShortcut(SETTINGAPNAME, "com.android.settings.wifi.WifiSettings");
                deleteShortcut(CAMERANAME, "com.android.camera.CameraLauncher");
                ...
                deleteShortcut(NOTEBOOKNAME);
                deleteShortcut(CONTACTNAME);
                ...
                createShortcut(CALENDERAPNAME, "com.android.calendar.AllInOneActivity");
                createShortcut(NOTEBOOKNAME);
                createShortcut(CONTACTNAME);

                createShortcut(SETTINGAPNAME, "com.android.settings.wifi.WifiSettings");
                createShortcut(CAMERANAME, "com.android.camera.CameraLauncher");
    ...
    private void deleteShortcut(String packageName) {
        ...
        try {
            ...
            deleteShortcut(packageName, info.activities[0].name);
          //the first activity tag in AndroidManifest
    ...
    private void deleteShortcut(String packageName, String activityName) {
        ...
        Intent shortcut = constructShortcutIntent(false, getShortcutName(packageName, activityName),
                getShortcutIcon(packageName, activityName),
                getShortcutIntent(packageName, activityName));
        cc.sendBroadcast(shortcut);
    }
    ...
    private void createShortcut(String packageName) {
        ...
        try {
            PackageManager pm = cc.getPackageManager();
            PackageInfo info = pm.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES);
            createShortcut(packageName, info.activities[0].name);
            //the first activity tag in AndroidManifest
        } catch (NameNotFoundException e) {
            CTSCLog.d(TAG, "NameNotFoundException " + e);
        }
    ...
    private void createShortcut(String packageName, String activityName) {
        CTSCLog.d(TAG, "createShortcut");
        Intent shortcut = constructShortcutIntent(true,
                getShortcutName(packageName, activityName),
                getShortcutIcon(packageName, activityName),
                getShortcutIntent(packageName, activityName));
        cc.sendBroadcast(shortcut);
    }
    ...
    private String getShortcutName(String packageName, String activityName) {
        ...
        try {
            ...
            //shortcut name default is application'a label
    ...
    private Parcelable getShortcutIcon(String packageName, String activityName) {
        ...
        try {
            ...
            if (null != activityName
                    && activityName.equalsIgnoreCase("com.android.camera.CameraLauncher")) {
...
class CheckDefaultIME extends CheckItemBase {
    ...
    private static final String IME2 =
      "com.google.inputmethod.latin/com.android.inputmethod.latin.LatinIME";
    private Context cc;
    ...
    public boolean onCheck() {
        ...
        String ime = Settings.Secure.getString(cc.getContentResolver(),
    ...
    public boolean onReset() {
        ...
        Settings.Secure.putString(cc.getContentResolver(),
                                  Settings.Secure.DEFAULT_INPUT_METHOD, IME1);
        mResult = check_result.RIGHT;
        return true;
    }
}
...
class CheckAddCallLog extends CheckItemBase {
    ...
    public boolean onReset() {
        ...
        try {
            // May block.
            Calls.addCall(null, getContext(), "10086", 0, 2, 0, null,
                          System.currentTimeMillis(), 0, new Long(0));
        } catch (Exception e) {
