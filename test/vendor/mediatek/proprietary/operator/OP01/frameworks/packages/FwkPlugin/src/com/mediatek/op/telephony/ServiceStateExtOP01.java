    ...
    public int needAutoSwitchRatMode(int phoneId,String nwPlmn){
        ...
        if (SubscriptionManager.isValidPhoneId(phoneId)){
            if (phoneId == (switchStatus - 1)){
                //[ALPS01646248]-Start: auto switch rat mode to 2G only when inserting
                // non-CMCC China operator Card
                ...
                if (simOperator == null || simOperator.equals("")) {
                    //maybe Phone process hasnot start read IMSI,
                    //so we use SystemProperties from RILD frist
                    simOperator = SystemProperties.get(propStr, "");
                    log("needSwitchRatMode:simOperator=" + simOperator +
                            ",from gsm.ril.sst.mccmnc");
                } else {
                ...
                //if((miscFeatureConfig &
                //            PhoneConstants.MISC_FEATURE_CONFIG_MASK_AUTO_SWITCH_RAT) == 1) {
                //    if (userType == TYPE3_USER){//type3
                //        if (!SystemProperties.get("ro.mtk_world_phone_policy").equals("1")) {
                            /* switch Rat Mode to 2G only when inserting
                               non-CMCC China operator Card */
                //            log("needSwitchRatMode:set Rat to 2G when inserting non-CMCC Card");
