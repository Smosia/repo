...
public class Op09MessageListItemExt extends DefaultOpMessageListItemExt {
    ...
    public void bindCommonMessage(Button downloadButton, TextView dateView) {
        ...
        if (mMessageItemExt.isReceivedMessage() && MessageUtils.isShowDateManagementEnable()) {
            String dateStr = null;
            if (mMessageItemExt.isSms() && !mMessageItemExt.isSubMsg()) {
                if (mMessageItemExt.mSmsSentDate == 0) {
                    dateStr = mResourceContext.getString(R.string.status_ct_unknown);
                } else {
            ...
            if (!TextUtils.isEmpty(dateStr)) {
                Log.d(TAG, "bindCommonMessage dateStr:" + dateStr);
