...
public class Op09DialogModeActivityExt extends DefaultOpDialogModeActivityExt
    ...
    public void initDialogView(TextView subName, Button download, TextView sentTime,
            TextView sentTitle, LinearLayout timeLayout, LinearLayout counterLinearLayout,
            TextView counter, Cursor cursor) {
        mSubName = subName;
        mMmsDownloadBtn = download;
        mSentTitle = sentTitle;
        mSentTime = sentTime;
    ...
    public void setDialogView(Context applicationContext, int subId, boolean isCurSMS, int type,
        ...
        Log.d(TAG, "setDialogView, subId = " + subId + ", isCurSMS = " + isCurSMS +
            ", type = " + type + ", sentTime = " + sentTime + ", receivedTime = " + receivedTime);
        mSentTime.setText(sentTime);
        mCurrentSubId = subId;
        Op09MessageListItemExt.setDualTime(mResourceContext, true, subId, mSentTitle,
                mTimeLayout, receivedTime);

        setRecvtimerViewToFitBigIcon(mSentTime);
        ...
        if (isCurSMS) {
            ...
            if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == type) {
                mSentTime.setText("");
                hideDualTimePanel(mSentTitle, mTimeLayout);
