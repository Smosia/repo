...
#define Hynix_LSC_SW
#ifdef Hynix_LSC_SW
	UINT16 QCT_LSC[1768] = {0,};  
	 void DoHynixLSC(UINT16 *);
	//#include "./hynix_lsc_conversion.h"
	 extern UINT16 MTK_EEPROM_DATA[1000];
	 extern float Gr_ratio_GLOBAL; //70%
	 extern float Gb_ratio_GLOBAL;
	 extern float R_ratio_GLOBAL;
	 extern float B_ratio_GLOBAL;
	 extern int ALLROUND_color_channel; 
#endif 
// hynix add end  
...
#ifdef Hynix_LSC_SW
UINT32 DoCamCalHynix2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData);
#endif 
...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
...
#ifdef Hynix_LSC_SW	
	{//CALIBRATION_LAYOUT_SENSOR_OTP
		0x00000001, 0x420800ff, CAM_CAL_SINGLE_OTP_DATA,
		{		
			{0x00000001, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
			{0x00000001, 0x00000005, 0x00000002, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
			{0x00000001, 0x00000017, 0x00000001, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE  DoCamCalHynixSingleLsc
			{0x00000001, 0x00000007, 0x0000000E, DoCamCalHynix2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN     DoCamCalHynix2AGain
			{0x00000000, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
		}
	}
#else
	{//CALIBRATION_LAYOUT_SENSOR_OTP
		...
		{		
			...
			{0x00000001, 0x00000005, 0x00000002, DoCamCalPartNumber}, //CAMERA_CAM_CAL_DATA_PART_NUMBER
			{0x00000001, 0x00000017, 0x00000001, DoCamCalSingleLsc}, //CAMERA_CAM_CAL_DATA_SHADING_TABLE  DoCamCalHynixSingleLsc
			{0x00000001, 0x00000007, 0x0000000E, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN     DoCamCalHynix2AGain
			{0x00000000, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
		}
	}
#endif 
...
static MINT32 dumpEnable=1; // To Open Log Change 0 -> 1 
	...
	*/
    ...
    { 
        ...
        else
        {
			...
			CAM_CAL_ERR("S2aBitEn=0x%x", pCamCalData->Single2A.S2aBitEn);
            ...
            if(0x1&AWBAFConfig){
                ...
                if(ioctlerr>0)
                {
                    ...
                    CAM_CAL_ERR("UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d", CalR, CalG, CalB, tempMax);
                ...
                if(ioctlerr>0)
                {
                    ...
                    CAM_CAL_ERR("Extract CalGain OKK\n");
                    ...
                    CAM_CAL_ERR("GoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d", FacR, FacG, FacB, tempMax);
    	        ...
    	        CAM_CAL_ERR("Start assign value\n");
                ...
                CAM_CAL_ERR("======================AWB CAM_CAL==================\n");
                CAM_CAL_ERR("[CalGain] = 0x%x\n", CalGain);
                CAM_CAL_ERR("[FacGain] = 0x%x\n", FacGain);
                CAM_CAL_ERR("[rCalGain.u4R] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4R);
                CAM_CAL_ERR("[rCalGain.u4G] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4G);
                CAM_CAL_ERR("[rCalGain.u4B] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4B);	
    		 CAM_CAL_ERR("[rFacGain.u4R] = %d\n", pCamCalData->Single2A.S2aAwb.rGoldGainu4R);
                CAM_CAL_ERR("[rFacGain.u4G] = %d\n", pCamCalData->Single2A.S2aAwb.rGoldGainu4G);
                CAM_CAL_ERR("[rFacGain.u4B] = %d\n", pCamCalData->Single2A.S2aAwb.rGoldGainu4B);	
                CAM_CAL_ERR("======================AWB CAM_CAL==================\n");
                #endif
            }
			if(0x2&AWBAFConfig){
                ...
                ////Only AWB Gain Gathering <////
                #if 1//def DEBUG_CALIBRATION_LOAD
               CAM_CAL_ERR("======================AF CAM_CAL==================\n");
               CAM_CAL_ERR("[AFInf] = %d\n", AFInf);
               CAM_CAL_ERR("[AFMacro] = %d\n", AFMacro);
               CAM_CAL_ERR("======================AF CAM_CAL==================\n");
                #endif
            }
...
UINT32 DoCamCalHynix2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	UINT8 AWBAFConfig;
	u16 AFInf, AFMacro;
	UINT16 tempMin = 1023, tempMax;
	UINT16 tempGain, CalR, CalGr, CalGb, CalG, CalB, FacR, FacGr, FacGb, FacG, FacB; // 20151126 UINT8 -> UINT16
        ...
        else
        {
	     ...
	     cam_calCfg.u4Offset = (start_addr+1);
            ...
            if(0x1&AWBAFConfig){
				...
				CalB  = (((CalGain>>16)&0xFF)<<8) |(((CalGain>>16)>>8)&0xFF);					
				CalGr = 1024; 	
				CalGb = 1024; 
				CalG = ((CalGr + CalGb) + 1) >> 1;

				CAM_CAL_ERR("Hi-842 Unit R Gain Value 0x%x\n",CalR);
				CAM_CAL_ERR("Hi-842 Unit B Gain Value 0x%x\n",CalB);
				CAM_CAL_ERR("Hi-842 Unit G Gain Value 0x%x\n",CalG);
				...
				FacB  = (((FacGain>>16)&0xFF)<<8) |(((FacGain>>16)>>8)&0xFF);					
				FacGr = 1024;
				FacGb = 1024;
                            FacG = ((FacGr + FacGb) + 1) >> 1;

				CAM_CAL_ERR("Hi-842 Golden R Gain Value 0x%x\n",FacR);
				CAM_CAL_ERR("Hi-842 Golden B Gain Value 0x%x\n",FacB);
				CAM_CAL_ERR("Hi-842 Golden G Gain Value 0x%x\n",FacG);				
			...
			if(0x2&AWBAFConfig){
                ////AF////
                cam_calCfg.u4Offset = (start_addr+10);//11
                cam_calCfg.u4Length = 2;
                cam_calCfg.pu1Params = (u8 *)&AFInf;
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                if(ioctlerr>0)
                ...
                cam_calCfg.u4Offset = (start_addr+12);//13
                cam_calCfg.u4Length = 2;
                cam_calCfg.pu1Params = (u8 *)&AFMacro;
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);
                if(ioctlerr>0)
...
/////////////////////////////////////////// Gr First 
        pCamCalData->SingleLsc.LscTable.MtkLcsData.PixId = 2;//hardcode.... need to fix

        cam_calCfg.u4Offset = (start_addr-2);
        cam_calCfg.u4Length = sizeof(table_size);
        cam_calCfg.pu1Params= (u8 *)&table_size;

        CAM_CAL_ERR("CAM_CALGetCalData u4Offset=%d u4Length=%d pu1Params= 0x%x ", cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);
        ...
        if(ioctlerr>0)
        {
          err = CAM_CAL_ERR_NO_SHADING;CAM_CAL_ERR("CAM_CALGetCal Dataioctlerr>0 \n");
        }
        CAM_CAL_ERR("CAM_CALGetCalData lsc table_size %d\n",table_size);
...
#ifdef Hynix_LSC_SW 
			for(int cnt = 0 ; cnt < 396 ; cnt ++)
			{
				if(MTK_EEPROM_DATA[cnt*2+1] < 0x20)
				{
					MTK_EEPROM_DATA[cnt*2+1] = 0x20 ; MTK_EEPROM_DATA[cnt*2] = 0x00;
				}
			}
			for(int cnt = 0 ; cnt < 792 ; cnt ++)
			{
				pCamCalData->SingleLsc.LscTable.Data[0x44 + cnt +4 ] = MTK_EEPROM_DATA[cnt];
			}
#endif 	
//013311
...
#if 0// def Hynix_LSC_Conversion_Use
			err = CAM_CAL_ERR_NO_ERR;
#endif 			
...
UINT32 DoCamCalLayoutCheck(void)
{
    ...
    UINT32 CheckID,i ;
    INT32 err;
	UINT8 ChipInfo; // 20151203 add 

	    CAM_CAL_ERR("DoCamCalLayoutCheck func start\n"); // temp. add log for check func call 
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (CAM_CALInit() != CAM_CAL_NONE) && (HI842_CAM_CALDeviceName(&cBuf[0]) == 0))
    {


	    CAM_CAL_ERR("DoCamCalLayoutCheck func Start Check Calibration Tool Version\n"); // temp. add log for check func call 
        ...
        for (i = 0; i< MAX_CALIBRATION_LAYOUT_NUM; i++)
        {
            ...
            {
                CAM_CAL_ERR("LayoutTbl Pass_xiaofei\n");
                CheckID = 0x00000000;
                cam_calCfg.u4Offset = CalLayoutTbl[i].HeaderAddr;
                cam_calCfg.u4Length = 4;
                cam_calCfg.pu1Params = (u8 *)&CheckID;
                CAM_CAL_LOG_IF(dumpEnable,"u4Offset=%d u4Length=%d pu1Params= 0x%x ", cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);
                CAM_CAL_ERR("Read cam_calCfg.pu1Params_xiaofei  = 0x%x \n",cam_calCfg.pu1Params);
		   CAM_CAL_ERR("Read cam_calCfg.u4Offset_xiaofei  = 0x%x \n",cam_calCfg.u4Offset);		
			...
			CAM_CAL_LOG_IF(dumpEnable,"Table[%d] ID= 0x%x",i, CheckID);
                     CAM_CAL_ERR("Read ID_xiaofei  = 0x%x \n",CheckID);
// Hynix Start 
#ifdef Hynix_LSC_SW  
		if(CheckID == 0x420800ff) // if hi-842 sensor 
		{
			CAM_CAL_ERR("Hi-842 Cal Detected");// Header Information Match Correct.
			
			cam_calCfg.u4Offset = 16;
			cam_calCfg.u4Length = 1768;
			cam_calCfg.pu1Params = (u8 *)QCT_LSC;
			CAM_CAL_LOG_IF(dumpEnable,"u4Offset=%d u4Length=%d pu1Params= 0x%x ", cam_calCfg.u4Offset, cam_calCfg.u4Length, cam_calCfg.pu1Params);

			err= ioctl(lCamcamFID, CAM_CALIOC_H_READ , &cam_calCfg);

			if(err< 0)
			{
				CAM_CAL_ERR("Hi-842 OTP Read  err\n");
			...
			{
				CAM_CAL_ERR("Hi-842 OTP Read  PASS!!!!!!! - Good!!!!!!\n");
				// Need insert con func
				Gr_ratio_GLOBAL=0.6; //60%
				Gb_ratio_GLOBAL=0.6;
				R_ratio_GLOBAL=0.6;
				B_ratio_GLOBAL=0.6;

// 20151203 add start 
				cam_calCfg.u4Offset = 2070;
				cam_calCfg.u4Length = 1;
				cam_calCfg.pu1Params = (u8 *)&ChipInfo;
				err= ioctl(lCamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);

				if(err< 0)
				{
					CAM_CAL_ERR("Hi-842 OTP Pixel ID Read err\n");
				...
				{
                    CAM_CAL_ERR("Hi-842 OTP Pixel ID is 0x%x\n",ChipInfo);

					if( 0 == ChipInfo )
						ALLROUND_color_channel = 3; // Pixel Order Select  0 : R , 1 : B , 2 : GB , 3 : GR 
					else
						ALLROUND_color_channel = 2; // Pixel Order Select  0 : R , 1 : B , 2 : GB , 3 : GR
				}
// 20151203 add end 	
...
*
*******************************************************************************/
UINT32 HI842_CAM_CALGetCalData(UINT32* pGetSensorCalData)
{	
    UCHAR cBuf[128] = "/dev/";	
    UCHAR HeadID[5] = "NONE";	
    UINT32 result = CAM_CAL_ERR_NO_DEVICE;
...
//    CAM_CAL_LOG_IF(dumpEnable,"CAM_CALGetCalData(0x%8x)----\n",(unsigned int)pGetSensorCalData);
    ...
    CAM_CAL_ERR("CAM_CALGetCalData func start\n"); // temp. add log for check func call 
    ...
    {  
        ...
        if ((CAM_CALInit() != CAM_CAL_NONE) && (HI842_CAM_CALDeviceName(&cBuf[0]) == 0))
        {
