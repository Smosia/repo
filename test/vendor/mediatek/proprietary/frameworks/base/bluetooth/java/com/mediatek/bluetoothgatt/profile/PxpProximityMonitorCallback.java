...
import com.mediatek.bluetoothgatt.characteristic.TxPowerLevel;

import java.util.UUID;
//........................................................................ Customized End: Import //

/**
 * This class is used to implement {@link PxpProximityMonitor} callbacks.
 */
public class PxpProximityMonitorCallback extends ClientBaseCallback {
    private static final boolean DBG = true;
    private static final String TAG = "PxpProximityMonitorCallback";
    ...
    void dispatchCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_LLS)) {
             if (charUuid.equals(GattUuid.CHAR_ALERT_LEVEL)) {
                ...
                this.onLlsAlertLevelReadResponse(
                        (AlertLevel) base, status);
                return;
            }
        }
        if (srvcUuid.equals(GattUuid.SRVC_TPS)) {
             if (charUuid.equals(GattUuid.CHAR_TX_POWER_LEVEL)) {
                base = new TxPowerLevel();
                base.setCharacteristic(characteristic);
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    base.setValue(characteristic.getValue());
                }
                this.onTpsTxPowerLevelReadResponse(
                        (TxPowerLevel) base, status);
                return;
            }
        }
    ...
    void dispatchCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_LLS)) {
             if (charUuid.equals(GattUuid.CHAR_ALERT_LEVEL)) {
                ...
                this.onLlsAlertLevelWriteResponse(
                        (AlertLevel) base, status);
                return;
            }
        }
     ...
     * Lls:AlertLevel characteristic read operation.
     *
     * @param alertLevel Lls:AlertLevel characteristic
     *                       that was read from the associated remote device.
     * @param status {@link BluetoothGatt#GATT_SUCCESS} if the read operation
     *               was completed successfully.
     */
    public void onLlsAlertLevelReadResponse(
            AlertLevel alertLevel, int status) {
    }

    /**
     * Callback reporting the result of a
     * Tps:TxPowerLevel characteristic read operation.
     *
     * @param txPowerLevel Tps:TxPowerLevel characteristic
     *                       that was read from the associated remote device.
     * @param status {@link BluetoothGatt#GATT_SUCCESS} if the read operation
     *               was completed successfully.
     */
    public void onTpsTxPowerLevelReadResponse(
            TxPowerLevel txPowerLevel, int status) {
    }


    /**
     * Callback indicating the result of a
     * Lls:AlertLevel characteristic write operation.
     *
     * @param alertLevel Lls:AlertLevel characteristic
     ...
     */
    public void onLlsAlertLevelWriteResponse(
            AlertLevel alertLevel, int status) {
    }
