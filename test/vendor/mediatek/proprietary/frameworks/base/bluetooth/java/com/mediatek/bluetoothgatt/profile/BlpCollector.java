...
/**
 * Public API for the GATT Blood Pressure Profile(Client).
 *
 * Name: Blood Pressure
 ...
 * Role: Collector
 */
public class BlpCollector extends ClientBase {
    private static final boolean DBG = true;
    private static final String TAG = "BlpCollector";

    /**
     * Create a BlpCollector object and init value.
     *
     * @param context App context
     */
    public BlpCollector(Context context) {
        super(context);

        if (DBG) {
            Log.d(TAG, "BlpCollector()");
        }
    }
    ...
    protected boolean handleServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        return true &&
                (gatt.getService(GattUuid.SRVC_BLS) != null);
    }

    /**
     * Reads Bls:BloodPressureFeature characteristic
     * from the associated remote device.
     *
     * @return true, if the read operation was initiated successfully
     */
    public boolean readBlsBloodPressureFeature() {
        if (DBG) {
            Log.d(TAG, "readBlsBloodPressureFeature()");
        }
    ...
    /**
     * Reads Bls:BloodPressureMeasurement:Cccd descriptor
     * from the associated remote device.
     *
     * @return true, if the read operation was initiated successfully
     */
    public boolean readBlsBloodPressureMeasurementCccd() {
        if (DBG) {
            Log.d(TAG, "readBlsBloodPressureMeasurementCccd()");
        }
    ...
    /**
     * Reads Bls:IntermediateCuffPressure:Cccd descriptor
     * from the associated remote device.
     *
     * @return true, if the read operation was initiated successfully
     */
    public boolean readBlsIntermediateCuffPressureCccd() {
        if (DBG) {
            Log.d(TAG, "readBlsIntermediateCuffPressureCccd()");
        }
        return readDescriptor(
                GattUuid.SRVC_BLS,
    ...
    /**
     * Writes Bls:BloodPressureMeasurement:Cccd descriptor and
     * its values to the associated remote device.
     *
     * @param value Descriptor value to write on the remote device
     * @return true, if the write operation was initiated successfully
     */
    public boolean writeBlsBloodPressureMeasurementCccd(byte[] value) {
        if (DBG) {
            Log.d(TAG, "writeBlsBloodPressureMeasurementCccd()");
        }
    ...
    /**
     * Writes Bls:IntermediateCuffPressure:Cccd descriptor and
     * its values to the associated remote device.
     *
     * @param value Descriptor value to write on the remote device
     * @return true, if the write operation was initiated successfully
     */
    public boolean writeBlsIntermediateCuffPressureCccd(byte[] value) {
        if (DBG) {
            Log.d(TAG, "writeBlsIntermediateCuffPressureCccd()");
        }
        return writeDescriptor(
                GattUuid.SRVC_BLS,
                GattUuid.CHAR_INTERMEDIATE_CUFF_PRESSURE,
