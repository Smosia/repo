...
/**
 * This class is used to implement {@link HrpCollector} callbacks.
 */
public class HrpCollectorCallback extends ClientBaseCallback {
    private static final boolean DBG = true;
    private static final String TAG = "HrpCollectorCallback";
    ...
    void dispatchCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_BODY_SENSOR_LOCATION)) {
                base = new BodySensorLocation();
                base.setCharacteristic(characteristic);
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    base.setValue(characteristic.getValue());
                }
                this.onHrsBodySensorLocationReadResponse(
                        (BodySensorLocation) base, status);
                return;
            }
        }
    ...
    void dispatchCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_CONTROL_POINT)) {
                base = new HeartRateControlPoint();
                base.setCharacteristic(characteristic);
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    base.setValue(characteristic.getValue());
                }
                this.onHrsHeartRateControlPointWriteResponse(
                        (HeartRateControlPoint) base, status);
                return;
            }
        }
    ...
    void dispatchCharacteristicChanged(BluetoothGatt gatt,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_MEASUREMENT)) {
                base = new HeartRateMeasurement(characteristic.getValue(), characteristic);
                this.onHrsHeartRateMeasurementNotify(
                        (HeartRateMeasurement) base);
                return;
            }
        }
    ...
    void dispatchDescriptorRead(BluetoothGatt gatt,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_MEASUREMENT) &&
                ...
                this.onHrsHeartRateMeasurementCccdReadResponse(descriptor, status);
                return;
            }
        }
    ...
    void dispatchDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_MEASUREMENT) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onHrsHeartRateMeasurementCccdWriteResponse(descriptor, status);
                return;
            }
        }
     ...
     * Hrs:BodySensorLocation characteristic read operation.
     *
     * @param bodySensorLocation Hrs:BodySensorLocation characteristic
     *                       that was read from the associated remote device.
     * @param status {@link BluetoothGatt#GATT_SUCCESS} if the read operation
     *               was completed successfully.
     */
    public void onHrsBodySensorLocationReadResponse(
            BodySensorLocation bodySensorLocation, int status) {
    }
     ...
     * Hrs:HeartRateControlPoint characteristic write operation.
     *
     * @param heartRateControlPoint Hrs:HeartRateControlPoint characteristic
     *                       that was written to the associated remote device.
     * @param status The result of the write operation
     *               {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    public void onHrsHeartRateControlPointWriteResponse(
            HeartRateControlPoint heartRateControlPoint, int status) {
    }


    /**
     * Callback reporting the result of a
     * Hrs:HeartRateMeasurement:Cccd descriptor read operation.
     *
     ...
     *               was completed successfully
     */
    public void onHrsHeartRateMeasurementCccdReadResponse(
            BluetoothGattDescriptor descriptor, int status) {
    }
    ...
    /**
     * Callback indicating the result of a
     * Hrs:HeartRateMeasurement:Cccd descriptor write operation.
     *
     ...
     *               {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    public void onHrsHeartRateMeasurementCccdWriteResponse(
            BluetoothGattDescriptor descriptor, int status) {
    }
     ...
     * Hrs:HeartRateMeasurement characteristic notification.
     *
     * @param heartRateMeasurement Characteristic that has been updated as a result
     *                       of a remote notification event.
     */
    public void onHrsHeartRateMeasurementNotify(
