...
static unsigned char CTPM_YS_FW[] = {
	#include "FT5436I_5136_ID5F_V0x14_20160124_app.i"
};
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client)
{
	...
	int i_ret;
	printk("sym fts_ctpm_fw_upgrade_ReadVendorID! \n");
	//}
	//*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (i_ret < 0) {
			printk("failed writing  0x55 and 0xaa ! \n");
			continue;
		}
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {
			printk("[FTS] sym xinan  Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			break;
		} 
		else 
		{
			printk(&client->dev, "[FTS] sym xinan  Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			continue;
		}
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			//*ucPVendorID = 0;
			printk("sym xinan In upgrade Vendor ID Mismatch, REG1 = 0x%x, REG2 = 0x%x, Definition:0x%x, i_ret=%d\n", reg_val[0], reg_val[1], 0, i_ret);
		} 
		else 
		{
			//*ucPVendorID = reg_val[0];
			printk("xym xinan In upgrade Vendor ID, REG1 = 0x%x, REG2 = 0x%x\n", reg_val[0], reg_val[1]);
			break;
		}
	}
	msleep(50);
	/*********Step 5: reset the new FW***********************/
	TPD_DEBUG("xym xinan Step 5: reset the new FW\n");
...
static int fts_GetFirmwareSize(char *firmware_name)
{
	...
	memset(filepath, 0, sizeof(filepath)); 
       sprintf(filepath, "xinan %s%s", FTXXXX_INI_FILEPATH_CONFIG, firmware_name);
	if (NULL == pfile)
	{
	...
	if (IS_ERR(pfile)) 
	{
		pr_err("xinan error occured while opening file %s.\n", filepath);
		return -EIO;
	}
...
msleep(200);
	...
	fts_i2c_write(client, auc_i2c_write_buf, 1);
	msleep(200);
	fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver_1);
	printk("sym uc_tp_fm_ver_1 = %x\n",uc_tp_fm_ver_1);
	msleep(200);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("sym 111uc_tp_fm_ver = %x\n",uc_tp_fm_ver);
	
	fts_read_reg(client, 0xa8, &vend_id);
	printk("sym  222 vend_id= %x \n",vend_id);
	msleep(1);
	if(vend_id != 0x67 && vend_id != 0x5f)
		{
	...
	{
		msleep(100);
		printk("sym xinan if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver < uc_host_fm_ver ) \n");
