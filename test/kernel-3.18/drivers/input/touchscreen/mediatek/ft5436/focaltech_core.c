...
int fts_i2c_read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
{
	...
	if(writelen>0)//if((NULL!=client) && (writelen>0) && (writelen<=128))
	{
		// DMA Write
	...
	if((readlen>0))//((NULL!=client) && (readlen>0) && (readlen<=128))
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		set_current_state(TASK_RUNNING);
			...
			if (ret<0)
			{
				printk("sym [Focal][Touch] read value fail");
				//return ret;
			}
			ret = fts_read_reg(fts_i2c_client, 0xd2,&state1);
			if (ret<0)
			{
				printk("sym [Focal][Touch] read value fail");
				//return ret;
			...
			//printk("tpd fts_read_Gestruedata state=%d\n",state);
		     	if(state ==1)
		     	{
		     		printk("sym fts_read_Gestruedatal");
...
static struct class *ft5436_class;
static bool TP_gesture_Switch = false;
static ssize_t enable_ft5436_show(struct class *dev, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s:%d\n", "gesenable",TP_gesture_Switch);
}
static ssize_t enable_ft5436_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long state;
	ssize_t ret = -EINVAL;
...
static CLASS_ATTR(gesenable, 00774, enable_ft5436_show, enable_ft5436_store);
#endif
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int retval = TPD_OK;
...
#ifdef FTS_GESTRUE_EN
	ft5436_class = class_create(THIS_MODULE, "syna");
	err = class_create_file(ft5436_class, &class_attr_gesenable);
#endif
//add by fangkai.yao
/*9375 info*/
/*i	int sensor_id;
...
static void tpd_resume(struct device *h)
{
	...
	msleep(200);
		...
		if(TP_gesture_Switch)
			{
			printk("sym %s fts_write_reg(fts_i2c_client,0xD0,0x00); ",__func__);
...
static void tpd_suspend(struct device *h)
{
	...
	printk("TPD enter sleep\n");
		...
		//	memset(coordinate_y,0,255);
			...
			fts_read_reg(i2c_client, 0xd2, &state1);
			printk("sym 2 state1 0xd2 =%x\n",state1);
			if(state1 != 0xff)
				fts_write_reg(i2c_client, 0xd2, 0xff);
			
			fts_write_reg(i2c_client, 0xd3, 0xff);
			fts_write_reg(i2c_client, 0xd4, 0xff);
			...
			for(i = 0; i < 10; i++)
			{
				...
				if(state == 1)
				{
					printk("sym 1 TPD gesture write 0x01\n");
	        			return;
				}
				else
				{
					printk("sym 2 TPD gesture write 0x01\n");
		   			fts_write_reg(i2c_client, 0xd0, 0x01);
					 msleep(10);
	   				fts_read_reg(i2c_client, 0xd0, &state);
	   				if(state == 1)
	   				{
	   	    				printk("sym 3 TPD gesture write 0x01 again OK\n");
	   					return;
	   				}
					else
					{
		...
		if(i >= 9)
		{
			printk("sym TPD gesture write 0x01 to d0 fail \n");
			return;
		}
	}
	#endif
