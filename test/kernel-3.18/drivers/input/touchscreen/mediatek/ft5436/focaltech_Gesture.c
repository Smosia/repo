...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	switch(gesture_id)
	{
	        ...
	        case GESTURE_DOUBLECLICK:
	                input_report_key(input_dev, KEY_GESTURE_F, 1);
	                input_sync(input_dev);
	                input_report_key(input_dev, KEY_GESTURE_F, 0);
	                input_sync(input_dev);
	                break;
	        case GESTURE_O:
 ...
 unsigned char buf[FTS_GESTRUE_POINTS * 4+2+6] = { 0 };
   ...
   {            
	...
	if (0x24 == buf[0])
	{
	    gestrue_id = 0x24;
	    fts_check_gesture(fts_input_dev,gestrue_id);
		printk( "sym %d check_gesture gestrue_id.\n", gestrue_id);
	    return -1;
	}
	...
	gestrue_id = fetch_object_sample(buf, pointnum);//sym modified need gesture lib.a
	//gestrue_id = 0x24;
	fts_check_gesture(fts_input_dev,gestrue_id);
	printk( "sym gestrue_id=%x   pointnum=%d  read gestrue_id.\n", gestrue_id,pointnum);
