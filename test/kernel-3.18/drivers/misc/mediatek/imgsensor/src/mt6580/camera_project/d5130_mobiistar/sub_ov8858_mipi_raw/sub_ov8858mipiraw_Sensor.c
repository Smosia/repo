...
SUB_OV8858_VERSION sub_ov8858version = SUB_OV8858R2A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = SUB_OV8858_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	printk("otp_flag2 = 0x%x\n",otp_flag);	
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x7011; // base address of info group 1
		//MID = read_cmos_sensor(0x7011);
		printk("MID5 = 0x%x\n", MID);
		printk("group is 1 otp_flag2 = 0x%x\n",otp_flag);	
	}
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				if((read_cmos_sensor(0x302A)) == 0XB2){
 				sub_ov8858version = SUB_OV8858R2A;
				SENSORDB("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,sub_ov8858version);	
				return ERROR_NONE;
				}
