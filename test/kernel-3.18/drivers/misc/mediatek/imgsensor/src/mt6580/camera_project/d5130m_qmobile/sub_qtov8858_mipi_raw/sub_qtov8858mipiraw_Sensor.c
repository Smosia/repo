...
#include "sub_qtov8858mipiraw_Sensor.h"

//#define PFX "OV8858R2A"

#define SENSORDB LOG_INF1

#define SUB_QTOV8858R2AOTP
extern int ov8858cjy;
static kal_uint16 MID = -1;
/****************************Modify Following Strings for Debug****************************/
#define PFX "SUB_QTOV88582LANE"
...
/****************************   Modify end    *******************************************/
 ...
 typedef enum {
	SUB_QTOV8858R2A,
	SUB_QTOV8858R1A
}SUB_QTOV8858_VERSION;
SUB_QTOV8858_VERSION sub_qtov8858version = SUB_QTOV8858R2A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = SUB_QTOV8858_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	MID = read_cmos_sensor(0x7011);
	printk("MID8 = 0x%x\n", MID);
	//write_cmos_sensor(0x7011, 0);
	printk("MID10 = 0x%x\n", MID);
	otp_flag = read_cmos_sensor(0x7010);
	printk("otp_flag1 = 0x%x\n",otp_flag);	
	//MID = read_cmos_sensor(0x7011);
	printk("MID6 = 0x%x\n", MID);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x7011; // base address of info group 1
		printk("group is 1 otp_flag1 = 0x%x\n",otp_flag);	
	}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) - 1;
			if ((*sensor_id == imgsensor_info.sensor_id)&&(MID == 0x06)) {
				if((read_cmos_sensor(0x302A)) == 0XB2){
 				sub_qtov8858version = SUB_QTOV8858R2A;
				SENSORDB("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,sub_qtov8858version);	
				return ERROR_NONE;
			  }
