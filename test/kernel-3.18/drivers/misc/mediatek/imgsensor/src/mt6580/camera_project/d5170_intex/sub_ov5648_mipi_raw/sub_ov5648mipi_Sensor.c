...
#include "sub_ov5648mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SUB_OV5648_camera_sensor"
#define LOG_1 LOG_INF("SUB_OV5648,MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SUB_OV5648MIPI_SENSOR_ID,
...
/*
	...
	mt_set_gpio_pull_enable(GPIO_SUB_CAM_ID_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_SUB_CAM_ID_PIN, GPIO_PULL_DOWN);
	comp =  mt_get_gpio_in(GPIO_SUB_CAM_ID_PIN);
	printk("SUBcompID = %d\n",comp);
	if(comp != 0 )
	{
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id()-1;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("huyuan i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("huyuan Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 SUB_OV5648MIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SUB_OV5648MIPISensorInit    */
