...
{
	printk("enter 553 init guoameng\n");
	//====================================================================
...
printk("leave 553 init guoameng\n");
}	/*	sensor_init  */
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	//if (flag==0)
		...
		//else
			//{while(1);}
	printk("hi553 [get_imgsensor_id] guoameng ");
	...
	printk("leave hi553 [get_imgsensor_id] guoameng ");
	return ERROR_NONE;
}
...
{   
	  printk("enter 553 open guoameng\n");
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("hi553 i2c write id guoameng: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("hi553 Read sensor id guoameng fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    spin_unlock(&imgsensor_drv_lock);
    printk("leave 553 open guoameng\n");
    return ERROR_NONE;
}    /*    open  */
...
{
	//LOG_INF("E");
printk("enter  553 preview guoameng\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	//LOG_INF("E");
	printk("enter 553 capture guoameng\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	//LOG_INF("E");
		printk("enter 553 normal_video guoameng\n");
	spin_lock(&imgsensor_drv_lock);
...
{
    //LOG_INF("E\n");
printk("enter 553  hs_video guoameng\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	printk("enter 553  slim_video guoameng\n");
    //LOG_INF("E\n");
spin_lock(&imgsensor_drv_lock);
