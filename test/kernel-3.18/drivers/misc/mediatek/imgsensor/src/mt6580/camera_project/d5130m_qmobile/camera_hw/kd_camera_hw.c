...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "imx135mipiraw"))) ||
			...
			//sub_ov8858
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SUB_OV8858_MIPI_RAW,currSensorName)))
			  {
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
             ...
             if (GPIO_CAMERA_INVALID != pinSet[pinSetIdxTmp][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdxTmp, CAMRST,
						pinSet[pinSetIdxTmp][IDX_PS_CMRST + IDX_PS_OFF]);
		
					  		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdxTmp][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdxTmp, CAMPDN,
						pinSet[pinSetIdxTmp][IDX_PS_CMPDN + IDX_PS_OFF]);
	
	            }
			  }	
			  		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SUB_QTOV8858_MIPI_RAW,currSensorName)))
			  {
	          	if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,
						pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
	...
	//		mtkcam_gpio_set(pinSetIdx, CAMLDO, 0);
        ...
        //sub_ov8858
				...
				{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_OV8858_MIPI_RAW-----FMYFMY \n");
