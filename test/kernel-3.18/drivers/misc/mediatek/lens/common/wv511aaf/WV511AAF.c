...
#define AF_DRVNAME "WV511AAF_DRV"
...
inline static int getAFInfo(__user stAF_MotorInfo * pstMotorInfo)
{
...
long WV511AAF_Ioctl(struct file * a_pstFile,unsigned int a_u4Command,unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        ...
        case WV511AAFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stMotorMETAInfo *)(a_u4Param));
        break;
        #endif
	case AFIOC_T_MOVETO:
    ...
    *g_pAF_Opened = 1;
    spin_unlock(g_pAF_SpinLock);
  	...
  	mt_set_gpio_dir(GPIO_CAMERA_AF_EN_PIN, GPIO_DIR_OUT);
		...
		printk(" TURN ON AF EN PIN!!,%d\n",mt_get_gpio_out(GPIO_CAMERA_AF_EN_PIN));
#endif 
#endif
    LOG_INF("End \n");
...
MODULE_LICENSE("GPL");
*/
void WV511AAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
