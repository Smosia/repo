...
public final class IoBridge {
    ...
    private static void connectErrno(FileDescriptor fd, InetAddress inetAddress, int port, int timeoutMs) throws ErrnoException, IOException {
        ...
        try {
            /// M: debug logging
            System.out.println("[CDS]connect[" +
                    inetAddress.toString() + ":" + port + "] tm:" + (timeoutMs / 1000));
            Libcore.os.connect(fd, inetAddress, port);
    ...
    private static void setSocketOptionErrno(FileDescriptor fd, int option, Object value) throws ErrnoException, SocketException {
        ...
        /// M: support snd timeout socket option @{
        case SocketOptions.SO_SND_TIMEOUT:
            int sndMillis = (Integer) value;
            StructTimeval sndTv = StructTimeval.fromMillis(sndMillis);
            Libcore.os.setsockoptTimeval(fd, SOL_SOCKET, SO_SNDTIMEO, sndTv);
            return;
