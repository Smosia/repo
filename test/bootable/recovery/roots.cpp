...
int ensure_path_mounted(const char* path) {
    //A: DWYQLSSMY-419 kuangyunfeng - for mount sideload dir(start)
    if (!strncmp(path, "/sideload", strlen("/sideload"))) {
        return 0;
    }
    //A: DWYQLSSMY-419 kuangyunfeng - for mount sideload dir(end)
...
//adupsfota end
	...
	LOGI("[1]v->mount_point = %s, v->flags = %d, v->fs_options=%s\n", v->mount_point, v->flags, v->fs_options);
#if defined(GMT_RECOVERY_MODE_ROCK_GOTA_SUPPORT) 
	if(gmt_mount_rw) {
