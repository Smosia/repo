...
import com.mediatek.galleryfeature.video.FrameHandler;
...
public class MoviePlayer implements
	...
	private File screenshotsDirectory = new File(
...
//    };
    ...
    public MoviePlayer(View rootView, final MovieActivity movieActivity, IMovieItem info,
		...
		if (screenshotsDirectory.toString().contains("sdcard0")) {
			ScreenshotStorage = mActivityContext
					.getResources().getString(R.string.picture_store_in) + ": "
					+ mActivityContext.getResources().getString(R.string.phone_storage)
					+ "/"  + Environment.DIRECTORY_PICTURES
					+ SNAP_SHOT_PATH;
		} else {
			ScreenshotStorage = mActivityContext.getResources()
					.getString(R.string.sdcard_storage)
					+ "/"  + Environment.DIRECTORY_PICTURES
					+ SNAP_SHOT_PATH;
		...
		//A: DWQBE-86 gulincheng 20151015{
        mController.setIVideoWidgetFunc(this);
		//A: }
...
//        });
   ...
   ///@}
	...
	public void snapshot() {
		...
		final Activity mActivity = mActivityContext;
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				final Bitmap bitmap = getVideoFrame(mItem.getUri(),
						mVideoView.getCurrentPosition());
				if (bitmap != null) {
					if (!GalleryUtils.saveBitmap(savePath.getPath(), bitmap)) {
		...
		Toast.makeText(mActivityContext, ScreenshotStorage, Toast.LENGTH_SHORT)
				.show();
		if (isPlaying) {
			mVideoView.start();
		}
	...
	public Bitmap getVideoFrame(Uri uri, int time) {
		Log.d(TAG, "time:" + time);
	...
	//todo reserve for new feature, onclick in mutiscreenview.
 	public void setOrientation(){
 		if (mContext.getResources().getBoolean(R.bool.new_feature_for_video_player)){
 			mActivityContext.setRequestedOrientation(
