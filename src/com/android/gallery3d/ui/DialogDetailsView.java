...
// add Bug_id:JLLBLS-270 liupeng 20150925  (end)
                ...
                switch (detail.getKey()) {
                    ...
                    case MediaDetails.INDEX_PATH:
                        ...
                        if (!is2SdCardSwapSupport()) {
                            if(GalleryUtils.isHaveExternalSDCard() )
                            {
                                if(value.contains("emulated/0")) {
                                    value= value.replace("emulated/0", context.getResources().getString(R.string.phone_storage));
                            }else {
                                    value= value.replace("sdcard1",  context.getResources().getString(R.string.sdcard_storage));
                                }
                            } else{
                                value= value.replace("emulated/0", context.getResources().getString(R.string.phone_storage));
                            }
                            ...
                            if(GalleryUtils.isHaveExternalSDCard() )
                            {
                                if(value.contains("sdcard0")) {
                                    value= value.replace("sdcard0", context.getResources().getString(R.string.sdcard_storage));
                                }else {
                                    value= value.replace("sdcard1",  context.getResources().getString(R.string.phone_storage));
                                }
                            } else{
                                value= value.replace("sdcard0", context.getResources().getString(R.string.phone_storage));
                            }
                        }
                 ...
                 //add BUG_ID:DWQBJ-48 liqiang 20151210  (start)
                if(MediaDetails.INDEX_MODEL == detail.getKey()){
                    if(value != null){
                        value = value.replace("SENSEIT ", "");
                    }
                }
                if(MediaDetails.INDEX_MAKE == detail.getKey()){
                    if(value != null && value.trim().equalsIgnoreCase("tehnoliga")){
                        value = "SENSEIT";
                    }
                }
                //add BUG_ID:DWQBJ-48 liqiang 20151210 (end)
